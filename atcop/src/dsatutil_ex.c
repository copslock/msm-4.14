...
$Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatutil_ex.c#2 $ $DateTime: 2019/11/09 00:16:19 $ $Author: pwbldsvc $
...
02/08/17   skc     Fixed issue on automatic answer on slot 2.
10/08/16   skc     Added enhancement on CHLD and CLCC for VoLTE MPTY Call.
08/10/15   skc     Removing usage of enum TUPLE_TYPE_IPv4_254
...
#define DSATUTIL_EX_ASSERT(expression) \
         dsatutil_ex_assert_wrapper(__LINE__, expression)
...
LOCAL dsat_string_item_type  dsat_tmgi_val[TMGI_STRING_MAX_LEN];
...
dsat_timer_s_type dsatutil_timer_table[] =
{
  {DSAT_TIMER_VOICE_RING,
   ...
   0, FALSE, FALSE, &dsatvoice_ring_info[SYS_MODEM_AS_ID_1]}  /* Duration varies, set in cmd handler */

#ifdef FEATURE_DUAL_ACTIVE
  ,{DSAT_TIMER_VOICE_RING_2,
   &dsatvoice_ring_timer_2, dsatutil_timer_cb,
   dsatvoice_timer_expired_handler,
   0, FALSE, FALSE, &dsatvoice_ring_info[SYS_MODEM_AS_ID_2]}  /* Duration varies, set in cmd handler */
...
/** -------------------------------------------------------------------------
  Config file name for ATCoP to store filepaths of EFS NV items.
---------------------------------------------------------------------------*/
#define DSAT_NV_CONF_EFS_FILE_NAME "/nv/item_files/conf/atcop_config_info.conf"

#define DSAT_NV_CONF_MAX_PATH_SIZE   128  /**< Maximum path size of an efs nv item. */

/** -------------------------------------------------------------------------
  EFS config path to be written to EFS.
---------------------------------------------------------------------------*/
static const char *dsat_nv_conf_efs_path_array[] =                         //For future pupose, making it array of strings
{
  "/nv/item_files/modem/data/interface/atcop_disable"
};

/** -------------------------------------------------------------------------
  NV item enum indexes corresponding to EFS config paths
---------------------------------------------------------------------------*/
typedef enum 
{
  DSAT_NV_CONF_ATCOP_DISABLE
} dsat_nv_conf_enum_type;

/** -------------------------------------------------------------------------
  Accessor macros to access config item from config item array
---------------------------------------------------------------------------*/
#define DSAT_NV_CONF_ITEM_INFO(item) (dsat_nv_conf_efs_path_array[(int)item])
...
FUNCTION DSATUTIL_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATUTIL_EX ASSERT
...
===========================================================================*/
static void dsatutil_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATUTIL_EX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatutil_ex_assert_wrapper */
...
)
{
      ...
      ('\0' != *tok_ptr->arg[token_index]) )
  {
    ...
    DS_ATCOP_ERROR_LOG_1("[INT] Conversion Fail or Out of Range token: =d", token_index);
...
)
{
      ...
      ('\0' != *tok_ptr->arg[token_index]) )
  {
    ...
    DS_ATCOP_ERROR_LOG_1("[String] Conversion Fail or Out of Range token: =d", token_index);
...
{
  if( (profile_id >= DSAT_LEGACY_PROFILE_NUM_MIN) && 
      (profile_id <= DSAT_LEGACY_PROFILE_NUM_MAX))
  {
    return TRUE;
  }
  return FALSE;
...
)
{
  ...
  if( profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
  { 
    DS_ATCOP_ERROR_LOG_1("Error end tx profile_status : =d",profile_status);
  }
  return profile_status;
}
/*===========================================================================
...
/** ========================================================================
  FUNCTION DSATUTIL_STRNCMP_UTF16_CASE

DESCRIPTION
  This function compares two strings for <n> UTF16 characters, ignoring case.
  Lower case letters are converted to upper case before the 
  difference is taken.
...
int dsatutil_strncmp_utf16_case
(
  const uint16 *s1,             /*  Pointer to string 1 */
  const uint16 *s2,             /*  Pointer to string 2 */
  uint32        n               /* Number of characters to compare */
)
{
  uint16 c1, c2;
...
)
{
  ...
  {
    DS_AT_MSG_SPRINTF_4_LOW("Allocated DSM item 0x%x in pool 0x%x (%s, %d)",
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Timer ID or handler not found in table: =d",
         *timer_id_ptr);
  }
...
)
{
  ...
  default:
    DS_ATCOP_ERROR_LOG_1("Unsupported tuple type: =d", ttype);
    return DSAT_ERROR;
  }
...
#define NUM_BITS_VAL 32
    ...
    *num_ptr = 0;
    ...
    for (i = num_tokens - 1; i >= 0; i--)
    {
      ...
      if (max_tokens_per_number <= j)
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("exceeded num_ptr_max_len =d",num_ptr_max_len);
          return DSAT_ERROR;
        }
  ...
  else /* INT_TO_STRING */
  {
    ...
    else
    {
      ...
      for (i=0; i < num_tokens; i++)
      {
        psutil_itoa (val[i], work_buf, radix);
...
)
{
           ...
           (INT_TO_STRING == mode && DS_PROFILE_3GPP_IP_V6 == addr_ptr->ip_vsn) )
  {
    ...
    if ( DSAT_OK != dsatutil_convert_tuple(
                                     ...
                                     DSAT_CONVERT_IP) )
    {
     DS_ATCOP_ERROR_LOG_0("Failed IPV6 address conversion");
      return DSAT_ERROR;
    }
    if ( STRING_TO_INT == mode )
    {
  ...
  else /* Assume IPV4 conversion */
  {
    if ( DSAT_OK != dsatutil_convert_tuple(mode,
                                           ...
                                           DSAT_CONVERT_IP) )
    {
     DS_ATCOP_ERROR_LOG_0("Failed IPV4 address conversion");
      return DSAT_ERROR;
    }
    if ( STRING_TO_INT == mode )
    {
...
)
{
  ...
  {
    DS_AT_MSG_SPRINTF_4_LOW("Allocated heap memory block 0x%x of size %d (%s, %d)",
...
{
  DS_AT_MSG1_LOW("modem_mem_free 0x%x:",(unsigned int)data_ptr);
  if (NULL != data_ptr)
  {
...
)
{
  ...
  {
    DS_AT_MSG_SPRINTF_4_LOW("Allocated DS command buffer 0x%x of payload size %d (%s, %d)",
...
)
{
  ...
  case DSAT_VENDOR_DSCI_IDX:
   ...
   case DSAT_VENDOR_QCNSP_IDX:
     ...
     break;
   case DSAT_VENDOR_QCCALLATTR_IDX:
     resp = &dsat_basic_vend_ss_vals.dsat_qccallattr_val[0];
     break;
   default:
    resp = NULL;
    break;
...
)
{
  ...
  case DSATETSI_EXT_ACT_CTZR_ETSI_IDX:
   ...
   case DSATETSI_EXT_ACT_CMCCS_ETSI_IDX:     
     resp = &dsat_etsi_ext_act_ss_vals.dsat_cmccs_val;
     break;
...
)
{
  ...
  case DSATETSI_EXT_CGEQMIN_IDX:
    ...
    case  DSATETSI_EXT_CVHU_IDX:
       ...
       break;
    case DSATETSI_EXT_CEMBMSCFG_IDX:
       resp = &dsat_etsi_ext_ss_vals.dsat_embmscfg_val;
       break;
    case DSATETSI_EXT_CEMBMSSRV_IDX:
       resp = &dsat_etsi_ext_ss_vals.dsat_embmssrv_val[0];
       break;
    case DSATETSI_EXT_CEMBMSSAI_IDX:
       resp = &dsat_etsi_ext_ss_vals.dsat_embmssai_val[0];
       break;
   default:
    resp = NULL;
    break;
...
)
{
 ...
 /* 1X command tables*/
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Table not found for =d index",cmd_id);
    DSATUTIL_EX_ASSERT(0);
  }
  return resp;
...
void dsatutil_default_constructor(void)
{
  ...
  dsat_etsi_ext_ss_vals.dsat_embmssrv_val[2].string_item = dsat_tmgi_val;
...
)
{
  ...
  if (addr == NULL)
  {
    DS_ATCOP_ERROR_LOG_1("Invalid addr for mem =d", mem_type);
...
)
{
  ...
  if (addr == NULL)
  {
    DS_ATCOP_ERROR_LOG_1("Invalid addr for slot ID =d", slot_id);
...
)
{
  ...
  if (addr == NULL)
  {
    DS_ATCOP_ERROR_LOG_1("Invalid addr for apps ID =d", apps_id);
...
PARAMETERS
  ...
  nv_info_ptr - Struct for NV item(s)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    retval = -1;
    err_info = DSAT_ERR_PTR_VARIABLE_NULL;
    goto result;
  }
  ...
  result:
   DS_AT_MSG2_MED("dsat_read_nv_efs retval:%d, err_info:%d",retval, err_info);
   return retval;
} /* dsat_read_efs_nv() */

/*===========================================================================
FUNCTION DSAT_WRITE_EFS_NV
...
PARAMETERS
  item_file_path - File path to item in NV
  nv_info_ptr - Pointer to NV item
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    retval = efs_errno;
    err_info = DSAT_ERR_NV_EFS_WRITE;
    goto result;
  }
  ...
  result:
   DS_AT_MSG2_MED("dsat_write_efs_nv retval:%d, err_info:%d",retval, err_info);
   return retval;
} /* dsat_write_efs_nv() */

/*===========================================================================
FUNCTION DSAT_PATH_IS_DIRECTORY
...
)
{
  ...
  if( 0 != rsp )
  {
    ...
    if( ENOENT != rsp )
    {
      DS_ATCOP_ERROR_LOG_1("efs_lstat error =d", rsp);
      return rsp;
    }
  }
  ...
  return -1;
} /* dsat_path_is_directory */

/*===========================================================================
FUNCTION DSAT_CREATE_EFS_CONFIG_FILE
...
SIDE EFFECTS
  None
===========================================================================*/
int dsat_create_efs_config_file
(
  const char *conf_file_path
)
{
  int                 config_fd, retval;
  dsat_err_e_type     err_info = DSAT_ERR_NO_ERROR;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  retval = dsat_path_is_directory("/nv");
  if( 0 != retval )
  {
    /* Directory doesn't exist yet */
    retval = efs_mkdir( "/nv", S_IREAD|S_IWRITE|S_IEXEC);
    if ( -1 == retval )
    {
      retval = efs_errno;
      err_info = DSAT_ERR_NV_EFS_CREATE_DIR;
      goto result;
    } 
  }

  retval = dsat_path_is_directory("/nv/item_files");
  if( 0 != retval )
  {
    /* Directory doesn't exist yet */
    retval = efs_mkdir( "/nv/item_files", S_IREAD|S_IWRITE|S_IEXEC);
    if ( -1 == retval )
    {
      retval = efs_errno;
      err_info = DSAT_ERR_NV_EFS_CREATE_DIR;
  ...
  retval = dsat_path_is_directory("/nv/item_files/conf");
  if( 0 != retval )
  {
    /* Directory doesn't exist yet */
    retval = efs_mkdir( "/nv/item_files/conf", S_IREAD|S_IWRITE|S_IEXEC);
    if ( -1 == retval )
    {
      retval = efs_errno;
      err_info = DSAT_ERR_NV_EFS_CREATE_DIR;
  ...
  {
    retval = efs_errno;
    err_info = DSAT_ERR_NV_EFS_OPEN_CONFIG_FILE;
    goto result;
  }

  retval = efs_close (config_fd);
  if ( 0 != retval )
  {
    retval = efs_errno;
    err_info = DSAT_ERR_NV_EFS_CLOSE_CONFIG_FILE;
    goto result;
  }
  ...
  result:
   DS_AT_MSG2_MED("dsat_create_efs_config_file retval:%d, err_info:%d",retval, err_info);
   return retval;

} /* dsat_create_efs_config_file() */


/** ===========================================================================
FUNCTION DSAT_CFG_GET_FILE
...
)
{
  ...
  *config_fd = efs_open (config_path, O_RDWR);
  ...
  if ( 0 > *config_fd )
  {
    ...
    {
      retval = dsat_create_efs_config_file(config_path);
      if ( 0 != retval )
      {
        err_info = DSAT_ERR_NV_EFS_CREATE_CONFIG_FILE;
        goto result;
      }
      *flag_new = TRUE;
  ...
  result:
   DS_AT_MSG2_MED("dsat_conf_get_file retval:%d, err_info:%d",retval, err_info);
   return retval;
}/* dsat_conf_get_file */


/** =========================================================================
FUNCTION DSAT_EFS_GET_CONF_STRING

DESCRIPTION
  Returns the file descriptor for the nv file path supplied.
  
PARAMETERS  
  dsat_cfg_item_info - array of pointers to nv item file paths.
...
RETURN VALUE 
  int - number of bytes in config string
  
SIDE EFFECTS 
  None.
===========================================================================*/

LOCAL int dsat_efs_get_conf_string
(
  const char * dsat_nv_conf_item_info[],
  char * file_path_buf,
  int dsat_efs_conf_items
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("file_path_buf is NULL");
    return -1;
  }
  
  file_path_buf[0] = '\0';
  file_path_buf_len = dsat_efs_conf_items * DSAT_NV_CONF_MAX_PATH_SIZE;
  /*---------------------------------------------------------------------
    Update data_config_info.conf with the list of nv item files.
  ----------------------------------------------------------------------*/
  /* Browse through dsat_cfg_item_info and copy all items in local
     buffer to be written to efs in one shot */
  for (index = 0; index < dsat_efs_conf_items; index ++)
  {
    /* Copy current path to buffer and append \n */
    position += snprintf( file_path_buf + position, 
                          file_path_buf_len - position,
                          "%s\n", 
                          dsat_nv_conf_item_info[index] );		
    /* Check for truncation */
    if( position >= file_path_buf_len)
    {
      DS_ATCOP_ERROR_LOG_1 ("Position =d exceeds max length", file_path_buf_len);
      return -1;
    }							
  }

  return position;
}/* dsat_efs_get_conf_string */


/** =========================================================================
  FUNCTION DSAT_EFS_WRITE_ITEMS
...
===========================================================================*/
LOCAL int dsat_efs_write_items
(
  char * data_conf_path,
  const char * dsat_nv_conf_item_info[],
  int dsat_efs_conf_items,
...
)
{
  ...
  if (NULL == dsat_nv_conf_item_info || NULL == data_conf_path)
  {
    retval = -1;
    err_info = DSAT_ERR_PTR_VARIABLE_NULL;
    goto result;
  ...
  do {
    ...
    retval = dsat_conf_get_file( &conf_fd, data_conf_path, &flag_new_file );
  
    if ( 0 != retval )
    {
      retval = -1;
      err_info = DSAT_ERR_NV_EFS_GET_CONF_FILE;
      goto result;
    }

    /* Allocate memory for file_read_buf */
    file_path_buf = dsat_alloc_memory(dsat_efs_conf_items*max_path_size, TRUE);
    ...
    conf_info_size = dsat_efs_get_conf_string(dsat_nv_conf_item_info, 
                                              file_path_buf, 
                                              dsat_efs_conf_items);
    ...
    /* Allocate memory for file_read_buf */
    file_read_buf = dsat_alloc_memory(conf_info_size, TRUE);
    ...
    if (FALSE == flag_new_file)
    {
      ...
      if( efs_ret < 0 )
      {
        DS_ATCOP_ERROR_LOG_0("Error reading file:" );
      }
    }
        ...
        || TRUE == flag_new_file ) 
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("efs_write error, bytes: =d", conf_info_size);
        retval = -1;
      }
    }
  } while (0);
  ...
  dsatutil_free_memory_ext((void**)&file_read_buf);
  dsatutil_free_memory_ext((void**)&file_path_buf);
  ...
  result:
   DS_AT_MSG2_MED("dsat_efs_write_items retval:%d, err_info:%d",retval, err_info);
   return retval;
}/* dsat_efs_write_items */


/** ========================================================================
  FUNCTION DSAT_EFS_INIT
  
  DESCRIPTION
    Initialization function for creating NV item file paths in EFS under
    atcop_config_info.conf. The function will put all the required 
    atcop nv item file paths in efs. The required atcop nv item file paths
    are specified in dsat_nv_conf_path_array and reference using the 
    indicies from dsat_nv_conf_enum_type
...
{
  if (0 != dsat_efs_write_items(DSAT_NV_CONF_EFS_FILE_NAME, 
                               dsat_nv_conf_efs_path_array,
                               ARR_SIZE(dsat_nv_conf_efs_path_array),
                               DSAT_NV_CONF_MAX_PATH_SIZE))

  {
   DS_ATCOP_ERROR_LOG_0("Item files write failed.");
  }
