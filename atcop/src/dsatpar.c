...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatpar.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/30/17   rk      Removed DS Flag FEATURE_DSAT_EXT_CLIENT_SUPPORT.
05/10/17   rk      Fixed issue related to Command token preparation
...
#define DSATPAR_ASSERT(expression) \
         dsatpar_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATPAR_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATPAR ASSERT
...
===========================================================================*/
static void dsatpar_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATPAR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatpar_assert_wrapper */
    ...
    switch ( UPCASE( *curr_char_ptr ) )
    {
      ...
      case 'S':
        ...
        /*--------------------------------------------------------------------
          ASKEY command
        --------------------------------------------------------------------*/
        token.cmd_category = ASKEY_AT_CMD;
        curr_char_ptr = parse_extended_command( curr_char_ptr, &token );
        if ( result_code != DSAT_ERROR )
        {
          result_code = dsatcmdp_queue_token( &token );
      ...
      case 'D':
        ...
        DSATPAR_ASSERT( result_code != DSAT_UNREC );
        if ( result_code != DSAT_ERROR )
        {
          result_code = dsatcmdp_queue_token( &token );          
        }
...
)
{
  ...
  while ( (c = *a_ptr) != '\0' )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Maximum line size exceeded: =d",MAX_LINE_SIZE);
    ...
    if ( ! quote_toggle )         /*  Allow ';' and ',' inside '"'  */
    {
      if ( c == ',' && (tokens_res->op & AR) )
      {
        *working_ptr = '\0';                /*  Terminate current argument  */
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Number of parameters exceeded: =d",MAX_ARG);
          result_code = DSAT_ERROR;
          break;
        }
      }
    ...
    while ( (c = *r_ptr) != '\0' && c != ';' && t != 0 && *htp_ptr )
    {
      if ( c != ' ' )
      {
        if ( UPCASE( c ) == *htp_ptr )
        {
          ...
          DSATPAR_ASSERT( w_ptr < &tok_ptr->working_at_line[MAX_LINE_SIZE] );
