...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatsms.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
8/23/19    sp      ATCOP changes to support new WMS callback function
12/18/15   skc     Fixed KW issues
04/16/15   sc      Fixed issue in event handling to use subs_id provided in 
                   event info.
02/11/15   tk      Fixed issues in subscription specific WMS API calls.
...
#define DSATSMS_ASSERT(expression) \
         dsatsms_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATSMS_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATSMS ASSERT
...
===========================================================================*/
static void dsatsms_assert_wrapper
(
...
#endif /* FEATURE_CDMA_SMS */
  {
    DS_ATCOP_ERROR_LOG_1("We are in wrong mode: =d",cmd_mode);
    return;
  }
...
dsatsms_fmt_mode_e_type dsatsms_get_msg_fmt_mode (void)
{
  ...
  {
    DSATSMS_ASSERT(cmd_mode == CDMA_CMD_MODE);
  }
...
)
{
  ...
  else
  {
    ...
    if(((dsatsmsi_pres_cmd.cmd_name==SMS_CMSS)||(dsatsmsi_pres_cmd.cmd_name == SMS_CMGS))
        && (sms_cmd_ptr->cmd_info == WMS_CMD_ERR_MSG_ACCESS_BARRED))
    {
...
void dsatsms_init_sms ( void )
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Could not register call back function with WMS");
  }
...
void dsatsms_deinit_sms ( void )
{
  ...
  if (result != WMS_CLIENT_ERR_NONE)
  {
   DS_ATCOP_ERROR_LOG_0("Could not deregister call back function with WMS");
  }
...
)
{
  ...
  if( dsatsmsi_pres_cmd.cmd_name == SMS_CPMS )
  {
    ...
    /* MO template is also added now. The side effects is out of scope; 
       ...
       causes any  trouble. */
    DSATSMS_ASSERT((((mem_status_ptr->tag == WMS_TAG_NONE)||
           (mem_status_ptr->tag == WMS_TAG_STATUS_RPT) ||
           (mem_status_ptr->tag == WMS_TAG_MO_TEMPLATE )) == TRUE));
    ...
    if (dsatsmsi_mem_report.mem == CPMS_MEM3) 
    {
      ...
      if (dsatsmsi_mem_report.is_read) 
      {
             ...
             (list_ptr_m3 == NULL) )
        {
          /* Its a bigger problem, we are neither in ETSI mode, 
             nor in CDMA mode */
         DS_ATCOP_ERROR_LOG_0("Wrong operating mode  ");
          DSAT_SMS_STATE_CLEAN();
          return DSAT_ERROR;
        } 
      ...
      if( dsatsmsi_mem_report.mem3_changed )
      {
        ...
        {
          result = dsatsmsi_route_config (CNMI_GET_CURRENT_MT_VAL,CNMI_GET_CURRENT_DS_VAL,event_info->subs_id); 
        }
#endif /* FEATURE_ETSI_SMS */
        else
        {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Wrong cpms mem =d",dsatsmsi_mem_report.mem);
      DSAT_SMS_STATE_CLEAN();
      return DSAT_ERROR;
    }
          ...
          ( !dsatsmsi_cmd_sm.one_index ))
  {
    ...
    else
    {
      ...
      *data_ptr++ = '(';
      ...
      {
        data_ptr = psutil_itoa(event_info->u.msg_list.indices[loop],
                                 data_ptr,
                                 10);
        *data_ptr++ = ',';
      }
...
)
{
  ...
  if ( event_info->u.msg_list.len == 0 ) 
  {
    ...
    if (event_info->u.msg_list.mem_store == WMS_MEMORY_STORE_SIM )
    {
       if ( wms_cfg_ms_get_message_list( dsatsmsi_client_id,
                                         ...
                                         WMS_TAG_MO_TEMPLATE ) != WMS_OK_S )
       {
        DS_ATCOP_ERROR_LOG_0("Failed to get msg list info from WMS");
         is_pending = FALSE;
       }
    }
    else
  ...
  else
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Failed to read template from WMS");
      is_pending = FALSE;
    }
  }
