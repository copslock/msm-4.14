...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatcmif.c#3 $ $DateTime: 2019/11/09 00:16:19 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/31/19   ks      Enhanced COPS to support NR5G.
...
09/15/15   skc     Ignoring the SYS_SRV_STATUS_LIMITED_REGIONAL when req_id matched
02/17/15   tk      Fixed issue in subscription not available event handling.
02/12/15   tk      Fixed +COPS issue in handling of CM PH command errors.
...
06/11/09   nc      Modified the prototype of dsatcmif_answer_call 
...
#define DSATCMIF_ASSERT(expression) \
         dsatcmif_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATCMIF_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATCMIF ASSERT
...
===========================================================================*/
static void dsatcmif_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATCMIF_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatcmif_assert_wrapper */
...
#define CALL_END 6
  ...
  DSATCMIF_ASSERT((call_event != NULL));
  ...
  if (NULL != res_buff)
  {
    ...
    /* <mpty>,[bs_type] and [cause] TBD */
      ...
      res_buff->used = (word)snprintf ((char*)data_ptr,
                                               ...
                                               stat,((call_event->event_info.call_type == CM_CALL_TYPE_VOICE ||
                                               call_event->event_info.call_type == CM_CALL_TYPE_VT)? 0:1)  );
  ...
  switch (call_event->event)
  {
    ...
    case CM_CALL_EVENT_INCOM:
      ...
      if ( IS_ETSI_MODE(current_mode) )
      {
        ...
        /* prepare the received number for saving to the received ph book */
        {
          uint8 type =  0x80;
		  
          /* Type bits - bit7 = 1, bits 6-4 = number type, bits 3-0 = number plan */
          type |= ((call_event->event_info.num.number_type & 0x07) << 4);
          type |= (call_event->event_info.num.number_plan & 0x0F);
          dsatme_last_call_init_record((char *)call_event->event_info.num.buf, type, NULL, LAST_CALL_RECEIVED);
        }
...
#ifdef FEATURE_DSAT_ETSI_MODE
      /* If the call_state is CM_CALL_STATE_INCOM then
         Report +CCWA unsolicited result code dependent
         upon active call or not */
      if ( IS_ETSI_MODE(current_mode) )
      {
        if (call_event->event_info.call_state != CM_CALL_STATE_INCOM)
        {
          break;
        }
        if(call_event->event_info.report_ccwa == TRUE)
    ...
    case CM_CALL_EVENT_MODIFY_COMPLETE_CONF:
      /* No more events related to call modification, clear requested call type */
      call_da_val->call_modify_param.req_call_type = CM_CALL_TYPE_NONE;
      seq_num = dsatact_callid_to_seqnum(subs_id, call_event->event_info.call_id);
      /* Check if AT+CCMMD was executed */
      if(CHECK_PENDING(DSATETSI_EXT_ACT_CCMMD_ETSI_IDX ,0, DSAT_PENDING_TRUE))
      {
        SET_PENDING(DSATETSI_EXT_ACT_CCMMD_ETSI_IDX ,0, DSAT_PENDING_FALSE)
        /* Return OK if CCMMD was REJECT and modify error code comes with UPGRADE/DOWNGRADE REJECT */
        if(call_da_val->call_modify_param.last_ccmmd_neg_status == DSAT_NEG_STATUS_REJECT &&
           call_event->event_info.dsat_mode_info.info.ip_call.modify_error_code == CM_CALL_END_UPGRADE_DOWNGRADE_REJ)
        {
          call_da_val->call_modify_param.last_ccmmd_neg_status = DSAT_NEG_STATUS_NONE;
          result = DSAT_OK;
          break;
        }
        /* Update Call type if call modification was successfull */
        if(!call_event->event_info.dsat_mode_info.info.ip_call.call_modify_err_status &&
           seq_num != CALL_SEQNUM_INVALID)
        {
          call_da_val->dsat_seqnum_callid[seq_num - 1].call_type = call_event->event_info.call_type;
          result = DSAT_OK;
        }
        else
        {
      ...
      {
        /* Update call type; other client may have requested for call modification */
        if(!call_event->event_info.dsat_mode_info.info.ip_call.call_modify_err_status &&
           seq_num != CALL_SEQNUM_INVALID)
        {
          call_da_val->dsat_seqnum_callid[seq_num - 1].call_type = call_event->event_info.call_type;
    ...
    case CM_CALL_EVENT_MODIFY_IND:
      /*Saving requested call type which will be used in AT+CCMMD command for accepting or 
        rejecting the request */
      call_da_val->call_modify_param.req_call_type = call_event->event_info.call_type;
      break;
    case CM_CALL_EVENT_MODIFY_REQ:
    case CM_CALL_EVENT_MODIFY_RES:
      /* Wait for Call modification complete confirmation event*/
      result = DSAT_ASYNC_EVENT;
      break;
    default:
      break;
...
#endif /* FEATURE_DSAT_ETSI_MODE*/
  /* Report +CMCCSI URC if enabled*/
  dsatetsicmif_report_cmccs(subs_id, call_event);
...
)
{
  ...
  switch (call_cmd->cmd)
  {
    ...
    case CM_CALL_CMD_MODIFY_RES:
      /* Wait for CM Events if no error */
      result = (call_cmd->cmd_err != CM_CALL_CMD_ERR_NOERR)? DSAT_ERROR : DSAT_ASYNC_EVENT;
      break;
    default:
    break;
  }
...
)
{
  ...
  else if ( DSAT_COPS_MODE_AUTO == ph_ss_ms_val->net_pref.mode )
  {
   ...
   {
     DS_ATCOP_ERROR_LOG_0(" Other modes should not invoke subcription available");
   }
...
)
{
  ...
  switch (event_ptr->event)
  {
    ...
    default:
     DS_ATCOP_ERROR_LOG_0("Unexpected CM Serving System event");
      break;
  }
...
)
{
  ...
  if( CDMA_CMD_MODE == dsatcmdp_get_operating_cmd_mode() )
  {
    ...
    switch( dsat_sysinfo_cmd.dsat_rtre_config )
    {
      ...
      default:
       DS_ATCOP_ERROR_LOG_0("RTRE control has incorrect value");
    }
...
dsat_result_enum_type dsatcmif_get_cm_ss_info ( void )
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Could not request CM SS phone info");
    result = DSAT_ERROR;
  }
...
// philip, 20180810, [Generic][TMO] Set preferred band
      ...
      if ( CHECK_NOT_PENDING(DSAT_ASKEY_ACT_SET_PERFERRED_BAND_IDX ,0,DSAT_PENDING_FALSE) )
      {  
...
// jelly, Set prefnet
	  ...
	  if ( CHECK_NOT_PENDING(DSAT_ASKEY_ACT_PREFNET_IDX ,0,DSAT_PENDING_FALSE) )
      {  
      ...
      if (CM_PH_CMD_ERR_NOERR != ph_cmd->cmd_err)
      {
        if ( CHECK_NOT_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_NONE))
        {
  ...
  default:
    DS_ATCOP_ERROR_LOG_1("Unsupport CM PH command: =d",ph_cmd->cmd);
    result = DSAT_ERROR;
    break;
  }
...
)
{
  ...
  dsatcmif_servs_state_ms_info    *ph_ss_ms_val = NULL;
  dsatetsicall_call_state_ms_info *call_ms_val = NULL;
  dsat_cops_async_state_e_type     prev_cmd_state = DSAT_COPS_ASTATE_NULL;
...
// philip, 20180810, [Generic][TMO] Set preferred band
#ifdef ASKEY_SET_PREFERRED_BAND
	  if ( CHECK_PENDING(DSAT_ASKEY_ACT_SET_PERFERRED_BAND_IDX ,0,DSAT_PENDING_TRUE) )
	  {  
	    SET_PENDING(DSAT_ASKEY_ACT_SET_PERFERRED_BAND_IDX ,0, DSAT_PENDING_FALSE)
		result = DSAT_OK;
	  }
...
// jelly, Set prefnet
#ifdef ASKEY_PREF_NET
	  if ( CHECK_PENDING(DSAT_ASKEY_ACT_PREFNET_IDX ,0,DSAT_PENDING_TRUE) )
      {  
        SET_PENDING(DSAT_ASKEY_ACT_PREFNET_IDX ,0, DSAT_PENDING_FALSE)
        result = DSAT_OK;
      }
    ...
    if (TRUE == cmif_is_gw_subscription_available( subs_id, ph_event))
    {
            ...
            but after deregistration command */
      {
        ...
        {
#endif /* FEATURE_SGLTE */
          prev_cmd_state = ph_ss_ms_val->dsatetsicall_network_list.cmd_state;
          result = cmif_subscription_available_process_cops(ph_ss_ms_val, subs_id);
          /* reset the net_pref structure and make the boolean false */
          ph_ss_ms_val->net_pref.subscription_invoked = FALSE;
#ifdef FEATURE_SGLTE
      ...
      {
        if(prev_cmd_state != ph_ss_ms_val->dsatetsicall_network_list.cmd_state)
        {
          return result;
        }
        else
    ...
    case CM_PH_EVENT_INFO:
    {
...
#ifdef CM_API_AOC_INFO
     {
        if (TRUE == ph_event->event_info.aoc_info.aoc_ready)
       {
        ...
        {
          call_ms_val->dsat_acm_res_type = DSAT_ACM_NONE;
         DS_ATCOP_ERROR_LOG_0("Advice of Charge is invalid/not ready");
          result = dsat_send_cme_error(DSAT_CME_SIM_BUSY);
        }
      }  
      ...
      {
        call_ms_val->dsat_acm_res_type = DSAT_ACM_NONE;
       DS_ATCOP_ERROR_LOG_0("CM_API_AOC_INFO not defined");
        result = dsat_send_cme_error(DSAT_CME_OP_NOT_SUPPORTED);
      }
#endif /* CM_API_AOC_INFO */
       else if ( ph_ss_ms_val->dsat_ws46_res_type != DSAT_WS46_NONE )
      {
        result = dsatact_process_ws46_cmd (&ph_event->event_info.pref_info, subs_id);
      }
#endif /* FEATURE_DSAT_ETSI_MODE */
    ...
    case CM_PH_EVENT_TERMINATE_GET_NETWORKS:
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Received Terminate GET_NETWORKS event from CM");
        ph_ss_ms_val->dsatetsicall_network_list.cmd_state = DSAT_COPS_ASTATE_NULL;
        ph_ss_ms_val->dsatetsicall_network_list.cmd_idx = CMD_IDX_NONE;
    ...
    default:
    { 
      DS_ATCOP_ERROR_LOG_1("Unsupported CM PH event: =d", ph_event->event);
      break;
    }
  }
  return result;
...
)
{
  ...
  DSATCMIF_ASSERT (((list_ptr != NULL) == TRUE));
  ...
  info_list_ptr->number_of_voice_calls = 0;
  for (i = 0; i < list_ptr->number_of_active_calls; i++)
  {
    ...
    /* Count number of voice calls */
    if(info_list_ptr->info[i].call_type == CM_CALL_TYPE_VOICE ||
       info_list_ptr->info[i].call_type == CM_CALL_TYPE_VT)
    {
      info_list_ptr->number_of_voice_calls++;
    }
  }
       ...
       ( DSAT_COPS_ASTATE_WAIT_2 == ph_ss_ms_val->dsatetsicall_network_list.cmd_state ) )
  {
    ...
    /* Change to auto registration on mismatch. Set COPS mode accordingly */
          ...
          /* For manual, mismatch means no requested network service */
                    ...
                    ph_ss_ms_val->dsatetsicall_network_list.requested_pref.mode )
          {
           DS_ATCOP_ERROR_LOG_0 ("COPS manual reg NO SRV");
            ph_ss_ms_val->dsatetsicall_network_list.cmd_state = DSAT_COPS_ASTATE_NULL;
            ph_ss_ms_val->dsatetsicall_network_list.cmd_idx = CMD_IDX_NONE;
          ...
          /* incase of QCCOPS command even in auto mode we need to wait for request ID match*/
              ...
              && (ph_ss_ms_val->dsatetsicall_network_list.cmd_idx == CMD_IDX_QCCOPS ||
                  ph_ss_ms_val->dsatetsicall_network_list.cmd_idx == CMD_IDX_QCCSGCOPS))
          {
           DS_ATCOP_ERROR_LOG_0 ("QCCOPS/CSGCOPS auto reg NO SRV");
            ph_ss_ms_val->dsatetsicall_network_list.cmd_state = DSAT_COPS_ASTATE_NULL;
            ph_ss_ms_val->dsatetsicall_network_list.cmd_idx = CMD_IDX_NONE;
            result = dsat_send_cme_error(DSAT_CME_NO_NETWORK_SERVICE);
          }
         ...
         ( SYS_SYS_MODE_HDR == sys_mode ) ) )
  {
    ...
    if (NULL != res_buff_ptr)
    {
      res_buff_ptr->used = (word)snprintf ((char*)res_buff_ptr->data_ptr,
                                           ...
                                           "^MODE:%d",
                                           sys_mode);
      dsatcmdp_send_urc_per_subs(dsat_get_current_1x_subs_id(), res_buff_ptr, DSAT_COMPLETE_RSP);
    }
...
)
{
  ...
  DSATCMIF_ASSERT( ((inband_info_ptr != NULL) == TRUE) );
