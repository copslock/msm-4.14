...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2001 - 2012,2014-2016 by Qualcomm Technologies Incorporated.
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatprep.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
-------    ---     ---------------------------------------------------------
10/30/17   rk      Removed DS Flag FEATURE_DSAT_EXT_CLIENT_SUPPORT.
09/23/16   skc     Fixed UE Crash issue seen during processing AT Commands as part
                   of regression testing.
08/29/16   skc     Fixed issue with AT Command processing in multiport scenario.
...
#define DSATPREP_ASSERT(expression) \
         dsatprep_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATPREP_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATPREP ASSERT
...
===========================================================================*/
static void dsatprep_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATPREP_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatprep_assert_wrapper */
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Invalid SIO port: =d",port);
    return;
  }
...
#endif /* defined(FEATURE_ETSI_SMS) || defined(FEATURE_ETSI_SMS) */
  default:
    DS_ATCOP_ERROR_LOG_1("Invalid SIO data preprocessor mode =d", mode);
  }
...
)
{
  ...
  */
  ...
  if(abort_call == FALSE &&
     (active_port = ds3g_siolib_get_ds3g_siolib_active_port()) != DS3G_SIOLIB_PORT_NONE &&
     atprep_dsm_item_ptr == NULL &&
     ...
     preprocessor_mode == DSATI_AT_CMD_MODE)
  {
    ...
    ** is non empty.
    */
    state = ds3g_siolib_get_ds3g_siolib_state(active_port);
    if(NULL == state)
    {
      return abort_call;
    }
    DS_AT_MSG2_HIGH("Active port %d current_cnt %d", active_port, state->serial_info.rx_wm_ptr.current_cnt);
    if(state->serial_info.rx_wm_ptr.current_cnt == 0)
    {
      ds3g_siolib_set_ds3g_siolib_active_port(DS3G_SIOLIB_PORT_NONE);
    }
  }
 ...
 ~>|  HUNT  | /,A,a  | FOUND_A |------>| FOUND_AT|                   +-------+
           ...
           (atprep_dsm_item_ptr = dsm_dequeue( wm_item_ptr )) != NULL) )
  {
    ...
    while ( processing_cmd_line && atprep_dsm_item_ptr->used )
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Invalid SIO port: =d freed remain DSM item in WM ",port);
        do
        {
          if( atprep_dsm_item_ptr != NULL)
          {
      ...
      case DSAT_CMD_PREP_STATE_FOUND_A:              
        ...
        else if ( UPCASE( cc ) != 'A' )
        {
          ...
          if ( !(wm_item_ptr->current_cnt) && !(atprep_dsm_item_ptr->used) ) 
          {
            ...
            {
	      if( dsatcmdp_processing_async_cmd != TRUE)
	      {
  ...
  DSATPREP_ASSERT( echo_item_ptr == NULL );
  if(at_detect == FALSE && dsatcmdp_processing_async_cmd == FALSE) 
  {
         ...
         cscs_val =(dsat_num_item_type)dsatutil_get_val(
               ...
               || (!unicode_mode && (cc == '\n' || cc == '\r' )) )
          {
            ...
            else
            {
             ...
             DS_ATCOP_ERROR_LOG_0("SMS message is too large!");
            }
          }
