...
$Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsat707mip.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/16   tk      Optimized debug macros usage in ATCoP.
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == NA)) 
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCMIP: Invalid Number of arguements  =d", tok_ptr->args_found);
      return DSAT_ERROR;
    }
    ds_nv_item.ds_qcmip = (byte) temp_val;
    (void) dsatutil_put_nv_item_per_subs( NV_DS_QCMIP_I, &ds_nv_item,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == NA)) 
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCMIPP: Invalid Number of arguements  =d", tok_ptr->args_found);
      return DSAT_ERROR;
    }
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == NA)) 
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCMIPT: Invalid Number of arguements  =d", tok_ptr->args_found);
      return DSAT_ERROR;
    }
    ds_nv_item.ds_mip_2002bis_mn_ha_auth = (byte) temp_val;
    (void) dsatutil_put_nv_item_per_subs( NV_DS_MIP_2002BIS_MN_HA_AUTH_I, &ds_nv_item,
                                          dsat_get_current_1x_subs_id() );
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
  {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == NA)) 
  {
    ...
    (void) dsatutil_get_nv_item_per_subs(NV_DS_MIP_ENABLE_PROF_I,&ds_nv_item,
                                               dsat_get_current_1x_subs_id());
    dsmip_get_1x_profile_val_num(DS_QCMIPP_VAL,(void *)&temp_index,0);
    ds_nv_item.ds_mip_enable_prof[temp_index ] = (byte) temp_val;
    (void) dsatutil_put_nv_item_per_subs(NV_DS_MIP_ENABLE_PROF_I,&ds_nv_item,
                                               dsat_get_current_1x_subs_id());
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
  {
    ...
    case (NA|QU):     /* --------------     query value    -------------- */
      ...
      else
      {
        ...
        else
        {
          ...
          (void) dsatutil_get_nv_item_per_subs(NV_DS_MIP_SS_USER_PROF_I,&ds_nv_item,
                                               dsat_get_current_1x_subs_id());
        } 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      ...
      if(NULL == temp_shadow_nv)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCMIPMASPI: Failed to update profile,arg_found =d", tok_ptr->args_found);
      result = DSAT_ERROR;
    }
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
...
*)temp_val,2)) )
      ...
      if(NULL == temp_shadow_nv)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCMIPMHSPI: Failed to update profile,arg_found =d", tok_ptr->args_found);
      result = DSAT_ERROR;
    }
    ...
    case (NA|QU):     /* ---------------     query value    -------------- */
      ...
      else
      {
        ...
        else
        {
          ...
          atmip_nv_status = dsatutil_get_nv_item_per_subs( NV_DS_MIP_SS_USER_PROF_I,
                                                           &ds_nv_item,
                                                           dsat_get_current_1x_subs_id() );
         ...
         (TRUE == dsmip_set_1x_profile_val_num(DS_QCMIPRT_VAL,(void *)temp_val,2)) )
  {
      ...
      switch(temp_val[0])
    {
      ...
      case 1:
        ...
        (void) dsatutil_get_nv_item_per_subs(NV_DS_MIP_GEN_USER_PROF_I,&ds_nv_item,
                                    dsat_get_current_1x_subs_id());

          ds_nv_item.ds_mip_gen_user_prof.rev_tun_pref =((dsat_num_item_type)temp_val[0] ? TRUE : FALSE);
        atmip_nv_status = dsatutil_put_nv_item_per_subs( NV_DS_MIP_GEN_USER_PROF_I, 
                                                &ds_nv_item,
                                               dsat_get_current_1x_subs_id() );
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCMIPRT: Failed to update profile,arg_found =d", tok_ptr->args_found);
      result = DSAT_ERROR;
    }  
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
      DS_ATCOP_ERROR_LOG_1("$QCMIPGETP: Invalid Number of arguements  =d", tok_ptr->args_found);
    return DSAT_ERROR;
  }
    ...
    atmip_nv_status = dsatutil_get_nv_item_per_subs(NV_DS_MIP_GEN_USER_PROF_I, &ds_nv_item,
                                               dsat_get_current_1x_subs_id());
    if(atmip_nv_status != NV_DONE_S)
    {
      return DSAT_ERROR;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    switch(tok_ptr->args_found)
    {
      ...
      default:
       DS_ATCOP_ERROR_LOG_0("Incorrect number of args found!");
        result = DSAT_ERROR;
        return result;
    }
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Second argument must be 0 or 1");
      result = DSAT_ERROR;
      return result;
    }
    ...
    else
    {
      ...
      switch(nv_pref)
      {
        ...
        case '1':
          ...
          (void) dsatutil_get_nv_item_per_subs( NV_DS_MIP_GEN_USER_PROF_I,
                                       &ds_nv_item,
                                               dsat_get_current_1x_subs_id() );
        ...
        default:
         DS_ATCOP_ERROR_LOG_0("Invalid NV preference");
          result = DSAT_ERROR;
          break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  rb_ptr = psutil_itoa((uint16) profile, rb_ptr, 10);
  *rb_ptr++ = ' ';
  ...
  *rb_ptr++ = (byte) temp_s4_val;
  ...
  rb_ptr = psutil_itoa(ds_nv_item.ds_mip_gen_user_prof.mn_aaa_spi,
                         rb_ptr,
                         10);
  ...
  *rb_ptr++ = (byte) temp_s4_val;
  ...
  rb_ptr = psutil_itoa(ds_nv_item.ds_mip_gen_user_prof.mn_ha_spi,
                         rb_ptr,
                         10);
  ...
  *rb_ptr++ = (byte) temp_s4_val;
  ...
  rb_ptr = psutil_itoa(ds_nv_item.ds_mip_gen_user_prof.rev_tun_pref,
                         rb_ptr,
                         10);
          ...
          case '1':
            ...
            *shadow_nv_ip_set = FALSE;
            ...
            switch(table_entry->special)
            {
              ...
              default:
               DS_ATCOP_ERROR_LOG_0("Unknown AT command");
                result = DSAT_ERROR;
                break;
            }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( dsat707_strip_quotes( (char *) tok_ptr->arg[0],
                            ...
                            table_entry->compound ) == FALSE )
  {
   DS_ATCOP_ERROR_LOG_0("Bad quotes (or input too long)!");
    return DSAT_ERROR;
  }
    ...
    case '1':
      ...
      *shadow_nv_ss_len = 0;
      ...
      /*---------------------------------------------------------------------
        Write the profile with the new value to NV.
      ---------------------------------------------------------------------*/
      atmip_nv_status = dsatutil_put_nv_item_per_subs( NV_DS_MIP_SS_USER_PROF_I, 
                                                      &ds_nv_item,
                                                      dsat_get_current_1x_subs_id());
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid hex string!");
    return DSAT_ERROR;
  }
