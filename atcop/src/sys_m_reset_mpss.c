...
$Header: //components/rel/core.mpss/9.0.1/debugtools/sys_m/src/sys_m_reset_mpss.c#1 $
...
extern void sys_m_process_rcinit_shutdown_init(void);
...
{
   SYS_M_LOCK_INIT();
   sys_m_process_rcinit_shutdown_init();
}
...
void sys_m_initiate_shutdown(void)
{
   ...
   TMS_SHUTDOWN_MSG_HIGH_1("Initiate shutdown from 0x%x task context", qurt_thread_get_id());

   if (0 < rcecb_getregistrants_name(SYS_M_RCECB_FWD_SHUTDOWN)) // count of registrants
   {
...
void sys_m_initiate_poweroff(void)
{
   ...
   TMS_SHUTDOWN_MSG_HIGH_1("Initiate Power off from 0x%x context", qurt_thread_get_id());
...
ssreq_status_t sys_m_initiate_restart_ssreq(ssreq_qmi_ind_cb_fn_type *ind_cb_fn)
{
   ...
   if(TRUE == get_ssreq_client_init_status())
   {
       ...
       TMS_SHUTDOWN_MSG_HIGH_2("System restart SSREQ over QMI from 0x%x, ret: %d", qurt_thread_get_id(), ssreq_ret);
   }
   SYS_M_UNLOCK();
   return ssreq_ret;
}
...
ssreq_status_t sys_m_request_peripheral_restart_ssreq(ssreq_qmi_ind_cb_fn_type *ind_cb_fn)
{
   ...
   if(TRUE == get_ssreq_client_init_status())
   {
       ...
       TMS_SHUTDOWN_MSG_HIGH_2("Peripheral restart SSREQ over QMI from 0x%x, ret: %d", qurt_thread_get_id(), ssreq_ret);
   }
   SYS_M_UNLOCK();
   return ssreq_ret;
}
...
)
{
   ...
   if(TRUE == get_ssreq_client_init_status())
   {
       ...
       TMS_SHUTDOWN_MSG_HIGH_3("System restart reason: %d SSREQ over QMI from 0x%x, ret: %d", res_code, qurt_thread_get_id(), ssreq_ret);
   }
   SYS_M_UNLOCK();
   return ssreq_ret;
}
...
)
{
   ...
   if(TRUE == get_ssreq_client_init_status())
   {
       ...
       TMS_SHUTDOWN_MSG_HIGH_3("Peripheral restart reason: %d SSREQ over QMI from 0x%x, ret: %d", res_code, qurt_thread_get_id(), ssreq_ret);
   }
   SYS_M_UNLOCK();
   return ssreq_ret;
}
