...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsat707smsc.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
/* Read NV_RTRE_CONFIG_I to determine RUIM-Config */
  ...
  if ( tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == (NA|QU)) )
  {
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("QCPMS: mem1 is needed");
#ifdef FEATURE_DSAT_EXTENDED_SMS
        res_buff_ptr->used = 
  ...
  {
   DS_ATCOP_ERROR_LOG_0("$QCPMS need at least one argument");
#ifdef FEATURE_DSAT_EXTENDED_SMS
    res_buff_ptr->used = 
      dsatsmsi_cms_error(res_buff_ptr, (int)DSAT_CMS_OP_NOT_ALLOWED);
    return DSAT_CMD_ERR_RSP;
...
/*-------------------------------------------------------------------------*/
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
                                ...
                                DSAT707_SMS_QCMGF_IDX,0,0,NUM_TYPE))
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("More then two Args in QCMGS");
        return DSAT_ERROR;
      }
      ...
      if(*tok_ptr->arg[0] != '\0' )
      {
        ...
        if ( !dsatsmsi_strip_quotes( tok_ptr->arg[0], tok_ptr->arg[0], len_quoted_string) )
        {
         ...
         DS_ATCOP_ERROR_LOG_0("<oa> without quotes - not supported");
          res_buff_ptr->used = (uint16) dsatsmsi_cms_error(
        ...
        {
         DS_ATCOP_ERROR_LOG_0("<oa> length greater than 20 not supported");
          res_buff_ptr->used = (uint16) dsatsmsi_cms_error(
            res_buff_ptr, (int)DSAT_CMS_INVALID_TXT_PARAM);
          result = DSAT_CMD_ERR_RSP;
    ...
    else
    {
      ...
      DS_ATCOP_ERROR_LOG_1("PDU Mode not supported in Cmd =d",dsatsmsi_pres_cmd.cmd_name);
      return DSAT_ERROR;
    }
  } /* ( tok_ptr->op == (NA|EQ|AR) ) */
...
#endif /* FEATURE_DSAT_EXTENDED_SMS */
    {
      DS_ATCOP_ERROR_LOG_1("Invalid =d Arg in QCMGR",tok_ptr->args_found);
      return DSAT_ERROR;
    }
...
/*-------------------------------------------------------------------------*/
  ...
  if ( ( tok_ptr->op == (NA|EQ|AR) ) || ( tok_ptr->op == (NA) ) )
  {
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      if( tok_ptr->args_found > 1 )
      {
       DS_ATCOP_ERROR_LOG_0("More then one Args in QCMGL");
        return DSAT_ERROR;
      }
#ifdef FEATURE_DSAT_EXTENDED_SMS
      if (TRUE == temp_hc_cmd )
      ...
      if ( !tag )
      {
        /* Bad Tag */
        /* Return an error */
        DS_ATCOP_ERROR_LOG_1("Bad tag input in =d cmd ",dsatsmsi_pres_cmd.cmd_name);
        DSAT_SMS_STATE_CLEAN();
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    if( tok_ptr->args_found > 3 )
    {
     DS_ATCOP_ERROR_LOG_0("More then three Args in QCMSS");
      return DSAT_ERROR;
    }
    ...
    if( tok_ptr->args_found == 1 )
    {
      ...
      if (msg_status  != WMS_OK_S)
      {
       DS_ATCOP_ERROR_LOG_0("Error sending the SMS");
        DSAT_SMS_STATE_CLEAN();
        result =  DSAT_ERROR;
      }
...
)
{
  ...
  {
    /* PDU Mode not Supported */
   DS_ATCOP_ERROR_LOG_0("PDU Mode not Supported");
