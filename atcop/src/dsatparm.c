...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatparm.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
             ...
             (CONFIG) )
    {
      ...
      if (atoi_return == ATOI_OK)
      {
        ...
        if (0 == (mixed_def_list[index]->attrib & BROKEN_RANGE))
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1("The argument =d is out of range",arg_in_num);
            return FALSE;
          }
          else
          {
        ...
        else
        {
          ...
          while (MAX_BROKEN_RANGE != dflm_ptr->default_v)
          {
            ...
            if ( (dflm_ptr - dflm_ptr_orig) >= 2)
            {
             ...
             DS_ATCOP_ERROR_LOG_0("Currently this is not supported ");
              return FALSE;
            }
          }
          if (FALSE == hit)
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Argument cannot be converted to number");
        return FALSE;
      }
