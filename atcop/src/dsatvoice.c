...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatvoice.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/13/17   skc     Added support for VT Call.
08/01/17   skc     Fixed $QCCHV issue
10/29/15   skc     Fix added to handle aborting of ATD when there
...
)
{
  ...
  for ( call_id = 0; call_id < CM_CALL_ID_MAX; call_id++ )
  {
    ...
    if ( voice_state[subs_id][call_id].state == DSAT_VOICE_STATE_WAITING )
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("ATCOP's answer-call is denied by CM");
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Could not find DSAT_VOICE_STATE_CONNECTING voice call");
    return result;
  }
...
)
{
  ...
  else
  {
   ...
   DS_ATCOP_ERROR_LOG_0("End call failed");
#ifdef FEATURE_DSAT_ETSI_MODE
    if ( IS_ETSI_MODE(current_mode) )
    {
