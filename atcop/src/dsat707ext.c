...
Copyright (c) 1995 - 2017 by Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsat707ext.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/30/17   rk      Fixed KW issue.
08/18/17   rg      Added  +CDV support for 1x on secondary sub 
02/07/17   skc     Removed limitation of slot id dependancy in Single SIM mode.
04/01/16   tk      ATCoP changes for dynamic memory optimizations.
...
#define DSAT707EXT_ASSERT(expression) \
         dsat707ext_assert_wrapper(__LINE__, expression)
...
FUNCTION DSAT707EXT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSAT707EXT ASSERT
...
===========================================================================*/
static void dsat707ext_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSAT707EXT_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsat707ext_assert_wrapper */
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("tok_ptr->arg[0]= NULL");
    return DSAT_ERROR;
  }
  ...
  {
   DS_ATCOP_ERROR_LOG_0("empty or error in dial string");
    return DSAT_ERROR;
  }
  ...
  if (voice_call_id == CM_CALL_ID_INVALID)
  {
    ...
    call_status = cm_mm_call_cmd_orig_per_subs(dsatcmif_call_cmd_cb_func,
                                      NULL,
                                      dsatcm_client_id,
                                      CM_CALL_TYPE_VOICE,
    ...
    else
    {
     ...
     DS_ATCOP_ERROR_LOG_0("call status error");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == NA)) 
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("+CRM: Invalid Number of arguements", tok_ptr->args_found);
      return DSAT_ERROR;
    }
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
...
/* Read command */
  if (tok_ptr->op == NA)
  {
    ...
    rb_ptr += snprintf( (char *)rb_ptr, res_buff_ptr->size,
                              "%s: ", tab_entry->name );
#endif/* FEATURE_DSAT_EXTENDED_CMD */
    rb_ptr = psutil_itoa (temp, rb_ptr, 10);
...
)
{
  ...
  /* Supported only Name Only and Read command */
  if (tok_ptr->op == (NA) ||
      tok_ptr->op == (NA|QU))
  {
    memset((void*)&dbi , 0, sizeof(dbi) );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if (tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == NA)) 
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("+CTA: Invalid Number of arguements  d", tok_ptr->args_found);
      return DSAT_ERROR;
    }
  }
  else if (tok_ptr->op == (NA|EQ|QU) )
...
#define RETURN_RESPONSE() \
  ...
  if (tok_ptr->op == NA)
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Error retrieving IMSI MCC from NV");
      RETURN_RESPONSE()
    }
    else
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Error retrieving IMSI MNC from NV");
        RETURN_RESPONSE()
      }
      else
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Error retrieving MIN2 from NV");
          RETURN_RESPONSE()
        }
        else
        {
          ...
          {
           DS_ATCOP_ERROR_LOG_0("Error retrieving MIN1 from NV");
            RETURN_RESPONSE()
          }
        }
      }
...
)
{
  ...
  if ((tok_ptr->op == NA)||(tok_ptr->op == (NA|QU)))
  {
    ...
    /* MDN Reporting */
    nv_status = dsatutil_get_nv_item_per_subs (NV_DIR_NUMBER_PCS_I, &ds_nv_item,
...
)
{
  ...
  DSAT707EXT_ASSERT( ((( (tab_entry->attrib & HEX) == 0 ) && 
          ( (tab_entry->attrib & LIST) != 0 )) == TRUE) );
  ...
  {
   DS_ATCOP_ERROR_LOG_0("DSAT_ERROR");
    return(DSAT_ERROR);
  }

  if ( tok_ptr->args_found  > MAX_ARG )
  {
   DS_ATCOP_ERROR_LOG_0("Invalid number of args found");
    return DSAT_ERROR;
  }
...
)
{
  ...
  case MMGSDI_CODE_BLOCKED:
    ...
    case DSAT_CDMA_CPWD_MMGSDI_PIN1_CHANGE:
      ...
      {
       DS_ATCOP_ERROR_LOG_0("PIN1 status is permanently blocked");
        result = dsat_send_cme_error ( DSAT_CME_SIM_FAILURE );
      }
      else
      {
    ...
    case DSAT_CDMA_CPWD_MMGSDI_PIN2_CHANGE:
      ...
      {
       DS_ATCOP_ERROR_LOG_0("PIN2 status is permanently blocked");
        result = dsat_send_cme_error ( DSAT_CME_SIM_FAILURE );
      }
      else
      {
...
#define PIN_LEN_OK(pinstr)\
  ...
  if ( IS_707_PUK_STATE(DSAT_CPIN_SIM_PIN_REQUIRED) )
  {
    ...
    {
      result = ext_cpin_unblock_pin ( tok_ptr, MMGSDI_PIN1 );
    }
    else
    {
      result = DSAT_ERROR;
  ...
  else if ( IS_707_PUK_STATE(DSAT_CPIN_SIM_PIN2_REQUIRED) )
  {
    ...
    if ( PIN_LEN_OK(temp0_mix_type) && 
         PIN_LEN_OK(temp1_mix_type) ) 
    {
      result = ext_cpin_unblock_pin ( tok_ptr, MMGSDI_PIN2 );
    }
    else
    {
      result = DSAT_ERROR;
...
)
{
  ...
  if( CHECK_PENDING(DSAT707_EXT_ACT_QCPIN_IDX,0,DSAT_PENDING_CLCK_MMGSDI_FAC_QUERY_CMD) )
  {
    ...
    else
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("MMGSDI call was not sucess status=d",cmd_info_ptr->cmd_status);
        return dsat_send_cme_error(DSAT_CME_SIM_FAILURE);
      }
      result = DSAT_OK;
      ...
      switch (cmd_info_ptr->pin1.status)
      {
        ...
        case MMGSDI_PIN_PERM_BLOCKED:
         DS_ATCOP_ERROR_LOG_0("PIN1 is permanently blocked");
          result = dsat_send_cme_error(DSAT_CME_SIM_FAILURE);
          break;
  ...
  if (dsat707_pin_error == DSAT_CME_NO_ERROR)
  {
    ...
    else if ( CHECK_PENDING(DSAT707_EXT_ACT_QCPIN_IDX,0,DSAT_PENDING_CPIN_MMGSDI_EXECUTE_CMD))
    {
          ...
          ))) )
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Null new PIN string invalid");
          return DSAT_ERROR;
        }
      }
    ...
    if (DSAT_OK == result)
    {
      ...
      for(index = 0; index < 2 && VALID_TOKEN(index); index++)
      {
        ...
        while ( '\0' != *c_ptr )
        {
          ...
          if (!(('0' <= *c_ptr) && ('9' >= *c_ptr)))
          { 
           DS_ATCOP_ERROR_LOG_0("PIN char outside range [0-9]");
            return DSAT_ERROR;
          }
          c_ptr++;
        }
      ...
      {
        DS_ATCOP_ERROR_LOG_1("MMGSDI returned false for PIN status query =d",
                   (int)mmgsdi_status); 
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
   ...
   /* dsat707_clck_fac_valstr[] */
    ...
    if (VALID_TOKEN(1))
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Cannot convert MODE parameter");
        return dsat_send_cme_error(DSAT_CME_UNKNOWN);
      }
      /* Test for in-range */
      if ((dsat_num_item_type)DSAT_CLCK_MODE_MAX <= clck_mode)
      {
       DS_ATCOP_ERROR_LOG_0("MODE parameter outside range");
        return dsat_send_cme_error(DSAT_CME_OP_NOT_SUPPORTED);
      }
    }
    else
    {
     DS_ATCOP_ERROR_LOG_0("+CLCK MODE parameter mandatory");
      return dsat_send_cme_error(DSAT_CME_UNKNOWN);
    }
    ...
    if (VALID_TOKEN(2))
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Password string too long");
        return dsat_send_cme_error(DSAT_CME_UNKNOWN);
      }
      /* Strip quotes */
      if (!dsatutil_strip_quotes_out(tok_ptr->arg[2],
                                     ...
                                     (word)passwd_len))
      {
       DS_ATCOP_ERROR_LOG_0("Problem stripping quotes");
        return dsat_send_cme_error(DSAT_CME_UNKNOWN);
      }
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Incorrect number of Args =(d)",
                 tok_ptr->args_found); 
      return dsat_send_cme_error(DSAT_CME_UNKNOWN);
    }
    ...
    switch (clck_mode)
    {
      case DSAT_CLCK_MODE_LOCK:
        ...
        { /* Operation is for FD phone book */
          ...
          if (mmgsdi_status != MMGSDI_SUCCESS) 
          {
            ...
            SET_PENDING(DSAT707_EXT_ACT_QCPIN_IDX ,0, DSAT_PENDING_CPIN_MMGSDI_CMD_NONE)
           DS_ATCOP_ERROR_LOG_0("MMGSDI returned error for FDN enabling");
            result = dsat_send_cme_error(DSAT_CME_UNKNOWN);
          }
        }
        else 
        { /* Operation is for SC */
          ...
          if (mmgsdi_status != MMGSDI_SUCCESS) 
          {
            DS_ATCOP_ERROR_LOG_1("MMGSDI returned false for SET PIN status =d",
                       (int)mmgsdi_status);
            /* MMGSDI command queue was full */
            SET_PENDING(DSAT707_EXT_ACT_QCPIN_IDX ,0, DSAT_PENDING_CPIN_MMGSDI_CMD_NONE)
            result = dsat_send_cme_error(DSAT_CME_UNKNOWN);
      ...
      case DSAT_CLCK_MODE_UNLOCK:
        ...
        { /* Operation is for FD phone book */
          ...
          {
            SET_PENDING(DSAT707_EXT_ACT_QCPIN_IDX ,0, DSAT_PENDING_CPIN_MMGSDI_CMD_NONE)
           DS_ATCOP_ERROR_LOG_0("GSDI returned error for FDN disabling");
            result = dsat_send_cme_error(DSAT_CME_UNKNOWN);
          }
        }
        else 
      ...
      case DSAT_CLCK_MODE_QUERY:
        ...
        { 
          ...
          {
           DS_ATCOP_ERROR_LOG_0("GSDI returned false for FDN status query");
            result = dsat_send_cme_error(DSAT_CME_UNKNOWN);
          }
        }
        else 
      ...
      /* Clear password */
      dsatutil_reset_password (clck_passwd, (word)MMGSDI_PIN_MAX_LEN+1);
  }
  /*--------------------------------------
...
#define CPWD_MAX_ARG (3)
    ...
    if (mmgsdi_status != MMGSDI_SUCCESS)
    {
      DS_ATCOP_ERROR_LOG_1("MMGSDI returned ERROR = d",(int)mmgsdi_status);
      /* Clear Flag of user-initiated MMGSDI action */
      SET_PENDING(DSAT707_EXT_ACT_QCPIN_IDX ,0, DSAT_PENDING_CPIN_MMGSDI_CMD_NONE)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DSAT707EXT_ASSERT((((dest) && (src)) == TRUE));
