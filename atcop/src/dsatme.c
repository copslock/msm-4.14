...
EXTERNALIZED FUNCTIONS INTERNAL TO DSAT UNIT
  ...
  dsatme_last_call_init_record
    Prepare the last_call_rec for saving number into ph book.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatme.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/29/18   skc     Added support to save incoming call to phonebook.
02/22/18   skc     Fixed AT+CLCK query issue while aborting.
10/26/17   rk      Added PBM_init completion check.
08/03/17   skc     Replaced strcmp() by strncmp().
08/01/17   skc     Fixed issue related to duplicate call of csp, spn
...
03/22/17   skc     Fixed issue with card power down and up during LPM and
                   ONLINE respectively for slot 2.
...
10/09/15   skc     Fixed compilation issue with WTGL Flavour
...
#define DSATME_ASSERT(expression) \
         dsatme_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATME_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATME ASSERT
...
===========================================================================*/
static void dsatme_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATME_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatme_assert_wrapper */
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Invalid APPS id =d",sim_info->apps_id);
  ...
  switch( client_ref )
  {
    ...
    default:
        ...
        DS_ATCOP_ERROR_LOG_1("Unsupported MMGSDI SIM read event: =d",
                           sim_info->client_ref);
...
)
{
  ...
  if (number != NULL)
  {
    if (NULL == pbm_ms_val->last_call_rec_ptr)
    {
      pbm_ms_val->last_call_rec_ptr = dsat_alloc_memory(sizeof(last_call_rec_type), FALSE);
    }

    /* Saving state */
    pbm_ms_val->last_call_rec_ptr->state = state;

    /* prepare the ld_rec for updating last dial ph book */
    pbm_ms_val->last_call_rec_ptr->rec.index = -1;
	/* need to verify with CPB_MAX_NUM_LENGTH */
    (void) strlcpy(pbm_ms_val->last_call_rec_ptr->rec.number, number, PBM_NUM_SIZE);

    if(type != -1)
    {
      pbm_ms_val->last_call_rec_ptr->rec.type= (uint8) type;
    }
    else
    {
      ...
      if(number[0] == '+')
      {
        pbm_ms_val->last_call_rec_ptr->rec.type = TA_INTERNATIONAL;
      }
      else
      {
        pbm_ms_val->last_call_rec_ptr->rec.type = TA_OTHER;
      }
    }
    ...
    (void) dsatutil_memscpy((void*)pbm_ms_val->last_call_rec_ptr->rec.text,
            PBM_TEXT_SIZE_BYTES,(void*)text,PBM_TEXT_SIZE_CHARS);  
    }
    else
    {
      pbm_ms_val->last_call_rec_ptr->rec.text[0] = '\0';
    }
    ...
    pbm_ms_val->last_call_rec_ptr->rec.time = time_jul_to_secs(&julian_time);
    pbm_ms_val->last_call_rec_ptr->rec.duration = 0;

    pbm_ms_val->last_call_rec_ptr->rec.is_answered = FALSE;
  }
  else
  {
    DS_AT_MSG0_LOW("dsatme_last_call_init_record called with no number");
  }  
} /* dsatme_last_call_init_record */
...
)
{
  ...
  default:
    DS_ATCOP_ERROR_LOG_1("Unsupported MMGSDI status : =d",mmgsdi_result);
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Too many arguements");
    return(DSAT_ERROR);
  }

  /* Make sure PBM has finished initializing */
  if (!pbm_is_init_completed())
  {
      return DSAT_CMD_ERR_RSP;
  }
...
)
{
  ...
  DSATME_ASSERT(res_buff_ptr->used == 0);
...
)
{
  ...
  if( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if( (result = cpbw_parse_arguments(tok_ptr, record_ptr, res_buff_ptr)) == DSAT_OK )
    {
      ...
      if (!((1 == tok_ptr->args_found )&&(record_ptr->index != -1)))
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Error: Could not allocate size");
  ...
  else if ( tok_ptr->op == (NA|EQ|QU) )
  {
    ...
    {
      (void) psutil_itoa((word) me_cpbs_entry.text_len, 
...
)
{
  ...
  static const struct {
    ...
    { DSAT_CME_INSUFFICIENT_RESOURCES, "insufficient resources"},
    { DSAT_CME_UNKNOWN_APN,             "missing or unknown APN"},
    { DSAT_CME_UNKNOWN_PDP_TYPE,        "unknown PDP address or PDP type"},
    { DSAT_CME_USER_AUTH_FAILED,        "user authentication failed"},
    { DSAT_CME_GGSN_REJECTED_ACTIVATION,     "activation rejected by GGSN, Serving GW or PDN GW"},
    { DSAT_CME_ACTIVATION_REJECTED_UNSPECIFIED, "activation rejected, unspecified"},
    ...
    { DSAT_CME_FEATURE_NOT_SUPPORTED,     "feature not supported"},
    { DSAT_CME_SEMANTIC_ERROR_TFT,        "semantic error in the TFT operation"},
    { DSAT_CME_SYNTACTICAL_ERROR_TFT,     "syntactical error in the TFT operation"},
    { DSAT_CME_UNKNOWN_PDP_CONTEXT,       "unknown PDP context"},
    { DSAT_CME_SEMANTIC_ERRORS_PKT_FILTER,       "semantic errors in packet filter(s)"},
    { DSAT_CME_SYNTACTICAL_ERRORS_PKT_FILTER,    "syntactical errors in packet filter(s)"},
    { DSAT_CME_PDP_ACTIVATE_WITHOUT_ACTIVE_TFT,  "PDP context without TFT already activated"},
    ...
    { DSAT_CME_OP_DETERMINED_BARRING,    "Operator Determined Barring"},
    { DSAT_CME_PDP_CNTXT_REACHED_MAX,    "maximum number of PDP contexts reached"},
    { DSAT_CME_REQUESTED_APN_NOT_SUPPORTED, "requested APN not supported in current RAT and PLMN combination"},
    { DSAT_CME_REQUEST_REJECTED,         "request rejected, Bearer Control Mode violation"},
    ...
    (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_CMEE_IDX,0,0,NUM_TYPE) == 2 )
  {
    ...
    if(cme_string == NULL)
    {
      DS_ATCOP_ERROR_LOG_1("Incorrect cme code =d", error_code);
      return;
    }
...
)
{
  ...
  if(tok_ptr->op == NA)
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("SPN info not available");
    }
  }
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_0("Invalid number of Arguments in QCSIMAPP");
      return DSAT_ERROR;
    }
                      ...
                      tok_ptr->arg[0], 10) != ATOI_OK ) || ( qcsimapp_val >= max_subs ))
    {
      ...
      DS_ATCOP_ERROR_LOG_1("Arg 1 must be a number between 0-=d", (max_subs - 1));
      return DSAT_ERROR;
    }
  ...
  switch (cmd_info_ptr->cmd_id)
  {
    ...
    case CPIN_MMGSDI_PERSO_GET_IND_RSP:
      if(dsatme_pending_mmgsdi_command[cmd_info_ptr->apps_id] && 
         dsat_curr_cmd_var.tok_ptr && dsat_curr_cmd_var.tok_ptr->name &&
         (0 == (dsatutil_strncmp_ig_sp_case((const byte *)dsat_curr_cmd_var.tok_ptr->name,
          (const byte *)"+CLCK" ,strlen((const char *)"+CLCK")))))
      {
        dsm_item_type *res_buff_ptr;
...
dsat_result_enum_type dsatme_init_sim_filename( void )
{
  ...
  if( result == DSAT_OK )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("CPLS value not supported");
      result = DSAT_ERROR;
    }
    ...
    if(record_ptr->group[0] != '\0')
    {
      ...
      */
      ...
      if(grp_len != 0 && grp_id != 0)
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Record length Exceeded \n");
        }
        else
        {
          raw_rsp_ptr->used=(word)snprintf((char *)raw_rsp_ptr->data_ptr,
...
)
{
  ...
  switch (status)
  {
    ...
    case PBM_ERROR_NOT_AVAILABLE:
      ...
      DS_ATCOP_ERROR_LOG_1("Wrong pbm_result: =d", status);
      result = DSAT_ERROR;
      break;
...

{
  ...
  if(status == PBM_SUCCESS)
  {
    ...
    if(get_pb_storage(info_ptr->pbm_device, storage))
    {
      ...
      {
        if(!strncmp( (char *) dsat_cpbs_storage_valstr[index], 
                    (char *) storage, CPBS_VAL_LEN + 1))
        {
          find_index = TRUE;
          break;
        }
      ...
      else
      {
        ...
        DS_ATCOP_ERROR_LOG_1("pbm_device =d not supported",info_ptr->pbm_device);
        result = DSAT_ERROR;
      }
    }
    else
    {
      ...
      DS_ATCOP_ERROR_LOG_1("No storage for pbm_device =d",info_ptr->pbm_device);
      result = DSAT_ERROR;
    }
...
LOCAL void cpb_pbm_cb_file_info_handler_output_info(int used)
{
  ...
  if (used < 0 || me_cpbs_entry.total < 1) 
  {
    ...
    DSATME_ASSERT(me_cpbs_entry.total == -1);
...
)
{
  ...
  if(status == PBM_SUCCESS)
  {
    ...
    DSATME_ASSERT(dd_val_ptr->mem[0] != '\0');
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid length");
...
)
{
  ...
  switch (pbm_result) 
  {
    ...
    case PBM_ERROR_INVALID_REC:
      /* pbm should not return this result at this time */
      DS_ATCOP_ERROR_LOG_1("Wrong pbm_result: =d", pbm_result);
      result = DSAT_ERROR;
      cb_expected = CB_NONE;
      break;
...
)
{
  ...
  if(!dev_ptr)
  {
   DS_ATCOP_ERROR_LOG_0("NULL dev_ptr pointer");
    return FALSE;
  }
...
)
{
  ...
  if(!storage_ptr)
  {
   DS_ATCOP_ERROR_LOG_0("NULL pb pointer");
    return FALSE;
  }
...
)
{
 ...
 /* update the cpbs parameter */
  for (index = 0; index < CPBS_SUPPORTED_PHBKS_NUM; index++)
  {
      ...
      {
        /* can't find a pbm device for the default storage type */
       DS_ATCOP_ERROR_LOG_0("No pbm_device mapped");
        return DSAT_ERROR;
      }
...
)
{
  ...
  else /* user give a text */
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("extended CMD requires coding field when text field specified");
      return DSAT_ERROR;
    }
    else
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("invalid coding mode specified (=d)",encoding);
        return DSAT_ERROR;
      }
    }
    ...
    {
      DS_ATCOP_ERROR_LOG_1("tried cpbw with encoding =d but currently only support 2 = ASCII",
                tok_ptr->arg[4][0]);
      dsatme_set_cme_error(DSAT_CME_OP_NOT_SUPPORTED, res_buff_ptr);
      return DSAT_CMD_ERR_RSP;
    }
  ...
  {
    DS_ATCOP_ERROR_LOG_1("cpbw incorrect additional address type =d",tok_ptr->arg[6]);
    return DSAT_ERROR;
  }
   /* ui can't handle a CPB_MAX_NUM_LENGTH international number since UI 
     always add + sign before the number.  This makes it 
                                 ...
                                 dsm_item_type * err_buff_ptr)
{
  ...
  if(error_code == DSAT_CME_MAX)
  {
    ...
    DS_ATCOP_ERROR_LOG_1("unrecognized pbm_return_type error =d",pbm_err);
...
)
{
  ...
  if (pbm_result == PBM_SUCCESS )
  {
    ...
    {
      /* the number was dialed, init the last_call_rec */
      dsatme_last_call_init_record(dd_val_ptr->record.number, dd_val_ptr->record.type, 
                                   dd_val_ptr->record.text, LAST_CALL_DIALED);
      dsatme_last_call_save(dsat_get_current_subs_id(FALSE));
    }
#endif /* FEATURE_DSAT_ETSI_MODE */
  }
  else if (pbm_result == PBM_ERROR_NOT_FOUND)
...
)
{
  ...
  while(dsat_cpbs_storage_valstr[dd_val_ptr->srch_pb][0] != '\0')
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Wrong mem");
      /* can't map mem to a valid pbm device */
      return DSAT_ERROR;
    }
...
)
{
  ...
  if (DSAT_ASYNC_CMD == result)
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Problem updating PIN status");
      result = DSAT_ERROR;
    }
    else
    {
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Invalid APPS ID =d",apps_id);
    return DSAT_ASYNC_EVENT;
  }
  ...
  case MMGSDI_SUCCESS:
    ...
    case CPIN_MMGSDI_PIN1_VERIFY:
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Problem verifying PIN1 status");
        return DSAT_ERROR;
      }
      break;
    ...
    case CPIN_MMGSDI_PIN2_VERIFY:
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Problem verifying PIN2 status");
        return DSAT_ERROR;
      }
      break;
...
LOCAL dsat_result_enum_type dsatme_report_clck_fdn_status(ds_at_cmd_status_type *cmd_info_ptr)
{
  ...
  if (!IS_VALID_GW_APPS_ID(cmd_info_ptr->apps_id))
  {
    DS_ATCOP_ERROR_LOG_1("Invalid APPS ID =d",cmd_info_ptr->apps_id);
    return DSAT_ASYNC_EVENT;
  }
  ...
  switch (mmgsdi_status)
  {
    ...
    case MMGSDI_CODE_BLOCKED:
      ...
      break;
    default:
      DS_ATCOP_ERROR_LOG_1(" CLCK query status is =d (NOT SUCCESS)",mmgsdi_status);
      result = DSAT_ERROR;
      break;
  }
...
)
{
  ...
  {
    SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_NONE)
...
#define DSAT_APP3_LPB  5
  ...
  if( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if ( DSAT_OK == result )
    {
        ...
        if ( IS_ETSI_MODE(current_mode) )
        {
          switch ( temp1 )
          {
            case DSAT_APP1_LPB:
            {
              ...
              {
                DS_ATCOP_ERROR_LOG_1(" Provision apps =d not ready ",DSAT_APPS_ID_GW_PRIMARY);
                result = DSAT_ERROR;
              }
              break;
            }
            ...
            case DSAT_APP2_LPB:
            {
              ...
              {
                DS_ATCOP_ERROR_LOG_1(" Provision apps =d not ready ",PBM_PROVISION_GW_SECONDARY);
                result = DSAT_ERROR;
              }
              break;
            }
            ...
            case DSAT_APP3_LPB:
            {
              ...
              {
                DS_ATCOP_ERROR_LOG_1(" Provision apps =d not ready ",DSAT_APPS_ID_GW_TERTIARY);
                result = DSAT_ERROR;
              }
              break;
            }
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Invalid Preference =d not supported",temp1);
        result = DSAT_ERROR;
      }
    }
  }  /* Process Test Command */
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Syntax Error in $QCPBMPREF command");
    return DSAT_ERROR;
  }
...
#endif /* FEATURE_DSAT_ETSI_MODE */
    default:
      DS_ATCOP_ERROR_LOG_1("Invalid client ref =d",temp_client_ref);
  }
...
)
{
  ...
  if ( cb_cmd == CB_NONE )
  {
   DS_ATCOP_ERROR_LOG_0("Get File attributes request on an invalid file ");
    return FALSE;
  }
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Failed to post get attributes for file =d",file_name);
    return FALSE;
  }
...
)
{
  ...
  me_ms_val = dsat_get_base_addr_per_session(sim_info->apps_id, FALSE);
...
)
{
  ...
  DSATME_ASSERT( (res_buff_ptr != NULL) );
  DSATME_ASSERT( (cmd_ptr != NULL) );
            ...
            curr_byte++ )
  {
    ...
    curr_char = psutil_itoa( (uint32)*curr_byte, curr_char, 16 );
...
#define FIRST_RECORD 1
  ...
  DSATME_ASSERT(me_ms_val->dsat_cuad_recs.curr_rec == sim_info->accessed_rec_num);
...
)
{
  ...
  switch (cnf_ptr->response_header.response_type)
  {
    ...
    default:
      DS_ATCOP_ERROR_LOG_1("Not a valid mmgsdi cb rsp: =d", 
...
)
{
  ...
  DSATME_ASSERT( NULL != resp_ptr);
  DSATME_ASSERT( NULL != user_data_ptr);
  ...
  switch (resp_ptr->msg_type)
  {
    ...
    case SIMLOCK_GET_STATUS_MSG:
      ...
      break;
    default:
     DS_ATCOP_ERROR_LOG_0("Not a valid mmgsdi cb rsp"); 
      break;
  }
...
)
{
  ...
  if(MMGSDI_SUCCESS == status)
  {
	...
	dst_plmn_info_ptr->tac_5g = src_plmn_info_ptr->tac_5g;
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Unexpected MMGSDI status - =d",oper_name->mmgsdi_status);
    result = DSAT_ERROR;
  }
  else
  {
...
)
{
  ...
  if ( CHECK_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_WRITE_CB) )
  {
  ...
  if (CHECK_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_WRITE))
  {
    SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_NONE)
   /* If not resetting and current and requested opmode 
    are the same just return OK */
    if ( (dsat_num_item_type)dsatutil_get_val(
          DSAT_EXT_CFUND_IDX,0,1,MIX_NUM_TYPE) == (dsat_num_item_type)DSAT_ME_DONT_RESET &&
    ...
    else
    {
     ...
     /* Reset support for CFUN = 6 */
      if ( (dsat_num_item_type)DSAT_ME_DO_RESET == 
           (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_CFUND_IDX,0,1,MIX_NUM_TYPE) ||
            ...
            DSAT_EXT_CFUND_IDX,0,0,MIX_NUM_TYPE))) )
      {
         ...
         if(FALSE == ((DSAT_ME_FUNC_OFFLINE == old_opmode )&&((dsat_num_item_type)DSAT_ME_FUNC_RESET == (dsat_num_item_type)dsatutil_get_val(
            DSAT_EXT_CFUND_IDX,0,0,MIX_NUM_TYPE))))
         {
           DSATUTIL_SET_VAL(DSAT_EXT_CFUND_IDX,0,0,0,DSAT_ME_FUNC_OFFLINE,MIX_NUM_TYPE)
         }
      ...
      if ( (dsat_num_item_type)dsatutil_get_val(
            DSAT_EXT_CFUND_IDX,0,0,MIX_NUM_TYPE) == (dsat_num_item_type)DSAT_ME_FUNC_MIN)
      {
      ...
      temp_mix_type = (dsat_mixed_param_val_type *)dsatutil_get_val(
                                    DSAT_EXT_CFUND_IDX,0,0,MIX_INDEX_TYPE);
  ...
  if ( CHECK_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_READ))
  {
    dsm_item_type * res_buff_ptr;
  ...
  if (TRUE == to_at_state)
  {
    ...
    case SYS_OPRT_MODE_LPM:
      ...
      *at_mode_ptr = DSAT_ME_FUNC_MIN;
      ...
      if ( CHECK_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_WRITE_RFOFF) )
      {
  ...
  else
  {
    ...
    case DSAT_ME_FUNC_MIN:
      *cm_mode_ptr = SYS_OPRT_MODE_LPM;
      SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_WRITE_MIN)
      break;
    
    case DSAT_ME_FUNC_FULL:
      *cm_mode_ptr = SYS_OPRT_MODE_ONLINE;
       SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_WRITE_FULL)
      break;    

    case DSAT_ME_FUNC_FTM:
      *cm_mode_ptr = SYS_OPRT_MODE_FTM;
      SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_WRITE_FTM)
      break;
...
#endif /* defined(FEATURE_DSAT_EXTENDED_CMD) */
      SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_WRITE_OFFLINE)
      break;
      
    case DSAT_ME_FUNC_RESET:
      *cm_mode_ptr = SYS_OPRT_MODE_RESET;
       SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_WRITE_RESET)
      break;
    ...
    case DSAT_ME_FUNC_RFOFF:
      ...
      *cm_mode_ptr = SYS_OPRT_MODE_LPM;
#endif /* defined(FEATURE_DSAT_EXTENDED_CMD) */
      SET_PENDING(DSAT_EXT_CFUND_IDX ,0, DSAT_PENDING_CFUN_WRITE_RFOFF)
      break;
  ...
  for (slot_id = slot_id_start; slot_id <= max_subs; slot_id++)
    {
      ...
      {
        result = DSAT_ASYNC_CMD;
        SET_PENDING(DSAT_EXT_CFUND_IDX, 0, DSAT_PENDING_CFUN_WRITE_CARD_DOWN);
  ...
  if (TRUE == me_ss_val->dsat_is_card_power_down)
  {
    ...
    for (slot_id = slot_id_start; slot_id <= max_subs; slot_id++)
    {
      ...
      if (MMGSDI_RESTRICTED_IN_ONCHIP_MODE != ret_val)
      {
        ...
        result = DSAT_ASYNC_CMD;
        SET_PENDING(DSAT_EXT_CFUND_IDX, 0, DSAT_PENDING_CFUN_WRITE_CARD_PUP);
