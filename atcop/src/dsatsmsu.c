...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatsmsu.c#1 $
...
{\
 DS_ATCOP_ERROR_LOG_0("Invalid CPMS memory type");\
  return 0;\
}

#define DSATSMSU_ASSERT(expression) \
         dsatsmsu_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATSMSU_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATSMSU ASSERT
...
===========================================================================*/
static void dsatsmsu_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATSMSU_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatsmsu_assert_wrapper */

/*===========================================================================
...
)
{
  ...
  while ( i < 7 )
  {
    if ( tag == tag_array[i] )
    {
      ...
      {
        /* mode TRUE: PDU mode */
        res_ptr = psutil_itoa(i, res_ptr, 10);
      }
      else
#endif /* FEATURE_ETSI_SMS */
      {
...
#endif /* FEATURE_DSAT_EXTENDED_SMS */
    {
     DS_ATCOP_ERROR_LOG_0("Unsupported Address character");
      return FALSE;
    }
  }
#endif /* FEATURE_CDMA_SMS */
...
)
{
  ...
  w_ptr = psutil_itoa((uint32)error, w_ptr, 10);
...
)
{
  ...
  if(list_ptr == NULL)
  {
   DS_ATCOP_ERROR_LOG_0("SMS memory settings unavailable");
    return FALSE;
  }
  mem_valstr = list_ptr->list_v[mem_val];
  /* Check if the CMGL is for Status report */
  if ( strncmp ("SR",(char *)mem_valstr, strlen((char*)"SR")) == 0)
  {
#ifdef FEATURE_DUAL_SIM
