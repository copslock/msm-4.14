...
/*===========================================================================
  ...
  $PVCSPath:  L:/src/asw/MSM5200/data/vcs/dsatetsicall.c_v   1.7   12 Jul 2002 10:05:08   randrew  $
   $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsicall.c#6 $ $DateTime: 2019/12/03 20:48:56 $ $Author: psahithi $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/4/19    ps      Added support for printing ACT value in decimal
12/03/19   ks      Added support for CREG to display missing lac, cell id and ACT value in LTE/NSA.
11/26/19   ks      Disabled CREG URC for NGRAN as per spec.
11/22/19   ks      Added Rac info parameter to CGREG command.
10/31/19   ks      Enhanced COPS and CREG to support NR5G.
07/25/19   ps      CREG URC Act value validation.
03/22/18   skc     Saving previous COPS format to restore if ERROR occurred.
...
08/04/17   skc     Fixed abort issue related to CPOL command.
...
07/27/17   skc     Stored VoLTE conference server call id.
07/26/17   skc     Fixed issue with A+CHLD=2 for IP Call.
03/31/17   skc     Fixed KW issue.
01/26/17   skc     Fixed issue related to AT+COPS.
...
04/01/16   skc     Fixed issue where wrong call id is stored during one 
                   active call and one held call and at+chld=2 is issued.
11/03/15   skc     Fixed issue with chld=0 where call id of active call was sent
...
02/27/15   tk      Fixed +COPN issue in MNC display logic.
...
#include "dsutil.h"
#include "sys_v.h"
#ifdef FEATURE_DATA_AT20_DEV
#include "cm.h"
#endif /* FEATURE_DATA_AT20_DEV */
...
#define DSATETSICALL_ASSERT(expression) \
         dsatetsicall_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATETSICALL_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATETSICALL ASSERT
...
===========================================================================*/
static void dsatetsicall_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATETSICALL_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatetsicall_assert_wrapper */
...
)
{
  ...
  if(cr_val == 1)
  {
    ...
    if(serv_string == NULL)
    {
      DS_ATCOP_ERROR_LOG_1("Incorrect service type =d", service);
      return;
    }
    ...
    if( DSAT_CES_DOMAIN_CS == call_da_val->dsatetsicmif_call_end_status.domain)
    {
          ...
          ( CM_CALL_END_NETWORK_END == info_ptr->end_status ) )
      {
        ...
        else
        {
         ...
         DS_ATCOP_ERROR_LOG_0("Cause and reject information unavailable");
...
)
{
  ...
  if(crc_val == 1)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Incorrect crc result code =d", crc_code);
      return NULL;
    }
...
)
{
  ...
  if (tok_ptr->op == NA)
  {
    ...
    if( call_da_val->etsicall_wait_call_present )
    {
      ...
      if( dsatcmif_answer_call(subs_id, call_da_val->etsicall_wait_call_id, FALSE,
                               voice_state[subs_id][call_da_val->etsicall_wait_call_id].call_mode) )
      {
        result = DSAT_ASYNC_CMD;
      }
      else
...
)
{
  ...
  else if (tok_ptr->op == (NA|QU) )
  {
    ...
    {
      creg_val =(dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CEREG_IDX,subs_id,0,NUM_TYPE);
      ...
      res_buff_ptr->used =
        (word)snprintf((char*)res_buff_ptr->data_ptr,
                                res_buff_ptr->size,
                                "+CEREG: %d,%d", creg_val, DSAT_NET_REG_NONE);
      return result;
    }
#endif /*# FEATURE_DSAT_LTE*/

    if((sys_mode != DSAT_MODE_GSM  && sys_mode != DSAT_MODE_WCDMA) &&
       (0 == dsatutil_strcmp_ig_sp_case((byte*)"+CGREG", (const byte *)parse_table->name)))
    {
      creg_val =(dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CGREG_IDX,subs_id,0,NUM_TYPE);
       /* <stat> 4 unknown (e.g. out of GERAN/UTRAN coverage) */
      res_buff_ptr->used =
        (word)snprintf((char*)res_buff_ptr->data_ptr,
                                res_buff_ptr->size,
                                "+CGREG: %d,%d", creg_val, DSAT_NET_REG_NONE);
      return result;
    }
    ...
    if(0 == dsatutil_strcmp_ig_sp_case((byte*)"+CREG", (const byte *)parse_table->name))
    {
                         ...
                         subs_id,0,NUM_TYPE) == CREG_DISP_LAC_CELL_ID)
      {
        ...
        {
          if((ph_ss_ms_val->dsat_net_reg_state.dsat_creg_reported.cell_id != 0xFFFFFFFF)&&
...
)
{
  ...
  if(NULL == ph_ss_ms_val)
  {
    return DSAT_REG_REGISTERED_MAX;
  }
...
)
{
  ...
  dsati_mode_e_type  curr_mode = dsatcmdp_get_current_mode_per_subs(subs_id);
  ...
  if ((curr_mode == DSAT_MODE_GSM || curr_mode == DSAT_MODE_WCDMA  ||
        curr_mode == DSAT_MODE_LTE || curr_mode == DSAT_MODE_1XLTE ) && 
      (0 < (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CREG_IDX,
                   subs_id,0,NUM_TYPE))) 
  {
#ifdef FEATURE_SGLTE
  ...
  if ((curr_mode == DSAT_MODE_GSM || curr_mode == DSAT_MODE_WCDMA) &&
      (0 < (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CGREG_IDX,
               subs_id,0,NUM_TYPE)))
  {
  ...
  if ((curr_mode == DSAT_MODE_LTE || curr_mode == DSAT_MODE_1XLTE) &&
      (0 < (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CEREG_IDX, subs_id,0,NUM_TYPE)))
  {
    DS_AT_MSG0_HIGH("Unsolicited cell id indication");
    if ( NULL == res_buff_ptr )
    {
    ...
    /* +CEREG displays lac and cell_id only when camped */
          ...
          ((report_creg &(REPORT_CEREG | REPORT_CREG_LAC | REPORT_CREG_CELL_ID))))
      {
        ...
        (void) snprintf(fmt_string, sizeof(fmt_string),
                        ",\"%X\",\"%X\",%d",
                        ph_ss_ms_val->dsat_net_reg_state.dsat_creg_reported.lte_tac,
                        (unsigned int)ph_ss_ms_val->dsat_net_reg_state.dsat_creg_reported.cell_id,
                        current_act);
...
FUNCTION DSAT_FILL_SUPS_TIME_INFO

DESCRIPTION
  This function fill CM structure with time information for call forwarding.

DEPENDENCIES
  None
  
RETURN VALUE
  TRUE  : if given time is parsed successfully
  FALSE : if invalid format for time is given.
...
===========================================================================*/
/* ARGSUSED */
boolean dsat_fill_sups_time_info
(
  const byte                      *time_tok_str,
  uint8                            len,
  cm_ip_sups_call_fwd_time_s_type *call_fwd_time
)
{
  dsat_time_s_type time_info;
  memset(&time_info, 0, sizeof(dsat_time_s_type));

  if(time_tok_str == NULL ||
     call_fwd_time == NULL)
  {
    return FALSE;
  }

  if(dsatetsime_parse_time(time_tok_str, len, &time_info) == FALSE)
  {
    return FALSE;
  }
  call_fwd_time->year = 0;
  call_fwd_time->month = 0;
  call_fwd_time->day = 0;
  call_fwd_time->hour = time_info.hour;
  call_fwd_time->minute = time_info.minute;
  call_fwd_time->second = time_info.second;
  call_fwd_time->time_zone = 0;
...
)
{
  ...
  /* MAX_TIME_STRING_LENGTH + 1 -->  +1 for NULL Termination */
  byte time_tok_str[MAX_TIME_STRING_LENGTH + 1];
  ...
  if(0 == (dsatutil_strncmp_ig_sp_case((const byte *)parse_table->name,
          (const byte *)"$QCCCFC" ,strlen((const char *)"$QCCCFC"))))
  {
    is_ext_cmd = TRUE;
  }
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    if (VALID_TOKEN(0))
    {
      ...
      /* Test for in-range */
      if ((!is_ext_cmd && (dsat_num_item_type)DSAT_CCFC_REASON_MAX <= temp) ||
          (is_ext_cmd && temp != DSAT_CCFC_REASON_UNCOND))
      {
    ...
    {
      DS_AT_MSG0_HIGH("REASON parameter mandatory");
      return DSAT_ERROR;
    }
    ...
    {
      DS_AT_MSG0_HIGH("MODE parameter mandatory");
      return DSAT_ERROR;
    }
    ...
    if ( is_ext_cmd && VALID_TOKEN(8))
    {
      /* Time based Input */
      byte *str;
      uint8 i = 0;
      int len = strlen((char*)tok_ptr->arg[8]);

      /* Ignore the quotes surrounding the string */
      if(dsatutil_strip_quotes_out(tok_ptr->arg[8], tok_ptr->arg[8],
                                    len) == FALSE)
      {
        return DSAT_ERROR;
      }
      ...
      str =  tok_ptr->arg[8];
      /* MAX_TIME_STRING_LENGTH + 1 --> +1 to read delimeter to break */
      while((i < (len - 2)) && (i < (MAX_TIME_STRING_LENGTH + 1)) && *str != delimiter)
      {
        time_tok_str[i] = *str++;
        i++;
      }

      if( !(i == MAX_TIME_STRING_LENGTH && *str == delimiter) )
      {
        /* Call forward start time is not in proper format or delimeter not found */
        return DSAT_ERROR;
      }

      time_tok_str[i] = '\0'; /* Null terminating start of time */
	  
      if(!dsat_fill_sups_time_info(time_tok_str, i, &sscf_ptr->sups_time_info.call_fwd_start_time))
      {
        /* Issue with getting call fwd start time */
        return DSAT_ERROR;
      }

      str++;/* Removing delimiter */

      /* Now str will point to call fwd end time 
	     i-3 is reduced from total length
	     i is length of call fwd start time 
	     1 is length of delimiter #
	     2 is length of double quotes */
      if(!dsat_fill_sups_time_info(str, len - i - 3, &sscf_ptr->sups_time_info.call_fwd_end_time))
      {
        /* Issue with getting call fwd end time */
        return DSAT_ERROR;
      }
        
      sscf_ptr->sups_time_info.time_present = TRUE;
    }
    else
    {
      /* Default */
      sscf_ptr->sups_time_info.time_present = FALSE;
    }
    ...
    {
      /* Set pending for respective command */
      if(is_ext_cmd)
      {
        SET_PENDING(DSATETSI_EXT_ACT_CCFC_ETSI_IDX, 0, DSAT_PENDING_CCFC_EXTENDED)
      }
      else
      {
        SET_PENDING(DSATETSI_EXT_ACT_CCFC_ETSI_IDX, 0, DSAT_PENDING_CCFC_LEGACY)
      }
    }
  }
...
FUNCTION DSATETSICALL_CCFCU_FREE_URI

DESCRIPTION
  This function is called by destructor to release buffer allocated
  during registration of call forwarding with forward to number in URI
  format with AT+CCFCU Command.
...
LOCAL void dsatetsicall_ccfcu_free_uri
(
  void *forward_to_addr_uri
)
{
  cm_forward_to_addr_uri_s_type  *forward_to_add_uri_temp = (cm_forward_to_addr_uri_s_type *) forward_to_addr_uri;

  /* We should not come here if forward_to_add_uri_temp is NULL */
  if(forward_to_add_uri_temp == NULL)
  {
    DSATETSICALL_ASSERT(0);
  }

  if (forward_to_add_uri_temp->fwd_to_uri != NULL)
  {
    dsatutil_free_memory_ext((void**)&forward_to_add_uri_temp->fwd_to_uri);
  }
  dsatutil_free_memory_ext((void**)&forward_to_add_uri_temp);
}
...
{
#define CCFCU_TON_FOR_URI 0
  dsat_result_enum_type result = DSAT_OK;
  dsatetsicmif_sups_state_ss_info  *sups_ss_val = NULL;
  dsatetsicmif_sups_state_ms_info  *sups_ms_val = NULL;

  enum{
    CCFCU_ERROR_TOKEN_NO_ERROR = 0,
    CCFCU_ERROR_TOKEN_0_CONVERSION_FAILED,
    CCFCU_ERROR_TOKEN_0_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_0_INVALID,
    CCFCU_ERROR_TOKEN_1_CONVERSION_FAILED ,
    /* 5 */
    CCFCU_ERROR_TOKEN_1_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_1_INVALID,
    CCFCU_ERROR_TOKEN_2_CONVERSION_FAILED ,
    CCFCU_ERROR_TOKEN_2_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_2_INVALID,
    /* 10 */
    CCFCU_ERROR_TOKEN_3_CONVERSION_FAILED ,
    CCFCU_ERROR_TOKEN_3_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_3_UNSUPPORTED,
    CCFCU_ERROR_TOKEN_3_INVALID,
    CCFCU_ERROR_TOKEN_4_INVALID_LENGTH ,
    /* 15 */    
    CCFCU_ERROR_TOKEN_4_INVALID_STRING,
    CCFCU_ERROR_TOKEN_4_INVALID_DIGIT,
    CCFCU_ERROR_TOKEN_4_INVALID,
    CCFCU_ERROR_TOKEN_4_MEM_ALLOC_FAILED,
    CCFCU_ERROR_TOKEN_4_MEM_ALLOC_FAILED1,
    /* 20 */
    CCFCU_ERROR_TOKEN_4_INVALID_URI,
    CCFCU_ERROR_TOKEN_5_CONVERSION_FAILED ,
    CCFCU_ERROR_TOKEN_5_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_5_NOT_SUPPORTED,
    CCFCU_ERROR_TOKEN_5_INVALID,
    /* 25 */
    CCFCU_ERROR_TOKEN_6_CONVERSION_FAILED ,
    CCFCU_ERROR_TOKEN_6_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_6_NOT_SUPPORTED,
    CCFCU_ERROR_TOKEN_6_INVALID,    
    CCFCU_ERROR_TOKEN_7_CONVERSION_FAILED ,
    /* 30 */
    CCFCU_ERROR_TOKEN_7_INVALID_STRING,
    CCFCU_ERROR_TOKEN_7_NOT_SUPPORTED,
    CCFCU_ERROR_TOKEN_7_ENCODING_FAILED,
    CCFCU_ERROR_TOKEN_8_CONVERSION_FAILED ,
    CCFCU_ERROR_TOKEN_8_INVALID_RANGE,
    /* 35 */
    CCFCU_ERROR_TOKEN_8_NOT_SUPPORTED,
    CCFCU_ERROR_TOKEN_8_INVALID,
    CCFCU_ERROR_TOKEN_9_CONVERSION_FAILED ,
    CCFCU_ERROR_TOKEN_9_INVALID_RANGE,
    CCFCU_ERROR_TOKEN_9_NOT_SUPPORTED,
    /* 40 */
    CCFCU_ERROR_TOKEN_9_INVALID
  }ccfcu_error = CCFCU_ERROR_TOKEN_NO_ERROR;
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    do
    {
      if (VALID_TOKEN(0))
      {
        ...
        if (ATOI_OK != dsatutil_atoi(&temp, tok_ptr->arg[0], 10))
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_0_CONVERSION_FAILED;
          break;
        }
        /* Test for supported reason */
        if ((dsat_num_item_type)DSAT_CCFCU_REASON_MAX <= temp &&
             DSAT_CCFCU_REASON_COMM_DEF != temp)
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_0_INVALID_RANGE;
          break;
        }

        temp = (temp == DSAT_CCFCU_REASON_COMM_NOT_LOGGED_IN) ? 
                DSAT_CCFC_REASON_NOTREACH : temp;

        sscf_ptr->reason = (ccfc_reason_e_type)temp;
      }
      else
      {
        ccfcu_error = CCFCU_ERROR_TOKEN_0_INVALID;
        break;
      }
      ...
      if (VALID_TOKEN(1))
      {
        ...
        if (ATOI_OK != dsatutil_atoi(&temp, tok_ptr->arg[1], 10))
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_1_CONVERSION_FAILED;
          break;
        }
        /* Test for in-range */
        if ((dsat_num_item_type)DSAT_CCFC_MODE_MAX <= temp)
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_1_INVALID_RANGE;
          break;
        }
        sscf_ptr->mode = (ccfc_mode_e_type)temp;
      }
      ...
      {
        ccfcu_error =CCFCU_ERROR_TOKEN_1_INVALID;
        break;
      }
      ...
      */
      if (sscf_ptr->mode == DSAT_CCFC_MODE_REG && VALID_TOKEN(2))
      {
        /* Convert to number */
        if (ATOI_OK != dsatutil_atoi(&temp, tok_ptr->arg[2], 10))
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_2_CONVERSION_FAILED;
          break;
        }
        /* Test for supported NumberType */
        if ((dsat_num_item_type)DSAT_CCFCU_NUMBERTYPE_MAX <= temp ||
            DSAT_CCFCU_NUMBERTYPE_INVALID == temp)
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_2_INVALID_RANGE;
          break;
        }
        sscf_ptr->fwd_type = ((dsat_ccfcu_numbertype_e_type)temp == DSAT_CCFCU_NUMBERTYPE_URI) ? 
                             CM_FORWARD_TO_URI : CM_FORWARD_TO_NUMBER;
      }
      else
      {
        if(sscf_ptr->mode == DSAT_CCFC_MODE_REG)
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_2_INVALID;
          break;
        }
      }

      /* Process TYPE parameter 
         <ton>: type of number in integer format (refer 3GPP TS 24.008 [8] subclause 10.5.4.7). The parameter is also set to zero when it has no meaningful content, e.g. when <numbertype>=1.
      */
      if (sscf_ptr->mode == DSAT_CCFC_MODE_REG && VALID_TOKEN(3))
      {
        /* Convert to number */
        if (ATOI_OK != dsatutil_atoi(&temp, tok_ptr->arg[3], 10))
        {
          /* Cannot convert TYPE parameter */
          ccfcu_error = CCFCU_ERROR_TOKEN_3_CONVERSION_FAILED;
          break;
        }
        if(sscf_ptr->fwd_type == CM_FORWARD_TO_NUMBER)
        {
          /* Test for in-range */
          if ((mixed_def_list[3]->def->dflm.upper <  temp) ||
              (mixed_def_list[3]->def->dflm.lower >  temp))
          {
            /* TYPE parameter outside range */
            ccfcu_error = CCFCU_ERROR_TOKEN_3_INVALID_RANGE;
            break;
          }
        }
        else
        {
          if(temp != CCFCU_TON_FOR_URI)
          {
            ccfcu_error = CCFCU_ERROR_TOKEN_3_UNSUPPORTED;
            break;
          }
        }
        sscf_ptr->type = (uint8)temp;
      ...
      else
      {
        if(sscf_ptr->mode == DSAT_CCFC_MODE_REG)
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_3_INVALID;
          break;
        }
      }

      /* Validate NUMBER parameter */
      if (sscf_ptr->mode == DSAT_CCFC_MODE_REG && VALID_TOKEN(4))
      {
        byte *digit = tok_ptr->arg[4];
		uint8 num_string_len = strlen((char*)tok_ptr->arg[4]);
		uint8 num_len = 0;

        /* Test length */
        if (mixed_def_list[4]->def->string_len < num_string_len)
        {
          /* NUMBER parameter too long */
          ccfcu_error = CCFCU_ERROR_TOKEN_4_INVALID_LENGTH;
          break;
        }
        ...
        {
          if (!dsatutil_strip_quotes_out(
                 tok_ptr->arg[4], tok_ptr->arg[4],
                 mixed_def_list[4]->def->string_len))
          {
            /* Quote stripping failed */
            ccfcu_error = CCFCU_ERROR_TOKEN_4_INVALID_STRING;
            break;
          }
        }
		
        /*Hold number in Ascii format for MMI string constuction */
        memset(&sups_ss_val->dsat_reg_number[0],0,CM_CALLED_PARTY_BCD_NO_LENGTH);

        if(CM_FORWARD_TO_URI == sscf_ptr->fwd_type)
        {          
          if(FALSE == dsatact_get_number_from_uri(tok_ptr->arg[4], num_string_len - 2,
		  	            sups_ss_val->dsat_reg_number, &num_len))
          {
            ccfcu_error = CCFCU_ERROR_TOKEN_4_INVALID_URI;
            break;
          }

          /* Get number pointer to validate it */
          digit = sups_ss_val->dsat_reg_number;

          sscf_ptr->fwd_to_uri = dsat_alloc_memory( sizeof(cm_forward_to_addr_uri_s_type), FALSE );

          if(sscf_ptr->fwd_to_uri == NULL)
          {
            ccfcu_error = CCFCU_ERROR_TOKEN_4_MEM_ALLOC_FAILED;
            break;
          }

          sscf_ptr->fwd_to_uri->fwd_to_uri = dsat_alloc_memory( sizeof(char) * (num_string_len - 2), FALSE );
          if(sscf_ptr->fwd_to_uri->fwd_to_uri == NULL)
          {
            ccfcu_error = CCFCU_ERROR_TOKEN_4_MEM_ALLOC_FAILED1;
            dsatutil_free_memory_ext((void **) &sscf_ptr->fwd_to_uri);
            break;
          }

          (void) dsatutil_memscpy((void*)sscf_ptr->fwd_to_uri->fwd_to_uri,
                  num_string_len - 2,(void*)tok_ptr->arg[4],
                  num_string_len - 2);
		  
          sscf_ptr->fwd_to_uri->fwd_to_uri_length = num_string_len - 2;
          /* Initializing object reference */
          ref_cnt_obj_init(sscf_ptr->fwd_to_uri, dsatetsicall_ccfcu_free_uri);
        }
        else
        {
          (void) dsatutil_memscpy((void*)&sups_ss_val->dsat_reg_number[0],
        ...
        while (*digit)
        {
          if ( ((*digit < '0') || (*digit > '9')) &&
               ...
               (*digit != 'a') && (*digit != 'b') && (*digit != 'c') )
          {
            ccfcu_error = CCFCU_ERROR_TOKEN_4_INVALID_DIGIT;
            break;
          }       
          digit++;
        }

        if(CM_FORWARD_TO_NUMBER == sscf_ptr->fwd_type)
        {
          /* Format number structure, accounting for '+' prefix. */
          /* '+' is stripped if detected.                        */
          cm_num_fill(&number, (byte *)tok_ptr->arg[4],
                      (byte)strlen((char*)tok_ptr->arg[4]),
                      CM_DIGIT_MODE_8BIT_ASCII);
          if (number.len)
          {
            /* Number type documented in GSM 04.08 section 10.5.4.7  */
      ...
      else
      {
        if(sscf_ptr->mode == DSAT_CCFC_MODE_REG)
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_4_INVALID;
          break;
        }
      }
      ...
      if (VALID_TOKEN(5))
      {
        ...
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_5_CONVERSION_FAILED;
          break;
        }
        /* Test for in-range */
        if ((mixed_def_list[5]->def->dflm.upper <  temp) &&
            (mixed_def_list[5]->def->dflm.lower >  temp))
        {
          ccfcu_error = CCFCU_ERROR_TOKEN_5_INVALID_RANGE;
          break;
        }
        ...
        if ((dsat_ccfc_allowed_classes != (dsat_ccfc_allowed_classes | temp)) &&
            ((dsat_num_item_type)DSAT_SS_CLASS_ALL != temp))
        {
          /* Class not mapped to allowed BSG */
          ccfcu_error = CCFCU_ERROR_TOKEN_5_NOT_SUPPORTED;
          break;
        }
        sups_ms_val->dsat_ss_cm_data.user_data.class = (uint8)temp;
      }
      ...
      {
        /* Default */
        sups_ms_val->dsat_ss_cm_data.user_data.class = (uint8)mixed_def_list[5]->def->dflm.default_v;
      }

      /* Token 6 : Ruleset 
         We are not supporting as of now
      */
      ...
      if (VALID_TOKEN(8))
      {
        ...
        {
          /* Cannot convert SATYPE parameter */
          ccfcu_error = CCFCU_ERROR_TOKEN_8_CONVERSION_FAILED;
          break;
        }
        /* Test for in-range */
        if ((mixed_def_list[8]->def->dflm.upper <  temp) ||
            (mixed_def_list[8]->def->dflm.lower >  temp))
        {
          /* SATYPE parameter outside range */
          ccfcu_error = CCFCU_ERROR_TOKEN_8_INVALID_RANGE;
          break;
        }
        sscf_ptr->satype = (uint8)temp;
      }
      ...
      {
        /* Test length */
        if (mixed_def_list[7]->def->string_len <
            strlen((char*)tok_ptr->arg[7]))
        {
          /* SUBADDR parameter too long */
          ccfcu_error = CCFCU_ERROR_TOKEN_7_CONVERSION_FAILED;
          break;
        }
        ...
        {
          if (!dsatutil_strip_quotes_out(
                 tok_ptr->arg[7], tok_ptr->arg[7],
                 mixed_def_list[7]->def->string_len))
          {
            /* Quote stripping failed */
            ccfcu_error = CCFCU_ERROR_TOKEN_7_INVALID_STRING;
            break;
          }
        }
      
        /* Encode SubAddress and SaType */
        etsicall_encode_subaddr(tok_ptr->arg[7], sscf_ptr->satype,
                                sscf_ptr->subaddr, &length);
        if (0 == length)
        {
          /* Cannot encode SubAddress parameter */
          ccfcu_error = CCFCU_ERROR_TOKEN_7_ENCODING_FAILED;
          break;
        }
      }
      ...
      {
        /* Convert to number */
        if (ATOI_OK != dsatutil_atoi(&temp, tok_ptr->arg[9], 10))
        {
          /* Cannot convert TIME parameter */
          ccfcu_error = CCFCU_ERROR_TOKEN_9_CONVERSION_FAILED;
          break;
        }
        /* Test for in-range */
        if ((mixed_def_list[9]->def->dflm.upper <  temp) ||
            (mixed_def_list[9]->def->dflm.lower >  temp))
        {
          /* TIME parameter outside range */
          ccfcu_error = CCFCU_ERROR_TOKEN_9_INVALID_RANGE;
          break;
        }
        sscf_ptr->time = (uint8)temp;
      }
      ...
      {
        /* Default */
        sscf_ptr->time = (uint8)mixed_def_list[9]->def->dflm.default_v;
      }
    ...
    if(ccfcu_error != CCFCU_ERROR_TOKEN_NO_ERROR)
    {
      DS_AT_MSG1_ERROR("+CCFCU Command processing fails with error %d", ccfcu_error);
      if(CM_FORWARD_TO_URI == sscf_ptr->fwd_type &&
         sscf_ptr->fwd_to_uri != NULL)
      {
        /* Releasing object reference */
        ref_cnt_obj_release(sscf_ptr->fwd_to_uri);
      }
      return DSAT_ERROR;
    }
    /* Initiate CM command */
    ...
    if(result == DSAT_ASYNC_CMD)
    {
      SET_PENDING(DSATETSI_EXT_ACT_CCFCU_ETSI_IDX, 0, DSAT_PENDING_CCFCU)
    }
  }
  /*--------------------------------------
  Processing for a TEST command:
  ...
  {
    byte buffer[((uint8)(DSAT_CCFCU_REASON_MAX - 1)*2)];
    byte * s_ptr = buffer;
    uint8 i = 0;

    /* -1 as Reason Deflection is not supported */
    ccfc_reason_e_type ccfcu_reason_list[DSAT_CCFCU_REASON_MAX - 1] =
    {
      DSAT_CCFC_REASON_UNCOND,
      DSAT_CCFC_REASON_BUSY,
      DSAT_CCFC_REASON_NOREPLY,
      ...
      DSAT_CCFC_REASON_ALLCALL,
      DSAT_CCFC_REASON_ALLCOND,
      DSAT_CCFCU_REASON_COMM_NOT_LOGGED_IN
    };

    /* Generate reason list */
    while (i < (uint8)(DSAT_CCFCU_REASON_MAX -1))
    {
      *s_ptr++ = (byte)ccfcu_reason_list[i] + '0';
      *s_ptr++ = ',';
      i++;
    }
    /* Replace last comma */
  ...
  else if (tok_ptr->op == (NA|QU) )
  {
    ...
    res_buff_ptr->used =
      (word)snprintf((char*)res_buff_ptr->data_ptr,res_buff_ptr->size,
                            "%s: %d,%d,%d",
                            parse_table->name,
                            (dsat_num_item_type)dsatutil_get_val( DSATETSI_EXT_ACT_CQI_ETSI_IDX,0,0,NUM_TYPE),
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
      case PIN_FAC:
        isFDN = ( (0 == strncmp((char *)tok_ptr->arg[0], "\"FD\"", strlen((char*)"\"FD\"")))||
                  (0 == strncmp((char *)tok_ptr->arg[0], "\"fd\"", strlen((char*)"\"fd\""))) );
        ...
        switch (ssfl_ptr->mode)
        {
          ...
          case DSAT_CLCK_MODE_QUERY:
            ...
            { /* Operation is for FD phone book */
              ...
              {
               DS_ATCOP_ERROR_LOG_0("SIM capabilities returned failure ");
            ...
            break;
          default:
           DS_ATCOP_ERROR_LOG_0("Unsupported MODE parameter");
            return DSAT_ERROR;
        }
      ...
      case PERSO_FAC:
        {
          ...
          req_data.sfs_type = SIMLOCK_SFS_LOCAL;
          ...
          if( DSAT_CLCK_MODE_LOCK == ssfl_ptr->mode)
          {
            ...
            simlock_result = simlock_ck_lock(&req_data,dsatme_sim_mmgsdi_perso_req_cb, user_data);
          ...
          else if (DSAT_CLCK_MODE_UNLOCK == ssfl_ptr->mode)
          {
            ...
            memset(&req_data_unlock, 0, sizeof(req_data_unlock));
            req_data_unlock.slot = req_data.slot;

            req_data_unlock.ignore_ck= FALSE;
            req_data_unlock.probe_ck = FALSE;
            ...
            simlock_result = simlock_unlock(req_data_unlock,dsatme_sim_mmgsdi_perso_req_cb, user_data);
            result = dsat_map_simlock_result(simlock_result);
            if (DSAT_ASYNC_CMD == result)
            {
              dsatme_pending_mmgsdi_command[apps_id] = TRUE;
          ...
          else if (DSAT_CLCK_MODE_QUERY == ssfl_ptr->mode)
          {
            ...
            simlock_result = simlock_get_status(dsatme_sim_mmgsdi_perso_req_cb, user_data);
            result = dsat_map_simlock_result(simlock_result);
            if (DSAT_ASYNC_CMD == result)
            {
              dsatme_pending_mmgsdi_command[apps_id] = TRUE;
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid no of Arguments in +CPWD Command");
      return DSAT_ERROR;
    }
    /* Initialize data structure */
    memset (&sups_ms_val->dsat_ss_cm_data.user_data, 0, sizeof(ss_user_data_s_type));
    ...
    if (VALID_TOKEN(1))
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Incorrect OLD Password Length =d",temp);
        return DSAT_ERROR;
      }
      else
      {
    ...
    if (VALID_TOKEN(2))
    {
      ...
      if ( (max_pw_len < temp) || (temp < min_pw_len) )
      {
        DS_ATCOP_ERROR_LOG_1("Incorrect NEW Password Length =d",temp);
        return DSAT_ERROR;
      }
      else
      {
      ...
      tok_ptr->op == NA)
  {
    ...
    if (DSAT_OK == result)
    {
      ...
      switch (mixed_val_list[0].num_item)
      {
        ...
        default:
          DS_ATCOP_ERROR_LOG_1("Unsupported CUSD <n> parameter: =d",
...
)
{
  ...
  dsatetsicall_call_state_da_info  *call_da_val = NULL;
  int i = 0, j = 0;
  boolean found_conf_participant = FALSE;
    ...
    if (VALID_TOKEN(0) && tok_ptr->args_found == 1 )
    {
      ...
      switch (sups_cmd)
      {
        ...
        case CM_CALL_SUPS_TYPE_SELECT_CALL_X:
          ...
          /*Special handling for sups_cmd CM_CALL_SUPS_TYPE_ACTIVE_CALL_X_REL
            Check whether seq_num is related to conference participant
          */
          if ( sups_cmd == CM_CALL_SUPS_TYPE_ACTIVE_CALL_X_REL)
          {
            ...
            {
              sups_cmd = CM_CALL_SUPS_TYPE_RELEASE_CALL_X_FROM_CONFERENCE;
              /* Searching participant info with this seq_num */
              for(i = 0; i < CM_CALL_ID_MAX; i++)
              {
                if(call_da_val->conf_participant_info[i].seq_number == seq_num)
                {
                  //Found participant and fill sups_params with URI details
                  found_conf_participant = TRUE;
                  sups_params.sups_number.len = call_da_val->conf_participant_info[i].user_uri_len;
                  for(j = 0; j < call_da_val->conf_participant_info[i].user_uri_len && j < CM_MAX_NUMBER_CHARS; j++)
                  {
                    sups_params.sups_number.buf[j] = call_da_val->conf_participant_info[i].user_uri[j];
                  }
                  sups_params.call_id = call_da_val->conf_linked_call_id;
                  break;
                }
              }
              if(found_conf_participant == FALSE)
              {
                DS_AT_MSG1_HIGH("No mapping of Sequence number %d with conference participant", seq_num);
                return DSAT_ERROR;
              }
            }
            else
            {
              if(call_id < CM_CALL_ID_MAX && 
                 call_da_val->mode_info[call_id] == CM_CALL_MODE_INFO_IP )
              {
                sups_cmd = CM_CALL_SUPS_TYPE_ACTIVE_CALL_REL;
              }
            }
          }
          break;
...
)
{
      ...
      if ( call_event->event_info.call_ss_success)
      {
        ...
        switch ( call_event->event_info.sups_type )
        {
          ...
          case CM_CALL_SUPS_TYPE_RELEASE_CALL_X_FROM_CONFERENCE:
            /* +CHLD=1x - Release a specific call with sequence number x */
            /* We will update cache during call end event or with disconnected status with call event CM_CALL_EVENT_CONFERENCE_INFO.
               For now we are freeing the terminal
            */
            clr_pending = TRUE;
            break;
          case CM_CALL_SUPS_TYPE_HOLD_CALL:
            ...
            /* Scenario:
               IP Voice Call: During 46(hold) or 47(retrieved) events, we will have moved
               active call to hold or hold call to active
            */
            if((call_da_val->was_active_call_hold || call_da_val->was_hold_call_retrieved) &&
                call_event->event_info.dsat_mode_info.info_type == CM_CALL_MODE_INFO_IP )
            {
              clr_pending = TRUE;
              DS_AT_MSG0_HIGH("Already active call moved to hold or hold to active");
              break;
            }
  ...
  /* It is possible that merging of calls may be initiated by client other than ATCoP.
     So store conference server call id. */
  if(call_event->event_info.sups_type == CM_CALL_SUPS_TYPE_MULTIPARTY_CALL &&
     call_event->event_info.dsat_mode_info.info_type == CM_CALL_MODE_INFO_IP)
  {
...
)
{
      ...
      (tok_ptr->op == (NA) ))
  {
    ...
    if ( VALID_TOKEN(0) )
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("PIN2 not specified");
        result = DSAT_ERROR;
      }
    }
    ...
    {
     DS_ATCOP_ERROR_LOG_0("PIN2 enabled but not specified");
  ...
  else if (tok_ptr->op == (NA|QU))
  {
                                    ...
                                    dsat_get_current_subs_id(FALSE)) )
    {
     ...
     DS_ATCOP_ERROR_LOG_0("Unable to retreive phone information");
    }
#else
      ...
      (tok_ptr->op == (NA) ))
  {
    ...
    if (tok_ptr->op == (NA|EQ|AR))
    {
      ...
      if ( VALID_TOKEN(0) )
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Cannot convert ACMMAX parameter");
          return DSAT_ERROR;
        }
      }
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Wrong ACMMAX value specified");
        result = DSAT_ERROR;
      }
    }
  }
  ...
  if (tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if ( VALID_TOKEN(1) )
    {
      ...
      while ('\0' != *digit)
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Invalid character in real number string");
          return DSAT_ERROR;
        }
        ...
        if ('.' == *digit)
        {
          ...
          {
           DS_ATCOP_ERROR_LOG_0("Multiple decimals in real number string");
            return DSAT_ERROR;
          }
          decimal_found = TRUE;
        }
      ...
      {
        DS_ATCOP_ERROR_LOG_0("PPU exponent exceeds limit");
         return DSAT_ERROR;
      }
...
/*---------------------------------------------------------
  ...
  if (tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    dsat_num_item_type prev_format = (dsat_num_item_type)dsatutil_get_val(
               DSATETSI_EXT_ACT_COPS_ETSI_IDX,subs_id,1,MIX_NUM_TYPE); /* list index */
    ...
    if ( ( DSAT_ERROR == result ) || ( DSAT_CMD_ERR_RSP == result ) )
    {
      ...
      DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_COPS_ETSI_IDX,subs_id,
                     1,0,(dsat_num_item_type)prev_format,MIX_NUM_TYPE)
    }
    return result;
  }
      ...
      (DSAT_COPS_ASTATE_WAIT_2 == ph_ss_ms_val->dsatetsicall_network_list.cmd_state) )
  {
    ...
    switch(ph_ss_ms_val->net_pref.mode)
    {
      ...
      case DSAT_COPS_MODE_MANUAL:
...
#else
          (void) cm_ph_cmd_subscription_available4 ( dsatcmif_ph_cmd_cb_func,
                                   NULL,
                                   dsatcm_client_id,
                                   dsat_get_current_subs_id(FALSE),
                                   CM_SUBSCRIPTION_STATUS_NO_CHANGE,
        ...
        break;
      default:
       DS_ATCOP_ERROR_LOG_0("Unsupported COPS mode ");
        break;
    }
...
)
{
  ...
  case DSAT_COPS_ASTATE_ANETS:
    {
      ...
      if(plmn_id_list.num_of_plmn_ids < 1 )
      {
        ph_ss_ms_val->dsatetsicall_network_list.cmd_state = DSAT_COPS_ASTATE_NULL;
        ph_ss_ms_val->dsatetsicall_network_list.cmd_idx = CMD_IDX_NONE;
       DS_ATCOP_ERROR_LOG_0("Invalid List length");
  ...
  default:
    DS_ATCOP_ERROR_LOG_1("Unsupported COPS state: =d", cmd_state);
    break;
  }
...
)
{
   ...
   if(ph_ss_ms_val->dsatetsicall_network_list.cmd_idx == CMD_IDX_QCCSGCOPS )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid or No csg_id, csg_rat");
      return DSAT_ERROR;
    }
    else
    {
    ...
    if(VALID_TOKEN(3))
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("cops_rat and csg_rat invalid pair");
        return DSAT_ERROR;
      }
    }
    else
    {
      if( !( (net_pref_ptr->csg_rat == SYS_RAT_UMTS_RADIO_ACCESS) || 
             (net_pref_ptr->csg_rat == SYS_RAT_LTE_RADIO_ACCESS) ) )
      {
          DS_ATCOP_ERROR_LOG_0("Invalid CSG_RAT");
           return DSAT_ERROR;
      }
    }
  }
         ...
         VALID_TOKEN(2))) && (DSAT_COPS_MODE_DEREG != net_pref_ptr->mode) )
    {
      ...
      if (( !VALID_TOKEN(2)) && 
          (ph_ss_ms_val->dsatetsicall_network_list.cmd_idx != CMD_IDX_QCCOPS || 
           ph_ss_ms_val->dsatetsicall_network_list.cmd_idx != CMD_IDX_QCCSGCOPS))
      {
       DS_ATCOP_ERROR_LOG_0("Missing COPS network identifier");
        return DSAT_ERROR;
      }
      ...
      case DSAT_COPS_FMT_LONG:
...
#endif /*FEATURE_MMGSDI*/
        {
          /* Name lookup failed so return error */
            DS_ATCOP_ERROR_LOG_0("COPS network name lookup failed");
      ...
      default:
    {
      DS_ATCOP_ERROR_LOG_1("Error: Unsupported format: =d", fmt);
      return DSAT_ERROR;
      break;
    }
    }
...
#define COPS_NETWORK_REPORTED 0xFF  /* control value to flag reported */
  ...
  {
   DS_ATCOP_ERROR_LOG_0 ( "Invalid state");
    return DSAT_ERROR;
  }
  memset ((void*)buffer, 0x0, sizeof(buffer));
  ...
  if(alist->length < 1)
  {
   DS_ATCOP_ERROR_LOG_0("+COPS command got aborted");
  ...
  else if( oper_name->client_ref == DSAT_MMGSDI_GET_CURR_OPER_NAME)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1(" Incorrect MMGSDI data found - =d plmn_ids ",oper_name->num_of_plmn_ids);
      return DSAT_ERROR;
    }
    ...
    default:
      DS_ATCOP_ERROR_LOG_1 ( "Unsupported format code: =d",fmt);
      return DSAT_ERROR;
    }
  ...
  switch(enc_type)
  {
    ...
    case MMGSDI_EONS_ENC_UCS2_PACKED:
        ...
        else
        {
         ...
         DS_ATCOP_ERROR_LOG_0("Empty buffer returned in UCS2 decoding");
          num_of_chars = 0;
        }
       break;
    default:
...
)
{
  ...
  DSATETSICALL_ASSERT(plmn_info_ptr != NULL);
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("etsicall_convert_mminfo_oper_name called on ntw name with max_len "
  ...
  case SYS_NW_NAME_CODING_SCHEME_UCS2:
    ...
    if( NULL == dsatutil_convert_chset( (char*)name_ptr, ALPHA_UCS2,
                                        ...
                                        num_chars, TRUE ))
    {
      DS_ATCOP_ERROR_LOG_0("UCS2 decode of MM info failed");
       return FALSE;
    }
    break;
...
)
{
  ...
  if ( VALID_TOKEN(index) )
  {
    ...
    { 
      DS_ATCOP_ERROR_LOG_1("+CBST parameter conversion error: =d",index);
      return FALSE; 
    }
  }
  else
...
)
{
  ...
  switch(*op_code)
  {
    ...
    default:
      ...
      DS_ATCOP_ERROR_LOG_1("Invalid operation code =d", *op_code);
      result = DSAT_ERROR;
  }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("LineID Buffer overflow =d", call_da_val->dsat_lineid_buf_length);
    return;
  }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Called LineID Buffer overflow =d", call_da_val->dsat_called_lineid_buf_length);
    return;
  }
...
)
{
  ...
  */
  ...
  DS_AT_MSG2_HIGH("dsatetsicall_report_clip_result(): Number len %d, pi %d", call_event_ptr->event_info.num.len, call_event_ptr->event_info.num.pi);
  if((call_event_ptr->event_info.dsat_mode_info.info_type == CM_CALL_MODE_INFO_GW_CS) &&
     ((boolean)((call_event_ptr->event_info.dsat_mode_info.info.gw_cs_call.calling_party_bcd_number.data[0]
  ...
  if(dsat_cli_presence == DSAT_CLI_ALLOWED)
  {
    ...
    if(((call_event_ptr->event_info.dsat_mode_info.info_type == CM_CALL_MODE_INFO_GW_CS) &&
       (call_event_ptr->event_info.dsat_mode_info.info.gw_cs_call.calling_party_subaddress.present == TRUE ))||
       (call_event_ptr->event_info.dsat_mode_info.info_type == CM_CALL_MODE_INFO_IP)) 
    {
      /* Subaddress */
  ...
  /* CLIP URC is prepared. It should be sent along with RING URC only */
...
)
{
  ...
  DSATETSICALL_ASSERT(NULL != tok_ptr);
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0(" Wrong function invocation ");
  }
  return DSAT_ASYNC_EVENT;
...
)
{
  ...
  if(tok_ptr->op == (NA|QU))
  {
    ...
    if( FALSE == ecall_session_get_call_info(
                    (dsatetsicall_cecall_cb_func)))
    {
     DS_ATCOP_ERROR_LOG_0("Unable to read status of the call");
      result = DSAT_ERROR;
    }
  ...
  else if(tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("CEcall :: Incorrect parameter  =d",cecall_type);
      result = DSAT_ERROR;
    }
    else
    { 
      ...
      switch(cecall_type)
      {
        case DSAT_CECALL_TEST:
             if( FALSE == ecall_session_start(
                             ...
                             (dsat_ecall_cb_func)))
             {
              DS_ATCOP_ERROR_LOG_0("Unable to start TEST call");
               result = DSAT_ERROR;
             }
             break;
        ...
        case DSAT_CECALL_RECONF:
            if( FALSE == ecall_session_start(
                            ...
                            (dsat_ecall_cb_func)))
            {
             DS_ATCOP_ERROR_LOG_0("Unable to start RECONFIG CEcall");
              result = DSAT_ERROR;
            }
            break;
        ...
        case DSAT_CECALL_MANUAL:
            if( FALSE == ecall_session_start(
                            ...
                            (dsat_ecall_cb_func)))
            {
             DS_ATCOP_ERROR_LOG_0("Unable to start Manual CEcall");
              result = DSAT_ERROR;
            }
            break;
        ...
        case DSAT_CECALL_AUTO:
            if( FALSE == ecall_session_start(
                            ...
                            (dsat_ecall_cb_func)))
            {
             DS_ATCOP_ERROR_LOG_0("Unable to start Auto CEcall");
              result = DSAT_ERROR;
            }
            break;
        
        default:
         DS_ATCOP_ERROR_LOG_0("Invalid CEcall option");
          result = DSAT_ERROR;
          break;
      }
    }
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid option");
    result = DSAT_ERROR;
  }
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("+CVMOD: Invalid arguement value  =d", voice_domain_pref);
      return DSAT_ERROR;
    }
  }else if(tok_ptr->op == (NA|QU))
  {
...
dsat_result_enum_type dsatetsicall_process_plmn_info(void)
{
  ...
  if(NULL == ph_ss_ms_val || NULL == call_da_val)
  {
    return result;
  }
   ...
   {
     DS_ATCOP_ERROR_LOG_0("Droping request since voice call is active");
      return DSAT_ERROR;
   }
   ...
   /* If the dereg_success should be true and the requested mode should not
    ...
    if ( ph_ss_ms_val->net_pref.last_dereg_success == TRUE )
    {
                                               ...
                                               DSAT_COPS_MODE_SETFMT)
       {
           ...
           {
            DS_ATCOP_ERROR_LOG_0("SIM is in illegal subscription state");
             /* Restore previous COPS value in case of ERROR */
             result = dsat_send_cme_error(DSAT_CME_SIM_FAILURE);
             return result;
           }
           if ( cm_ph_cmd_subscription_available4 ( dsatcmif_ph_cmd_cb_func,
                                   NULL,
                                   dsatcm_client_id,
                                   subs_id,
                                   CM_SUBSCRIPTION_STATUS_NO_CHANGE,
                                   CM_SUBSCRIPTION_STATUS_CHANGE) == TRUE )
    ...
    switch (ph_ss_ms_val->net_pref.mode)
    {
       ...
       default:
          DS_ATCOP_ERROR_LOG_1("Error: Unsupported mode: =d", 
                     ph_ss_ms_val->net_pref.mode);
          result = DSAT_ERROR;
          break;
    }
...
)
{
  ...
  else if( tok_ptr->op == (NA|QU) )
  {
    res_buff_ptr->used = (word)snprintf((char*)res_buff_ptr->data_ptr,
                                                res_buff_ptr->size,
                                                "+CMCCS: %d", (dsat_num_item_type)dsatutil_get_val(
  ...
  else if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if(ATOI_OK != dsatutil_atoi(&cmccs_val, tok_ptr->arg[0], 10))
    {
      return DSAT_ERROR;
    }

    DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CMCCS_ETSI_IDX, 0, 0, 0, cmccs_val, NUM_TYPE)

    if(cmccs_val == DSAT_CMCCS_VAL_LIST_SUPPORTED_SUPS)
    {
      /* Display supported URCs for supplementary service.
         Currently we are not supporting URCs for supplementary service */
      res_buff_ptr->used = (word)snprintf((char*)res_buff_ptr->data_ptr,
                                                res_buff_ptr->size,
                                                "+CMCCS:");
    }
  }
  /* Spec does not talk about if +CMCCS= is given. Doing nothing  and returning OK */
...
{
  dsat_num_item_type    ccidx = 0, neg_status;
  sys_modem_as_id_e_type subs_id = dsat_get_current_subs_id(FALSE);
  dsatetsicall_call_state_da_info  *call_da_val = dsat_get_base_addr_per_subs(DSAT_CALL_DA_VALS, subs_id, TRUE);
  cm_ip_call_attrib_info_s_type  call_attrib;
  cm_call_type_e_type call_type = CM_CALL_TYPE_NONE;
  cm_call_id_type     call_id;
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    do
    {
      ...
      /* Allow command only when at least there is one active call. */
      if(call_da_val->etsicall_num_active_calls < 1)
      {
        error_idx = 2;
        break;
      ...
      /* Validate ccidx: sequence number */
      if(!VALID_TOKEN(0) ||
          (ATOI_OK != dsatutil_atoi(&ccidx, tok_ptr->arg[0], 10)))
      {
        error_idx = 3;
        break;
      }
      /* ccidx should be in valid range */
      if(ccidx > CM_CALL_ID_MAX || ccidx < 1)
      {
        error_idx = 4;
        break;
      }
      /* Call type for ccidx should not be other than VOICE or VT */
      if(call_da_val->dsat_seqnum_callid[ccidx - 1].call_type != CM_CALL_TYPE_VOICE &&
         call_da_val->dsat_seqnum_callid[ccidx - 1].call_type != CM_CALL_TYPE_VT)
      {
        error_idx = 5;
        break;
      }
      /* call id should be valid for ccidx */
      if((call_id = dsatetsicall_seqnum_to_callid(subs_id, ccidx)) == CM_CALL_ID_INVALID)
      {
        error_idx = 6;
        break;
      }
      ...
      if(!VALID_TOKEN(1) ||
         (ATOI_OK != dsatutil_atoi(&neg_status, tok_ptr->arg[1], 10))||
         (neg_status > DSAT_NEG_STATUS_REJECT || neg_status < DSAT_NEG_STATUS_DESIRED))
      {
        error_idx = 7;
        break;
      }
      ...
      /* Validate SDP Information.
         As of now, only "m=audio" and "m=video" is supported.
      */
      if(VALID_TOKEN(2))
      {
        if( !dsatutil_strip_quotes_out ( tok_ptr->arg[2], tok_ptr->arg[2],
                                         (uint16)strlen((char*)tok_ptr->arg[2]) ) )
        {
          error_idx = 8;
          break;
        }
        if(0 == (dsatutil_strncmp_ig_sp_case((const byte *)tok_ptr->arg[2],
                  (const byte *)"m=audio" ,strlen((const char *)"m=audio"))))
        {
          call_type = CM_CALL_TYPE_VOICE;
        }
        else if(0 == (dsatutil_strncmp_ig_sp_case((const byte *)tok_ptr->arg[2],
                      (const byte *)"m=video" ,strlen((const char *)"m=video"))))
        {
          call_type = CM_CALL_TYPE_VT;
        }
        /* Return error if call type is same as before */
        if(call_da_val->dsat_seqnum_callid[ccidx - 1].call_type == call_type ||
           call_type == CM_CALL_TYPE_NONE)
        {
          /* Exception for Video call */
          if(call_type != CM_CALL_TYPE_VT)
          {
            error_idx = 9;
            break;
          }
      ...
      {
        /* SDP_MD is mandate for neg_status 1 and 2 */
        if(neg_status == DSAT_NEG_STATUS_DESIRED ||
           neg_status == DSAT_NEG_STATUS_PROPOSED )
        {
          error_idx = 10;
          break;
        }
        /* ccidx voice state shoulb be active*/
        if(voice_state[subs_id][call_da_val->dsat_seqnum_callid[ccidx - 1].call_id].state == DSAT_VOICE_STATE_ACTIVE)
        {
          call_type = call_da_val->call_modify_param.req_call_type;
        }
      }
      /* Setting call attributes */
      memset( &call_attrib , 0 , sizeof(call_attrib));
      call_attrib.audio_attrib = CM_CALL_ATTRIB_TX_RX;

      if(call_type == CM_CALL_TYPE_VOICE)
      {
        call_attrib.video_attrib = CM_CALL_ATTRIB_NONE;
      }
      else if(call_type == CM_CALL_TYPE_VT)
      {
        ...
        {
          call_attrib.video_attrib = CM_CALL_ATTRIB_RX_ONLY;
        }
        else if(call_attr == DSAT_QCCALLATTR_CALL_ATTR_TX)
        {
          call_attrib.video_attrib = CM_CALL_ATTRIB_TX_ONLY;
        }
        else
        {
          call_attrib.video_attrib = CM_CALL_ATTRIB_TX_RX;
        }
      }
      else
      {
      ...
      if(neg_status == DSAT_NEG_STATUS_DESIRED ||
         neg_status == DSAT_NEG_STATUS_PROPOSED )
      {
        /* Calling CM API for requesting call modification */
        if(!cm_mm_call_cmd_modify_req_ext( dsatcmif_call_cmd_cb_func,
                                       NULL, 
                                       dsatcm_client_id,
                                       call_id,
                                       call_type,
      ...
      {
        /* Calling CM API for accepting or rejecting call modification */
        call_da_val->call_modify_param.modify_response = (neg_status == DSAT_NEG_STATUS_ACCEPT)?TRUE:FALSE;
        if(!cm_mm_call_cmd_modify_res_ext( dsatcmif_call_cmd_cb_func,
                                       NULL, 
                                       dsatcm_client_id,
                                       call_id,
                                       call_da_val->call_modify_param.modify_response,
                                       NULL,
                                       call_type,
                                       &call_attrib))
        {
      ...
      SET_PENDING(DSATETSI_EXT_ACT_CCMMD_ETSI_IDX, 0, DSAT_PENDING_TRUE)
      call_da_val->call_modify_param.last_ccmmd_neg_status = neg_status;
      return DSAT_ASYNC_CMD;
    }
    while (0);
  }
  /* Doing nothing for read and test command and returning OK Only */
  ...
  DS_ATCOP_ERROR_LOG_1("Operation not allowed error =d", error_idx);
