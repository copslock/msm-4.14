...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsime_ex.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
#define DSATETSIME_EX_ASSERT(expression) \
         dsatetsime_ex_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATETSIME_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATETSIME_EX ASSERT
...
===========================================================================*/
static void dsatetsime_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATETSIME_EX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatetsime_ex_assert_wrapper */
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Invalid subscription specified: =d",subs_id);
  ...
  {
    /* Report error */
    DS_ATCOP_ERROR_LOG_1("Unsupported indicator specified: =d",indicator);
    result = DSAT_ERROR;
  }
...
)
{
  ...
  switch (mmgsdi_event)
  {
    case MMGSDI_SUBSCRIPTION_READY_EVT:
      ...
      {
       DS_ATCOP_ERROR_LOG_0(" Initial status query for all PINs failed");
      }
    break;
...
#define SET_FILENAME(app_type, file_gsm, file_umts)\
   ...
   DSATETSIME_EX_ASSERT(filename != NULL ); 
