...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatctab_ex.c#5 $ $DateTime: 2019/11/08 09:46:23 $ $Author: pwbldsvc $
...
03/22/17   skc     Fixed AT$QCSIMSTAT test command display issue.
04/07/16   skc     Fixed issue with AT Command +IPR and $QCTER.
01/27/16   skc     Added $QCRSCP and $QCECNO command Support.
03/17/15   tk      Fixed issue in getting QCSIAMPP value.
...
/*$QCCALLTYPE Command*/
LOCAL const dflm_type dsat_qccallattr_calltype_dflm[] =
{
  { 0, 0, 1},
};

LOCAL const dflm_type dsat_qccallattr_attr_dflm[] =
{
  { 0, 0, 2},
};

LOCAL const mixed_def_s_type dsat_qccallattr_calltype_def_val =
{
  CONFIG | LOCAL_TEST, (mixed_def_u_type *) &dsat_qccallattr_calltype_dflm
};

LOCAL const mixed_def_s_type dsat_qccallattr_attr_def_val =
{
  CONFIG | LOCAL_TEST, (mixed_def_u_type *) &dsat_qccallattr_attr_dflm
};

LOCAL const mixed_def_s_type * dsat_qccallattr_mixed_dfl[2] = 
{
  &dsat_qccallattr_calltype_def_val,
  &dsat_qccallattr_attr_def_val
};
...
/* $QCCCFC Command */
extern const mixed_def_s_type * dsat_ccfc_mixed_dfl[];
...
/* Exclude CDMA (for now, may be required later) */
  ...
  { "+CFUND",    EXTENDED | CONFIG | LOCAL_TEST | LIST | MIXED_PARAM | 
    ...
    SPECIAL_NONE, 2,DSAT_EXT_CFUND_IDX,   dsat_cfun_mixed_dfl },
...
const dsati_cmd_type dsat_vendor_table [] =
{
  ...
  { "$QCSIMAPP",  EXTENDED | CONFIG | LOCAL_TEST | RESTRICTED | COMMON_CMD| NOT_FORWARDABLE,
  ...
  { "$QCPWRDN",  EXTENDED | COMMON_CMD| RESTRICTED | AT_FWD_CMDS,
...
#ifdef FEATURE_WCDMA
  ,{ "$QCRSCP", READ_ONLY| EXTENDED | UMTS_CMD,
     SPECIAL_NONE, 0 , DSAT_VENDOR_QCRSCP_IDX, NULL }
  ,{ "$QCECNO", READ_ONLY| EXTENDED | UMTS_CMD,
     SPECIAL_NONE, 0 , DSAT_VENDOR_QCECNO_IDX, NULL }
#endif /* FEATURE_WCDMA */

,{ "$QCPCOLIST",  EXTENDED | LOCAL_TEST | COMMON_CMD | STRING ,
     SPECIAL_NONE,  1,  DSAT_VENDOR_QCPCOLIST_IDX,    NULL }

 ,{ "$QCMSISDN",  EXTENDED | LOCAL_TEST | COMMON_CMD,
     SPECIAL_NONE,  1,  DSAT_VENDOR_QCMSISDN_IDX,    NULL }

,{ "$QCPCOMSISDN",  EXTENDED | LOCAL_TEST | UMTS_CMD | STRING,
     SPECIAL_NONE,  12,  DSAT_VENDOR_QCPCOMSISDN_IDX,    NULL }

,{ "$QCCALLATTR",	EXTENDED | CONFIG | MIXED_PARAM | LIST | LOCAL_TEST | UMTS_CMD,
     SPECIAL_NONE,	2,  DSAT_VENDOR_QCCALLATTR_IDX,	 &dsat_qccallattr_mixed_dfl[0] }

,{ "$QCIMSSRV",  EXTENDED | LOCAL_TEST | COMMON_CMD,
     SPECIAL_NONE,	1,	DSAT_VENDOR_QCIMSSRV_IDX,	 NULL }

,{ "$QCCCFC",    EXTENDED | LOCAL_TEST | LIST | MIXED_PARAM | UMTS_CMD,
    SPECIAL_NONE, 9, DSATETSI_EXT_ACT_CCFC_ETSI_IDX, &dsat_ccfc_mixed_dfl[0] }
...
void dsat_init_qcsimapp_table(void)
{
  ...
  dsat_qcsimapp_table[DSAT_SLOT_IDX_2].apps_id_mapped[DSAT_APPS_ID_1X_SECONDARY] = TRUE;
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Subs_id is not mapped to any Slot, ignoring setting it as qcsimapp_val");
    return;
  }
