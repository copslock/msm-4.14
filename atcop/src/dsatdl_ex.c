...
$Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatdl_ex.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
#define DSATDL_EX_ASSERT(expression) \
         dsatdl_ex_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATDL_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATDL_EX ASSERT
...
===========================================================================*/
static void dsatdl_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATDL_EX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatdl_ex_assert_wrapper */
...
)
{
  ...
  DSATDL_EX_ASSERT(symbol_table_ptr != NULL );
...
LOCAL void dsatdl_log(void)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("DL Info: error =d", dsatdl_info.error);
  }
...
LOCAL void dsatdl_unload(void)
{
  ...
  if (NULL != dsatdl_info.handle)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("dlclose returned =d", dlclose_ret);
