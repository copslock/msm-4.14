...
INITIALIZATION AND SEQUENCING REQUIREMENTS
   Copyright (c) 2001 - 2017 Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatvoice_ex.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
dsatvoice_ring_info_s_type dsatvoice_ring_info[DSAT_SYS_MODEM_AS_ID_MAX];
/*===========================================================================
...
#define DSATVOICE_EX_ASSERT(expression) \
         dsatvoice_ex_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATVOICE_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATVOICE_EX ASSERT
...
===========================================================================*/
static void dsatvoice_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATVOICE_EX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatvoice_ex_assert_wrapper */
...
)
{
  ...
  DSATVOICE_EX_ASSERT( NULL != call_info_ptr );
  ...
  if ( dsatcmif_is_voice_call_type (call_info_ptr->call_type) )
  {
             ...
             voice_state[subs_id][call_info_ptr->call_id].cmd_active )
    {
      ...
      else
      {
        ...
        /* Call in LTE will be ended after SRVCC but call will be still up in GW.
           So dont return NO_CARRIER */
        else if(call_da_val->dsatetsicmif_call_end_status.error_info.cs_domain.end_status == 
                CM_CALL_END_SRVCC_IMS_END)
        {
          result = DSAT_ASYNC_EVENT;
        }
        else
...
)
{
  ...
  dsatetsicall_call_state_da_info *call_da_val = NULL;
  ds_at_call_info_s_type *call_info_ptr = &call_event->event_info;
  ...
  if(dsatcmif_is_voice_call_type (call_info_ptr->call_type ))
  {
    if(dsatact_callid_to_seqnum(subs_id, call_info_ptr->call_id) != CALL_SEQNUM_INVALID && call_info_ptr->call_id < CM_CALL_ID_MAX)
    {
      voice_state[subs_id][call_info_ptr->call_id].call_mode = call_info_ptr->dsat_mode_info.info_type;
      call_id_found = TRUE;
    }
    ...
    {
      if (call_info_ptr->call_state == CM_CALL_STATE_INCOM)
      {
        /*--------------------------------------------------------------------------------
          Scenerio:
          1) Start an MT voice call over IMS(3rd party), RING is sent to UI.
          2) Don’t answer the call, and do SRVCC to GSM
          3) Voice call handover complete successful on GSM
          4) Answer incoming voice call using ‘ATA’ command  
          
          In case of 3rd party IMS, after SRVCC to GSM, ATCoP is unaware of unanswered
          incoming calls events hence answering with ATA fails.
          This is to handle answering unanswered incoming calls after SRVCC to GSM
          using ATA.
          No harm in handling this event with QC-IMS when call state is CM_CALL_STATE_INCOM.
        ------------------------------------------------------------------------------------*/
        dsatact_track_sequence_number(subs_id,
                     call_info_ptr->call_id,
                     CM_CALL_EVENT_INCOM,
        ...
        dsatetsicall_call_event_incom(call_event ,subs_id);
      }
    }
#endif /* FEATURE_DSAT_ETSI_MODE */
  }

  /* Set the flag handover complete, the flag will be used if handover happen from LTE to GW
     i.e. SRVCC in MPTY Case */
  if (NULL != call_da_val && 
      call_da_val->etsicall_num_participant_in_conf > 0)
  {
    call_da_val->handover_complete = TRUE;    
  }
  
  DS_AT_MSG6_HIGH("evt_ho:Hand over complete for call id = %d call mode = %d call type %d call state %d subs_id %d call id found = %d",
                       call_info_ptr->call_id,
                       call_info_ptr->dsat_mode_info.info_type,
                       call_info_ptr->call_type,
...
)
{
  ...
  if ( dsatcmif_is_voice_call_type (call_info_ptr->call_type) )
  {
         ...
         (call_info_ptr->call_type == CM_CALL_TYPE_VT))
    {
      ...
      call_da_val->call_info_before_connect.call_id = call_info_ptr->call_id;
      call_da_val->call_info_before_connect.info_type = call_info_ptr->dsat_mode_info.info_type;
    }
  }
...
)
{
  ...
  switch (call_info_ptr->local_hold_state)
  {
    ...
    default:
     DS_ATCOP_ERROR_LOG_0("Invalid local hold state");
      break;
  }
...
)
{
  ...
  dsatvoice_ring_info_s_type voice_ring_info;
  ...
  switch (*timer_id_ptr)
  {
    case DSAT_TIMER_VOICE_RING:
      /* continue ring te */
      {
        ...
        {
          voice_ring_info.sys_mode = (sys_sys_mode_e_type)((dsatvoice_ring_info_s_type *)(t_ptr->user_data))->sys_mode;
          voice_ring_info.subs_id = (sys_modem_as_id_e_type)((dsatvoice_ring_info_s_type *)(t_ptr->user_data))->subs_id;
          voice_ring_te(voice_ring_info.subs_id, voice_ring_info.sys_mode);
        }
      }
...
)
{
  ...
  for ( call_id = 0; call_id < CM_CALL_ID_MAX; call_id++ )
  {
    ...
    if ( voice_state[subs_id][call_id].state == DSAT_VOICE_STATE_CONNECTING )
    {
      ...
      if(!call_status)
      {
       DS_ATCOP_ERROR_LOG_0("ATCOP's end call request is denied by CM");
      }
      return;
    } 
  }

 DS_ATCOP_ERROR_LOG_0("ATD/ATA abort handler and no connecting call");
} /* dsatvoice_voice_call_abort_handler */
...
)
{
  ...
  for ( call_id = 0; call_id < CM_CALL_ID_MAX; call_id++ )
  {
    ...
    if ( voice_state[subs_id][call_id].state == DSAT_VOICE_STATE_WAITING )
    {
                                   ...
                                   DSAT_SREG_S0_IDX,0,0,NUM_TYPE) != 0 )
        {
                                         ...
                                         DSAT_SREG_S0_IDX,0,0,NUM_TYPE))
          {
            ...
            if (TRUE == dsatvoice_validate_call_cmd(subs_id, DSAT_CALL_CMD_ANSWER))
            {
              ...
              if(dsatcmif_answer_call(subs_id,call_id,TRUE,voice_state[subs_id][call_id].call_mode))
              {
              ...
              else
              {
               ...
               DS_ATCOP_ERROR_LOG_0("ATCOP's auto answer-call is denied by CM");
      ...
      if( !call_answered )
      {
       ...
       if(t_ptr->user_data != NULL)
       {
         ((dsatvoice_ring_info_s_type *)(t_ptr->user_data))->sys_mode = sys_mode;
         ((dsatvoice_ring_info_s_type *)(t_ptr->user_data))->subs_id = subs_id;
       }
        /*---------------------------------------------------------------------
 ...
 DS_ATCOP_ERROR_LOG_0("voice_ring_te() called and no DSAT_VOICE_STATE_WAITING call found");
...
)
{
...
#endif /* FEATURE_DSAT_ETSI_MODE */
  {
    ...
    else if ( TRUE == dsat_pdp_state.pending )
    {
      ...
      DSATVOICE_EX_ASSERT(((NULL != t_ptr && t_ptr->user_data != NULL) == TRUE));
      ...
      if (!cm_result)
      {
       DS_ATCOP_ERROR_LOG_0("Problem invoking CM API on deactivation");
      }
...
{
  qsh_mdump_collect_high(&voice_state, DSAT_SYS_MODEM_AS_ID_MAX * CM_CALL_ID_MAX * sizeof(voice_state_type));
}
#endif /* FEATURE_QSH_MDUMP */
