...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsismsu.c#1 $
...
===========================================================================*/
#define DSATETSISMSU_ASSERT(expression) \
         dsatetsismsu_assert_wrapper(__LINE__, expression)
...
#define CONDITIONAL_NULL_CHECK(ptr)    \
  if (ptr == NULL)                   \
  {                                    \
   DS_ATCOP_ERROR_LOG_0("Unexpected NULL pointer");  \
    return 0;                          \
  }
...
FUNCTION DSATETSISMSU_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATETSISMSU ASSERT
...
===========================================================================*/
static void dsatetsismsu_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATETSISMSU_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatetsismsu_assert_wrapper */
...
)
{
  ...
  switch ( tpdu_msg_ptr->u.gw_pp.tpdu_type )
  {
    case WMS_TPDU_DELIVER:
    {
      ...
      *w_ptr++ = ',';
      ...
      if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CSDH_IDX,0,0,NUM_TYPE))
      {
        ...
        *w_ptr++ = ',';
        w_ptr = psutil_itoa((uint32) type_of_addr, w_ptr, 10);
            ...
            dsatsmsi_pres_cmd.cmd_name == SMS_CNMA)
        {
          ...
          *w_ptr++ = ',';
          w_ptr = psutil_itoa((uint32) raw_data[0], w_ptr, 10);

          /* write <pid> to the result buffer */
          *w_ptr++ = ',';
          w_ptr = psutil_itoa((uint32)deliver_msg_ptr->pid, w_ptr, 10);

          /* write <dcs> to the result buffer */
          /* get length of TP-OA */
          *w_ptr++ = ',';
          w_ptr = psutil_itoa((uint32) dcs_val, w_ptr, 10);
        ...
        *w_ptr++ = ',';
        ...
        w_ptr = psutil_itoa((uint32) len, w_ptr, 10); 
      }/* if (dsat_csdh_val)*/
        ...
        *w_ptr++ = ',';
        ...
        if( dsatsmsi_pres_cmd.cmd_name == SMS_CMGR )
        {
          ...
          *w_ptr++ = ',';
          ...
          /* write <pid> to the result buffer */
          *w_ptr++ = ',';
          w_ptr = psutil_itoa((uint32) submit_msg_ptr->pid, w_ptr, 10);
          ...
          *w_ptr++ = ',';
          ...
          {
            w_ptr = psutil_itoa((uint32) raw_data[tp_da_len + 4], w_ptr, 10);
          }
    ...
    case WMS_TPDU_STATUS_REPORT:
    {
      ...
      *w_ptr++ = ',';
      w_ptr = psutil_itoa(status_report_ptr->message_reference, w_ptr, 10);
      ...
      *w_ptr++ = ',';
      w_ptr = psutil_itoa((uint32) status_report_ptr->tp_status, w_ptr, 10);
      res_buff_ptr->used = (uint16)(w_ptr - res_buff_ptr->data_ptr);
    }
    break;
    ...
    case WMS_TPDU_COMMAND:
    {
      ...
      *w_ptr++ = ',';
      w_ptr = psutil_itoa((uint32) command_msg_ptr->command, w_ptr, 10);
                                    ...
                                    && (dsatsmsi_pres_cmd.cmd_name == SMS_CMGR))
      {
        /* write <pid> to the result buffer */
        *w_ptr++ = ',';
        w_ptr = psutil_itoa((uint32) command_msg_ptr->pid, w_ptr, 10);

        /* write <mn> to the result buffer */
        *w_ptr++ = ',';
        w_ptr = psutil_itoa(command_msg_ptr->message_number, w_ptr, 10);
        ...
        *w_ptr++ = ',';
        w_ptr = psutil_itoa((uint32) command_msg_ptr->command_data_len, 
                              w_ptr, 10);
        *w_ptr++ = '\n';
...
)
{
       ...
       dsatsmsi_pres_cmd.cmd_name == SMS_CNMA )
  {
      ...
      */
      if(is_alpha_present)
      {
        ...
        for( i=0; i<len; i++)
        {
          ...
          w_ptr = psutil_itoa((uint32) me_ss_val->dsat_alpha_text[i], w_ptr, 16 );
        }
        is_alpha_present = FALSE;
      }
      *w_ptr++ = ',';
    ...
    w_ptr = psutil_itoa((uint32)msg_ptr->u.gw_message.raw_ts_data.len,
                          w_ptr,10);
    *w_ptr++ = '\n';
...
)
{
  ...
  else 
  {
   ...
   DS_ATCOP_ERROR_LOG_0("Service Center address not given");
    return FALSE;
  }
} /* dsatetsismsi_get_sca */

...
#define UCS2 2
  ...
  DSATETSISMSU_ASSERT( res_ptr != NULL);
  DSATETSISMSU_ASSERT( type_of_addr != NULL);

  if ( addr.number_of_digits > WMS_ADDRESS_MAX )
  {
    DS_ATCOP_ERROR_LOG_1("Invalid num of digits =d in address",addr.number_of_digits);
    return NULL;
  }
  cscs_val =(dsat_num_item_type)dsatutil_get_val(
                                             DSATETSI_EXT_CSCS_IDX,0,0,NUM_TYPE);
  ...
  while(bcd_idx < addr.number_of_digits)
  {
                   ...
                   DSATETSI_EXT_CMGF_IDX,0,0,NUM_TYPE)) || is_csca )
    {
      switch(addr.digits[bcd_idx])
      {
          ...
          default:
            addr_ira_ptr = 
              (char *)psutil_itoa( (uint32) addr.digits[bcd_idx], 
                                     (byte *)addr_ira_ptr, 
                                     10 );
            break;
      }/* switch */
    ...
    {
      res_ptr = psutil_itoa((uint32) addr.digits[bcd_idx], res_ptr, 16);
      ++num_len;
    }
    bcd_idx++;
  }
...
)
{
  ...
  res_ptr = psutil_itoa( (uint32) len, res_ptr, 16 );
  ...
  if ( addr.number_of_digits )
  {
    ...
    res_ptr = psutil_itoa((uint32) type_of_addr, res_ptr, 16);
    ...
    while( (loop < addr.number_of_digits ) && (loop < (sizeof(addr.digits) -1)))
    {
      ...
      {
        *res_ptr++ = 'F';
        res_ptr = psutil_itoa((uint32) addr.digits[loop], res_ptr, 16);
      }
      else
      {
        res_ptr = psutil_itoa((uint32) addr.digits[loop+1], res_ptr, 16);
        res_ptr = psutil_itoa((uint32) addr.digits[loop], res_ptr, 16);
      }
      loop = loop + 2;
    }
...
)
{
  ...
  if (vp_ptr == NULL)
  {
   DS_ATCOP_ERROR_LOG_0("Unexpected NULL ptr");
    return FALSE;
  }
