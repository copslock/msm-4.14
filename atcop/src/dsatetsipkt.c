...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsipkt.c#2 $   $DateTime: 2019/10/31 06:19:41 $ $Author: yarora $
  
when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/29/18   skc     APN name with '-' hyphen or '.' dot is not allowed.
08/01/17   rg      Fixed +CGDCONT delete profile issue on non-DDS
07/21/17   ss      Added support for eMBMS AT commands
07/18/17   rk      Added PDP reject cause codes.
03/18/16   skc     Fixed issue with getting primary profile list
03/06/15   sc      Fixed +CGDCONT AT read command parameter display issue.
02/11/15   sc      Fixed compilation errors when feature flag
...
#define DSATETSIPKT_ASSERT(expression) \
         dsatetsipkt_assert_wrapper(__LINE__, expression)
...
/*-------------------------------------------------------------------
 eMBMS Global Variables 
 ------------------------------------------------------------------*/
dsat_embms_svc_interest_req_type *dsat_embms_svc_interest_list = NULL;
dsat_embms_tmgi_list_type *dsat_embms_active_tmgi_list = NULL;
static uint8 embms_status = DSAT_EMBMS_DISABLED;
byte activate_tmgi_str[TMGI_HEX_MAX_LENGTH];
...
LOCAL boolean dsatetsipkt_embms_change_status
(
  uint8 new_status
);

LOCAL dsat_result_enum_type dsatetsipkt_embms_query_tmgi_list
(
  dsat_num_item_type service_type ,
  dsm_item_type *res_buff_ptr ,
  sys_modem_as_id_e_type subs_id
);

LOCAL dsat_result_enum_type dsatetsipkt_embms_convert_tmgi_str_to_rrc_fmt 
(
  byte *in_ptr,
  byte *out_ptr,
...
LOCAL dsat_num_item_type dsatetsipkt_embms_get_inter_freq_count
(
  dsat_embms_sai_info_type * sai_list 
);

LOCAL boolean dsatetsipkt_embms_convert_str_to_rrc_list
(
byte *in_str,
uint8 max_items,
void *out_list,
uint8 * out_list_len,
dsat_embms_list_type list_type
);
...
FUNCTION DSATETSIPKT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATETSIPKT ASSERT
...
===========================================================================*/
static void dsatetsipkt_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATETSIPKT_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatetsipkt_assert_wrapper */
...
)
{
  ...
  *total_profiles = 0;
  ...
  while(profile_status != DS_PROFILE_REG_RESULT_LIST_END)
  {
    ...
    if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
    {
      DS_ATCOP_ERROR_LOG_1 ("Getting profile info by iterator failed. Error =d",
                         profile_status);
      result = DSAT_ERROR;
      goto return_result;
    }
...
return_result:
  ...
  if (itr != NULL)
  {
    ...
    if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS) 
    { 
      DS_ATCOP_ERROR_LOG_1 ("Destroying iterator failed. Error =d", profile_status); 
    }
  }
  return result;
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
...

    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Error CGDCONT PDP profile read: =d",val);
    ...
    if (1 == tok_ptr->args_found)
    {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Cannot reset SECONDARY context with CGDCONT: =d",val);
          return DSAT_ERROR;
        }
        else
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1("Error resetting linked context: =d",val);
            return DSAT_ERROR;
          }
    ...
    else
    {
                                            ...
                                            ))
      {
        ...
        else
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1("Error CGDCONT PDP profile reset: =d",cid);
            return DSAT_ERROR;
          }
        }
        ...
        if( VALID_TOKEN(6) )
        {
          ...
          {
           DS_ATCOP_ERROR_LOG_0("Cannot convert ipv4 addr alloc type parameter");
            return DSAT_ERROR;
          }
          else
          {
        ...
        if( VALID_TOKEN(7) )
        {
          ...
          {
           DS_ATCOP_ERROR_LOG_0("Cannot convert emergency flag");
            return DSAT_ERROR;
          }
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Error CGDCONT PDP profile write: =d",cid);
          return DSAT_ERROR;
        }
        ...
        if(dsatetsipkt_read_write_param_table(dsat_cgdcont_ext_table,
                                              ...
                                              dsat_get_current_ds_profile_subs_id()) == DSAT_ERROR)
        {
          DS_ATCOP_ERROR_LOG_1("Error CGDCONT EXT Table: =d",cid);
          return DSAT_ERROR;
        }
      }
      else
  ...
  else if (tok_ptr->op == (NA|EQ|QU))
  {
    ...
    if (0 == dsatetsipkt_cgdcont_response_fmt (parse_table,
                                               ...
                                               MULTI_INDEX_TEST))
    {
     DS_ATCOP_ERROR_LOG_0("Problem generating query response");
      result = DSAT_ERROR;
    }
  }
...
send_error:
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Error:=d",err_info.errval);
  }
  else
  {
...
)
{
  ...
  else if (tok_ptr->args_found == 2) 
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Invalid profile =d",cid);
      return DSAT_ERROR;
    }
    /* Make conditional edits to profile */
  ...
  else if ( tok_ptr->args_found == 1) 
  {
    ...
    for (i = 0; i < total_profiles; i++) 
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Error PDP profile read: =d",profile_list[i]);
        return DSAT_ERROR;
      }
      if(secon_flag == TRUE)
      {
...
)
{
  ...
  switch (format)
  {
    case MULTI_INDEX_TEST:
      ...
      /*Range we support for 3GPP only profiles is: 1-24 */
    ...
    case MULTI_INDEX_READ:
      ...
      for (i = 0; i < total_profiles; i++) 
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("CGDCONT Read Failed");
          return 0;
        }
        ...
        if(dsatetsipkt_read_write_param_table(dsat_cgdcont_ext_table,
                                              ...
                                              dsat_get_current_ds_profile_subs_id()) == DSAT_ERROR)
        {
         DS_ATCOP_ERROR_LOG_0("CGDCONT Ext Read Failed");
          return 0;
        }
        ...
        {
         DS_ATCOP_ERROR_LOG_0("PDP Type is invalid for ATCoP");
          return 0;
        }
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    else
    {
                                            ...
                                            ))
      {
        ...
        if(ds_profile_reset_pdp_context_per_sub(DS_PROFILE_TECH_3GPP,cid,dsat_get_current_ds_profile_subs_id()) != DS_PROFILE_REG_RESULT_SUCCESS)
        {
            DS_ATCOP_ERROR_LOG_1("Error CGDSCONT PDP profile reset: =d",cid);
            return DSAT_ERROR;
        }
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Error CGDSCONT PDP profile write: =d",cid);
          return DSAT_ERROR;
        }
      }
      else
...
)
{
  ...
  uint8             total_profiles = 0;
   ...
   memset(&pdata, 0, sizeof(ds_profile_3gpp_pdp_context_type));
    ...
    case MULTI_INDEX_READ:
      ...
      for ( i = 0; i < total_profiles; i++) 
      {
        ...
        if(dsatetsipkt_read_write_param_table(context_param_tbl,sizeof(context_param_tbl)/sizeof(atcop_profile_3gpp_param_list_type),
                                 profile_list[i],READ_COMMAND,(byte*)&pdata,sizeof(pdata),dsat_get_current_ds_profile_subs_id()) == DSAT_ERROR)
        {
          DS_ATCOP_ERROR_LOG_1("Error CGDSCONT PDP profile read: =d",profile_list[i]);
          return DSAT_ERROR;
        }
          size = snprintf ((char *)res_buff_ptr->data_ptr, 
                    res_buff_ptr->size,
    ...
    else
    {
      ...
      if ( VALID_TOKEN((uint32)CGTFT_PFI) )
      {
        ...
        */
        ...
        {
         DS_ATCOP_ERROR_LOG_0(" MAXIMUM two filter can set ");
          return DSAT_ERROR;
        }
    ...
    case MULTI_INDEX_READ:
                                           ...
                                           (const byte *)"+QCGTFT" ,strlen((const char *)"+QCGTFT"))))
      {
        if (DSAT_ERROR == etsipkt_get_profile_list( DS_PROFILE_TECH_EPC, &profile_list[0],
                                                    ...
                                                    dsat_get_current_ds_profile_subs_id()) )
        {
         DS_ATCOP_ERROR_LOG_0("Error getting EPC profile list!");
        }
      }
      else
      {
      ...
      for (i=0; i < total_profiles; i++) 
      {
        ...
        for (j=0; j < DS_PROFILE_3GPP_MAX_TFT_PARAM_SETS; j++)
        {
          ...
          if(dsatetsipkt_read_write_profile_param(profile_list[i],READ_COMMAND,
                     ...
                     &pdata,sizeof(ds_profile_3gpp_tft_params_type)) == DSAT_ERROR)
          {
            DS_ATCOP_ERROR_LOG_1("Error get param CID : =d ",profile_list[i]);
            continue;
          }
          else
          {
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    else
    {
                                            ...
                                            ))
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Error get param : =d",cid);
          return DSAT_ERROR;
        }
        if(dsatetsipkt_read_write_profile_param(cid,READ_COMMAND,
        ...
        if(dsatetsipkt_read_write_profile_param(cid,WRITE_COMMAND,
               ...
               &pdata,sizeof(ds_profile_3gpp_3gpp_qos_params_type)) == DSAT_ERROR)
        {
          DS_ATCOP_ERROR_LOG_1("Error set param : =d",cid);
          return DSAT_ERROR;
        }
    ...
    case MULTI_INDEX_READ:
      ...
      for (i=0; i < total_profiles; i++) 
      {
        ...
        if(dsatetsipkt_read_write_profile_param(profile_list[i],READ_COMMAND,
               ...
               &pdata,sizeof(ds_profile_3gpp_3gpp_qos_params_type)) == DSAT_ERROR)
        {
          DS_ATCOP_ERROR_LOG_1("Error get param : =d",profile_list[i]);
          continue;
        }
        if(dsatetsipkt_read_write_profile_param(profile_list[i],READ_COMMAND,
               DS_PROFILE_3GPP_PROFILE_PARAM_SRC_STAT_DESC_REQ ,
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    else
    {
                                            ...
                                            ))
      {
        ...
        if ( DS_PROFILE_REG_RESULT_SUCCESS != profile_status )
        {
          DS_ATCOP_ERROR_LOG_1("Unable to Reset for <cid> : =d",cid);
          return DSAT_ERROR;
        }
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    else
    {
                                            ...
                                            ))
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Error get param CID : =d ",cid);
          return DSAT_ERROR;
        }
        /* Make conditional edits to profile */
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid number of Arguments in CGCONTRDP");
      result = DSAT_ERROR;
    }
    else
    {
      ...
      else
      {
        ...
        if( ATOI_OK == dsatutil_atoi (&p_cid, tok_ptr->arg[0], 10) )
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1("CID out of range value=d",p_cid);
            result = DSAT_ERROR;
          }
        }
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    if(tok_ptr->args_found > 1)
    {
     DS_ATCOP_ERROR_LOG_0("Invalid number of Arguments in CGSCONTRDP");
      result = DSAT_ERROR;
    }
    else
    {
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    if(tok_ptr->args_found > 1)
    {
     DS_ATCOP_ERROR_LOG_0("Invalid number of Arguments in CGEQOSRDP");
      result = DSAT_ERROR;
    }
    else
    {
      ...
      else
      {
        ...
        if( ATOI_OK  != dsatutil_atoi (&cid, tok_ptr->arg[0], 10) )
        {
          DS_ATCOP_ERROR_LOG_1("CID out of range value=d",cid);
          result = DSAT_ERROR;
        }
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    if(tok_ptr->args_found > 1)
    {
     DS_ATCOP_ERROR_LOG_0("Invalid number of Arguments in CGTFTRDP");
      result = DSAT_ERROR;
    }
    else
    {
    ...
    else
    {
                                            ...
                                            ))
      {
        ...
        if(FALSE == dsatetsipkt_is_profile_context_valid(cid, DS_PROFILE_TECH_3GPP,
                                                         dsat_get_current_ds_profile_subs_id()))
        {
          DS_ATCOP_ERROR_LOG_1("Profile =d is not valid",cid);
          return DSAT_ERROR;
        }
...
{
  dsat_result_enum_type result = DSAT_OK;
  uint32                        subs_id = dsat_get_current_ds_profile_subs_id();
  ps_sys_pdu_session_info_type  pdu_session_info;
  int16                         ps_errno = 0;
  ...
  */
  for( ;(dsat_pdp_state.change_index < dsat_pdp_state.valid_profile_count); )
  {
    ...
    memset(&pdu_session_info, 0, sizeof(pdu_session_info));
    pdu_session_info.profile_id_3gpp = dsat_pdp_cid[dsat_pdp_state.change_index];

    result = ps_sys_ioctl_ex(PS_SYS_TECH_ALL,
                             PS_SYS_IOCTL_GET_PDU_SESSION_PARAM,
                             (ps_sys_subscription_enum_type) subs_id,
                             &pdu_session_info,
                             &ps_errno);
    if(result == -1 )
    {
      DS_ATCOP_ERROR_LOG_2("PDP request failed for CID =d ps_errno %d",
      dsat_pdp_cid[dsat_pdp_state.change_index], ps_errno);
      result = DSAT_ERROR;
      break;
    }

    if(!pdu_session_info.call_allowed)
    {
      DS_ATCOP_ERROR_LOG_2("PDP request failed for CID =d net_down_reason = d",
           dsat_pdp_cid[dsat_pdp_state.change_index],
                           pdu_session_info.net_down_reason );
      result = DSAT_ERROR;
      break;
    }

    result = ds_3gpp_rmsm_atif_cgact_handler(dsat_pdp_cid[dsat_pdp_state.change_index],
           (ds_3gpp_atif_cgact_act_mode_e_type)dsat_pdp_state.demand_state,
            NULL,
                        dsat_get_current_subs_id(FALSE),
                        &pdu_session_info.pdu_session_param);
    ...
    {
      DS_ATCOP_ERROR_LOG_1("PDP request failed for CID =d",
        dsat_pdp_cid[dsat_pdp_state.change_index]);
      break;
    }
    ...
    {
      DS_ATCOP_ERROR_LOG_1("PDP request failed result code is =d ",result);
      DSATETSIPKT_ASSERT(0);
    }
  }
...
)
{
  static const struct {
    ...
    dsat_cme_error_e_type          cme_error_code;
  }ps_down_table[] = {
    { PS_NET_DOWN_REASON_OPERATOR_DETERMINED_BARRING,      DSAT_CME_OP_DETERMINED_BARRING },
    { PS_NET_DOWN_REASON_INSUFFICIENT_RESOURCES,           DSAT_CME_INSUFFICIENT_RESOURCES },
    { PS_NET_DOWN_REASON_UNKNOWN_APN,                      DSAT_CME_UNKNOWN_APN },
    { PS_NET_DOWN_REASON_UNKNOWN_PDP,                      DSAT_CME_UNKNOWN_PDP_TYPE },
    { PS_NET_DOWN_REASON_AUTH_FAILED,                      DSAT_CME_USER_AUTH_FAILED },
    { PS_NET_DOWN_REASON_GGSN_REJECT,                      DSAT_CME_GGSN_REJECTED_ACTIVATION},
    { PS_NET_DOWN_REASON_ACTIVATION_REJECT,                DSAT_CME_ACTIVATION_REJECTED_UNSPECIFIED},

    { PS_NET_DOWN_REASON_OPTION_NOT_SUPPORTED,             DSAT_CME_SERV_OP_NOT_SUPPORTED},
    { PS_NET_DOWN_REASON_OPTION_UNSUBSCRIBED,              DSAT_CME_REQ_SERV_OP_NOT_SUBSCRIBED},
    { PS_NET_DOWN_REASON_OPTION_TEMP_OOO,                  DSAT_CME_SERV_OP_TEMP_OUT_OF_ORDER},

    { PS_NET_DOWN_REASON_FEATURE_NOT_SUPPORTED,            DSAT_CME_FEATURE_NOT_SUPPORTED},
    { PS_NET_DOWN_REASON_TFT_SEMANTIC_ERROR,               DSAT_CME_SEMANTIC_ERROR_TFT},
    { PS_NET_DOWN_REASON_TFT_SYNTAX_ERROR,                 DSAT_CME_SYNTACTICAL_ERROR_TFT},
    { PS_NET_DOWN_REASON_UNKNOWN_PDP_CONTEXT,              DSAT_CME_UNKNOWN_PDP_CONTEXT},
    { PS_NET_DOWN_REASON_FILTER_SEMANTIC_ERROR,            DSAT_CME_SEMANTIC_ERRORS_PKT_FILTER},
    { PS_NET_DOWN_REASON_FILTER_SYNTAX_ERROR,              DSAT_CME_SYNTACTICAL_ERRORS_PKT_FILTER},
    { PS_NET_DOWN_REASON_PDP_WITHOUT_ACTIVE_TFT,           DSAT_CME_PDP_ACTIVATE_WITHOUT_ACTIVE_TFT},
    { PS_NET_DOWN_REASON_ACTIVATION_REJECTED_BCM_VIOLATION,  DSAT_CME_REQUEST_REJECTED},
    { PS_NET_DOWN_REASON_MAX_ACTIVE_PDP_CONTEXT_REACHED,   DSAT_CME_PDP_CNTXT_REACHED_MAX},
    { PS_NET_DOWN_REASON_UNSUPPORTED_APN_IN_CURRENT_PLMN,  DSAT_CME_REQUESTED_APN_NOT_SUPPORTED},
    { PS_NET_DOWN_REASON_NO_GW_SRV,                        DSAT_CME_NO_NETWORK_SERVICE},
    { PS_NET_DOWN_REASON_NO_SRV,                           DSAT_CME_NO_NETWORK_SERVICE},
    { PS_NET_DOWN_REASON_MAX,                              DSAT_CME_UNKNOWN}
  };
  ...
  else
  {
    ...
    {
      int index = 0;
      while(ps_down_table[index].ps_down_reason != pdp_info_ptr->iface_net_down)
      {
       index++;
       if(ps_down_table[index].cme_error_code == DSAT_CME_UNKNOWN)
       {
        break;
       }
      }
      result = dsat_send_cme_error(ps_down_table[index].cme_error_code);
    }
    else
    {
...
)
{
  ...
  if(dsat_pdp_state.pending == TRUE)
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("No pending command, ignore");
      return;
    }
  ...
  {
   DS_ATCOP_ERROR_LOG_0("No pending +CGACT command");
  }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid Response");
  }
...
)
{
  ...
  if ( tok_ptr->op == (NA|QU) )
  {
      ...
      if(DSAT_OK != ds_3gpp_rmsm_atif_cgact_query(&profile_status,
                 dsat_get_current_subs_id(FALSE)))
      {
       DS_ATCOP_ERROR_LOG_0("CGACT query failed");
        return DSAT_ERROR;
      }
      else if (profile_status.profile_count > DSAT_MAX_ALLOWED_PROFILES)
      {
       DS_ATCOP_ERROR_LOG_0("Invalid Profiles provided by 3GPP ATIF");
        return DSAT_ERROR;
      }
  ...
  else if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if ( !etsipkt_build_context_processing_list(tok_ptr, 
                                                ...
                                                &state_changed) )
    {
     DS_ATCOP_ERROR_LOG_0("Error building CGACT processing order list");
      return DSAT_ERROR;
    }
    ...
    if(dsat_pdp_state.valid_profile_count == 0)
    {
     ...
     DS_ATCOP_ERROR_LOG_0("Empty CGACT processing order list");
      return DSAT_OK;
    }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("CGACT query failed for AMPV Command");
    return DSAT_ERROR;
  }
  DS_AT_MSG1_HIGH("CGACT query Total_Profile: %d ",profile_status.profile_count);
...
)
{
  ...
  for (index = 0; index < MAX_TCLASS_ENTRIES; index++)
  {
        ...
        traffic_class_dependency_table[index].traffic_class)
    {
      ...
      if (FALSE == hit)
      {
         DS_ATCOP_ERROR_LOG_0("SDU error ratio not valid for TClass");
          return DSAT_ERROR;
      }
      ...
      if (FALSE == hit)
      {
       DS_ATCOP_ERROR_LOG_0("Residual BER not valid for TClass");
        return DSAT_ERROR;
      }
          ...
          ( pdata_ptr->traffic_class != DS_PROFILE_3GPP_TC_BACKGROUND ) )
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Transfer delay not valid for =d TClass ",
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Grntd bitrate not valid for TClass");
        return DSAT_ERROR;
      }
      break;
    }
...
)
{
  ...
  while (MAX_INDEX != sdu_error_conv_table[cntr].index )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid Access into sdu_error_conv_table");
      return FALSE;
    }
  }
  return FALSE;
...
)
{
  ...
  while (MAX_INDEX != res_ber_conv_table[cntr].index )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid Access into res_ber_conv_table");
      return FALSE;
    }
...
)
{
  ...
  while (MAX_INDEX != dorder_conv_table[cntr].index )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid Access into dorder_conv_table");
      return FALSE;
    }
  }
  return FALSE;
...
)
{
  ...
  while (MAX_INDEX != sdu_dlvr_conv_table[cntr].index )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid Access into sdu_dlvr_conv_table");
      return FALSE;
    }
...
)
{
  ...
  while (MAX_INDEX != tclass_conv_table[cntr].index )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid Access into tclass_conv_table");
      return FALSE;
    }
...
)
{
  ...
  DSATETSIPKT_ASSERT (NULL != tok_ptr);
...
)
{
  ...
  DSATETSIPKT_ASSERT((((NULL != buf_ptr) && (0 < buf_size)) == TRUE));
  ...
  for (i=0; i < total_profiles; i++) 
  {
    ...
    IS_BASE_CONTEXT_VALID(i + 1,result);
    if( result == FALSE )
    {
      continue;
    }
    ...
    if(dsatetsipkt_read_write_param_table(context_param_tbl,sizeof(context_param_tbl)/sizeof(atcop_profile_3gpp_param_list_type),
                             profile_list[i],READ_COMMAND,(byte*)&pdata,sizeof(pdata),dsat_get_current_ds_profile_subs_id()) == DSAT_ERROR)
    {
      DS_ATCOP_ERROR_LOG_1("Error CGDSCONT PDP profile write: =d",profile_list[i]);
      return DSAT_ERROR;
    }
      ...
      if ( buf_size >= ((c - buf_ptr) + 3) )
      {
        ...
        /* for two digit, use the utility function */
          c = psutil_itoa (pdata.pdp_context_number, c, 10); 
        }
        *c++ = ',';
      }
      else
...
)
{
  ...
  DSATETSIPKT_ASSERT((((NULL != from_ptr) && (NULL != to_ptr) &&
         (NULL != buf_ptr) && (0 < buf_size)) == TRUE));
...
)
{
  ...
  DSATETSIPKT_ASSERT( (((primary_id > 0) &&
          (NULL != size_ptr) && (NULL != list_ptr)) == TRUE) ); 
...
)
{
  ...
  *size_ptr = 0;
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Error PDP profile read: =d",cid);
    return FALSE;
  }
...
)
{
  ...
  if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS)  
  { 
    DS_AT_MSG1_HIGH("Error on profile context delete: %d",primary_id);
    return FALSE;
  }
  return TRUE;
...
)
{
  ...
  DSATETSIPKT_ASSERT (NULL != res_buff_ptr);
...
)
{
  ...
  if ( type_list == DSAT_CQM_CGACT_WRITE )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Cannot convert to integer parameter: =d",0);
      return FALSE;
    }

    if ((dsat_num_item_type)DSAT_PDP_CONNECT_MAX <= demand_state)
    {
      DS_ATCOP_ERROR_LOG_1("Demand state invalid: =d",demand_state);
      return FALSE;
    }
  }
  ...
  if ( no_cid_list == FALSE )
  {
    ...
    for ( i = 1; i <= cid_limit; i++)
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Cannot convert to integer parameter: =d",i);
        return FALSE;
      }
      /* Test for CID within valid range */
      if (!DSAT_IS_PROFILE_ID_WITHIN_RANGE(cid))
      {
        DS_ATCOP_ERROR_LOG_1("Connection ID invalid: =d",cid);
        return FALSE;
      }
      ...
      if ( !dsatetsipkt_is_profile_context_valid((uint16) cid,
                                                 ...
                                                 dsat_get_current_ds_profile_subs_id()))
        {
        DS_ATCOP_ERROR_LOG_1("Profile id =d is not a 3gpp profile",cid);
          return FALSE;
        }
  ...
  dsat_pdp_state.change_index = 0;
    ...
    /* Copy to structure */
    DS_AT_MSG3_HIGH ( "Valid PDP profile count:%d, prim_count:%d, sec_count:%d", 
...
)
{
  ...
  DSATETSIPKT_ASSERT( NULL != cid_list );
  DSATETSIPKT_ASSERT( NULL != index );
         ...
         ( *index < DSAT_MAX_ALLOWED_PROFILES ) )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Cannot convert CID value: index=d",*index);
      result = DSAT_ERROR;
      break;
    }
  } /* while */
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Buffer NULL");
    return FALSE;
  }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1(" Info type mismatch with DS_3GPP_ATIF_PRIM_INFO =d ",dynamic_param_ptr->info_type);
    DSATETSIPKT_ASSERT(0);
    return FALSE;
  }
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|QU))
  {
    ...
    for(i = 0;(i< PS_SYS_MAX_AVAIL_PDNS+1)&&(i< prim_profile_list.data_len); ++i)
    {
      res_buff_ptr->used += (word)snprintf(
           (char *)&res_buff_ptr->data_ptr[res_buff_ptr->used], 
           res_buff_ptr->size - res_buff_ptr->used,
    ...
    if( NULL == cid_list_ptr )
    {
      ...
      for(i = 0;(i< DS_3GPP_MAX_FLOW_CONTEXT)&&(i< flow_profile_list.data_len); ++i)
      {
                                  ...
                                  subs_id,DS_3GPP_ATIF_SEC_INFO,dynamic_param_ptr))
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1(" Info type mismatch with DS_3GPP_ATIF_SEC_INFO =d ",dynamic_param_ptr->info_type);
            DSATETSIPKT_ASSERT(0);
          }
        }
      }/* for Loop */
    ...
    if( NULL == cid_list_ptr )
    {
      ...
      for(i = 0;(i< DS_3GPP_MAX_FLOW_CONTEXT)&&(i< flow_profile_list.data_len); ++i)
      {
                                  ...
                                  subs_id,DS_3GPP_ATIF_QOS_INFO,dynamic_param_ptr))
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1(" Info type mismatch with DS_3GPP_ATIF_QOS_INFO =d ",dynamic_param_ptr->info_type);
            DSATETSIPKT_ASSERT(0);
          }
        }
      }/* for */
...
)
{
  ...
  {
     DS_ATCOP_ERROR_LOG_1(" Info type mismatch with DS_3GPP_ATIF_TFT_INFO =d ",dynamic_param_ptr->info_type);
     DSATETSIPKT_ASSERT(0);
     return FALSE;
  }
   return TRUE;
      ...
      if(VALID_TOKEN(1))
      {
        ...
        else
        {
          ...
          if( FALSE == dsatetsipkt_is_profile_context_valid((uint16)cid_result, DS_PROFILE_TECH_3GPP,
                                                            dsat_get_current_ds_profile_subs_id()) )
          {
            DS_ATCOP_ERROR_LOG_1("Context ID =d is not valid",cid_result);
                  ...
                  (uint16)cid_result , &secon_flag))
          {
            ...
            else
            {
              /* We currently do not support secondary PDP activations 
                 through TE */
               DS_ATCOP_ERROR_LOG_1("Context ID =d is secondary",cid_result);
               return dsat_send_cme_error(DSAT_CME_OP_NOT_SUPPORTED);
            }
          }
          else
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if ( !etsipkt_build_context_processing_list(tok_ptr, 
                                                ...
                                                NULL) )
    {
     DS_ATCOP_ERROR_LOG_0("+CGCMOD <cid> list contained invalid parameter");
      return DSAT_ERROR;
    }
    ...
    {
      memset ((void*)&dsat_pdp_state, 0x0,sizeof(dsat_pdp_state));
     DS_ATCOP_ERROR_LOG_0("Empty CGCMOD processing order list");
      return DSAT_OK;
    }
  ...
  else if (tok_ptr->op == (NA|EQ|QU))
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("CGACT query failed for CGCMOD query");
      return DSAT_ERROR;
    }
    else if (profile_status.profile_count > DSAT_MAX_ALLOWED_PROFILES)
    {
...
#define RESPONSE_TYPE_MAX 2
  ...
  if ( ( mode == DSAT_CMD ) && (tok_ptr->op == (NA|EQ|AR)) )
  {
    if ( (dsat_num_item_type)dsatutil_get_val( DSATETSI_EXT_CGAUTO_IDX,
                                     0,0,NUM_TYPE) == DSAT_CGAUTO_AUTO_ANS_ON )
    {
     DS_ATCOP_ERROR_LOG_0(" +CGANS not allowed when +CGAUTO = 1");
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("invalid table or data");
    return DSAT_ERROR;
  }
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid Tech Type");
    return DSAT_ERROR;
  }
...
)
{
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid data or data size");
      return DSAT_ERROR;
    }
                                                   ...
                                                   dsat_get_current_ds_profile_subs_id()))
   {
    ...
    {
     DS_ATCOP_ERROR_LOG_1("Cannot get tech_type for profile id =d ",
                       profile_number);
      return DSAT_ERROR;
    }
...
FUNCTION DSATETSIPKT_EMBMS_INDICATION_HANDLER
...
void dsatetsipkt_embms_indication_handler(void* ind, msgr_umid_type ind_type)
{

  ds_cmd_type                   *cmd_buf = NULL;
  boolean *is_embms_enabled = NULL;
  dsat_embms_tmgi_list_type *avail_tmgi_list = NULL;
  dsat_embms_tmgi_list_type *active_tmgi_list = NULL;
  dsat_embms_svc_interest_ind_type *svc_interest_ind = NULL;
  dsat_embms_sai_info_type *sai_info_list = NULL;
  lte_rrc_embms_deact_tmgi_cnf_s *deact_tmgi_cnf_msg;
  lte_rrc_embms_act_tmgi_cnf_s *act_tmgi_cnf_msg;
  uint8 local_err = dsat_embms_local_err_00;

  DS_AT_MSG1_HIGH(" dsatetsipkt_embms_indication_handler indication = %d " , ind_type);
  ...
  switch ( ind_type )
  {
    case DSAT_EMBMS_STATUS_CHANGE_IND:

       /*-----------------------------------------------------------------
       eMBMS status change indication 
       ----------------------------------------------------------------*/

      is_embms_enabled = (boolean *)ind;
      embms_status = (*is_embms_enabled == TRUE) ? DSAT_EMBMS_ENABLED : DSAT_EMBMS_DISABLED;
      DS_AT_MSG1_HIGH("DSAT_EMBMS_STATUS_CHANGE_IND - New eMBMS status = %d ", embms_status);
      break;

    case DSAT_EMBMS_AVAIL_TMGI_LIST_IND:

      /*-------------------------------------------------------------------
      Available list of TMGI indication from RRC 
      ------------------------------------------------------------------*/
      cmd_buf = dsat_get_cmd_buf(sizeof(dsat_embms_tmgi_list_type), FALSE);
      if(cmd_buf == NULL)
      {
        local_err = dsat_embms_local_err_01;
        break;
      }
      avail_tmgi_list = cmd_buf->cmd_payload_ptr;

      memset( avail_tmgi_list,0,sizeof(dsat_embms_tmgi_list_type) );
      avail_tmgi_list->num_tmgi = ((dsat_embms_tmgi_list_type *)ind)->num_tmgi;

      /*-----------------------------------------------------------------------
      If num of TMGIs is 0 in the list , no need to allocate memory
      for TMGI list . just post the command to atcop for async command processing 
      -----------------------------------------------------------------------*/
      if ( avail_tmgi_list->num_tmgi != 0 )
      {
        avail_tmgi_list->list = dsat_alloc_memory(avail_tmgi_list->num_tmgi * sizeof(dsat_embms_tmgi_type),FALSE);
        if ( NULL == avail_tmgi_list->list )
        {
          local_err = dsat_embms_local_err_02;
          ds_release_cmd_buf(&cmd_buf);
          break;
        }
        /*--------------------------------------------------------------------
         Copy the incoming List of avail TMGI to the command buffer
         --------------------------------------------------------------------*/
        memscpy(avail_tmgi_list->list,
                avail_tmgi_list->num_tmgi * sizeof(dsat_embms_tmgi_type),
                ((dsat_embms_tmgi_list_type*)ind)->list,
                avail_tmgi_list->num_tmgi * sizeof(dsat_embms_tmgi_type));
      }

      cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_EMBMS_AVAIL_TMGI_LIST;
      ds_put_cmd(cmd_buf);
      break;

    case DSAT_EMBMS_INTEREST_SERVICE_IND:

      /*-------------------------------------------------------------------------
       Service Interest Indication from RRC ( Counting Procedure )
      -------------------------------------------------------------------------*/
      cmd_buf = dsat_get_cmd_buf(sizeof(dsat_embms_svc_interest_ind_type), FALSE);
      if (cmd_buf == NULL)
      {
        local_err = dsat_embms_local_err_03;
        break;
      }

      svc_interest_ind = cmd_buf->cmd_payload_ptr;
      svc_interest_ind->debug_trace_id = ((dsat_embms_svc_interest_ind_type *)ind)->debug_trace_id;

      cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_EMBMS_INTEREST_TMGI_LIST;
      ds_put_cmd(cmd_buf);
      break;

    case DSAT_EMBMS_SAI_LIST_IND:

      /*-------------------------------------------------------------------------
       SAI list for carrier and neighbouring frequency Indication from RRC
      -------------------------------------------------------------------------*/
      cmd_buf = dsat_get_cmd_buf(sizeof(dsat_embms_sai_info_type), FALSE);
      if (cmd_buf == NULL)
      {
        local_err = dsat_embms_local_err_04;
        break;
      }

      sai_info_list = cmd_buf->cmd_payload_ptr;
      sai_info_list->num_frequencies =  ((dsat_embms_sai_info_type *)ind)->num_frequencies;

      if ( sai_info_list->num_frequencies != 0 )
      {
        sai_info_list->freq_sai_info = dsat_alloc_memory(sai_info_list->num_frequencies * sizeof(dsat_embms_frequency_sai_info),
                                                         FALSE);

        if ( NULL == sai_info_list->freq_sai_info )
        {
          local_err = dsat_embms_local_err_05;
          ds_release_cmd_buf(&cmd_buf);
          break;
        }
        memscpy( sai_info_list->freq_sai_info ,
                 sai_info_list->num_frequencies * sizeof(dsat_embms_frequency_sai_info),
                 ((dsat_embms_sai_info_type *)ind)->freq_sai_info,
                 ((dsat_embms_sai_info_type *)ind)->num_frequencies * sizeof(dsat_embms_frequency_sai_info));
      }

      cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_EMBMS_SAI_LIST_UPDATE;
      ds_put_cmd(cmd_buf);
      break;

    case DSAT_EMBMS_ACTIVE_TMGI_LIST_IND:

      /*----------------------------------------------------------------
      Active list of TMGI indication from RRC
      ---------------------------------------------------------------*/

      cmd_buf = dsat_get_cmd_buf(sizeof(dsat_embms_tmgi_list_type), FALSE);
      if(cmd_buf == NULL)
      {
        local_err = dsat_embms_local_err_06;
        break;
      }

      active_tmgi_list = cmd_buf->cmd_payload_ptr;
      memset(active_tmgi_list,0,sizeof(dsat_embms_tmgi_list_type));
      active_tmgi_list->num_tmgi = ((dsat_embms_tmgi_list_type *)ind)->num_tmgi;

      /*-----------------------------------------------------------------------
      If num of TMGIs is 0 in the list , no need to allocate memory
      for TMGI list . just post the command to atcop for async command processing 
      -----------------------------------------------------------------------*/
      if ( active_tmgi_list->num_tmgi != 0)
      {
        active_tmgi_list->list = dsat_alloc_memory(active_tmgi_list->num_tmgi * sizeof(dsat_embms_tmgi_type),FALSE);
        if ( NULL == active_tmgi_list->list )
        {
          local_err = dsat_embms_local_err_07;
          ds_release_cmd_buf(&cmd_buf);
          break;
        }
        /*--------------------------------------------------------------------
          Copy the incoming List of active TMGI to the command buffer
         --------------------------------------------------------------------*/
        memscpy(active_tmgi_list->list,
                active_tmgi_list->num_tmgi * sizeof(dsat_embms_tmgi_type),
                ((dsat_embms_tmgi_list_type*)ind)->list,
                active_tmgi_list->num_tmgi * sizeof(dsat_embms_tmgi_type)); 
      }
      cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_EMBMS_ACTIVE_TMGI_LIST;
      ds_put_cmd(cmd_buf);
      break;

    case DSAT_EMBMS_TMGI_DEACT_CNF_IND:

      /*---------------------------------------------------------------------
       TMGI deactivation Confirmation from RRC
       --------------------------------------------------------------------*/

      /* Proceed only if some asynchronous AT cmd is waiting for confirmation */
      if ( !CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_DEACT_TMGI) &&
           !CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_DEACT_TMGI) )
      {
       DS_ATCOP_ERROR_LOG_0("dsatetsipkt_embms_indication_handler: Deactivation not pending ");
        return;
      }

      cmd_buf = dsat_get_cmd_buf(sizeof(lte_rrc_embms_deact_tmgi_cnf_s), FALSE);
      if(cmd_buf == NULL)
      {
        local_err = dsat_embms_local_err_08;
        break;
      }

      deact_tmgi_cnf_msg = cmd_buf->cmd_payload_ptr;

      memset(deact_tmgi_cnf_msg,0,sizeof(lte_rrc_embms_deact_tmgi_cnf_s));

      /*------------------------------------------------------------------------
       Copy the deactivation status and RRC session info of the deactivated TMGI
      ------------------------------------------------------------------------*/
      deact_tmgi_cnf_msg->deact_status = ((lte_rrc_embms_deact_tmgi_cnf_s *)ind)->deact_status;
      dsatutil_memscpy(&deact_tmgi_cnf_msg->bearer_info,
                       sizeof(lte_rrc_embms_bearer_info_s),
                       &((lte_rrc_embms_deact_tmgi_cnf_s *)ind)->bearer_info,
                       sizeof(lte_rrc_embms_bearer_info_s));

      cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_EMBMS_TMGI_DEACT_CNF;
      ds_put_cmd(cmd_buf);
      break;

    case DSAT_EMBMS_TMGI_ACT_CNF_IND:

      /*-------------------------------------------------------------------------
      Activation Confirmation msg from RRC , will proceed only when async AT cmd
      waiting for activation indication else return
      ------------------------------------------------------------------------*/
      if ( !CHECK_PENDING(DSATETSI_EXT_CEMBMSDATA_IDX,0,DSAT_PENDING_TRUE))
      {
       DS_ATCOP_ERROR_LOG_0("dsatetsipkt_embms_indication_handler : Activation Not Pending");
        return;
      }

      cmd_buf = dsat_get_cmd_buf(sizeof(lte_rrc_embms_act_tmgi_cnf_s), FALSE);
      if(cmd_buf == NULL)
      {
        local_err = dsat_embms_local_err_09;
        break;
      }

      act_tmgi_cnf_msg = cmd_buf->cmd_payload_ptr;
      memset(act_tmgi_cnf_msg,0,sizeof(lte_rrc_embms_act_tmgi_cnf_s));

      act_tmgi_cnf_msg->act_status = ((lte_rrc_embms_act_tmgi_cnf_s *)ind)->act_status;

      /*-----------------------------------------------------------------------------
      Copy the incoming info from RRC for the activation cnf to the command 
      buffer payload
      ----------------------------------------------------------------------------*/
      dsatutil_memscpy(&act_tmgi_cnf_msg->bearer_info,
                       sizeof(lte_rrc_embms_bearer_info_s),
                       &((lte_rrc_embms_act_tmgi_cnf_s *)ind)->bearer_info,
                       sizeof(lte_rrc_embms_bearer_info_s));

      cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_EMBMS_TMGI_ACT_CNF;
      ds_put_cmd(cmd_buf);
      break;

    default:
      DS_ATCOP_ERROR_LOG_1("dsatetsipkt_embms_indication_handler : Invalid Indication type - =d " , ind_type);
      return;
  }

  if ( local_err )
  DS_ATCOP_ERROR_LOG_1("dsatetsipkt_embms_indication_handler : Memory Allocation failure Local Error =d",local_err);
...
FUNCTION DSATETSIPKT_EMBMS_CHANGE_STATUS

DESCRIPTION
  This function sends LTE_RRC_EMBMS_ENABLE_REQ/LTE_RRC_EMBMS_DISABLE_REQ
  message to RRC to request enable/disable of eMBMS.

DEPENDENCIES
  None

RETURN VALUE
  TRUE  : If message request sent successfully to RRC
  FALSE : On Message sending failure or invalid status request

SIDE EFFECTS
  None
  
===========================================================================*/
boolean dsatetsipkt_embms_change_status (uint8 new_status)
{
  lte_rrc_embms_enable_req_s embms_status_req_msg;
  sys_modem_as_id_e_type subs_id = dsat_get_current_subs_id(FALSE);
  boolean ret_val = FALSE;

  embms_status_req_msg.debug_trace_id = DSAT_EMBMS_DEBUG_TRACE_ID;

  switch ( new_status )
  {
    case DSAT_EMBMS_ENABLED:
      ret_val = ds3gmsgr_msg_send_ext( LTE_RRC_EMBMS_ENABLE_REQ,
                                       MSGR_DS_MSGRRECV,
                                       (msgr_hdr_struct_type*)(&embms_status_req_msg),
                                       sizeof(lte_rrc_embms_enable_req_s),
                                       subs_id);
      break;

    case DSAT_EMBMS_DISABLED:
      ret_val = ds3gmsgr_msg_send_ext( LTE_RRC_EMBMS_DISABLE_REQ,
                                       MSGR_DS_MSGRRECV,
                                       (msgr_hdr_struct_type*)(&embms_status_req_msg),
                                       sizeof(lte_rrc_embms_enable_req_s),
                                       subs_id);
    ...
    default:
      DS_ATCOP_ERROR_LOG_1("Invalid Embms Status =d Request" , new_status );
      break;
  }
...
FUNCTION DSATETSIPKT_EMBMS_GET_INTER_FREQ_COUNT

DESCRIPTION
  This function calculates the inter frequency count in a given
  SAI list provided by RRC.

DEPENDENCIES
  None

RETURN VALUE
  0 : if inter frequency count is 0 in the SAI list else
  returns no of inter frequencies > 0 in the SAI list 
...
LOCAL dsat_num_item_type dsatetsipkt_embms_get_inter_freq_count(dsat_embms_sai_info_type * sai_list)
{
  dsat_num_item_type inter_freq_count=0;
  dsat_num_item_type count;

  for ( count=0 ; count < sai_list->num_frequencies ; count++ )
  {
    if (sai_list->freq_sai_info[count].is_serving_frequency == FALSE )
    {
      inter_freq_count++;
    }
  }

  return inter_freq_count;  
}

/*===========================================================================

FUNCTION DSATETSIPKT_EMBMS_SAI_LIST_HANDLER

DESCRIPTION
  This function handles the SAI List updates coming from RRC.
  RRC sends SAI list for carrier and neighbouring frequencies if there is
  any update in the list or if any client requested for the list.
...
dsat_result_enum_type dsatetsipkt_embms_sai_list_handler ( ds_cmd_type * cmd_ptr)
{
  dsat_result_enum_type result = DSAT_ASYNC_EVENT;
  dsat_num_item_type num_freq=0;
  dsat_embms_sai_info_type *dsat_embms_sai_list_update;
  dsat_num_item_type sai_mode;
  dsat_num_item_type embms_prior;
  dsat_num_item_type inter_freq_count;
  dsat_num_item_type intra_freq_count;
  dsm_item_type            *res_buff_ptr;
  uint8 index;
  uint8 j;
  

  DS_AT_MSG0_HIGH(" dsatetsipkt_embms_sai_list_handler - Entry ");

  /*----------------------------------------------------------------------------------------
   dsat_embms_sai_list_update is the latest update for the SAI info from RRC
  ----------------------------------------------------------------------------------------*/

  dsat_embms_sai_list_update = (dsat_embms_sai_info_type *)dsat_get_cmd_payload_ptr(cmd_ptr);
  if ( NULL == dsat_embms_sai_list_update )
  {
    return DSAT_ERROR;
  }

  num_freq = dsat_embms_sai_list_update->num_frequencies;

  /*---------------------------------------------------------------------------------------------
   SAI list from RRC, will check if any async cmd for SAI req is processing or not
   and if ATCOP need to send indication or not
  ---------------------------------------------------------------------------------------------*/
  sai_mode = (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CEMBMSSAI_IDX,0,0,NUM_TYPE);
  embms_prior = (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CEMBMSSAI_IDX,0,1,NUM_TYPE);

  /*--------------------------------------------------------------------------------------------
   Get the inter-frequency and intra-frequency count from the SAI list
  --------------------------------------------------------------------------------------------*/

  inter_freq_count = dsatetsipkt_embms_get_inter_freq_count(dsat_embms_sai_list_update);
  intra_freq_count = dsat_embms_sai_list_update->num_frequencies - inter_freq_count;

  if ( CHECK_PENDING(DSATETSI_EXT_CEMBMSSAI_IDX,0,DSAT_PENDING_TRUE) )
  {
    /*--------------------------------------------------------------------
     Async command for SAI request is pending
    --------------------------------------------------------------------*/
    res_buff_ptr = dsat_dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL, FALSE);

    res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
                                          res_buff_ptr->size,
                                          "+CEMBMSSAI: USERDEF %d,%d,%d",
                                          sai_mode,embms_prior,num_freq);
    ...
    if ( num_freq > 0 )
    {
      for ( index = 0 ; index < num_freq ; index++ )
      {
        res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                               res_buff_ptr->size - res_buff_ptr->used,
                                               ",%lu",dsat_embms_sai_list_update->freq_sai_info[index].frequency);
      }
    }
    ...
    res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                           res_buff_ptr->size - res_buff_ptr->used,
                                           "+CEMBMSSAI: FCNT %d,%d",embms_prior,inter_freq_count);
    if ( inter_freq_count > 0 )
    {
      for( index= 0 ; index < inter_freq_count ; index++ )
      {
        if ( dsat_embms_sai_list_update->freq_sai_info[index].is_serving_frequency == FALSE )
        {
          res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                                 res_buff_ptr->size - res_buff_ptr->used,
                                                 "\n+CEMBMSSAI: INTER %lu %d",
                                                 dsat_embms_sai_list_update->freq_sai_info[index].frequency,
                                                 dsat_embms_sai_list_update->freq_sai_info[index].num_sais);
        }
        for ( j = 0 ; j < dsat_embms_sai_list_update->freq_sai_info[index].num_sais ; j++ )
        {
          res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                                 res_buff_ptr->size - res_buff_ptr->used,
                                                 ",%d",dsat_embms_sai_list_update->freq_sai_info[index].sai_list[j]);
        }
      }
    }

    if ( intra_freq_count > 0 )
    {
      for ( index = 0 ; index < intra_freq_count ; index++ )
      {
        if ( dsat_embms_sai_list_update->freq_sai_info[index].is_serving_frequency == TRUE )
        {
          res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                                 res_buff_ptr->size - res_buff_ptr->used,
                                                 "\n+CEMBMSSAI: INTRA %d",
    ...
    SET_PENDING(DSATETSI_EXT_CEMBMSSAI_IDX,0,DSAT_PENDING_CMD_NONE)
  ...
  {
    /*-----------------------------------------------------------------
    SAI list update indication on , send URCs
    ------------------------------------------------------------------*/
    res_buff_ptr = dsat_dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL, FALSE);

    res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                           res_buff_ptr->size - res_buff_ptr->used,
                                           "+CEMBMSSAII: FCNT %d,%d",embms_prior,inter_freq_count); 
    if ( inter_freq_count > 0 )
    {
      for( index= 0 ; index < inter_freq_count ; index++ )
      {
    ...
    dsatcmdp_send_urc_per_subs(dsat_get_current_subs_id(FALSE), res_buff_ptr, DSAT_COMPLETE_RSP);
  }

  if ( num_freq != 0 )
  {
    dsatutil_free_memory(dsat_embms_sai_list_update->freq_sai_info);
  }
  return result;
}
...
FUNCTION DSATETSIPKT_EMBMS_ACTIVATE_TMGI_CNF_HANDLER

DESCRIPTION
  This function handles the activation confirmation message coming from
  RRC for any particular TMGI.RRC sends activation confirmation msg
  to the clients each time a TMGI is activated.
...
dsat_result_enum_type dsatetsipkt_embms_activate_tmgi_cnf_handler( ds_cmd_type * cmd_ptr )
{
  dsat_result_enum_type result = DSAT_ASYNC_EVENT;
  lte_rrc_embms_act_tmgi_cnf_s *act_tmgi_cnf_info = NULL;
  
  dsat_string_item_type *act_tmgi_pending_str;
  dsat_string_item_type *rrc_plmn_id;
  dsat_string_item_type *rrc_svc_id;
  
  DS_AT_MSG0_HIGH("dsatetsipkt_embms_activate_tmgi_cnf_handler() - Entry");
  /*------------------------------------------------------------------------------------------
   Get the SVC ID , PLMN ID and activation status info from the Command Payload
   -----------------------------------------------------------------------------------------*/
  act_tmgi_cnf_info = ( lte_rrc_embms_act_tmgi_cnf_s *) dsat_get_cmd_payload_ptr(cmd_ptr);
  
  act_tmgi_pending_str = activate_tmgi_str;
  rrc_plmn_id = act_tmgi_cnf_info->bearer_info.session_info.tmgi.plmn_id.identity;
  rrc_svc_id = act_tmgi_cnf_info->bearer_info.session_info.tmgi.service_id;

  /*------------------------------------------------------------------------------------------
   Check if ATCOP received activation messsage for the same TMGI str of which the async cmd
   is waiting for . If yes report the result else return
   -----------------------------------------------------------------------------------------*/
  if ( !memcmp(act_tmgi_pending_str , rrc_svc_id, DSAT_EMBMS_RRCIF_SVC_ID_SIZE) &&
       !memcmp( act_tmgi_pending_str + DSAT_EMBMS_RRCIF_PLMN_OFFSET ,rrc_plmn_id,DSAT_EMBMS_RRCIF_PLMN_ID_SIZE))
  {
    memset(act_tmgi_pending_str,0,TMGI_HEX_MAX_LENGTH); 

    if ( CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_TMGI))
    {
      SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_CMD_NONE)
    }
    SET_PENDING(DSATETSI_EXT_CEMBMSDATA_IDX,0,DSAT_PENDING_CMD_NONE)

    switch ( act_tmgi_cnf_info->act_status )
    {
      ...
      case LTE_EMBMS_SUCCESS:
        DS_AT_MSG0_HIGH("dsatetsipkt_embms_activate_tmgi_cnf_handler :Activation Success ");
        result = DSAT_OK;
        break;
      default :
       DS_ATCOP_ERROR_LOG_0("dsatetsipkt_embms_activate_tmgi_cnf_handler :Activation Failure ");
        result = DSAT_ERROR;
        break;
    }
  }
...
FUNCTION DSATETSIPKT_EMBMS_DEACTIVATE_TMGI_CNF_HANDLER

DESCRIPTION
  This function handles the activation confirmation message coming from
  RRC for any particular TMGI.RRC sends deactivation confirmation msg
  to the clients each time a TMGI is deactivated.
...
dsat_result_enum_type dsatetsipkt_embms_deactivate_tmgi_cnf_handler( ds_cmd_type * cmd_ptr )
{
  dsat_result_enum_type result = DSAT_ASYNC_EVENT;
  lte_rrc_embms_deact_tmgi_cnf_s * deact_tmgi_cnf_info = NULL;

  dsat_string_item_type *deact_tmgi_pending_str;
  dsat_string_item_type *rrc_plmn_id;
  dsat_string_item_type *rrc_svc_id;


  DS_AT_MSG0_HIGH("dsatetsipkt_embms_deactivate_tmgi_cnf_handler()- Entry");
  
  deact_tmgi_cnf_info = ( lte_rrc_embms_deact_tmgi_cnf_s *) dsat_get_cmd_payload_ptr(cmd_ptr);
  deact_tmgi_pending_str = ( byte *) dsatutil_get_val(DSATETSI_EXT_CEMBMSSRV_IDX,0,2,MIX_STR_TYPE);

  /*------------------------------------------------------------------------------
   Copy the PLMN ID and SVC ID from the RRC payload
   ------------------------------------------------------------------------------*/
  rrc_plmn_id = deact_tmgi_cnf_info->bearer_info.session_info.tmgi.plmn_id.identity;
  rrc_svc_id = deact_tmgi_cnf_info->bearer_info.session_info.tmgi.service_id;

  /*-----------------------------------------------------------------------------------
   Check if the Deactivation Indication is for the same TMGI we deactivated using AT cmd
   If not , just ignore and return 
   ------------------------------------------------------------------------------------*/
  if ( !memcmp(deact_tmgi_pending_str , rrc_svc_id, DSAT_EMBMS_RRCIF_SVC_ID_SIZE) &&
       !memcmp( deact_tmgi_pending_str + DSAT_EMBMS_RRCIF_PLMN_OFFSET ,rrc_plmn_id,DSAT_EMBMS_RRCIF_PLMN_ID_SIZE))
  {
    memset(deact_tmgi_pending_str,0,TMGI_HEX_MAX_LENGTH);
    if ( CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_DEACT_TMGI) )
    {
      SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_TMGI)
    }
    else
    {
      SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_CMD_NONE)
    ...
    /*-------------------------------------------------------------------------
    Check the deactivation status to report the proper result to async cmd 
    -------------------------------------------------------------------------*/

    if ( deact_tmgi_cnf_info->deact_status == LTE_EMBMS_SUCCESS )
    {
      DS_AT_MSG0_HIGH("dsatetsipkt_embms_deactivate_tmgi_cnf_handler: TMGI deactivation Success ");
      result = DSAT_OK;
    }
    else
    {
     DS_ATCOP_ERROR_LOG_0("dsatetsipkt_embms_deactivate_tmgi_cnf_handler: TMGI deactivation Failure ");
      result = DSAT_ERROR;
    }
  }
  return result;
...
FUNCTION DSATETSIPKT_EMBMS_ACTIVE_TMGI_LIST_HANDLER

DESCRIPTION
  This function handles the active TMGI list indication coming from RRC.
  RRC sends the active TMGI list whenever there is any change in the list
  and this function will store the active TMGI list in a global variable.
...
dsat_result_enum_type dsatetsipkt_embms_active_tmgi_list_handler( ds_cmd_type * cmd_ptr )
{
  dsat_result_enum_type result = DSAT_ASYNC_EVENT;
  dsat_embms_tmgi_list_type *active_tmgi_list; 
  dsat_num_item_type num_active_tmgi=0;
  dsat_num_item_type count=0;
  dsat_num_item_type mode_val;

  dsm_item_type            *res_buff_ptr=NULL;

  mode_val = (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CEMBMSSRV_IDX,0,0,NUM_TYPE);
  active_tmgi_list = ( dsat_embms_tmgi_list_type *)dsat_get_cmd_payload_ptr(cmd_ptr);
  num_active_tmgi = active_tmgi_list->num_tmgi;

  DS_AT_MSG1_HIGH("dsatetsipkt_embms_active_tmgi_list_handler - Entry Num TMGI : %d",num_active_tmgi);

  /*--------------------------------------------------------------------------------------------
   If this is the first indication for active TMGI list , dsat_embms_active_tmgi_list will be NULL
   else free the previous available tmgi list and update the latest one
   --------------------------------------------------------------------------------------------*/

  if ( dsat_embms_active_tmgi_list != NULL)
  {
    if ( dsat_embms_active_tmgi_list->list != NULL )
    {
      dsatutil_free_memory(dsat_embms_active_tmgi_list->list);
    }
    dsatutil_free_memory(dsat_embms_active_tmgi_list);
  }

  dsat_embms_active_tmgi_list = dsat_alloc_memory(sizeof(dsat_embms_tmgi_list_type),FALSE);

  if ( NULL == dsat_embms_active_tmgi_list )
  {
   DS_ATCOP_ERROR_LOG_0("Could not allocate memory");
    return DSAT_ERROR;
  }
  dsat_embms_active_tmgi_list->num_tmgi = num_active_tmgi;
  dsat_embms_active_tmgi_list->list = active_tmgi_list->list;

  /*---------------------------------------------------------------
    If +CEMBMSSRV URC is set to on , send the response
   --------------------------------------------------------------*/
  if ( mode_val == 1 )
  {
    ...
    res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
                                          res_buff_ptr->size,
                                          "+CEMBMSSRVI:%d,%d",
                                          DSAT_ACTIVE_EMBMS_SERVICE,num_active_tmgi);

    for ( count = 0 ; count < num_active_tmgi ; count++ )
    {
      res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                             res_buff_ptr->size - res_buff_ptr->used,
                                             ",\"%02X%02X%02X%02X%02X%02X\",",
                                             active_tmgi_list->list[count].tmgi[0],
                                             active_tmgi_list->list[count].tmgi[1],
                                             active_tmgi_list->list[count].tmgi[2],
                                             active_tmgi_list->list[count].tmgi[3],
                                             active_tmgi_list->list[count].tmgi[4],
                                             active_tmgi_list->list[count].tmgi[5]);
    }
    dsatcmdp_send_urc_per_subs(dsat_get_current_subs_id(FALSE), res_buff_ptr, DSAT_COMPLETE_RSP);
    result = DSAT_OK;
  }
...
FUNCTION DSATETSIPKT_EMBMS_INTEREST_TMGI_HANDLER

DESCRIPTION
  This function handles the interested TMGI indication from RRC.
  On receiving this indication , ATCOP will send list of
  interested TMGIs user is interested in to RRC
...
dsat_result_enum_type dsatetsipkt_embms_interest_tmgi_handler ( ds_cmd_type * cmd_ptr)
{
  dsat_result_enum_type result = DSAT_ASYNC_EVENT;
  dsat_embms_svc_interest_ind_type* svc_interest_ind;

  dsm_item_type                             *dsm_ptr = NULL;
  msgr_attach_struct_type                   *att_ptr = NULL;
  lte_rrc_embms_service_interest_info_req_s  embms_tmgi_list_msg;
  uint16                                     dsm_item_size = 0;
  uint8                                      tmgi_index = 0;
  uint8                                      tmgi_info_index = 0;
  uint16                                     dsm_item_temp_size = 0;
  dsat_embms_svc_interest_req_type           *tmgi_interest_list = dsat_embms_svc_interest_list;
  uint8 local_err = dsat_embms_local_err_00;

  svc_interest_ind = (dsat_embms_svc_interest_ind_type *)dsat_get_cmd_payload_ptr(cmd_ptr);

  if ( svc_interest_ind == NULL )
  {
    result = DSAT_ERROR;
    return result;
  }
  DS_AT_MSG1_HIGH("dsatetsipkt_embms_interest_tmgi_handler: Debug Trace ID : %d",svc_interest_ind->debug_trace_id);

  /* Initialize the message header with 1 attachment */
  msgr_init_hdr_attach
  (
    &embms_tmgi_list_msg.msg_hdr, 
    MSGR_DS_MSGRRECV, 
  ...
  do
  {
    /* Set the debug trace id */
    embms_tmgi_list_msg.debug_trace_id = svc_interest_ind->debug_trace_id;

    if( tmgi_interest_list == NULL )
    {
      tmgi_interest_list = dsat_alloc_memory(sizeof(dsat_embms_svc_interest_req_type),FALSE);
      memset(tmgi_interest_list,0,sizeof(dsat_embms_svc_interest_req_type));
    }

    if ( tmgi_interest_list->num_tmgi == 0 )
    {
      if((dsm_item_size = dsm_pushdown(&dsm_ptr,
                                       &tmgi_interest_list->num_tmgi,
                                       sizeof(tmgi_interest_list->num_tmgi),
                                       DSM_DS_SMALL_ITEM_POOL))
         != sizeof(tmgi_interest_list->num_tmgi))
      {
        local_err = dsat_embms_local_err_01;
        dsm_item_temp_size = sizeof(tmgi_interest_list->num_tmgi);
    ...
    else
    {
      ...
      ------------------------------------------------*/
      for (tmgi_index = tmgi_interest_list->num_tmgi;
           tmgi_index > 0;
           tmgi_index--)
      {
        /* push down session id */
        if((dsm_item_size = 
              dsm_pushdown(&dsm_ptr,
                 &tmgi_interest_list->tmgi_list[tmgi_index-1].session_id,
                 sizeof(tmgi_interest_list->tmgi_list[tmgi_index-1].session_id),
                 DSM_DS_SMALL_ITEM_POOL))
           != sizeof(tmgi_interest_list->tmgi_list[tmgi_index-1].session_id)
          )
        {
          local_err = dsat_embms_local_err_02;
          dsm_item_temp_size = sizeof(tmgi_interest_list->tmgi_list[tmgi_index-1].session_id);
          result = DSAT_ERROR;
          dsm_free_packet(&dsm_ptr);
        }
        /* push down session id present flag */
        if((dsm_item_size = 
              dsm_pushdown(&dsm_ptr,
                           &tmgi_interest_list->
                                      tmgi_list[tmgi_index-1].session_id_present,
                           sizeof(tmgi_interest_list->
                                      tmgi_list[tmgi_index-1].session_id_present),
                           DSM_DS_SMALL_ITEM_POOL))
           != sizeof(tmgi_interest_list->
                                       tmgi_list[tmgi_index-1].session_id_present)
          )
        {
          ...
          dsm_free_packet(&dsm_ptr);
          result = DSAT_ERROR;
          dsm_item_temp_size = sizeof(tmgi_interest_list->tmgi_list[tmgi_index-1].session_id_present);
        }

        /* push down the service ID */
        for (tmgi_info_index = PS_IFACE_EMBMS_TMGI_SIZE - 3;
             tmgi_info_index > 0;
             tmgi_info_index--)
        {
          if((dsm_item_size = 
                dsm_pushdown(&dsm_ptr,
                             &tmgi_interest_list->
                                 tmgi_list[tmgi_index-1].tmgi[tmgi_info_index-1],
                             sizeof(tmgi_interest_list->
                                 tmgi_list[tmgi_index-1].tmgi[tmgi_info_index-1]),
                             DSM_DS_SMALL_ITEM_POOL))
             != sizeof(tmgi_interest_list->
            ...
            )
          {
            local_err = dsat_embms_local_err_04;
            dsm_free_packet(&dsm_ptr);
            dsm_item_temp_size = sizeof(tmgi_interest_list->
                                 tmgi_list[tmgi_index-1].tmgi[tmgi_info_index-1]);
            result = DSAT_ERROR;
          }
        }

        /* push down the PLMN ID */
        for (tmgi_info_index = PS_IFACE_EMBMS_TMGI_SIZE;
             tmgi_info_index > 3;
             tmgi_info_index--)
        {
          if((dsm_item_size = 
    ...
    /* Get the attachment ptr from the message */
    att_ptr = msgr_get_attach(&embms_tmgi_list_msg.msg_hdr, 0);
    if (att_ptr == NULL)
    {
      result = DSAT_ERROR;
      break;
    ...
    /* set the attachment ptr as the dsm_ptr for extraction */
    msgr_set_dsm_attach(att_ptr, dsm_ptr);

    result = msgr_send(&embms_tmgi_list_msg.msg_hdr,
                       sizeof(lte_rrc_embms_service_interest_info_req_s));

    if (result != DSAT_OK)
    {
      DS_ATCOP_ERROR_LOG_1("Error sending Service interest request msg to RRC Local Error = d",local_err);
      dsm_free_packet(&dsm_ptr);
      break;
    }
  } while (0);
  
  if ( dsat_embms_svc_interest_list == NULL)
  {
    dsatutil_free_memory(tmgi_interest_list); 
  }
  return result;
}
...
FUNCTION DSATETSIPKT_EMBMS_AVAIL_TMGI_LIST_HANDLER

DESCRIPTION
  This function handles the available TMGI list indication coming from RRC.
  RRC sends available TMGI list indication whenever there is any change in 
  the avail list or when any client queries for the avail TMGI list.
...
dsat_result_enum_type dsatetsipkt_embms_avail_tmgi_list_handler( ds_cmd_type * cmd_ptr )
{
  dsat_result_enum_type result = DSAT_ASYNC_EVENT;
  dsat_embms_tmgi_list_type *avail_tmgi_list; 
  dsat_num_item_type num_avail_tmgi=0;
  dsat_num_item_type count=0;
  dsm_item_type            *res_buff_ptr = NULL;
  dsat_num_item_type       mode_val;
  ...
  avail_tmgi_list = ( dsat_embms_tmgi_list_type *)dsat_get_cmd_payload_ptr(cmd_ptr);
  num_avail_tmgi = avail_tmgi_list->num_tmgi;

  DS_AT_MSG1_HIGH("dsatetsipkt_embms_avail_tmgi_list_handler - Entry Num TMGI : %d",num_avail_tmgi);
  ...
  if ( mode_val == 1 )
  {
    res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
                                          res_buff_ptr->size,
                                          "+CEMBMSSRVI:%d,%d",
                                          DSAT_AVAILABLE_EMBMS_SERVICE,num_avail_tmgi);
  }
  else
  {
    res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
  ...
  for ( count = 0 ; count < num_avail_tmgi ; count++ )
  {
    res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                           res_buff_ptr->size - res_buff_ptr->used,
                                           ",\"%02X%02X%02X%02X%02X%02X\",",
                                           avail_tmgi_list->list[count].tmgi[0],
                                           avail_tmgi_list->list[count].tmgi[1],
                                           avail_tmgi_list->list[count].tmgi[2],
                                           avail_tmgi_list->list[count].tmgi[3],
                                           avail_tmgi_list->list[count].tmgi[4],
                                           avail_tmgi_list->list[count].tmgi[5]);
  }

  /*------------------------------------------------------------------------------
  Check if async CEMBMSSRV cmd for querying Avail list is pending or not ,
  If yes , send the asynchronous response else send Indications as URC
  -----------------------------------------------------------------------------*/

  if ( CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_TMGI_AVAIL_LIST) )
  {
    DS_AT_MSG0_HIGH(" CEMBMSSRV Query Avail List Pending ");
    dsatcmdp_handle_async_cmd_rsp(res_buff_ptr, DSAT_COMPLETE_RSP);
    SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_CMD_NONE)
    result = DSAT_OK;
  }
  else if ( CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_AVAIL_LIST) )
  {
    DS_AT_MSG0_HIGH(" CEMBMSSRV Query Avail List Pending ");
    dsatcmdp_handle_async_cmd_rsp(res_buff_ptr, DSAT_COMPLETE_RSP);
    SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_TMGI)
  ...
  {
    /*-----------------------------------------------------------------------------
     +CEMBMSSRV URC is set to 1 , send the indications as URC
     ----------------------------------------------------------------------------*/
    dsatcmdp_send_urc_per_subs(dsat_get_current_subs_id(FALSE), res_buff_ptr, DSAT_COMPLETE_RSP);
    result = DSAT_OK;
  }
...
FUNCTION DSATETSIPKT_EMBMS_CONVERT_TMGI_STR_TO_RRC_FMT

DESCRIPTION
  This function converts ATCOP TMGI format of String in hex type to RRC TMGI
  format
...
RETURN VALUE
  DSAT_OK: ATCOP TMGI String is valid and converted successfully to RRC fmt
  DSAT_ERROR: ATCOP TMGI string is invalid or conversion to RRC fmt fails
...
LOCAL dsat_result_enum_type dsatetsipkt_embms_convert_tmgi_str_to_rrc_fmt ( byte * in_ptr , byte * out_ptr , dsat_num_item_type len )
{
  dsat_num_item_type result = DSAT_OK;
  uint8 index=0;
  uint8 count=0;
  dsat_num_item_type hex_val;
  dsat_num_item_type hex_val_next;
  byte temp[2];

  memset(out_ptr,0,TMGI_HEX_MAX_LENGTH);
  memset(temp,0,2); 
  
  /*-----------------------------------------------------------
    TMGI Str length is 12 if both SVC ID and PLMN ID is present
    else TMGI Str length is 6 if only SVC ID is present
  ------------------------------------------------------------*/
  if ( len != TMGI_STRING_MAX_LENGTH - 2 &&
       len != TMGI_STRING_MIN_LENGTH - 2 )
  {
    return DSAT_ERROR;
  }

  for ( index = 0 ,count = 0 ; count < (len/2 )  ; index++,count++ )
  {
    temp[0] = in_ptr[index];
    if ( dsatutil_atoi(&hex_val,temp,16) != ATOI_OK )
    {
      result = DSAT_ERROR;
      break;
    }
    ...
    temp[0]=in_ptr[++index];
    if ( dsatutil_atoi(&hex_val_next,temp,16) != ATOI_OK )
    {
      result = DSAT_ERROR;
      break;
    }
    out_ptr[count] = (hex_val << 4) | (hex_val_next);
  }
  return result;
}
...
FUNCTION DSATETSIPKT_EMBMS_QUERY_TMGI_LIST

DESCRIPTION
 This function is used while AT+CEMBMSSRV at cmd is executed for querying 
 Active/Available lists of TMGI.

DEPENDENCIES
  None

RETURN VALUE
  DSAT_OK: On Active TMGI list Query
  DSAT_ASYNC_CMD: On Available TMGI list Query
  DSAT_ERROR: On failure
...
LOCAL dsat_result_enum_type dsatetsipkt_embms_query_tmgi_list (dsat_num_item_type service_type ,dsm_item_type * res_buff_ptr , sys_modem_as_id_e_type subs_id)
{

  dsat_result_enum_type result = DSAT_ERROR;

  lte_rrc_embms_avail_tmgi_list_req_s  dsat_avail_tmgi_req_msg;
  dsat_embms_tmgi_list_type *active_tmgi_list = NULL;
  dsat_num_item_type index;
  uint8 num_active_tmgi = 0;
  boolean ret_val = TRUE;

  /*-----------------------------------------------------------------------------------

  Available TMGI List - ATCOP will send LTE_RRC_EMBMS_AVAIL_TMGI_LIST_REQ msg to RRC 
                        and will wait for the list of avail TMGI indication 

  Active TMGI List    - ATCOP updates global variable dsat_embms_active_tmgi_list whenever 
                        there is any indication from RRC for active TMGI list 
  ------------------------------------------------------------------------------------*/

  DS_AT_MSG1_HIGH("+CEMBMSSRV: dsatetsipkt_embms_query_tmgi_list - service type %d ",service_type);

  switch (service_type)
  {
    case DSAT_AVAILABLE_EMBMS_SERVICE:

      memset(&dsat_avail_tmgi_req_msg,0,sizeof(dsat_avail_tmgi_req_msg));
      dsat_avail_tmgi_req_msg.debug_trace_id = DSAT_EMBMS_DEBUG_TRACE_ID;

      /*--------------------------------------------------------------------------
      Send the message to RRC via DSMSGR
      --------------------------------------------------------------------------*/
      ret_val = ds3gmsgr_msg_send_ext
                 (LTE_RRC_EMBMS_AVAIL_TMGI_LIST_REQ, 
                  MSGR_DS_MSGRRECV,
                  (msgr_hdr_struct_type*)(&dsat_avail_tmgi_req_msg),
                  sizeof(lte_rrc_embms_avail_tmgi_list_req_s),
                  subs_id);

      if ( ret_val == FALSE )
      {
       DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV: Error Sending LTE_RRC_EMBMS_AVAIL_TMGI_LIST_REQ msg to RRC");
        result = DSAT_ERROR;
        break;
      }

      /*-------------------------------------------------------------------------------------
      ATCOP will wait for indication from RRC with avail list of TMGIs
      Return ASYNC_CMD
      -------------------------------------------------------------------------------------*/
      if ( !CHECK_PENDING (DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_TMGI))
      {
        SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_TMGI_AVAIL_LIST)
      }
      else
      {
        SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_AVAIL_LIST)
      }

      result = DSAT_ASYNC_CMD;
      break;

    case DSAT_ACTIVE_EMBMS_SERVICE:

      if ( dsat_embms_active_tmgi_list == NULL )
      {
        /*-----------------------------------------------------------------------------------
        There is no active TMGI  list updates from RRC yet , ATCOP will return 0 active TMGIs
        ----------------------------------------------------------------------------------*/
        res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
                                              res_buff_ptr->size,
                                              "+CEMBMSSRV:%d,%d",
      ...
      {
        /*-----------------------------------------------------------------------------------
        ATCOP will report the latest active TMGI list update from RRC 
        ----------------------------------------------------------------------------------*/
        active_tmgi_list = dsat_embms_active_tmgi_list;
        num_active_tmgi = active_tmgi_list->num_tmgi;
        res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
                                              res_buff_ptr->size,
                                              "+CEMBMSSRV:%d,%d",
                                              DSAT_ACTIVE_EMBMS_SERVICE,num_active_tmgi);
        for ( index = 0 ; index < num_active_tmgi ; index++ )
        {
          res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                                 res_buff_ptr->size - res_buff_ptr->used,
                                                 ",\"%02X%02X%02X%02X%02X%02X\",",
                                                 active_tmgi_list->list[index].tmgi[0],
                                                 active_tmgi_list->list[index].tmgi[1],
                                                 active_tmgi_list->list[index].tmgi[2],
                                                 active_tmgi_list->list[index].tmgi[3],
                                                 active_tmgi_list->list[index].tmgi[4],
                                                 active_tmgi_list->list[index].tmgi[5]);
        }
        result = DSAT_OK;
      }
      break;
    ...
    default:
      DS_ATCOP_ERROR_LOG_1("+CEMBMSSRV: Invalid Service Type =d " , service_type);
      result = DSAT_ERROR;
      break;
  }
  return result;
...
FUNCTION DSATETSIPKT_EMBMS_CONVERT_STR_TO_RRC_LIST

DESCRIPTION
  This function converts ',' seperated sai's or EARFCN's in a quoted string to
  a list of SAIs and EARFCNs of RRC format
...
LOCAL boolean dsatetsipkt_embms_convert_str_to_rrc_list( byte *in_str , uint8 max_items, void *out_list , uint8 *out_list_len , dsat_embms_list_type list_type)
{
  dsat_num_item_type val, val_lim;
  uint8 *count = out_list_len;
  uint16 *sai_list;
  uint32 *earfcn_list;
  byte c;
  boolean valid = TRUE;
  ...
  val = 0;
  *count = 0;
  val_lim = (dsat_num_item_type) ((unsigned int)MAX_VAL_NUM_ITEM / 10);
  if ( list_type == DSAT_EMBMS_SAI_LIST )
  {
    sai_list = (uint16 *)out_list;
  }
  else
  {
    earfcn_list = (uint32 *)out_list;
  } 
  ...
  while (  (c = *in_str++) != '\0' )
  {
    ...
    else
    {
      ...
      {
        val = (dsat_num_item_type) (val * 10 + c);
        if( *in_str == '\0' )
        {
          if ( list_type == DSAT_EMBMS_SAI_LIST)
          {
...
FUNCTION DSATETSIPKT_EXEC_CEMBMSCFG_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. It handles the +CEMBMSCFG command.
  This commands sets the eMBMS status
...
dsat_result_enum_type dsatetsipkt_exec_cembmscfg_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
)
{
  ...
  dsat_num_item_type prev_status;
  dsat_num_item_type new_status;
  const dflm_type * dflm_ptr;
  dsat_result_enum_type result = DSAT_OK;
  ...
  else if( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSCFG: Invalid no of Arguments");
      return DSAT_ERROR;
    }
    /* Validate the argument received */
    if( !VALID_TOKEN(0) || ATOI_OK != dsatutil_atoi(&new_status, tok_ptr->arg[0] ,10))
    {
      return DSAT_ERROR;
    }

    dflm_ptr= (const dflm_type *) parse_table->def_lim_ptr;
    if ( dflm_ptr->lower > new_status ||
         dflm_ptr->upper < new_status)
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSSCFG: Input argument out of range");
      return DSAT_ERROR;
    }

    /* eMBMS status can be changed by other clients */
    prev_status = (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CEMBMSCFG_IDX,0,0,NUM_TYPE);
    if ( new_status == prev_status && new_status == embms_status )
    {
      DS_AT_MSG0_HIGH("New Status same as current status , returning OK");
      return DSAT_OK;
    }

    if ( FALSE == dsatetsipkt_embms_change_status( (uint8)new_status) )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSCFG: dsatetsipkt_embms_change_status failure ");
      return DSAT_ERROR;
    }  
    DSATUTIL_SET_VAL(DSATETSI_EXT_CEMBMSCFG_IDX,0,0,0,new_status,NUM_TYPE);
  }
  else
  {
    /* All other opeartions are illegal*/
    result = DSAT_ERROR;
  }
  return result;
...
FUNCTION DSATETSIPKT_EXEC_CEMBMSSRV_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. It handles the +CEMBMSSRV command.
  This command is used to query Active/Available TMGI lists,
  configure a TMGI for activation and deactivating a TMGI.

DEPENDENCIES
  eMBMS must be enabled
...
RETURN VALUE
  ...
  possible values:
    DSAT_ERROR : if there was any problem in executing the command
    DSAT_OK : if it is a success.
    DSAT_ASYNC_CMD : On Successful msg sent to RRC and waiting for indication

SIDE EFFECTS
  None
===========================================================================*/

dsat_result_enum_type dsatetsipkt_exec_cembmssrv_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
)
{
  ...
  dsat_result_enum_type result = DSAT_OK;
  sys_modem_as_id_e_type subs_id = dsat_get_current_subs_id(FALSE);
  dsat_num_item_type num_args,mode_val,service_type;
  dsat_string_item_type *tmgi_str = NULL;
  dsat_num_item_type tmgi_str_len;
  byte tmgi_str_hex[TMGI_HEX_MAX_LENGTH];
  void *cmd_arg_ptr;
  lte_rrc_embms_deact_tmgi_req_s deact_tmgi_req_msg;
  boolean ret_val = FALSE;
  const dflm_type *dflm_ptr;
  const mixed_def_s_type **mixed_dflm_ptr = (const mixed_def_s_type **)parse_table->def_lim_ptr;
  uint8 curr_embms_status = embms_status;
  boolean is_mode_valid = FALSE;
  


  /* This command is valid only when eMBMS status is enabled */
  if ( curr_embms_status != DSAT_EMBMS_ENABLED )
  {
   DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV: eMBMS status disabled , Returning Error");
    return dsat_send_cme_error(DSAT_CME_OP_NOT_ALLOWED);
  }
  ...
  {
    num_args = tok_ptr->args_found;
    if( num_args < 1 || num_args > parse_table->compound)
    {
      return DSAT_ERROR;
     DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV:Invalid no of Arguments");
    }
    ...
    if ( VALID_TOKEN(0) && ATOI_OK == dsatutil_atoi(&mode_val,tok_ptr->arg[0],10) ) 
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV: Invalid Argument");
        return DSAT_ERROR;
      }
      dflm_ptr = (const dflm_type *)mixed_dflm_ptr[0]->def;
      if ( dflm_ptr->lower > mode_val ||
           dflm_ptr->upper < mode_val)
      {
       DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV: Invalid Argument");
        return DSAT_ERROR;
      }
      ...
      if ( num_args == 1 )
      {
        DSATUTIL_SET_VAL(DSATETSI_EXT_CEMBMSSRV_IDX,0,0,0,mode_val,NUM_TYPE)
        return DSAT_OK;
      }
    }

    /* Validate Arg 2  */
    if ( !VALID_TOKEN(1) || ATOI_OK != dsatutil_atoi(&service_type,tok_ptr->arg[1],10))
    {
      return DSAT_ERROR;
    }

    dflm_ptr = (const dflm_type *)mixed_dflm_ptr[1]->def;

    if ( dflm_ptr->lower > service_type ||
         dflm_ptr->upper < service_type )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV: Invalid Argument");
      return DSAT_ERROR;
    }

    if ( service_type == DSAT_AVAILABLE_EMBMS_SERVICE ||
         service_type == DSAT_ACTIVE_EMBMS_SERVICE ||
         service_type == DSAT_DEFAULT_EMBMS_SERVICE )
    {
      if ( num_args != 2 )
      {
        DS_ATCOP_ERROR_LOG_1("+CEMBMSSRV: Invalid no of arguments for service type =d",service_type);
        return DSAT_ERROR;
      }
    }

    /* If Service_type is 2 or 3 i.e configure TMGI or Deconfigure TMGI , no of args must be 3 */
    if ( service_type == DSAT_CONFIGURE_EMBMS_SERVICE || 
         service_type == DSAT_DECONFIGURE_EMBMS_SERVICE )
    {
      if( num_args != 3 || !VALID_TOKEN(2))
      {
       DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV: Configure/Deconfigure TMGI need 3 args");
        return DSAT_ERROR;
      }
      else
      {
        /* Validate the args for Configure/Deconfigure TMGI */
        if( strlen((const char*)tok_ptr->arg[2]) > mixed_dflm_ptr[2]->def->string_len )
        {
         DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV:TMGI string length invalid");
          return DSAT_ERROR;
        }

        if( !dsatutil_strip_quotes_out ( tok_ptr->arg[2], tok_ptr->arg[2], strlen((const char*)tok_ptr->arg[2]) ) )
        {
          DS_AT_MSG0_HIGH("+CEMBMSSRV: dsatutil_strip_quotes_out ERROR");
          return DSAT_ERROR;
        }
        tmgi_str = tok_ptr->arg[2];
        tmgi_str_len = strlen((const char*)tmgi_str);
      }
    }
    ...
    /* Service Type 4 currently not supported , just return OK */

    if ( service_type == DSAT_DEFAULT_EMBMS_SERVICE )
    {
      DS_AT_MSG0_HIGH("+CEMBMSSRV: Service type 4 not supported , return OK");
      return DSAT_OK;
    }
    ...
    switch (service_type)
    {
      ...
      case DSAT_ACTIVE_EMBMS_SERVICE:
        /*------------------------------------------------------
        Query Available / Active Lists of TMGI
        -----------------------------------------------------*/

        result = dsatetsipkt_embms_query_tmgi_list(service_type,res_buff_ptr,subs_id);
        break;

      case DSAT_CONFIGURE_EMBMS_SERVICE:
        /*------------------------------------------------------
         Configure a TMGI for Activation
         ------------------------------------------------------*/

        /* Convert input TMGI string to RRC hex format */
        if ( DSAT_OK != dsatetsipkt_embms_convert_tmgi_str_to_rrc_fmt( tmgi_str , tmgi_str_hex , tmgi_str_len ))
        {
          result = DSAT_ERROR;
          break;
        }

        /* Copy the configured TMGI to a Global variable which can be used by AT+CEMBMSDATA command */
        dsatutil_memscpy(activate_tmgi_str,
                         sizeof(activate_tmgi_str),
                         tmgi_str_hex,
                         sizeof(tmgi_str_hex));
      ...
      case DSAT_DECONFIGURE_EMBMS_SERVICE:

        /*---------------------------------------------------------
         Deconfigure / Deactivate a TMGI
         ---------------------------------------------------------*/
        ...
        /*----------------------------------------------------------------------
        Copy the Service ID and PLMN ID from the input TMGI String to
        deactivation request message to be sent to RRC
        ----------------------------------------------------------------------*/

        dsatutil_memscpy((void*)(deact_tmgi_req_msg.session_info.tmgi.service_id),
                         DSAT_EMBMS_RRCIF_SVC_ID_SIZE,
                         (void*)(tmgi_str_hex),
                         DSAT_EMBMS_RRCIF_SVC_ID_SIZE);

        dsatutil_memscpy((void*)(deact_tmgi_req_msg.session_info.tmgi.plmn_id.identity),
                         DSAT_EMBMS_RRCIF_PLMN_ID_SIZE,
                         (void*)(tmgi_str_hex + DSAT_EMBMS_RRCIF_PLMN_OFFSET),
                         DSAT_EMBMS_RRCIF_PLMN_ID_SIZE);

        deact_tmgi_req_msg.debug_trace_id =  DSAT_EMBMS_DEBUG_TRACE_ID;

        /*------------------------------------------------------------------------
        Send the message to RRC for deactivating a TMGI
        -----------------------------------------------------------------------*/
        ret_val = ds3gmsgr_msg_send_ext
        ...
        /*----------------------------------------------------------------------
        Message sending to RRC failed , return error
        ----------------------------------------------------------------------*/
        if ( ret_val == FALSE )
        {
          result = DSAT_ERROR;
        ...
        /*--------------------------------------------------------------------------
        Store the TMGI so that we can compare it on deactivation indication from RRC
        ---------------------------------------------------------------------------*/
        
        cmd_arg_ptr = dsatutil_get_val(DSATETSI_EXT_CEMBMSSRV_IDX,0,2,MIX_STR_TYPE);
        dsatutil_memscpy( cmd_arg_ptr , TMGI_HEX_MAX_LENGTH,
                          tmgi_str_hex , sizeof(tmgi_str_hex) );

        /*----------------------------------------------------------------------------
        ATCOP will wait for deactivation indication for this particaular TMGI 
        Return ASYNC_CMD
        ----------------------------------------------------------------------------*/

        if ( CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_TMGI) )
        {
          SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_DEACT_TMGI)
        }
        else
        {
          SET_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_DEACT_TMGI)
        }
      ...
      default:
       DS_ATCOP_ERROR_LOG_0(" +CEMBMSSRV: Invalid Service Type");
        result = DSAT_ERROR;
    }
  }
  else if (tok_ptr->op == (NA|EQ|QU))
  {
    res_buff_ptr->used = (word)snprintf ( (char*)res_buff_ptr->data_ptr,
                                                 res_buff_ptr->size,
                                                 "+CEMBMSSRV: (%d-%d),(%d-%d)",
                                                 ((const dflm_type *)mixed_dflm_ptr[0]->def)->lower,
                                                 ((const dflm_type *)mixed_dflm_ptr[0]->def)->upper,
                                                 ((const dflm_type *)mixed_dflm_ptr[1]->def)->lower,
                                                 ((const dflm_type *)mixed_dflm_ptr[1]->def)->upper);
    result = DSAT_OK;
  }
  else
  {
   DS_ATCOP_ERROR_LOG_0("+CEMBMSSRV:Operation not supported");
    result = DSAT_ERROR;
  }
  return result;
}
...
FUNCTION DSATETSIPKT_EXEC_CEMBMSDATA_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. It handles the +CEMBMSDATA command.
  This command is used to activate a TMGI
DEPENDENCIES
  Should be followed by +CEMBMSSRV cmd which will configure the TMGI to
  activate and eMBMS status must be enabled

RETURN VALUE
  returns an enum that describes the result of the command execution.
  possible values:
    DSAT_ERROR : if there was any problem in executing the command
    DSAT_ASYNC_CMD : On activation request sent to RRC

SIDE EFFECTS
  None
===========================================================================*/

dsat_result_enum_type dsatetsipkt_exec_cembmsdata_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
{
  dsat_result_enum_type result = DSAT_OK;
  dsat_num_item_type session_id;
  dsat_string_item_type *sai_list_str;
  dsat_string_item_type *freq_list_str;
  lte_rrc_embms_act_tmgi_req_s act_tmgi_req_msg;
  sys_modem_as_id_e_type subs_id = dsat_get_current_subs_id(FALSE);
  boolean ret_val = FALSE;
  uint8 curr_embms_status = embms_status;

  uint8 num_sai_to_search = 0; /*!< Number of SAIs in the list below */

  ps_iface_embms_sai_type sai_list[PS_IFACE_EMBMS_MAX_NUM_SAI_TO_SRCH]; 
                          /*! List of SAIs where TMGI can be found */

  uint8 num_earfcn_to_search = 0; /*!< Number of EARFCNs in the list below */

  ps_iface_earfcn_type earfcn_list[PS_IFACE_EMBMS_MAX_NUM_FREQUENCY_TO_SRCH]; 
                          /*! List of EARFCNs where TMGI can be found */

  if ( curr_embms_status != DSAT_EMBMS_ENABLED )
  {
   DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA: eMBMS disabled");
    return dsat_send_cme_error(DSAT_CME_OP_NOT_ALLOWED);
  }

  if ( tok_ptr->op == (NA|EQ|AR))
  {
    /* TMGI must be configured before using ++CEMBMSDATA at cmd */
    if ( !CHECK_PENDING(DSATETSI_EXT_CEMBMSSRV_IDX,0,DSAT_PENDING_ACT_TMGI) )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA:TMGI for activation not yet configured");
      return DSAT_ERROR;
    }
    /* Check for no of args */
    if ( tok_ptr->args_found != 3 )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA:Invalid no of Args ");
      return DSAT_ERROR;
    }
    ...
    /* Arg 1 i.e session ID must be 0 */
    if ( dsatutil_atoi(&session_id,tok_ptr->arg[0],10) == ATOI_OK )
    {
      if ( session_id != 0)
      {
       DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA: Session ID not supported , must be 0");
        return DSAT_ERROR;
      }
    }
    else
    ...
    if ( !dsatutil_strip_quotes_out(tok_ptr->arg[1],tok_ptr->arg[1],strlen((const char*)tok_ptr->arg[1])) )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA: SAI list must be in quotes");
      return DSAT_ERROR;
    }

    if ( !dsatutil_strip_quotes_out(tok_ptr->arg[2],tok_ptr->arg[2],strlen((const char*)tok_ptr->arg[2])) )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA: Frequency list must be in quotes");
      return DSAT_ERROR;
    }

    sai_list_str = tok_ptr->arg[1];
    freq_list_str = tok_ptr->arg[2];

    if ( FALSE == dsatetsipkt_embms_convert_str_to_rrc_list(sai_list_str,PS_IFACE_EMBMS_MAX_NUM_SAI_TO_SRCH,(void *)sai_list,
                                                            &num_sai_to_search , DSAT_EMBMS_SAI_LIST) )
    {
      return DSAT_ERROR;
    }

    if ( FALSE == dsatetsipkt_embms_convert_str_to_rrc_list(freq_list_str,PS_IFACE_EMBMS_MAX_NUM_FREQUENCY_TO_SRCH,(void *)earfcn_list,
                                                            &num_earfcn_to_search , DSAT_EMBMS_EARFCN_LIST) )
    {
      return DSAT_ERROR;
    }

    DS_AT_MSG2_HIGH("+CEMBMSDATA: Num SAI to search = %d, EARFCN = %d ",num_sai_to_search,num_earfcn_to_search);

    /* Fill the activation request msg based on the args */
    memset((void*)&act_tmgi_req_msg, 0 , sizeof(act_tmgi_req_msg));
    act_tmgi_req_msg.debug_trace_id = DSAT_EMBMS_DEBUG_TRACE_ID;

    act_tmgi_req_msg.num_earfcn_to_search = ( num_earfcn_to_search < PS_IFACE_EMBMS_MAX_NUM_FREQUENCY_TO_SRCH ) ?
                                              num_earfcn_to_search:
                                              PS_IFACE_EMBMS_MAX_NUM_FREQUENCY_TO_SRCH;

    act_tmgi_req_msg.num_sai_to_search    = ( num_sai_to_search < PS_IFACE_EMBMS_MAX_NUM_SAI_TO_SRCH ) ?
                                              num_sai_to_search:
                                              PS_IFACE_EMBMS_MAX_NUM_SAI_TO_SRCH;

    /*--------------------------------------------------------------------------------------------------------
     Copy SAI list , EARFCN list , PLMN ID , SVC ID to the RRC TMGI activation request msg
     -------------------------------------------------------------------------------------------------------*/
    ...
    dsatutil_memscpy((void*)(act_tmgi_req_msg.session_info.tmgi.service_id),
                     DSAT_EMBMS_RRCIF_SVC_ID_SIZE,
                     (void*)(activate_tmgi_str),
                     DSAT_EMBMS_RRCIF_SVC_ID_SIZE);

    dsatutil_memscpy((void*)(act_tmgi_req_msg.session_info.tmgi.plmn_id.identity),
                     DSAT_EMBMS_RRCIF_PLMN_ID_SIZE,
                     (void*)(activate_tmgi_str + DSAT_EMBMS_RRCIF_PLMN_OFFSET),
                     DSAT_EMBMS_RRCIF_PLMN_ID_SIZE);

    /*------------------------------------------------------------
      Send Activation request msg to RRC
      ----------------------------------------------------------*/
    ret_val = ds3gmsgr_msg_send_ext
    ...
    if ( ret_val == FALSE )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSDATA:Error sending TMGI Activation Req MSG");
      return DSAT_ERROR;
    }
    SET_PENDING(DSATETSI_EXT_CEMBMSDATA_IDX,0,DSAT_PENDING_TRUE)
    result = DSAT_ASYNC_CMD;
  }
  else
  {
    DS_AT_MSG0_HIGH("+CEMBMSDATA: No other operation permitted");
    result = DSAT_ERROR;
  }  
  return result;
}
...
FUNCTION DSATETSIPKT_EXEC_CEMBMSCNT_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. It handles the +CEMBMSCNT command.
  This command sets the interested TMGI list
DEPENDENCIES
  eMBMS must be enabled
...
dsat_result_enum_type dsatetsipkt_exec_cembmscnt_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
{
  dsat_result_enum_type result = DSAT_OK;
  dsat_num_item_type num_args,index;
  dsat_embms_svc_interest_req_type * tmgi_interest_list;
  uint8 len_quoted_str;
  dsat_string_item_type *tmgi_str;
  ...
  if ( curr_embms_status != DSAT_EMBMS_ENABLED )
  {
   DS_ATCOP_ERROR_LOG_0("+CEMBMSSAI: eMBMS disabled ");
    return dsat_send_cme_error(DSAT_CME_OP_NOT_ALLOWED);
  }
  ...
  if(tok_ptr->op == (NA|QU))
  {
    tmgi_interest_list = dsat_embms_svc_interest_list;

    /*--------------------------------------------------------------------
    If interest list is NULL , there is 0 TMGI of interest else
    report the list of interested TMGIs
    --------------------------------------------------------------------*/
    ...
    if ( NULL == tmgi_interest_list )
    {
      result = DSAT_OK;
    }
    else
    {
      for(index=0 ; index < tmgi_interest_list->num_tmgi ; index++)
      {
        res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                               res_buff_ptr->size - res_buff_ptr->used,
                                               "\"%02X%02X%02X%02X%02X%02X\"",
                                               tmgi_interest_list->tmgi_list[index].tmgi[0],
                                               tmgi_interest_list->tmgi_list[index].tmgi[1],
                                               tmgi_interest_list->tmgi_list[index].tmgi[2],
                                               tmgi_interest_list->tmgi_list[index].tmgi[3],
                                               tmgi_interest_list->tmgi_list[index].tmgi[4],
                                               tmgi_interest_list->tmgi_list[index].tmgi[5]);

        if ( index != (tmgi_interest_list->num_tmgi - 1))
        {
          res_buff_ptr->used += (word)snprintf ( (char*)res_buff_ptr->data_ptr + res_buff_ptr->used,
                                                 res_buff_ptr->size - res_buff_ptr->used,
                                                 ",");
  ...
  else if (tok_ptr->op == (NA|EQ|AR))
  {
    if ( num_args == 0 )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSCNT: No arguments");
      return DSAT_ERROR;
    }
    /*--------------------------------------------------------------------------------
     We will get args as list of TMGIs of interest 
     Validate the arguments and if valid store them in a global variable
    ---------------------------------------------------------------------------------*/
    tmgi_interest_list = dsat_alloc_memory(sizeof(dsat_embms_svc_interest_req_type),FALSE);
    memset(tmgi_interest_list,0,sizeof(dsat_embms_svc_interest_req_type));

    tmgi_interest_list->num_tmgi = num_args;
    tmgi_interest_list->debug_trace_id = DSAT_EMBMS_DEBUG_TRACE_ID;

    tmgi_interest_list->tmgi_list = dsat_alloc_memory(num_args * sizeof (dsat_embms_tmgi_type) ,FALSE);
    memset(tmgi_interest_list->tmgi_list,0, num_args * sizeof(dsat_embms_tmgi_type));

    for (index=0 ; index < num_args ;index++)
    {
      len_quoted_str = strlen((const char*) tok_ptr->arg[index]);

      if ( FALSE == dsatutil_strip_quotes_out(tok_ptr->arg[index],tok_ptr->arg[index],len_quoted_str) )
      {
        is_valid = FALSE;
        break;
      }
      tmgi_str = tok_ptr->arg[index];
      if ( DSAT_OK != dsatetsipkt_embms_convert_tmgi_str_to_rrc_fmt(tmgi_str,tmgi_str_hex,strlen((const char*)tmgi_str)))
      {
        is_valid = FALSE;
        break;
      }

      tmgi_interest_list->tmgi_list[index].session_id_present = FALSE;
      tmgi_interest_list->tmgi_list[index].session_id = 0;
      memscpy(tmgi_interest_list->tmgi_list[index].tmgi,
              sizeof(tmgi_interest_list->tmgi_list[index].tmgi),
              tmgi_str_hex,
              sizeof(tmgi_str_hex));
    ...
    {
      /*------------------------------------------------------------------
      Args received are not valid , Free the memory and return Error
      ------------------------------------------------------------------*/
      dsatutil_free_memory(tmgi_interest_list->tmgi_list);
      dsatutil_free_memory(tmgi_interest_list);
      tmgi_interest_list = NULL;
      return DSAT_ERROR;
    }

    /*--------------------------------------------------------------------
     Args passed are valid TMGIs, free the old list and update new list
     -------------------------------------------------------------------*/
    if (NULL == dsat_embms_svc_interest_list)
    {
      dsat_embms_svc_interest_list = tmgi_interest_list;
    }
    else
    {
      dsatutil_free_memory(dsat_embms_svc_interest_list->tmgi_list);
      dsatutil_free_memory(dsat_embms_svc_interest_list);
      dsat_embms_svc_interest_list = tmgi_interest_list;
    }
    result = DSAT_OK;
  ...
  {
    DS_AT_MSG0_HIGH("+CEMBMSCNT: No other operation supported");
    result = DSAT_ERROR;
  }
  return result;
}
...
FUNCTION DSATETSIPKT_EXEC_CEMBMSSAI_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. It handles the +CEMBMSSAI command.
  This command reports the latest SAI list.
DEPENDENCIES
  eMBMS must be enabled
...
RETURN VALUE
  ...
  possible values:
    DSAT_ERROR : if there was any problem in executing the command
    DSAT_OK : if it is a success
    DSAT_ASYNC_CMD : SAI list request successfully sent to RRC

SIDE EFFECTS
  None
===========================================================================*/

dsat_result_enum_type dsatetsipkt_exec_cembmssai_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
)
{
  ...
  {
    /*-----------------------------------------------------------
     Fill SAI request msg to be sent to RRC
    -----------------------------------------------------------*/
    memset(&sai_list_req_msg,0,sizeof(sai_list_req_msg));
    sai_list_req_msg.debug_trace_id = DSAT_EMBMS_DEBUG_TRACE_ID;

    /*-----------------------------------------------------------
     Send SAI list request msg to RRC
    -----------------------------------------------------------*/
    ...
    if ( FALSE == ret_val )
    {
     DS_ATCOP_ERROR_LOG_0("+CEMBMSSAI:Error sending SAI List Query Req Msg");
      return DSAT_ERROR;
    }

    /*------------------------------------------------------------
    Wait for SAI List indication from RRC , return ASYNC CMD
    ------------------------------------------------------------*/
    SET_PENDING(DSATETSI_EXT_CEMBMSSAI_IDX,0,DSAT_PENDING_TRUE)
    result = DSAT_ASYNC_CMD;
  }
  else
  {
   DS_ATCOP_ERROR_LOG_0("+CEMBMSSAI:Opeartion not supported");
    result = DSAT_ERROR;
  }
  return result;
}
...
{
  qsh_mdump_collect_high(&dynamic_param_ptr, sizeof(ds_3gpp_atif_dynamic_params_info_type *));
}
#endif /* FEATURE_QSH_MDUMP */
