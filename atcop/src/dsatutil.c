...
$Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatutil.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
#define DSATUTIL_ASSERT(expression) \
         dsatutil_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATUTIL_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATUTIL ASSERT
...
===========================================================================*/
static void dsatutil_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATUTIL_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatutil_assert_wrapper */
...
)
{
  ...
  if ( (NULL != val_ptr) && (!skip_fmt))
  {
    ...
    else if (tab_entry->compound > 1)
    {
      for (j=0; j < tab_entry->compound; ++j)
      {
        w_ptr = psutil_itoa(val_ptr[j], w_ptr, radix );
        *w_ptr++ = ',';
      }
      --w_ptr;
    }
    ...
    {
      w_ptr = psutil_itoa(*val_ptr, w_ptr, radix );
    }
...
)
{
  ...
  DSATUTIL_ASSERT(NULL != orig_dial_str);
  ...
  *s_ptr = '\0';
  ...
  while (i < dsat_pkt_dial_string_table[subs_id].size)
  {
     ...
     {
       DS_ATCOP_ERROR_LOG_1("Invalid prefix length : =d", prefix_len);
       return result;
     }
...
)
{
  ...
  DSATUTIL_ASSERT(NULL == state_ptr->cmd_mem_ptr);
...
)
{
  ...
  #define CVT_TO_HEX_PAIR(ch)\
    ...
    out_str_dup = (char *)psutil_itoa((uint32)ch, (byte *)out_str_dup, 16);
  ...
  DSATUTIL_ASSERT( in_str != NULL );
  DSATUTIL_ASSERT( out_str != NULL );
   ...
   switch ( in_chset )
    {
        ...
        switch ( out_chset )
        {
          ...
          case ALPHA_UCS2:
            ...
            if ( *in_str_dup == GSM_AT_CHAR_REPLACEMENT )
            {
              ...
              out_str_dup = 
                 (char *)psutil_itoa( 0x40, (byte *)out_str_dup, 16);
            }
            else
            {
              ...
              {
                *out_str_dup++ = '0';
                out_str_dup = (char *)psutil_itoa( 0, (byte *)out_str_dup, 16);
      ...
      case ALPHA_PBM_8BIT:
        switch ( out_chset )
        {
          ...
          /* If unsupported output character set specified */
          default:
           DS_ATCOP_ERROR_LOG_0("Conversion from PBM alpha limited to IRA, GSM, UCS2");
            return NULL;
        }
      ...
      case ALPHA_PBM_UCS2:
        switch ( out_chset )
        {
          ...
          /* If unknown output character set specified */
          default:
           DS_ATCOP_ERROR_LOG_0("Conversion from ALPHA_PBM_UCS2 is limited to IRA, GSM, UCS2");
            return NULL;
        }
      ...
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid input character set specified =d",
                   in_chset);
        return NULL;
    }
  }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("invalid input length =d",in_len);
  ...
  for (i = 0; i < in_len; i += HEX_CHAR_LEN)
  {
    ...
    if ( i == 0 )
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("invalid GSM coding scheme =d", coding_scheme);
        return DSAT_CME_INVALID_CHAR_IN_TEXT;
      }
      continue;
    }
                ...
                (coding_scheme == GSM_UCS2_CODING_SCHEME_THREE) ) )
    {
      ...
      if ( num_chars == 0 )
      {
       DS_ATCOP_ERROR_LOG_0("number of characters in coding scheme is 0");
        return DSAT_CME_UNKNOWN;
      }
    ...
    else if ( i == 4 )
    {
      ...
      else if ( coding_scheme == GSM_UCS2_CODING_SCHEME_THREE )
      {
        ...
        if (in_len < 6)
        {
          DS_ATCOP_ERROR_LOG_1("not enough characters (=d) for coding scheme 3",in_len);
          return DSAT_CME_UNKNOWN;
        }
    ...
    switch ( coding_scheme )
    {
      ...
      case GSM_UCS2_CODING_SCHEME_ONE:
      {
        ...
        if( FALSE == dsatutil_hex_to_uint16( &in_str[i], &ucs2_char_tmp ))
        {
          DS_ATCOP_ERROR_LOG_0("ATOI conversion failed ");
           return DSAT_CME_UNKNOWN;
        }
        ...
        if ( ucs2_char_tmp != 0xFFFF )
        {
          ...
          if ( out_idx >= out_max )
          {
           DS_ATCOP_ERROR_LOG_0("not enough room in output");
            return DSAT_CME_TEXT_STRING_TOO_LONG;
          }
      ...
      case GSM_UCS2_CODING_SCHEME_THREE:
      {
        ...
        /* If bit 8 of the octet is set to zero, the remaining 7 bits of the 
           ...
           value */
        if (FALSE == dsatutil_hex_to_uint8( &in_str[i], &octet_tmp ))
        {
          return DSAT_CME_UNKNOWN;
        }
        if ( octet_tmp & 0x80 )
        ...
        else
        {
          ...
          else
          {
            ...
            if ( octet_tmp == GSM_EXT_TBL_ESC_CHAR )
            {
              ...
              {
               DS_ATCOP_ERROR_LOG_0("input character stream incomplete for GSM escape sequence");
                return DSAT_CME_INVALID_CHAR_IN_TEXT;
              }
...
)
{
  ...
  DSATUTIL_ASSERT( ((NULL != gsm_in && NULL != *gsm_in && NULL != pbm_out) == TRUE) );
...
#define MASK_BITS_LESS_THAN_8 0x07
  ...
  DSATUTIL_ASSERT( NULL != addr_mask_num );
  DSATUTIL_ASSERT( NULL != addr_mask_str );
       ...
       (INT_TO_STRING == mode && DS_PROFILE_3GPP_IP_V6 == *ip_vsn) )
  {
    ...
    if (curr_char == NULL)
    {
     DS_ATCOP_ERROR_LOG_0("Invalid address mask string");
      return DSAT_ERROR;
    }
    ...
    if(INT_TO_STRING == mode)
    {
      ...
      {
        /* Convert to integer */
         psutil_itoa ((uint32)addr_mask_num->mask.prefix_len,(byte *) curr_char, 10);
      }
      else
      {
        if(1 == cgpiaf_add_format)
...
#define DSATUTIL_MMGSDI_UCS2_82 0x82
  ...
  switch (buf_in[0]) 
  {
    ...
    default:
     DS_ATCOP_ERROR_LOG_0("Input buffer not encoded in UCS2 80,81 or 82");
      break;
  }
...
)
{
  ...
  fs_handle = mcfg_fopen(file_path,
                         MCFG_FS_O_RDONLY,
                         MCFG_FS_DEFFILEMODE,
                         MCFG_FS_TYPE_EFS,
                         MCFG_FS_SUBID_NONE);
...
send_error:
  ...
  DS_AT_MSG2_HIGH ("dsat_efs_file_open() file_name invalid len, err:%d, "
                   "mcfg_efs_erno:", err_info, 
...
)
{
  ...
  ret = mcfg_fstat (fs_handle, &fs_buf, MCFG_FS_TYPE_EFS);

  if (-1 == ret)
  {
    DS_ATCOP_ERROR_LOG_1("dsat_get_string_efs_file_size() Invalid file, efs_erno:=d",
                     mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    return 0;
  }
...
)
{
  ...
  *buffer = (void *) dsat_alloc_memory (local_size , FALSE);
  ...
  ret = mcfg_fread(fs_handle, *buffer, local_size, MCFG_FS_TYPE_EFS);
  ...
  (void)mcfg_fclose(fs_handle, MCFG_FS_TYPE_EFS);

  return ret;

send_error:
  DS_AT_MSG2_HIGH ("dsat_read_efs_data() file_name invalid length, err:%d,"
                   "mcfg_efs_erno:%d", err_info,
                   mcfg_fs_errno(MCFG_FS_TYPE_EFS));
