...
INITIALIZATION AND SEQUENCING REQUIREMENTS
   ...
   Copyright (c) 2002 - 2014,2017 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatsmsu_ex.c#1 $
...
)
{
  ...
  {
    if(strncmp(wms_memory_table[index].mem_str, 
              (char *) mem_valstr, CPMS_MEM_STR_LEN + 1) == 0)
    {
      return_val = wms_memory_table[index].wms_memory;
      break;
    }
  }
  DS_AT_MSG2_HIGH("index = %d return_val = %d", index,return_val);
  return return_val;
} /* dsatsmsi_memory_lookup */
