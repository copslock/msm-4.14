...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatclient.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/30/17   rk      Removed DS Flag FEATURE_DSAT_EXT_CLIENT_SUPPORT.
06/22/17   skc     Fixed issue to send missing bytes to VSP client.
...
===========================================================================*/
  rex_crit_sect_type dsat_client_crit_sect;

  dsat_client_info_s_type dsat_client_info[DSAT_EXT_CLIENT_ID_MAX];

#define DSATCLIENT_ASSERT(expression) \
         dsatclient_assert_wrapper(__LINE__, expression)
         
/*-------------------------- Local Functions -------------------------------*/

/*===========================================================================

FUNCTION DSATCLIENT_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATCLIENT ASSERT
...
===========================================================================*/
static void dsatclient_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATCLIENT_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatclient_assert_wrapper */ 
...
{
  boolean result = TRUE;
  DSATCLIENT_ASSERT ( ((client_id_ptr != NULL) == TRUE) );
  ...
  {
    DS_ATCOP_ERROR_LOG_1(" Invalid client type specified =d",client_type);
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Client with type =d is already registered",client_type);
    *client_id_ptr = DSAT_EXT_CLIENT_ID_INVALID;
    result = FALSE;
  }
...
)
{
 ...
 /*-------------------------------------------------------
  ...
  {
    DS_ATCOP_ERROR_LOG_1(" Invalid client Client ID =d",client_id);
    return FALSE;
  }
    /*-------------------------------------------------------
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Function Pointer can't be NULL ");
    return FALSE;
  }
    ...
    rex_enter_crit_sect(&dsat_client_crit_sect);
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1(" Client id=d is not a registred client",client_id);
      result =  FALSE;
    }
    else
    {
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1(" Invalid length =d ",at_cmd.length);
    return FALSE;
  }
...
/*
  'response_type' is decided based on rearm_autodetect state, 
   ...
   If response buffer is greater than LARGE DSM ITEM (1536),
       -->we would have queued cmd and created new buffer.
       -->In last buffer only we will set rearm_autodetect.
       -->Response type will be START, INTERMEDIATE, INTERMEDIATE..., END
   If response buffer is less than LARGE DSM ITEM,
       -->we wont queue and set rearm_autodetect.
       -->If response buffer size is 
            >>> less than (MAX_RESP_SIZE - 1), response type will be COMPLETE.
            >>> in between (MAX_RESP_SIZE - 1) and (MAX_RESP_SIZE - 1)*2, response type will be START and END.
            >>> greater than (MAX_RESP_SIZE - 1)*2, response will be START, INTERMEDIATE and END.
*/
...
{ \
  ...
  { \
    ...
    { \
      if((sent_buff_len + res_buff_len) < response_buff_size) \
      { \
...
/*-------------------------------------------------------------------------------*/
  ...
  dsat_data_s_type response;
  dsat_sw_api_resp_s_type   resp_type;
  uint16                    response_buff_size = 0;
  uint16                    sent_buff_len = 0;
...
)
{
     ...
     (dsatcmdp_processing_fwd_cmd.client_id == client_id))
  {
   ...
   DS_ATCOP_ERROR_LOG_0("De-registration when a command processing has not been completed");
...
)
{
    ...
    */
    ...
    switch (client_resp_ptr->result_code)
    {
      ...
      case DSAT_CLIENT_RESULT_OK:
        ...
        break;
      default:
       DS_ATCOP_ERROR_LOG_0("Invalid result code-clearing res_buffer, return ERROR");
        if ( client_resp_ptr->resp_buff_ptr != NULL )
        {
          /* Free the dsm item pointer allocated */
          dsm_free_packet(&(client_resp_ptr->resp_buff_ptr));
  ...
  else
  {
   ...
   DS_ATCOP_ERROR_LOG_0("No AT Command forwarding is pending, ignoring");
    return DSAT_ASYNC_EVENT;
  }
  if( client_resp_ptr->resp_buff_ptr != NULL )
  {
  ...
  DS_ATCOP_ERROR_LOG_1("Result code =d", (int)result);
  return result;
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("In valid cmd_status =d ",client_urc_ptr->cmd_status);
