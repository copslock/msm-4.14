...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatcmdp.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/23/18   skc     Fixed issue while flushing queued URCs.
10/30/17   rk      Removed DS Flag FEATURE_DSAT_EXT_CLIENT_SUPPORT.
08/29/17   rg      Fix to send URC on to the VSP port from process_send_urc
08/23/17   rg      Ignore queuing URCs if the port isn't used atleast once
07/26/17   skc     Restricting URC to report if there is any active port
                   in case of multi-port scenrio.
05/04/17   skc     Fixed issue with AT Commands that may start with CDV
04/04/17   ss      Added support for remote AT commands based on EFS.
10/12/16   skc     Fixed issue with at_hanlde used during abort.
...
#define DSATCMDP_ASSERT(expression) \
         dsatcmdp_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATCMDP_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATCMDP ASSERT
...
===========================================================================*/
static void dsatcmdp_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATCMDP_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatcmdp_assert_wrapper */
...
)
{
  ...
  {
    DS_ATCOP_ERROR_FATAL("Invalid SIO Port ID");
  }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("AT command is already in process");
    return;
  }
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Async cmd handling error - formatted rsp item allocated!");
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid ATCoP state");
          ...
          && ((result_code == DSAT_OK) || (result_code == DSAT_NO_RESULT_CODE)) )
  {
        ...
        || ((strncmp((char *)token_ptr->token.name, cap_cdv, cmd_len)) == 0))
    {
      ...
      if((token_ptr->token.op & AR ) == 0  &&
         (token_ptr->token.op & QU ) != QU &&  //Filter out NA|EQ|QU or NA|QU
         (token_ptr->token.op & EQ ) != EQ)    //Filter out NA|EQ 
      {
    ...
    */
      ...
      if ( is_cmd_supported )
    {
      ...
      DSATCMDP_ASSERT((((token_ptr->token.cmd_category < NUM_AT_CMD_CATEGORIES)) == TRUE));
...
/*Displaying Command name */
      ...
      if (ETSI_CMD_MODE == curr_operating_mode)
      {
        ...
        else
        {
          ...
          {
           DS_ATCOP_ERROR_LOG_0("Command not allowed in restricted mode");
            result_code = DSAT_ERROR;
          }
#if defined(FEATURE_8960_SGLTE_FUSION)
          else if ( !(table_ptr->attrib & NOT_PROVISIONED) && DSAT_MMGSDI_INACTIVE == dsat_get_provision_state() )
          {
           DS_ATCOP_ERROR_LOG_0("Command is not allowed before provisioning");
            result_code = DSAT_ERROR;
          }
#endif /* defined(FEATURE_8960_SGLTE_FUSION) || defined(FEATURE_8960_DSDA_FUSION) */
        }
      ...
      if (result_code != DSAT_ERROR)
      {
        ...
        if ( table_ptr->attrib & ABORT_CMD )
        {
          ...
          if (temp_abort_ptr == NULL)
          {
            ...
            DS_ATCOP_ERROR_LOG_1("Invalid abort state: =d", table_ptr->cmd_id);
          }
        }
...
)
{
  ...
  if (buf_ptr != NULL && total_tokens < TOKEN_BUF_CNT)
  {
    ...
    {
      DSATCMDP_ASSERT( ((token_fill_ptr < &token_buffer[TOKEN_BUF_SIZE]) == TRUE) );
  ...
  else 
  {
   ...
   DS_ATCOP_ERROR_LOG_0("Data task AT token free buffer empty.");
...
)
{
  ...
  DSATCMDP_ASSERT( ((token_empty_ptr >= token_buffer &&
          token_empty_ptr < &token_buffer[TOKEN_BUF_SIZE]) == TRUE) );
...
void dsatcmdp_send_queued_cmd_results( void )
{
  ...
  while ( (dq_item_ptr = (dsm_item_type *)q_get(&cmd_results_hold_queue))
          != NULL )
  {
    DSATCMDP_ASSERT( ((dq_item_ptr->used > 0) == TRUE) );   
...
)
{
  ...
  DSATCMDP_ASSERT( ((raw_rsp_ptr != NULL) == TRUE) );
  ...
  if ( formatted_rsp_ptr == NULL )
  {
   DS_ATCOP_ERROR_LOG_0("Async cmd handling error - formatted rsp item not allocated!");
...
)
{
  ...
  if(msg_ptr == NULL)
  {
   DS_ATCOP_ERROR_LOG_0("Invalid Queue element");
...
boolean dsatcmdp_abort_command( void )
{
       ...
       ( abort_cmd_table_entry_ptr_ex != NULL ) )
  {
    ...
    DSATCMDP_ASSERT( ((abort_cmd_table_entry_ptr_ex->abort_func != NULL) == TRUE) );
  ...
  else if (curr_ext_at_abort_fwd_ptr != NULL) 
  {
    /* Verify abort handler pointer is valid.  Pointer should never be
       NULL when this function is called to abort a command. */
    DSATCMDP_ASSERT( ((curr_ext_at_abort_fwd_ptr->ext_at_abort_cb != NULL) == TRUE) );
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Invalid SIO port: =d",(int)port);
    return;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  default:
    DS_ATCOP_ERROR_FATAL( "Invalid Flow control value");
    break;
  }
...
)
{
  ...
  DS_AT_MSG2_MED("Set block indications flag to %d on port %d",
                 flag_val, port_id);
  if(dsatcmdp_processing_async_cmd == TRUE && flag_val == FALSE)
  {
    DS_AT_MSG0_HIGH("dsatcmdp_set_block_indications return false");
    return;
  }
  sio_info_ptr = dsatcmdp_get_sio_info_ptr(port_id);
  ...
  {	
                           ...
                           (const byte *)&fwd_at_cmd_p->cmd_name[0]) )
    {
                                       ...
                                       fwd_at_cmd_p->user_info_ptr))
      {
        ...
        {
          ASSERT( fwd_at_cmd_p->ext_at_abort_cb != NULL );
      ...
      else
      {
       ...
       DS_ATCOP_ERROR_LOG_0("Command cannot be forwarded, return ERROR");
        /* Clear the pending flag,client and port */
        dsatcmdp_processing_fwd_cmd.is_pending = FALSE;
        dsatcmdp_processing_fwd_cmd.port = DS3G_SIOLIB_DATA_PORT;
        dsatcmdp_processing_fwd_cmd.client_id = 0;
...
)
{
  ...
  boolean                      is_urc_sent_to_vsp_port = FALSE;
  ...
  if ((dsat_num_item_type) dsatutil_get_val(DSAT_BASIC_Q_IDX,0,0,NUM_TYPE) == 0)
  {
    ...
    for (port_id = DS3G_SIOLIB_DATA_PORT; port_id < DS3G_SIOLIB_PORT_MAX; port_id++)
    {
      if (port_id == DS3G_SIOLIB_CLIENT_VSP_PORT && !is_urc_sent_to_vsp_port)
      {
        if (buffer_size == dsm_dup_packet(&dup_buffer, formatted_res_buff, 0, buffer_size))
        {
          dsatclient_send_urc_response(dup_buffer, &ret_val, TRUE);
          is_urc_sent_to_vsp_port = TRUE;
        }
            ...
            else
            {
              ...
              else
              {
                ...
                dsatrsp_send_response(dup_buffer, TRUE);
                  if (port_id == DS3G_SIOLIB_DATA_PORT && !is_urc_sent_to_vsp_port)
                  {
                    /* As part of sending urc on to the port DS3G_SIOLIB_DATA_PORT,
                       urc for vsp port is also processed from function
...
{
  qsh_mdump_collect_high(&curr_token_ptr, sizeof(token_q_struct_type *));
  qsh_mdump_collect_high(&formatted_rsp_ptr, sizeof(dsm_item_type *));
  qsh_mdump_collect_high(&cmd_results_hold_queue, sizeof(q_type));
  qsh_mdump_collect_high(&dsatcmdp_sio_info_ptr, DS3G_SIOLIB_PORT_MAX * sizeof(dsat_sio_info_s_type *));
}
#endif /* FEATURE_QSH_MDUMP */
