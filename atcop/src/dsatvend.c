...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatvend.c#2 $ $DateTime: 2020/01/28 03:55:36 $ $Author: gkamma $
...
08/18/17   rg      Added ^prefmode support for 1x on secondary sub 
07/27/17   skc     Fixed issue with sending display buffer for asynchrouns command
                   AT$QCRMCALL and AT+CPOL command.
02/02/17   skc     Fixed issue with AT$QCMRUC for DSDS case.
09/21/16   skc     Fixed issue related to AT$QCPDPCFGE command.
08/31/16   skc     Fixed issue with AT$QCMRUE for GSM and WCDMA RAT.
02/11/16   skc     Fixed KW issues.
01/27/16   skc     Fixed rssi calculation issue for AT$QCSQ
01/27/16   skc     Added $QCRSCP and $QCECNO command Support.
10/12/15   skc     Fixed KW Issues
08/10/15   skc     Replacing TUPLE_TYPE_IPv4_254 with TUPLE_TYPE_IPv4 to remove
                   IP limitation for DNS
05/15/15   tk      Fixed MRU commands issue in Multi SIM configurations.
01/30/15   tk      Feature wrapped $QCVOIPM command with FEATURE_IMS.
01/26/15   sc      Fixed +CGTFT\+QCGTFT AT command issue.
11/19/14   sc      Fixed $QCPRFCRT profile creation issue.
...
#define DSATVEND_ASSERT(expression) \
         dsatvend_assert_wrapper(__LINE__, expression)
...
typedef struct
{
  dsat_qcimssrv_set_get_f_type qcimssrv_set_get;
  void *ims_data_ptr;
}dsat_qcimssrv_ims_s_type;

LOCAL dsat_qcimssrv_ims_s_type qcimssrv_ims;
...
dsat_profile_parameter_details_s_type profile_3gpp_parameter_details_list[]=
{
  ...
  {  "PDNLBL3GPP"            ,    DS_PROFILE_3GPP_PROFILE_PARAM_PDN_LABEL,
      0,                          STR_TYPE,   &apn_str_len[0]    },
...
dsat_qcprofile_param_details_s_type profile_qcpcomsisdn_list[] =
{
  {
    DS_PROFILE_3GPP_PROFILE_MSISDN_REQ_FLAG,
    sizeof(boolean),
    1 
  },
  {
    DS_PROFILE_3GPP_PROFILE_OP_RESERVED_PCO_ID_LIST,
    10*(sizeof(uint16)),
    0xFFFF  
  }
}; /* profile_qcpcomsisdn_list */

/*===========================================================================

FUNCTION DSATVEND_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATVEND ASSERT
...
===========================================================================*/
static void dsatvend_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATVEND_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatvend_assert_wrapper */
...
)
{
  ...
  dsat_error_info_s_type                    err_info;
  dsat_qcprofile_param_details_s_type      *param_list = NULL;
  ...
  err_info.errval   = DSAT_ERR_NO_ERROR;
  err_info.arg_num  = 0;
  param_list        = &profile_qcpdpcfge_param_details_list[0];
    ...
    for ( index = 0; index < total_profiles; index++)
    {
      ...
      for (i=0;i<ARR_SIZE(profile_qcpdpcfge_param_details_list);i++)
      {
                                                           ...
                                                           param_list[i].len))
        {
          if(token_idx == TOKEN_IDX_11)
          { 
            //Get only MNC information, exclude mnc_includes_pcs_digit
            profile_params = profile_params & 0xFFFF;
          }
          buf_curr_len += snprintf(&profile_param_data[buf_curr_len],MAX_PARAM_BUF - buf_curr_len,"%d",profile_params);
        }
        buf_curr_len += snprintf(&profile_param_data[buf_curr_len],MAX_PARAM_BUF - buf_curr_len,",");
  ...
  if (tok_ptr->op == (NA|EQ|AR) ) 
  {
        ...
        ( DSAT_IS_PROFILE_ID_WITHIN_RANGE(profile_id)))
    {
      if(!dsatetsipkt_is_profile_context_valid( profile_id,
                                                ...
                                                dsat_get_current_ds_profile_subs_id()))
      {
        DS_ATCOP_ERROR_LOG_1("Context =d is not valid",profile_id);
        return DSAT_ERROR;
      }
      if(VALID_TOKEN(1) && (ATOI_OK ==dsatutil_atoi((dsat_num_item_type*)&flag_i_val[0],tok_ptr->arg[1],10)))
      {
        if(0 == flag_i_val[0] || 1 == flag_i_val[0])
        {
          ...
          if(DSAT_OK != result )
          {
            DS_ATCOP_ERROR_LOG_1("$QCPDPIMSCFGE: Address flag write failed  =d", result);
            return DSAT_ERROR;
          }
        }
        else
      ...
      if(VALID_TOKEN(2) && (ATOI_OK ==dsatutil_atoi((dsat_num_item_type*)&flag_i_val[1],tok_ptr->arg[2],10)))
      {
        if(0 == flag_i_val[1] || 1 == flag_i_val[1])
        {
          ...
          if(DSAT_OK != result )
          {
            DS_ATCOP_ERROR_LOG_1("$QCPDPIMSCFGE: DHCP flag write failed  =d", result);
            return DSAT_ERROR;
          }
        }
        else
      ...
      if(VALID_TOKEN(3) && (ATOI_OK ==dsatutil_atoi((dsat_num_item_type*)&flag_i_val[2],tok_ptr->arg[3],10)))
      {
        if(0 == flag_i_val[2] || 1 == flag_i_val[2])
        {
          ...
          if(DSAT_OK != result )
          {
            DS_ATCOP_ERROR_LOG_1("$QCPDPIMSCFGE: IM CN flag write failed  =d", result);
            return DSAT_ERROR;
          }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("NULL pointer argument");
    return FALSE;
  }
...
)
{
  ...
  if ( FALSE == cm_ph_cmd_oprt_mode ( dsatcmif_ph_cmd_cb_func,
                                      ...
                                      new_mode ) )
  {
    DS_ATCOP_ERROR_LOG_1("Failed to change sys oprt mode to =d", new_mode);
...
#define DSAT_MAX_MRU_PLMN_LEN 6
  ...
  if(tok_ptr->op == (NA|QU)||(tok_ptr->op == (NA|EQ|AR)))
  {
    if ((dsat_num_item_type)dsatutil_get_val(
             DSAT_EXT_CFUND_IDX,0,0,MIX_NUM_TYPE) != DSAT_ME_FUNC_FULL)
    {
          ...
          /* Strip quotes */
           ...
           else
           {
            ...
            (void)strlcat ((char*)buffer,
             ...
             {
              DS_ATCOP_ERROR_LOG_0("Cannot convert MCC value");
               return DSAT_ERROR;
             }
             ...
             {
              DS_ATCOP_ERROR_LOG_0("Cannot convert MNC value");
               return DSAT_ERROR;
             }
...
)
{
  ...
  if (TRUE != cm_ph_cmd_read_mru( dsatcmif_ph_cmd_cb_func,
                                  ...
                                  index ))
#else
  if (TRUE != cm_ph_cmd_read_mru1( dsatcmif_ph_cmd_cb_func,
                                  &mru_table_entry,
                                  dsatcm_client_id,
                                  index,
  ...
  {
   DS_ATCOP_ERROR_LOG_0("CM command to read MRU table index is failed");
    SET_PENDING( DSAT_VENDOR_QCMRUE_IDX, 0, DSAT_PENDING_MRU_NONE );
    result = DSAT_ERROR;
  }
...
)
{
  ...
  switch(action)
  {
    ...
    {
      SET_PENDING( DSAT_VENDOR_QCMRUC_IDX, 0, DSAT_PENDING_MRU_UPDATE_RESULT );
	  ...
	  if (TRUE != cm_ph_cmd_mru_update( dsatcmif_ph_cmd_cb_func,
                                        ...
                                        CM_MRU_CLEAN ))
#else
      if (TRUE != cm_ph_cmd_mru_update1( dsatcmif_ph_cmd_cb_func,
                                        NULL,
                                        dsatcm_client_id,
                                        &mru_entry,
                                        0,
...
#endif /* FEATURE_DATA_MPSS_AT20_DEV */
      {
       DS_ATCOP_ERROR_LOG_0("CM command to update MRU table is failed");
    ...
    case DSAT_PENDING_MRU_WRITE:
    {
      ...
      switch(dsat_mru_input.rat)
      {
        ...
        case DSAT_GSM_RAT:
        {
          ...
          mru_entry.gwl.acq_mode = SYS_SYS_MODE_GSM;
          
          temp = vend_convert_mru_band_input_to_gsm_band_pref(dsat_mru_input.band);
          mru_entry.gwl.band_pref.bit_15_0= (word) ((word) temp & 0xffff );
          mru_entry.gwl.band_pref.bit_31_16 =(word) ((uint32)temp >> 16 );
          mru_entry.gwl.num_rats = 1; 
          mru_entry.gwl.rat[0] = SD_MODE_GSM;
        ...
        case DSAT_WCDMA_RAT:
        {
          mru_entry.mode = SYS_SYS_MODE_GWL;
          mru_entry.gwl.mode = SYS_SYS_MODE_GWL;
          mru_entry.gwl.acq_mode = SYS_SYS_MODE_WCDMA;
          
          temp = vend_convert_mru_band_input_to_wcdma_band_pref(dsat_mru_input.band);
          mru_entry.gwl.band_pref.bit_15_0= (word) ((word) temp & 0xffff );
          mru_entry.gwl.band_pref.bit_31_16 =(word) ((uint32)temp >> 16 );
          mru_entry.gwl.num_rats = 1; 
          mru_entry.gwl.rat[0] = SD_MODE_WCDMA;
          if(dsat_mru_input.is_plmn_valid )
          {
            mru_entry.gwl.net_select_mode = 4; /* SD_NET_SELECT_MODE_MANUAL_SYS */
           (void) dsatutil_memscpy((void*)&mru_entry.gwl.plmn_id.identity[0], 3,
      ...
      SET_PENDING( DSAT_VENDOR_QCMRUE_IDX, 0, DSAT_PENDING_MRU_UPDATE_RESULT );
...
)
{
  ...
  if(tok_ptr->op == (NA))
  {
    if(0 != lte_rrc_delete_acq_db((lte_rrc_mem_instance_type)dsat_get_current_subs_id(FALSE)))
    {
      result = DSAT_ERROR;
    }
  }
...
)
{
  ...
  if ( CHECK_PENDING( DSAT_VENDOR_QCMRUE_IDX, 0, DSAT_PENDING_MRU_UPDATE_RESULT ) )
  {
    SET_PENDING( DSAT_VENDOR_QCMRUE_IDX, 0, DSAT_PENDING_MRU_RESULT );
  }
  else if ( CHECK_PENDING( DSAT_VENDOR_QCMRUC_IDX, 0, DSAT_PENDING_MRU_UPDATE_RESULT ) )
  {
    SET_PENDING( DSAT_VENDOR_QCMRUC_IDX, 0, DSAT_PENDING_MRU_RESULT );
  }
  else
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Unexpected MRU read result");
...
)
{
  ...
  switch (msg_ptr->msg.ss_event.event_info[DSAT_STACK_MAIN].sys_mode)
  {
    ...
    case SYS_SYS_MODE_HDR:
      ...
      if (ds707_pkt_mgr_get_bearer_tech_info(&bearer_tech) == TRUE)
      {
        switch (bearer_tech.data.cdma_type.rat_mask)
        {
          ...
          default:
           DS_ATCOP_ERROR_LOG_0("Invalid HDR Protocol");

        }
      }
      else
      {
       DS_ATCOP_ERROR_LOG_0("Couldn't get HDR Protocol");
      }
#endif /* FEATURE_DATA_IS707 */
      break;
    ...
    default:
     DS_ATCOP_ERROR_LOG_0("Invalid Sys Mode");
      break;
  }
  ...
  if (DSAT_QCSYSMODE_ID_WCDMA == qcsysmode_id || DSAT_QCSYSMODE_ID_TDS == qcsysmode_id )
  {
    ...
    default:
       DS_ATCOP_ERROR_LOG_0("Invalid HS Indication");
      break;
  }
  }
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR) ) 
  {
   DS_ATCOP_ERROR_LOG_0("Write command is not supported ");
    return DSAT_ERROR;
  } 
  else if(tok_ptr->op == (NA|EQ|QU))
  {
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Cannot allocate serial port for SLIP connection");
    return DSAT_ERROR;
  }
...
RETURN VALUE
  ...
  SD_SS_BAND_PREF_E_TYPE value. 

SIDE EFFECTS
  None

===========================================================================*/
sd_ss_band_pref_e_type vend_process_inband_values
(
  dsat_bandpref_e_type in_band
)
{
  sd_ss_band_pref_e_type out_band;
  switch( in_band )
  {
    case DSAT_BAND_PREF_BC0_A :
      out_band = SD_SS_BAND_PREF_BC0_A;
      break;
    case DSAT_BAND_PREF_BC0_B :
      out_band = SD_SS_BAND_PREF_BC0_B;
      break;
    case DSAT_BAND_PREF_BC0 :
      out_band = SD_SS_BAND_PREF_BC0;
      break;
    case DSAT_BAND_PREF_BC1 :
      out_band = SD_SS_BAND_PREF_BC1;
      break;
    case DSAT_BAND_PREF_BC3 :
      out_band = SD_SS_BAND_PREF_BC3;
      break;
    case DSAT_BAND_PREF_BC4 :
      out_band = SD_SS_BAND_PREF_BC4;
      break;
    case DSAT_BAND_PREF_BC5 :
      out_band = SD_SS_BAND_PREF_BC5;
      break;
    case DSAT_BAND_PREF_GSM_DCS_1800 :
      out_band = SD_SS_BAND_PREF_GSM_DCS_1800;
      break;
    case DSAT_BAND_PREF_GSM_EGSM_900 :
      out_band = SD_SS_BAND_PREF_GSM_EGSM_900;
      break;
    case DSAT_BAND_PREF_GSM_PGSM_900 :
      out_band = SD_SS_BAND_PREF_GSM_PGSM_900;
      break;
    case DSAT_BAND_PREF_BC6 :
      out_band = SD_SS_BAND_PREF_BC6;
      break;
    case DSAT_BAND_PREF_BC7 :
      out_band = SD_SS_BAND_PREF_BC7;
      break;
    case DSAT_BAND_PREF_BC8 :
      out_band =  SD_SS_BAND_PREF_BC8;
      break;
    case DSAT_BAND_PREF_BC9 :
      out_band = SD_SS_BAND_PREF_BC9;
      break;
    case DSAT_BAND_PREF_BC10 :
      out_band = SD_SS_BAND_PREF_BC10;
      break;
    case DSAT_BAND_PREF_BC11 :
      out_band = SD_SS_BAND_PREF_BC11;
      break;
    case DSAT_BAND_PREF_BC12 :
      out_band = SD_SS_BAND_PREF_BC12;
      break;
    case DSAT_BAND_PREF_BC14 :
      out_band = SD_SS_BAND_PREF_BC14;
      break;
    case DSAT_BAND_PREF_BC15 :
      out_band = SD_SS_BAND_PREF_BC15;
      break;
    case DSAT_BAND_PREF_BC16 :
      out_band = SD_SS_BAND_PREF_BC16;
      break;
    case DSAT_BAND_PREF_GSM_450 :
      out_band = SD_SS_BAND_PREF_GSM_450;
      break;
    case DSAT_BAND_PREF_GSM_480 :
      out_band = SD_SS_BAND_PREF_GSM_480;
      break;
    case DSAT_BAND_PREF_GSM_750 :
      out_band = SD_SS_BAND_PREF_GSM_750;
      break;
    case DSAT_BAND_PREF_GSM_850 :
      out_band = SD_SS_BAND_PREF_GSM_850;
      break;
    case DSAT_BAND_PREF_GSM_RGSM_900 :
      out_band = SD_SS_BAND_PREF_GSM_RGSM_900;
      break;
    case DSAT_BAND_PREF_GSM_PCS_1900 :
      out_band = SD_SS_BAND_PREF_GSM_PCS_1900;
      break;
    case DSAT_BAND_PREF_WCDMA_I_IMT_2000 :
      out_band = SD_SS_BAND_PREF_WCDMA_I_IMT_2000;
      break;
    case DSAT_BAND_PREF_WCDMA_II_PCS_1900 :
      out_band = SD_SS_BAND_PREF_WCDMA_II_PCS_1900;
      break;
    case DSAT_BAND_PREF_WCDMA_III_1700 :
      out_band = SD_SS_BAND_PREF_WCDMA_III_1700;
      break;
    case DSAT_BAND_PREF_WCDMA_IV_1700 :
      out_band = SD_SS_BAND_PREF_WCDMA_IV_1700;
      break;
    case DSAT_BAND_PREF_WCDMA_V_850 :
      out_band = SD_SS_BAND_PREF_WCDMA_V_850;
      break;
    case DSAT_BAND_PREF_WCDMA_VI_800 :
      out_band = SD_SS_BAND_PREF_WCDMA_VI_800;
      break;
    case DSAT_BAND_PREF_WCDMA_VII_2600 :
      out_band = SD_SS_BAND_PREF_WCDMA_VII_2600;
      break;
    case DSAT_BAND_PREF_WCDMA_VIII_900 :
      out_band = SD_SS_BAND_PREF_WCDMA_VIII_900;
      break;
    case DSAT_BAND_PREF_WCDMA_IX_1700 :
      out_band = SD_SS_BAND_PREF_WCDMA_IX_1700;
      break;
    case DSAT_BAND_PREF_ANY :
      out_band = SD_SS_BAND_PREF_ANY;
      break;
    default:
      out_band = SD_SS_BAND_PREF_NONE;
  }
...
)
{
  ...
  sd_ss_band_pref_e_type  local_cm_band   = SD_SS_BAND_PREF_NONE;
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Error in stripping quotes");
    return FALSE;
  }
  ...
  while( loop < DSAT_MAX_BANDPREF_STRING_LENGTH && noquotes_band[loop] != '\0')
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1(" invalid band received =d",index_band);
      return FALSE;
    }
    index = 0;
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Not applicable in UMTS mode ");
    return DSAT_ERROR;
  }
  if ( (NA|EQ|AR) == tok_ptr->op )
  {
    ...
    switch( in_pref_val )
    {
        ...
        user_pref.mode_band.mode = MMODE_RAT_MASK_CDMA 
			                | MMODE_RAT_MASK_HDR 
			                | MMODE_RAT_MASK_GSM 
      ...
      case DSAT_PREFMODE_SYSMODE_HDR:
        user_pref.mode_band.mode = MMODE_RAT_MASK_HDR;
        break;
      case DSAT_PREFMODE_SYSMODE_HYBRID_CDMA_HDR:
        user_pref.mode_band.mode = MMODE_RAT_MASK_CDMA | MMODE_RAT_MASK_HDR;
    ...
    if ( FALSE == cm_is_valid_sd_mode_pref(user_pref.mode_band.mode))
    {
     DS_ATCOP_ERROR_LOG_0("Invalid CM MODE PREF setting");
    ...
    {
      SET_PENDING(DSATETSI_EXT_ACT_CVMOD_ETSI_IDX ,0, DSAT_PENDING_PREFMODE_NONE)
       result = DSAT_ERROR;
    }
    else
    {
...
)
{
  ...
  DSATVEND_ASSERT( pref_mode != NULL );
  ...
  switch ( cmd_type )
  {
      ...
      switch ( pref_mode->network_rat_mode_pref )
      {
		...
		*/
        ...
        default:
          ...
          DS_ATCOP_ERROR_LOG_1("Invalid pref mode =d received, seeting it to Automatic",
                                          pref_mode->network_rat_mode_pref);
      }
      res_buff_ptr = dsat_dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL, FALSE);
      res_buff_ptr->used = (word)snprintf((char *)res_buff_ptr->data_ptr,
    ...
    case DSAT_PENDING_PREFMODE_BANDPREF_READ:
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Band preferene query failed");
        result = DSAT_ERROR;
        break;
      }
    ...
    case DSAT_PENDING_PREFMODE_BANDPREF_VERIFY:
      {
        uint32 index = 0;
        sd_ss_band_pref_e_type temp_band_pref = SD_SS_BAND_PREF_NONE;
        sd_ss_band_pref_e_type req_band_pref  = SD_SS_BAND_PREF_NONE;
        cm_pref_term_e_type pref_term = CM_PREF_TERM_PWR_CYCLE;
	mmode_mode_band_s_type mode_band;

        memset(&mode_band, 0, sizeof(mmode_mode_band_s_type));
        ...
        {
          if ( SD_SS_BAND_PREF_NONE == 
                ( temp_band_pref = vend_process_inband_values(
                                               dsat_bandpref_val[index])))
          {
            DS_ATCOP_ERROR_LOG_1("Invalid band preference =d",dsat_bandpref_val[index ]);
        ...
        mode_band.mode = pref_mode->network_rat_mode_pref; 
        mode_band.bands.band = req_band_pref; 
        if ( FALSE == cm_is_valid_pref(&mode_band))
        {
         DS_ATCOP_ERROR_LOG_0("Invalid Mode+Band Preference");
          SET_PENDING(DSAT_VENDOR_QCBANDPREF_IDX ,0, DSAT_PENDING_PREFMODE_NONE)
          (void)dsat_send_cme_error(DSAT_CME_OP_NOT_ALLOWED);
          return DSAT_CMD_ERR_RSP;
        }
...
#ifdef FEATURE_DUAL_SIM
        user_pref.asubs_id =dsat_get_current_subs_id(FALSE);
#endif /* FEATURE_DUAL_SIM */
	user_pref.cmd_cb_func = dsatcmif_ph_cmd_cb_func;
	user_pref.data_block_ptr= NULL;
	user_pref.client_id = dsatcm_client_id;
	...
	user_pref.mode_band.bands.band = req_band_pref;
    ...
    default:
      DS_ATCOP_ERROR_LOG_1("Preference mode mismatch =d",pref_mode->network_rat_mode_pref);
      DSATVEND_ASSERT(0);
  }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("RTRE not configured yet");
    return DSAT_ERROR;
  }
...
| 
      ...
      case MMODE_RAT_MASK_TDS:
        at_pref_ptr->mode_val = DSAT_SYSCONFIG_MODE_TDSCDMA;
        break;
#endif /* FEATURE_TDSCDMA */		
      default:
        DS_ATCOP_ERROR_LOG_1("Unsupported pref mode =d ",cm_pref_ptr->network_rat_mode_pref);
    ...
    switch( cm_pref_ptr->acq_order_pref )
    {
      ...
      case CM_GW_ACQ_ORDER_PREF_NO_CHANGE:
        ...
        break;
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid acq_order =d ",cm_pref_ptr->acq_order_pref);
    ...
    switch( cm_pref_ptr->network_roam_pref )
    {
      ...
      case CM_ROAM_PREF_NO_CHANGE:
        ...
        break;
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid roam mode =d ",cm_pref_ptr->network_roam_pref);
    ...
    switch( cm_pref_ptr->service_domain_pref )
    {
      ...
      case CM_SRV_DOMAIN_PREF_NO_CHANGE:
        ...
        break;
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid roam mode =d ",cm_pref_ptr->service_domain_pref);
  ...
  else
  {
    switch( at_pref_ptr->mode_val )
    {
      case DSAT_SYSCONFIG_MODE_AUTOMATIC:
        cm_pref_ptr->network_rat_mode_pref = MMODE_RAT_MASK_CDMA | 
			                                 MMODE_RAT_MASK_HDR |
      ...
      case DSAT_SYSCONFIG_MODE_GSM:
        cm_pref_ptr->network_rat_mode_pref = MMODE_RAT_MASK_GSM;
        break;
      case DSAT_SYSCONFIG_MODE_WCDMA:
        cm_pref_ptr->network_rat_mode_pref = MMODE_RAT_MASK_WCDMA;
        break;
#ifdef FEATURE_TDSCDMA		
      case DSAT_SYSCONFIG_MODE_TDSCDMA:
        cm_pref_ptr->network_rat_mode_pref = MMODE_RAT_MASK_TDS;
        break;
#endif /* FEATURE_TDSCDMA */
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid pref mode =d ",at_pref_ptr->mode_val);
        return DSAT_ERROR;
    }
    ...
    switch( at_pref_ptr->order_val )
    {
      ...
      case DSAT_SYSCONFIG_ORDER_NO_CHANGE:
        ...
        break;
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid acq_order =d ",at_pref_ptr->order_val);
        return DSAT_ERROR;
    }
    ...
    switch( at_pref_ptr->roam_val )
    {
      ...
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid roam mode =d ",at_pref_ptr->roam_val);
        return DSAT_ERROR;
    }
    ...
    switch( at_pref_ptr->domain_val )
    {
      ...
      case DSAT_SYSCONFIG_DOMAIN_NO_CHANGE:
        ...
        break;
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid roam mode =d ",at_pref_ptr->domain_val);
        return DSAT_ERROR;
    }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Not applicable in CDMA mode ");
    return DSAT_ERROR;
  }
  if ( (NA|EQ|AR) == tok_ptr->op )
  {
    ...
    /* Verify if the pref values are correct */
    if ( FALSE == cm_is_valid_sd_mode_pref(ph_pref_type.network_rat_mode_pref))
    {
     DS_ATCOP_ERROR_LOG_0("Invalid CM MODE PREF setting");
      (void)dsat_send_cme_error(DSAT_CME_OP_NOT_ALLOWED);
      return DSAT_CMD_ERR_RSP;
    ...
    user_pref.data_block_ptr= NULL;
    user_pref.client_id = dsatcm_client_id;
    user_pref.mode_band.mode = ph_pref_type.network_rat_mode_pref;
    user_pref.pref_term = CM_PREF_TERM_PERMANENT;
    user_pref.misc_pref.pref_duration = 0;
    user_pref.misc_pref.acq_order_pref = ph_pref_type.acq_order_pref;
    user_pref.mode_band.bands.band = SD_SS_BAND_PREF_NO_CHANGE;
    user_pref.misc_pref.prl_pref = CM_PRL_PREF_NO_CHANGE;
    user_pref.misc_pref.roam_pref = ph_pref_type.network_roam_pref;
    user_pref.hybr_pref = CM_HYBR_PREF_NO_CHANGE;
    user_pref.srv_domain_pref = ph_pref_type.service_domain_pref;
...
)
{
  ...
  if ( tok_ptr->op == (NA|QU) )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("EONS files not read at Init");
    }
  }
  return result;
}/* dsatvend_exec_pacsp_cmd */
...
)
{
  ...
  if (tok_ptr->op == NA)
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Could not get DAL Device Handle, bailing out");
      return DSAT_ERROR;
    }
...
#define START_ECALL 1
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid Operating Mode");
    return result;
  }
      ...
      (tok_ptr->op == (NA|QU)))
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid params");
      return result;
    }
    result = dsatparm_exec_param_cmd ( mode,
                                        parse_table,
       ...
       (tok_ptr->op == (NA|EQ|AR)))
    {
                   ...
                   DSAT_VENDOR_ECALL_IDX,0,0,NUM_TYPE) == START_ECALL)
      {
        if( FALSE == ecall_session_start((ecall_type_of_call)(dsat_num_item_type)dsatutil_get_val(
                                         ...
                                         dsat_ecall_cb_func))
        {
         DS_ATCOP_ERROR_LOG_0("Unable to start the Ecall");
      ...
      else
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0("Unable to stop the Ecall");
...
{
  dsat_result_enum_type result = DSAT_OK;
  nv_item_type          ds_nv_item_meid;

  memset( (void*)&ds_nv_item_meid, 0, sizeof(ds_nv_item_meid));
  ...
  if (tok_ptr->op == NA)
  {
     ...
     nv_status = dsatutil_get_nv_item_per_subs( NV_MEID_I , &ds_nv_item_meid,
                                               dsat_get_current_1x_subs_id() );

     /* If MEID is not set, then report ESN. */
     if (( nv_status != NV_DONE_S ) || 
         ((0 == qw_hi(ds_nv_item_meid.meid)) && (0 == qw_lo(ds_nv_item_meid.meid)))) 
     {
     ...
     else
     {
            ...
            (!strncmp((const char *)parse_table->name,(const char *)"^GSN",strlen((const char *)"^GSN"))) )
       {
         res_buff_ptr->used = (word)snprintf(
                                              ...
                                              res_buff_ptr->size,
                                              "0x%06X%08X", 
                                              (unsigned int)qw_hi(ds_nv_item_meid.meid),
                                              (unsigned int)qw_lo(ds_nv_item_meid.meid));
       }
       else
...
)
{
  ...
  switch( action )
  {
    ...
    default:
    {
      DS_ATCOP_ERROR_LOG_1("Unregistered event =d recvd, ignoring", action);
      break;
    }
  } /* switch( event ) */
     ...
     case QCRMCALL_ACTION_QUERY: 
       ...
       else
       {
         ...
         for (i = 0; i < result_rmnet->list.num_qcrmcall; i++)
         {
           ...
           {
             DS_ATCOP_ERROR_LOG_1("Invalid Data with IP_Family: =d", ip_type);
           }
         }
         dsatcmdp_handle_async_cmd_rsp(res_buff_ptr, DSAT_COMPLETE_RSP);
       }
...
/*------------------------------------------------------------------------*/
  ...
  DSATVEND_ASSERT(NULL != sni_params );
  ...
  if ( (NA|EQ|AR) == tok_ptr->op )
  {
    ...
    if ( VALID_TOKEN(2))
    {
      if ( dsatutil_atoi( &temp_data, 
                       tok_ptr->arg[2], 10) != ATOI_OK )
      {
       DS_ATCOP_ERROR_LOG_0("Ascii to integer conversion failed");
        return FALSE;
      }
      if ( (QCRMCALL_IP_V4V6 < temp_data) || (temp_data < QCRMCALL_IP_V4) )
      {
        DS_ATCOP_ERROR_LOG_1("Invalid IP Type received [=d]",temp_data);
        return FALSE; 
      }
      switch (temp_data)
      {
    ...
    if ( VALID_TOKEN(3))
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Invalid Tech Pref [=d]", temp_data);
        return FALSE;
      }
    }
    ...
    if ( VALID_TOKEN(4))
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Invalid UMTS Profile [=d]", temp_data);
        return FALSE;
      }
    ...
    if ( VALID_TOKEN(6))
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("APN is too long [=d]",len);
        return FALSE;
      }
...
/* -----------------------------------------------------------------------------*/
  ...
  if ( (NA|EQ|AR) == tok_ptr->op )
  {
    ...
    if ( VALID_TOKEN(1))
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Invalid Params received; cannot proceed further");
        ...
        case QMI_AF_INET_4_6:
          {
            ...
            if(dsat_qcrmcall_info.state == DSAT_PENDING_QCRMCALL_NONE)
            {
             DS_ATCOP_ERROR_LOG_0("QCRMCALL V4/V6 Call failed");
              dsatutil_free_memory(sni_params_ptr);
              return DSAT_ERROR;
            }   
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid ARG 1 received; cannot proceed further");
      return DSAT_ERROR;
    }
  }
  else if ( (NA|EQ|QU) == tok_ptr->op )
...
{ 
  ...
  {		
    if(VALID_TOKEN(0) && (ATOI_OK == dsatutil_atoi( &mode_val, tok_ptr->arg[0], 10 )))
    {
       switch(mode_val)
       {
         case DSAT_QCNSP_RAT_MODE_AUTOMATIC:
           DSATUTIL_SET_VAL(DSAT_VENDOR_QCNSP_IDX,0,0,0,MMODE_RAT_MASK_CDMA | 
		   	                                            MMODE_RAT_MASK_HDR |
		   	                                            MMODE_RAT_MASK_GSM | 
		   	                                            MMODE_RAT_MASK_WCDMA |
		   	                                            MMODE_RAT_MASK_LTE | 
         ...
         case DSAT_QCNSP_RAT_MODE_GSM:
           DSATUTIL_SET_VAL(DSAT_VENDOR_QCNSP_IDX,0,0,0,MMODE_RAT_MASK_GSM,NUM_TYPE)
            break;
         case DSAT_QCNSP_RAT_MODE_WCDMA:
           DSATUTIL_SET_VAL(DSAT_VENDOR_QCNSP_IDX,0,0,0,MMODE_RAT_MASK_WCDMA,NUM_TYPE)
            break;
         case DSAT_QCNSP_RAT_MODE_LTE:
           DSATUTIL_SET_VAL(DSAT_VENDOR_QCNSP_IDX,0,0,0,MMODE_RAT_MASK_LTE,NUM_TYPE)
           break;
         default:
            return DSAT_ERROR;
       }
    ...
    {
    	 DSATUTIL_SET_VAL(DSAT_VENDOR_QCNSP_IDX,0,0,0,0,NUM_TYPE)
    }
...
)
{
  ...
  switch( cmd_type )
  {
   ...
   case DSAT_PENDING_QCNSP_WRITE:
    ...
    break;
   default:
     DS_ATCOP_ERROR_LOG_1("Invalid cmd type =d",cmd_type);
     result = DSAT_ERROR;
  }
  return result;
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Failure response received =d",rsp_info_ptr->rspType);
...
/* Check the number of arguments.*/
    if ( tok_ptr->args_found != parse_table->compound)
    {
      DS_ATCOP_ERROR_LOG_1("$QCVOIPM: Invalid Number of arguements  =d", tok_ptr->args_found);
      result = DSAT_ERROR;
    }
/*Arg 0  Indicates the session_timer_ims parameter ; Unit is Sec*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: SessionExpires is mandatory ");
      result = DSAT_ERROR;
    }
/*Arg 1  Indicates the value of min_se parameter in seconds ;*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: MinSessionExpiry is mandatory  ");
      result = DSAT_ERROR;
    }
/*Arg 2  Indicates the value of  rtp_rtcp_inactivity_timer_ims parameter in seconds;*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: RtpRtcpInactivityTimer is mandatory  ");
      result = DSAT_ERROR;
    }
...
/* IMS expects value in Ms, convert Sec into Ms*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: RingingTimer is mandatory  ");
      result = DSAT_ERROR;
    }
...
/* IMS expect value in Ms, convert Sec into Ms*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: RingbackTimer is mandatory  ");
      result = DSAT_ERROR;
    }
/*Arg 5  Indicates the value of amrwb parameter*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: AmrWbEnable is mandatory  ");
      result = DSAT_ERROR;
    }
/*Arg 6  Indicates the value of scr_amr parameter*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: ScrAmrEnable is mandatory  ");
      result = DSAT_ERROR;
    }
/*Arg 7 Indicates the value of scr_amrwb parameter*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: ScrAmrWbEnable  is mandatory  ");
      result = DSAT_ERROR;
    }
/*Arg 8 Indicates the value of amrmodeset parameter*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: AmrModeStr is mandatory  ");
      result = DSAT_ERROR;
    }
/*Arg 9 Indicates the value of amrwbmodset parameter*/
    ...
    {
     DS_ATCOP_ERROR_LOG_0("$QCVOIPM: Amr WbModeStr is mandatory  ");
      result = DSAT_ERROR;
    }
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    if ( dsatparm_exec_param_cmd(mode, parse_table, tok_ptr, res_buff_ptr) != DSAT_ERROR )
    {
      ...
      if ( cm_ph_cmd_set_drx_req_per_subs ( dsatcmif_ph_cmd_cb_func,
                                            ...
                                            &set_drx_info ) == FALSE )
      {
       DS_ATCOP_ERROR_LOG_0("No buffer to send request");
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("cmapi for meas info failed in CMAPI_SYS_MODE_LTE ");
  }
  ...
  {
   DS_ATCOP_ERROR_LOG_0("cmapi for meas info failed in CMAPI_SYS_MODE_WCDMA ");
  }
  if( rsr_count )
  {
    res_buff_ptr->used--;
  ...
  if ((tok_ptr->op == (NA|QU)) && (TRUE == dsat_subs_info[subs_id].is_sglte_sub))
  {
    ...
    else
    {
      ...
      else
      {
       ...
       DS_ATCOP_ERROR_LOG_0("mmgsdi_session_get_operator_name API call failed");
...
/*Valid terminator found ':' ; Terminate Param type*/
  ...
  {
    DS_ATCOP_ERROR_LOG_1("$QCPRFCRT: Invalid Tokens Name,Remain Size  =d", type_size);
    return DSAT_FAILURE;
  }
...
/* Valid  terminator found'\0' ; Terminate Param value*/
  ...
  {
    DS_ATCOP_ERROR_LOG_1("$QCPRFCRT: Invalid Tokens value,Remain Size  =d", value_size);
    return DSAT_FAILURE;
  }
...
)
{
  ...
  for(count = 0 ; count< tok_ptr->args_found; count++ )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Parsing failed for Argument =d",count);
      return DSAT_FAILURE;
    }
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Invalid param_type for arg num =d",count);
      return DSAT_FAILURE;
    }
        ...
        DSAT_3GPP2_PARAM_PDP_TYPE != dsat_3gpp2_profile_param_abbrv_type[index].param_type)
    {
      ...
      {
        DS_ATCOP_ERROR_LOG_1("ARG convertion failed =d",count);
        return DSAT_FAILURE;
      }
      switch( param_identifier)
      {
        case DSAT_3GPP2_PARAM_OPRT_TYPE:
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1("ARG out of range =d",value);
            return DSAT_FAILURE;
          }
          break;
        }
      ...
      else
      {
        switch( param_identifier )
        {
         case DSAT_3GPP2_PARAM_APN_NAME:
         {
           ...
           {
             DS_ATCOP_ERROR_LOG_1("ARG out of range =d",strlen(param_value));
             return DSAT_FAILURE;
           }
           break;
         }
 ...
 */
    ...
    switch( param_list[idx].type )
    {
      case STR_TYPE:
      {
            ...
            param_id == DS_PROFILE_3GPP_PROFILE_PARAM_OVERRIDE_HOME_PDP_TYPE)
        {
          ...
          {
            DS_ATCOP_ERROR_LOG_1("Invalid PDP TYPE =d",param_id );
            return DSAT_FAILURE;
          }
        }
      ...
      case NUM_TYPE:
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("ATOI failed for param_id =d",param_id );
          return DSAT_FAILURE;
        }
        ...
        if( param_id == DS_PROFILE_3GPP_PROFILE_PARAM_FAILURE_TIMERS)
        {
           ...
           if(profile_status == DS_PROFILE_REG_RESULT_SUCCESS)
           {
              ...
              {
                 DS_ATCOP_ERROR_LOG_1("ERROR: failure_timer invalid param index: =d" , i);
              }
           }
           break;
        }
  ...
  -----------------------------------------------------------------------*/
        ...
        if( FALSE == dsatutil_strip_quotes_out( (byte*) param_value,
                                                ...
                                                (word)  param_list[idx].range[0].lower ))
        {
          DS_ATCOP_ERROR_LOG_1("Strip quote failure for param_id =d",param_id);
          return DSAT_FAILURE;
        }
        ...
        if(DSAT_OK != dsatutil_convert_tuple( STRING_TO_INT,
                                              ...
                                              DSAT_CONVERT_IP) )
        {
          DS_ATCOP_ERROR_LOG_1("V4 addr conversion failure for param_id =d",param_id);
          return DSAT_FAILURE;
        }
      ...
      case ADDR_P2_V6_TYPE:
      {
        ...
        if(DSAT_OK != dsatutil_convert_tuple( STRING_TO_INT,
                                              ...
                                              DSAT_CONVERT_IP) )
        {
          DS_ATCOP_ERROR_LOG_1("V6 addr conversion failure for param_id =d",param_id);
          return DSAT_FAILURE;
        }
      ...
      case LNGR_TMR_TYPE:
      {
        if( ATOI_OK != dsatutil_atoi( &value, (byte*) param_value, 10 ))
        {
          DS_ATCOP_ERROR_LOG_1("ATOI failed for param_id =d",param_list[idx].identifier);
          return DSAT_FAILURE;
        }      
        /* Range Validation */
        for( i = 0; !(param_list[idx].range[i].lower == RANGE_END &&
 ...
 DS_ATCOP_ERROR_LOG_0("ARG not found!");
  return DSAT_FAILURE;
...
)
{
  ...
  while (index < param_list_size)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Invalid tech_type =d", tech_type);
      return DSAT_ERROR;
    }
       ...
       case STR_TYPE:
       {
        ...
        else
        {
         ...
         if(profile_status == DS_PROFILE_REG_RESULT_SUCCESS)
         {
           ...
           else if( param_list[current_index].identifier == DS_PROFILE_3GPP_PROFILE_PARAM_LTE_ROAMING_PDP_TYPE ||
                    param_list[current_index].identifier == DS_PROFILE_3GPP_PROFILE_PARAM_UMTS_ROAMING_PDP_TYPE)
           {
             /* Checking if Roaming pdp type is configured. If not, then don't display any PDP type. */
             if (DS_PROFILE_3GPP_ROAMING_PDP_MAX == num_val)
             {
               num_val = 3;
             }
             res_buff_ptr->used += (word) snprintf( (char*)&res_buff_ptr->data_ptr[res_buff_ptr->used],
      ...
      case ADDR_V4_TYPE:
      {
        ...
        if(profile_status == DS_PROFILE_REG_RESULT_SUCCESS)
        {
          if( DSAT_OK != dsatutil_convert_tuple( INT_TO_STRING,
                                                 ...
                                                 DSAT_CONVERT_IP))
          {
            DS_ATCOP_ERROR_LOG_1("IPV4 address conversion failed for param_id =d",param_list[current_index].identifier);
        ...
        if(profile_status == DS_PROFILE_REG_RESULT_SUCCESS)
        {
          ...
          if( DSAT_OK != dsatutil_convert_tuple( INT_TO_STRING,
                                                 ...
                                                 (byte*)value ,
                                                  ...
                                                  DSAT_CONVERT_IP))
          {
            DS_ATCOP_ERROR_LOG_1("IPV6 address conversion failed for param_id =d",param_list[current_index].identifier);
            DSAT_END_PROFILE_TRANSACTION(profile_hndl);
            return DSAT_ERROR;
          }
...
)
{
  ...
  if( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Invalid Number of arguements  =d", tok_ptr->args_found);
      return(DSAT_ERROR);
    }
    /*---------------------------------------------------------------
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Failed to get Profile ID");
      return DSAT_ERROR;
    }
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid first argument. Should be valid profile ID");
      return(DSAT_ERROR);
    }
     ...
     /*---------------------------------------------------------------
      ...
      if( !VALID_TOKEN(index) || DSAT_FAILURE == dsatvend_get_param_type_value( (char*) tok_ptr->arg[index],
                                                        (char*)param_type,
                                                         ...
                                                         DSAT_MAX_PROFILE_PARAM_VAL_LEN ))
      {
        DS_ATCOP_ERROR_LOG_1("Failed to tokenize argument no. =d", index);
        DSAT_END_PROFILE_TRANSACTION(profile_hndl);
        return DSAT_ERROR;
      } 
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Failed to set value for param for agr no. =d", index);
        DSAT_END_PROFILE_TRANSACTION(profile_hndl);
        return DSAT_ERROR;
      }
  ...
  else if( tok_ptr->op == (NA|QU) )
  {
    ...
    for(index = 0; tech_array[index] != DS_PROFILE_TECH_MAX; index++)
    {
      ...
      if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
      {
        DS_ATCOP_ERROR_LOG_1("$QCPRFMOD: Getting profile list iterator failed. Error =d ",
                           profile_status);
        continue;    
      }
      ...
      -----------------------------------------------------------------------*/
        ...
        if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
        {
          DS_ATCOP_ERROR_LOG_1 ("$QCPRFMOD: Getting profile info by iterator failed. Error =d",
                             profile_status);
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Getting Next iterator failed. Error =d",
                           profile_status);
          DSAT_PROFILE_DESTROY_ITERATOR(itr);
          return DSAT_ERROR;
        }
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("$QCPRFCRT: Invalid Number of arguements  =d", tok_ptr->args_found);
      return(DSAT_ERROR);
    }
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid parameter type or value.");
      return DSAT_ERROR;
    }
    /* Operation type is a mandatory parameter */
    if(0 == (param.param_present & DSAT_PROFILE_CREATE_MASK_OPRT) )
    {
     DS_ATCOP_ERROR_LOG_0("OPRT type is not present.");
      return DSAT_ERROR;
    }
    ...
    switch(param.oprt_val)
    {
      case DSAT_3GPP2_PROFILE_DELETE:
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1(" Profile Deletion Failed. num_of_Arg =d ",tok_ptr->args_found);
          return DSAT_ERROR;
        }
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Invalid tech_type for profile id =d ",param.profile_id_val);
          return DSAT_ERROR;
        }
      ...
      case DSAT_3GPP2_PROFILE_CREATE:
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Mandatory fields are missing profile id =d", profile_id);
          return DSAT_ERROR;
        }
        ...
        /*-------------------------------------------------------------------------
          ...
          {
            DS_ATCOP_ERROR_LOG_1("Profile ID =d already exists", profile_id);
             return DSAT_ERROR;
          }
          else
          {
...
#define BUFFER_LIMIT (DS_PROFILE_3GPP_MAX_APN_STRING_LEN + 156)
    ...
    for(index = 0; tech_array[index] != DS_PROFILE_TECH_MAX; index++)
    {
      ...
      if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
      {
        DS_ATCOP_ERROR_LOG_1("$QCPRFCRT: Getting profile list iterator failed. Error =d ",
                           profile_status);
        continue;    
      }
      ...
      -----------------------------------------------------------------------*/
        ...
        if(profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
        {
          DS_ATCOP_ERROR_LOG_1 ("$QCPRFCRT: Getting profile info by iterator failed. Error =d",
                             profile_status);
          (void) ds_profile_itr_destroy(itr);
          return DSAT_ERROR;
        }
        ...
        {
          DS_ATCOP_ERROR_LOG_1 ("Invalid PDP_TYPE: =d", num_val);
          DSAT_PROFILE_DESTROY_ITERATOR(itr);
          return DSAT_ERROR;
        }
...
#ifdef FEATURE_WCDMA
/*===========================================================================
FUNCTION DSATVEND_EXEC_QCRSCP_CMD

DESCRIPTION
  This function executes $QCRSCP and $QCECNO commands and provides response 
  Calls CM API to receive neighbour cell info like Primary Scrambling Code,
  UARFCN, RSCP, ECNO.
...
===========================================================================*/
dsat_result_enum_type dsatvend_exec_qcrscp_cmd
(
    dsat_mode_enum_type mode,             /*  AT command mode:            */
    const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
    const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
)
{
  #define CMD_RSCP 0
  #define CMD_ECNO 1
  #define MAX_CELLS_REPORTED CMAPI_WCDMA_MAX_CELLS_REPORTED
  ...
  res_buff_ptr->data_ptr[0] = '\0';
  if (0 == strncmp((const char *)parse_table->name,(const char *)"$QCRSCP",strlen((const char *)"$QCRSCP")))
  {
    res_buff_ptr->used = (word)snprintf((char*)res_buff_ptr->data_ptr,
                                                res_buff_ptr->size,
                                                "$QCRSCP: ");
  ...
  else if(0 == strncmp((const char *)parse_table->name,(const char *)"$QCECNO",strlen((const char *)"$QCECNO")))
  {
    res_buff_ptr->used = (word)snprintf((char*)res_buff_ptr->data_ptr,
                                                res_buff_ptr->size,
                                                "$QCECNO: ");
  ...
  if( err_wcdma_mode == CMAPI_SUCCESS )
  {
         ...
         meas_info.meas_info_u.wcdma_nbr_info.is_data_valid)
    {
     ...
     /*wcdma intra start: in intra cell nbr info each cell different rscp and ecno values get the info for all cells */
      DS_AT_MSG1_HIGH("WCDMA: num intra num cells: %d", nbr_info->cmapi_wcdma_intra_cell_info.num_nbr_cells);
      for( i = 0; i<nbr_info->cmapi_wcdma_intra_cell_info.num_nbr_cells && i < CMAPI_WCDMA_MAX_CELLS_REPORTED; i++ )
      {
        if(CMD_RSCP == cmd)
        {
        res_buff_ptr->used += (word)snprintf(
                                   ...
                                   res_buff_ptr->size - res_buff_ptr->used,
                                     "%3.3d,%d,\"%3.3d.%d0\",",
                                   nbr_info->cmapi_wcdma_intra_cell_info.nbr_cell[i].psc,
                                   nbr_info->cmapi_wcdma_intra_cell_info.freq,
                                     (nbr_info->cmapi_wcdma_intra_cell_info.nbr_cell[i].rscp)/10,
                                     DSAT_ABSOLUTE_VAL(nbr_info->cmapi_wcdma_intra_cell_info.nbr_cell[i].rscp)%10);
        }
        else
        {
          res_buff_ptr->used += (word)snprintf(
                                      ...
                                      nbr_info->cmapi_wcdma_intra_cell_info.nbr_cell[i].psc,
                                      nbr_info->cmapi_wcdma_intra_cell_info.freq,
                                      (nbr_info->cmapi_wcdma_intra_cell_info.nbr_cell[i].ecio)/10,
                                      DSAT_ABSOLUTE_VAL(nbr_info->cmapi_wcdma_intra_cell_info.nbr_cell[i].ecio)%10);
        }
        cell_count++;
      }
      /* wcdma intra end */

      /*wcdma inter start :in the inter  cell nbr info multiple ferequencies are available hence collect all cells info for each frequency*/
      DS_AT_MSG2_HIGH("WCDMA: num inter num freqs: %d current cell_count = %d",
                       nbr_info->cmapi_wcdma_inter_cell_info.num_freq, cell_count);
      for( i=0; ( i<nbr_info->cmapi_wcdma_inter_cell_info.num_freq && i < CMAPI_WCDMA_SRCH_INTER_FREQ_MAX) &&
                ( cell_count < MAX_CELLS_REPORTED); i++ )
      {
        DS_AT_MSG1_HIGH("WCDMA: num inter num cells: %d", nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].num_cells);
        
        for( j=0; ( j<nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].num_cells && j < CMAPI_WCDMA_MAX_CELLS_REPORTED) && 
                  ( cell_count < MAX_CELLS_REPORTED); j++ )
        {
          if(CMD_RSCP == cmd)
          {
          res_buff_ptr->used += (word)snprintf(
                                   ...
                                   res_buff_ptr->size - res_buff_ptr->used,
                                       "%3.3d,%d,\"%3.3d.%d0\",",
                                   nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].cell[j].psc,
                                   nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].freq,
                                       (nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].cell[j].rscp)/10,
                                       DSAT_ABSOLUTE_VAL(nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].cell[j].rscp)%10);
          }
          else
          {
            res_buff_ptr->used += (word)snprintf(
                                       ...
                                       nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].cell[j].psc,
                                       nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].freq,
                                       (nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].cell[j].ecio)/10,
                                       DSAT_ABSOLUTE_VAL(nbr_info->cmapi_wcdma_inter_cell_info.wcdma_inter_freq[i].cell[j].ecio)%10);
          }
          cell_count++;
        }
      }
  ...
  /*Get LTE cell Info: here we are collecting WCDMA nbr cells of current cell(LTE) 
     .their might be multiple uarfcn nbr info hence collect cell info for each uarfcn*/
  err_lte_mode = cmapi_get_rat_meas_info( CMAPI_SYS_MODE_LTE, &meas_info);
  DS_AT_MSG1_HIGH("err_lte_mode %d", err_lte_mode);
  if( (err_lte_mode == CMAPI_SUCCESS) && ( cell_count < MAX_CELLS_REPORTED))
  {
    DS_AT_MSG3_HIGH("is_service_available = %d sys_mode %d is_data_valid %d",
		             meas_info.is_service_available,
		             meas_info.sys_mode,
      ...
      meas_info.meas_info_u.lte_nbr_info.valid)
    {
     ...
     DS_AT_MSG1_HIGH("num_wcdma_freqs %d", nbr_info->wcdma.num_wcdma_freqs);
	 
     for( i=0; (i< nbr_info->wcdma.num_wcdma_freqs && i < CMAPI_LTE_NGBR_IND_NUM_FREQS) &&
                ( cell_count < MAX_CELLS_REPORTED) ; i++ )
     {
       DS_AT_MSG1_HIGH("LTE info : num cells = %d",nbr_info->wcdma.freq[i].num_wcdma_cells);
       for( j=0; ( j< nbr_info->wcdma.freq[i].num_wcdma_cells && j < CMAPI_LTE_NGBR_IND_NUM_CELLS) &&
                  ( cell_count < MAX_CELLS_REPORTED); j++ )
       {
         if(CMD_RSCP == cmd)
         {
         res_buff_ptr->used += (word)snprintf(
                                   ...
                                   res_buff_ptr->size - res_buff_ptr->used,
                                      "%3.3d,%u,\"%3.3d.%d0\",",
                                   nbr_info->wcdma.freq[i].cell[j].psc,
                                   nbr_info->wcdma.freq[i].uarfcn,
                                      (nbr_info->wcdma.freq[i].cell[j].cpich_rscp)/10,
                                      DSAT_ABSOLUTE_VAL(nbr_info->wcdma.freq[i].cell[j].cpich_rscp)%10);
         }
         else
         { 
           res_buff_ptr->used += (word)snprintf(
                                      ...
                                      nbr_info->wcdma.freq[i].cell[j].psc,
                                      nbr_info->wcdma.freq[i].uarfcn,
                                      (nbr_info->wcdma.freq[i].cell[j].cpich_ecno)/10,
                                      DSAT_ABSOLUTE_VAL(nbr_info->wcdma.freq[i].cell[j].cpich_ecno)%10);
          }
          cell_count++;
        }
      }
  ...
  if( err_wcdma_mode != CMAPI_SUCCESS && err_lte_mode != CMAPI_SUCCESS)
  {
    res_buff_ptr->used = 0;
    res_buff_ptr->data_ptr[0] = '\0';
    DS_AT_MSG1_HIGH("current mode %d", dsatcmdp_get_current_mode());
    if(dsatcmdp_get_current_mode() == DSAT_MODE_WCDMA)
    {
      err_cmapi = err_wcdma_mode;
    }
    ...
    DS_AT_MSG1_HIGH("err_cmapi %d", err_cmapi);
    switch( err_cmapi )
    {
      case CMAPI_NO_SERVICE:
        result = dsat_send_cme_error(DSAT_CME_NO_NETWORK_SERVICE);
  ...
  DS_AT_MSG1_HIGH("result %d", result);
  return result;
}
#endif /* FEATURE_WCDMA */
...
FUNCTION DSATVEND_EXEC_QCPCOLIST_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. This command is used to get APN PCO list info
...
===========================================================================*/
dsat_result_enum_type dsatvend_exec_qcpcolist_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
)
{
  ...
  ps_sys_pco_info_per_apn_type          apn_pco_list;
  int                                   cnt_pcos = 0;
  boolean                               valid_status = FALSE;
  byte                                  apn_buff[PS_SYS_MAX_APN_LEN+1];
  dsat_error_info_s_type                err_info;
  err_info.errval = DSAT_ERR_NO_ERROR;
  err_info.arg_num = 0;
	...
	else if(tok_ptr->op == (NA|EQ|AR))
	{
	  if(tok_ptr->args_found == 1)
	  {
    	...
    	if (valid_status)
    	{
    	  (void)strlcpy((char *)apn_pco_list.apn_name,(const char *)apn_buff,PS_SYS_MAX_APN_LEN + 1);
    	}
    	memset(&apn_pco_list.apn_pco_info, 0, sizeof(ps_sys_op_reserved_pco_list_type));
    	  
        /*-------------------------------------------------------------------------
           Retrieve pco information by calling into PS Sys Layer
        -------------------------------------------------------------------------*/                                  
        return_val = ps_sys_ioctl_ex(PS_SYS_TECH_ALL,
                                     PS_SYS_IOCTL_GET_APN_PCO_INFO,
        ...
        {
          DS_AT_MSG2_ERROR("$qcpcolist ps_sys_ioctl_ex ret [%d] errno [%d]", return_val, ps_errno);
          return DSAT_ERROR;
        }
    
        res_buff_ptr->used = (word) snprintf((char*)&res_buff_ptr->data_ptr[0],
                                                 res_buff_ptr->size,
                                                 "$QCPCOLIST: %s\n",(char *)apn_pco_list.apn_name);
    	apn_pco_list.apn_pco_info.pco_info[cnt_pcos].app_specific_info[0] = 1;
    	apn_pco_list.apn_pco_info.pco_info[cnt_pcos].app_specific_info[1] = 1;
    	apn_pco_list.apn_pco_info.num_pcos = 10;
    	for (cnt_pcos = 0; cnt_pcos < apn_pco_list.apn_pco_info.num_pcos; cnt_pcos++)
        {
          res_buff_ptr->used += (word) snprintf((char *)&res_buff_ptr->data_ptr[res_buff_ptr->used],
                                  res_buff_ptr->size - res_buff_ptr->used,
                                  "pco_id: %d,mcc: %d,mnc: %d,mnc_inclide_pcs_digit: %d,"
                                  "apninfo: %s, container_id: %d\n",
                                  apn_pco_list.apn_pco_info.pco_info[cnt_pcos].pco_id, 
                                  apn_pco_list.apn_pco_info.pco_info[cnt_pcos].mcc,
                                  apn_pco_list.apn_pco_info.pco_info[cnt_pcos].mnc ,
                                  apn_pco_list.apn_pco_info.pco_info[cnt_pcos].mnc_includes_pcs_digit,
                                  (char *)(apn_pco_list.apn_pco_info.pco_info[cnt_pcos].app_specific_info),
                                  apn_pco_list.apn_pco_info.pco_info[cnt_pcos].container_id); 
    	}
	  }
	  else
	  {
...
)
{
  ...
  int16                                 return_val = DSS_SUCCESS;
  uint32                                subs_id = dsat_get_current_ds_profile_subs_id();
  ps_sys_msisdn_info_per_apn_type       apn_msisdn_info;
  boolean                               valid_status = FALSE;
  byte                                  apn_buff[PS_SYS_MAX_APN_LEN+1];
  dsat_error_info_s_type                err_info;
  err_info.errval = DSAT_ERR_NO_ERROR;
  err_info.arg_num = 0;
	...
	else if(tok_ptr->op == (NA|EQ|AR))
	{
	  if(tok_ptr->args_found == 1)
	  {
	    ...
	    if (valid_status)
	    {
		  (void)strlcpy((char *)apn_msisdn_info.apn_name,(const char *)apn_buff,PS_SYS_MAX_APN_LEN + 1);
	    }
	    memset(&apn_msisdn_info.msisdn_info, 0, sizeof(ps_sys_msisdn_info_type));
	  
	    /*-------------------------------------------------------------------------
        Retrieve pco information by calling into PS Sys Layer
        -------------------------------------------------------------------------*/                                  
        return_val = ps_sys_ioctl_ex(PS_SYS_TECH_ALL,
                                     PS_SYS_IOCTL_GET_APN_MSISDN_INFO,
                                     (ps_sys_subscription_enum_type) subs_id,
                                     (void *)&apn_msisdn_info,
                                     &ps_errno);   

	    if (return_val != DSS_SUCCESS)
        {
          DS_AT_MSG2_ERROR("$qcmsisdn ps_sys_ioctl_ex ret [%d] errno [%d]", return_val, ps_errno);
          return DSAT_ERROR;
        }
      
	    res_buff_ptr->used = (word) snprintf((char*)&res_buff_ptr->data_ptr[0],
                                                    res_buff_ptr->size,
                                                    "$QCMSISDN: %s",(char *)(apn_msisdn_info.msisdn_info.msisdn));
	  }
	  else
	  {
	    result = DSAT_ERROR;
...
FUNCTION DSATVEND_EXEC_QCPCOMSISDN_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. It executes at$qcpcomsisdn command.
  at$qcpcomsisdn is used to edit MSISDN,PCO values. 
...
===========================================================================*/
/*ARGSUSED*/
dsat_result_enum_type dsatvend_exec_qcpcomsisdn_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
{
  #define PCO_LIST_MAX 10
  dsat_num_item_type                        profile_id        = 0;
  dsat_num_item_type                        profile_params    = 0;
  dsat_num_item_type                        profile_pco_params[PCO_LIST_MAX]    = {0};
  dsat_error_info_s_type                    err_info;
  dsat_qcprofile_param_details_s_type      *param_list = NULL;
  ...
  char                profile_param_data[MAX_PARAM_BUF];
  void                *value = NULL;
  uint16              value_pco[PCO_LIST_MAX] = {0};
  uint16              profile_pco_params1[PCO_LIST_MAX] = {0};
  boolean             valid_status = FALSE;

  err_info.errval   = DSAT_ERR_NO_ERROR;
  err_info.arg_num  = 0;
  param_list        = &profile_qcpcomsisdn_list[0];
...
/* Profile validation :- Arg 0 should present and have valid value*/
    ...
    for (i=0;i<PCO_LIST_MAX+2;i++)
    {
      ...
      if(i==0)
      {
        ...
        if(dsatetsipkt_read_write_profile_param((uint16)profile_id,
                                                WRITE_COMMAND,
                                                param_list[0].identifier, 
                                                value,
                                                param_list[0].len) != DSAT_OK)
        {
          err_info.errval  = DSAT_ERR_INVALID_ARGUMENT;
          err_info.arg_num = token_idx;
          goto send_error;
      ...
      else if( i < PCO_LIST_MAX+1)
      {
        VALID_NUM_TOKEN(token_idx, min_val, param_list[1].max_value,&profile_pco_params[i-1])
        if (valid_status)
        {
          value_pco[i-1] = (uint16)(profile_pco_params[i-1] & 0xFFFF);
          if(MIN_OPRT_RESV_PCO > profile_pco_params[i-1] || MAX_OPRT_RESV_PCO < profile_pco_params[i-1])
          {
            err_info.errval  = DSAT_ERR_INVALID_ARGUMENT;
            err_info.arg_num = token_idx;
            goto send_error;
      ...
      else
      {
        if(dsatetsipkt_read_write_profile_param((uint16)profile_id,
                                                  WRITE_COMMAND,
                                                  param_list[1].identifier, 
                                                  &value_pco[0],
                                                  param_list[1].len) != DSAT_OK)
        {
            err_info.errval  = DSAT_ERR_INVALID_ARGUMENT;
            err_info.arg_num = token_idx;
            goto send_error;
  ...
  else if(tok_ptr->op == (NA|QU))
  {
	...
	memset(&profile_pco_params1[0],0x0,PCO_LIST_MAX);
        if(DSAT_OK == dsatetsipkt_read_write_profile_param((uint16)profile_list[index],
                                                           READ_COMMAND,
                                                           param_list[0].identifier,
                                                           &profile_params,
	...
	if(DSAT_OK == dsatetsipkt_read_write_profile_param((uint16)profile_list[index],
                                                           READ_COMMAND,
                                                           param_list[1].identifier,
                                                           &profile_pco_params1[0],
                                                           param_list[1].len))
        {
          
	  for(i=0;i<PCO_LIST_MAX;i++)
          {
            buf_curr_len += snprintf(&profile_param_data[buf_curr_len],MAX_PARAM_BUF - buf_curr_len,"%d",profile_pco_params1[i]);
	    buf_curr_len += snprintf(&profile_param_data[buf_curr_len],MAX_PARAM_BUF - buf_curr_len,",");
          }
        }
...
}/*dsatvend_exec_qcpcomsisdn_cmd*/

/*===========================================================================

FUNCTION DSATVEND_EXEC_QCCALLATTR_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. This command is used to set call attributes
  like call type and video transmission attributes.
...
dsat_result_enum_type dsatvend_exec_qccallattr_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
)
{
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    /* If only one argument provided, second parameter call attribute will be default to bidirection */
    if(tok_ptr->args_found == 1)
    {
      DSATUTIL_SET_VAL(DSAT_VENDOR_QCCALLATTR_IDX,0,1,0,DSAT_QCCALLATTR_CALL_ATTR_TXRX,NUM_TYPE)
    }

    /* Only bi-directional call attribute is allowed for Voice call */
    if(DSAT_QCCALLATTR_CALL_TYPE_VOICE ==
       (dsat_qccallattr_call_type_e_type)dsatutil_get_val(DSAT_VENDOR_QCCALLATTR_IDX,0,0,NUM_TYPE) &&
       DSAT_QCCALLATTR_CALL_ATTR_TXRX != 
       (dsat_qccallattr_call_type_e_type)dsatutil_get_val(DSAT_VENDOR_QCCALLATTR_IDX,0,1,NUM_TYPE))
    {
      DSATUTIL_SET_VAL(DSAT_VENDOR_QCCALLATTR_IDX,0,1,0,DSAT_QCCALLATTR_CALL_ATTR_TXRX,NUM_TYPE)
      result = DSAT_ERROR;
    }
  ...
  return result;
}/* dsatvend_exec_qccallattr_cmd */

/*===========================================================================

FUNCTION DSATVEND_EXEC_QCIMSSRV_CMD

DESCRIPTION
  This function takes the result from the command line parser
  and executes it. This command is used to request enable or disable IMS service.
...
RETURN VALUE
  ...
  possible values:
    DSAT_ERROR : if there was any problem in executing the command
    DSAT_OK : if it is a success.
    DSAT_ASYNC_CMD: if command is given to read IMS Service status
...
dsat_result_enum_type dsatvend_exec_qcimssrv_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
{
  dsat_num_item_type ims_srv = 0;
  sys_modem_as_id_e_type subs_id = dsat_get_current_subs_id(FALSE);
  dsatcmif_servs_state_ms_info  *ph_ss_ms_val = 
              dsat_get_base_addr_per_subs(DSAT_SS_PH_MS_VALS, subs_id, FALSE);
  dsat_qcimssrv_ims_srv_status_s_type ims_data;
  
  if(NULL == ph_ss_ms_val || qcimssrv_ims.qcimssrv_set_get == NULL)
  {
    return DSAT_ERROR;
  }
  
  memset(&ims_data, 0, sizeof(dsat_qcimssrv_ims_srv_status_s_type));
  
  if(tok_ptr->op == (NA|EQ|AR))
  {
    if(tok_ptr->args_found != 1)
    {
      /* Command only accept only one argument */
      return DSAT_ERROR;
    }
    if(!VALID_TOKEN(0) ||
       (ATOI_OK != dsatutil_atoi(&ims_srv, tok_ptr->arg[0], 10)))
    {
      /* Invalid first argument */
      return DSAT_ERROR;
    }

    if(ims_srv == ph_ss_ms_val->ims_service_status || (ims_srv > 1))
    {
      /* Invalid input for IMS Service status */
      return DSAT_ERROR;
    }	

	
    ims_data.status = (boolean)ims_srv;
    ims_data.subs_id = subs_id;
	
    if(!qcimssrv_ims.qcimssrv_set_get(DSAT_QCIMSSRV_ACTION_WRITE,&ims_data,qcimssrv_ims.ims_data_ptr))
    {
        return DSAT_ERROR;
    }

    SET_PENDING(DSAT_VENDOR_QCIMSSRV_IDX, 0, DSAT_PENDING_QCIMSSRV_WRITE)
    return DSAT_ASYNC_CMD;
  }
  else if(tok_ptr->op == (NA|EQ|QU))
  {
  ...
  {
    ims_data.subs_id = subs_id;
    if(!qcimssrv_ims.qcimssrv_set_get(DSAT_QCIMSSRV_ACTION_READ,&ims_data,qcimssrv_ims.ims_data_ptr))
    {
        return DSAT_ERROR;
    }

    SET_PENDING(DSAT_VENDOR_QCIMSSRV_IDX, 0, DSAT_PENDING_QCIMSSRV_READ)
    return  DSAT_ASYNC_CMD;
  }
  /* No further support */
  return  DSAT_ERROR;
}/* dsatvend_exec_qcimssrv_cmd */

/*===========================================================================
FUNCTION DSATVEND_IMS_SRV_STATUS_HANDLER

DESCRIPTION
  Handler function for IMS Service Status response. 
...
======================================================================*/
/*ARGSUSED*/
dsat_result_enum_type dsatvend_ims_srv_status_handler
(
  ds_cmd_type         * cmd_ptr              /* DS Command pointer         */
)
{
  ...
  dsatcmif_servs_state_ms_info	*ph_ss_ms_val =
  	                       dsat_get_base_addr_per_subs(DSAT_SS_PH_MS_VALS, subs_id, FALSE);
  dsat_qcimssrv_ims_srv_status_s_type   *ims_status = NULL;
  dsm_item_type            *res_buff_ptr = NULL;
  
  if (NULL == ph_ss_ms_val || NULL == cmd_ptr)
  {
    return DSAT_ERROR;
  }
  
  if(cmd_ptr->hdr.cmd_id == DS_CMD_ATCOP_IMS_SRV_STATUS_IND)
  {
    ims_status = dsat_get_cmd_payload_ptr(cmd_ptr);
    ph_ss_ms_val->ims_service_status = ims_status->status;

    if(ims_status->result == TRUE)
    {
      if(CHECK_PENDING(DSAT_VENDOR_QCIMSSRV_IDX, 0, DSAT_PENDING_QCIMSSRV_READ))
      {
        SET_PENDING(DSAT_VENDOR_QCIMSSRV_IDX, 0, DSAT_PENDING_QCIMSSRV_NONE)
        res_buff_ptr = dsat_dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL, FALSE);

        res_buff_ptr->data_ptr[res_buff_ptr->used] = '\0';
        res_buff_ptr->used = (word) snprintf ((char *)res_buff_ptr->data_ptr,
                          res_buff_ptr->size,
                          "+QCIMSSRV: %d", ims_status->status);
      ...
      if(CHECK_PENDING(DSAT_VENDOR_QCIMSSRV_IDX, 0, DSAT_PENDING_QCIMSSRV_WRITE))
      {
        SET_PENDING(DSAT_VENDOR_QCIMSSRV_IDX, 0, DSAT_PENDING_QCIMSSRV_NONE)
        result = DSAT_OK;
      }   
...
/*===========================================================================
FUNCTION dsat_qcimssrv_ims_srv_status_cb_i
...
SIDE EFFECTS  
  None  
======================================================================*/
boolean dsat_qcimssrv_ims_srv_status_cb_i
(
...
)
{
  ...
  if( NULL == ims_srv_status)
  {
    return FALSE;
  }
	  
  cmd_buf = dsat_get_cmd_buf(sizeof(dsat_qcimssrv_ims_srv_status_s_type), FALSE);
  if (cmd_buf == NULL || 
      ((ims_status = cmd_buf->cmd_payload_ptr) == NULL))
  {
     return FALSE;
  }
      
  cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_IMS_SRV_STATUS_IND;
  ims_status->status  = ims_srv_status->status;
  ims_status->subs_id = ims_srv_status->subs_id;
  ims_status->result  = ims_srv_status->result;

  DS_AT_MSG3_MED("ims_service_status %d subscription %d result %d", ims_status->status,
                  ims_status->subs_id,ims_srv_status->result);
      
  ds_put_cmd(cmd_buf);
  return TRUE;
}  /* dsat_qcimssrv_ims_srv_status_cb_i */

/*===========================================================================
FUNCTION dsat_client_register_ims_srv_i
...
SIDE EFFECTS  
  None
===========================================================================*/
boolean dsat_client_register_ims_srv_i
(
  dsat_qcimssrv_set_get_f_type  f_ptr,
  void                         *ims_data
)
{
  qcimssrv_ims.qcimssrv_set_get = f_ptr;
  qcimssrv_ims.ims_data_ptr     = ims_data;
  return TRUE;
