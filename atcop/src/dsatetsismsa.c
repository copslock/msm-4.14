...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsismsa.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why

--------   ---     ----------------------------------------------------------
04/25/18   rg      Fixed to delete SM from GW RAM for no message class SM only
                   when CSMS is not set to 1 and <mt> in CNMI is not set
                   to 2
08/17/17   skc     Fixed made to delete SM from GW RAM for no message class SM.
...
)
{
  ...
  if (NULL != res_buff)
  {
    ...
    if ( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,0,0,NUM_TYPE) == 0 )
    {
      ...
      data_ptr = psutil_itoa((uint32)pdu->len, data_ptr, 10);
      ...
      *data_ptr++ = '\n';
      ...
      for( i=0; i < pdu->len; i++)
      {
        ...
        data_ptr = psutil_itoa( (uint32) pdu->data[i], data_ptr, 16 );
      }
      *data_ptr = '\0';
    ...
    else
    {
      ...
      *data_ptr++ = '\n';
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Not an error, but total Response is stripped");
      }
    }
    ...
    if ( dsatsmsi_pres_cmd.cmd_name == SMS_CNMA )
    {
                                               ...
                                               WMS_GW_MSG_WAITING_NONE)
      {
        ...
        if (((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CNMI_ETSI_IDX,
                                      event_info->subs_id,CNMI_MT_VAL,NUM_TYPE) != 2) &&
...
)
{
  ...
  else
  {
    ...
    if ( read_templ_ptr->mask & WMS_GW_TEMPLATE_MASK_SC_ADDR )
    {
      ...
      if( temp_ptr == NULL )
      {
       DS_ATCOP_ERROR_LOG_0("Reading SCA unsuccessful");
        return DSAT_ERROR;
      }
      *temp_ptr = '\0';
      if ( FALSE ==  dsatsmsi_strip_quotes( sca_ptr, no_quotes, sizeof(no_quotes) ) )
      {
       DS_ATCOP_ERROR_LOG_0("Strip Quotes failed");
        return DSAT_ERROR;
      }
      ...
      *data_ptr++ = ',';
      ...
      else
      {
        for( i=0; i<ack.sm_len; i++)
        {
          ...
          data_ptr = psutil_itoa( ack.sm_data[i], data_ptr, 16 );
         }
      }
    }
    res_buff->used =  (byte)(data_ptr - res_buff->data_ptr);
...
)
{
  ...
  if( status != WMS_OK_S )
  {
    /* We got an error while reading the message */
    /* Tell TE about it                          */
   DS_ATCOP_ERROR_LOG_0("ATCOP SMS: Error in STATUS REPORT decoding");
  ...
  if ( dsatsmsi_pres_cmd.cmd_name == SMS_CMGL)
  {
    ...
    if(dsatsmsi_cmd_sm.present_index >= dsatsmsi_cmd_sm.num_of_indices)
    {
      ...
      else
      {
        ...
        {
         DS_ATCOP_ERROR_LOG_0(" Could not get the msg list ");
          dsatutil_free_memory((void *)temp_ts_data);
          return DSAT_ERROR;
        }
        if ( dsatsmsi_cmd_sm.list_begin )
    ...
    else
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0(" Could not read the msg list ");
        dsatutil_free_memory((void *)temp_ts_data);
        return DSAT_ERROR;
      }
      /* Let ATCOP know that we are not done yet */
