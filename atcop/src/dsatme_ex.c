...
EXTERNALIZED FUNCTIONS INTERNAL TO DSAT UNIT
  ...
  dsatme_last_call_connected
    Change last_call_rec state to be connected.

  dsatme_last_call_save
    Store last_call_rec to the ph book.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatme_ex.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/29/18   skc     Added support to save incoming call to phonebook.
10/30/17   skc     Fixed issue where subscription status flag was not updated properly.
08/18/17   rk      Added support for CDMA(1X) 2nd slot.
08/01/17   skc     Fixed issue related to duplicate call of csp, spn and imsi
                   read handler.
06/15/16   skc     Fixed issue while saving last dialed number with ATD.
10/09/15   skc     Fixed compilation issue with WTGL Flavour
05/23/15   sc      Fixed +CLCK personalization pin issue.
...
#define DSATME_EX_ASSERT(expression) \
         dsatme_ex_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATME_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATME_EX ASSERT
...
===========================================================================*/
static void dsatme_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATME_EX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatme_ex_assert_wrapper */
...
FUNCTION DSATME_LAST_CALL_CONNECTED
...
===========================================================================*/
void dsatme_last_call_connected
(
...
)
{
  ...
  if ((NULL == pbm_ms_val) || (NULL == pbm_ms_val->last_call_rec_ptr))
  {
    return ;
  }

  if (pbm_ms_val->last_call_rec_ptr->state != LAST_CALL_NONE)
  {
    pbm_ms_val->last_call_rec_ptr->rec.is_answered = TRUE;
    ...
    pbm_ms_val->last_call_rec_ptr->conn_time = time_jul_to_secs(&julian_time);
  }
...
)
{
  ...
  if(pbm_ms_val->last_call_rec_ptr->state != LAST_CALL_NONE)
  {
    if(pbm_ms_val->last_call_rec_ptr->rec.is_answered)
    {
      /* get the duration of the call */
      time_julian_type     julian_time;
      dword               cur_time;
      
      ASSERT(pbm_ms_val->last_call_rec_ptr->rec.is_answered);
      ...
      pbm_ms_val->last_call_rec_ptr->rec.duration = cur_time - pbm_ms_val->last_call_rec_ptr->conn_time;

      if(pbm_ms_val->last_call_rec_ptr->state == LAST_CALL_DIALED)
      {
        /* Outgoing call is already saved to phonebook during dialing; so returning early*/
        return;
      }
    }

    if(pbm_ms_val->last_call_rec_ptr->rec.text[0] != '\0' )
    {
      (void) dsatutil_memscpy((void*)name,
            PBM_TEXT_SIZE_CHARS,(void*)pbm_ms_val->last_call_rec_ptr->rec.text,PBM_TEXT_SIZE_BYTES);
      data_sizes[num_fields++] = (pbmutils_wstrlen(name)+ 1) * sizeof(uint16);	  
    }

    data_sizes[num_fields++] =  strlen(pbm_ms_val->last_call_rec_ptr->rec.number)+ 1;
    ...
    pbm_fill_field(&ptyped_fields_ptr[field_index],PBM_FIELD_PHONE_GENERIC,
                   PBM_FT_PHONE,(byte *)pbm_ms_val->last_call_rec_ptr->rec.number, 
                   (uint16) data_sizes[field_index]);

    field_index++;

    pbm_fill_field(&ptyped_fields_ptr[field_index],PBM_FIELD_NUMBER_TYPE,
                   PBM_FT_BYTE, (byte *)&pbm_ms_val->last_call_rec_ptr->rec.type, 
                   (uint16) data_sizes[field_index]);
    ...
    switch(pbm_ms_val->last_call_rec_ptr->state)
    {
      case LAST_CALL_DIALED:
        pb_id_local.device_type = PBM_DIALED;
        pbm_cat = PBM_CAT_OUTGOING;
        break;
      case LAST_CALL_RECEIVED:
        pb_id_local.device_type = (pbm_ms_val->last_call_rec_ptr->rec.is_answered) ? PBM_RCVD : PBM_MISS;;
        pbm_cat = (pbm_ms_val->last_call_rec_ptr->rec.is_answered) ? PBM_CAT_INCOMING : PBM_CAT_MISSED;
        break;
      default:
        /* We shold not come here */
        ASSERT(0);
    }
    ...
    dsatutil_free_memory(pbm_ms_val->last_call_rec_ptr);
    pbm_ms_val->last_call_rec_ptr = NULL;
  }
} /* dsatme_last_call_save */
...
)
{
     ...
     cmd == DD_PBM_FILE_INFO)
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0("NULL info_ptr");
      return;
    }
  }
...
)
{
  ...
  DS_ATCOP_ERROR_LOG_1("wrong cmd =d",pbm_info->cmd);
  return DSAT_ASYNC_EVENT;
} /* dsatme_pbm_cb_cmd_handler */
...
)
{
  ...
  if(status == PBM_SUCCESS)
  {
    ...
    DSATME_EX_ASSERT(me_cpbs_entry.device == info_ptr->pbm_device);
  ...
  {
    DS_ATCOP_ERROR_LOG_1("pbm_file_info cb with status: =d",status);
  }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_0("session_info_ptr is NULL");
     return;
  }
...
)
{
  ...
  switch (mmgsdi_init_info->session_type)
  {
	...
	case MMGSDI_1X_PROV_SEC_SESSION:
	  dsat_qcsimapp_info[DSAT_APPS_ID_1X_SECONDARY].session_id=
	  	mmgsdi_init_info->session_id;
	  break;
...
)
{
  ...
  switch( event->evt )
  {
    ...
    case MMGSDI_REFRESH_EVT:
      ...
      {
       DS_ATCOP_ERROR_LOG_0("null ptr: me_mmgsdi_refresh_file_change_notification_cb");
...
LOCAL void dsatme_init_thin_ui_cfg(void)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("enable_thin_ui_cfg EFS stat errcode =d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));
  }
  else
  {
    ...
    if (fd < 0)
    {
      DS_ATCOP_ERROR_LOG_1("enable_thin_ui_cfg EFS open errcode =d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    }
    else
    {
      if(-1 == mcfg_fread(fd, &is_thin_ui, sizeof(is_thin_ui), MCFG_FS_TYPE_EFS))
      {
       DS_ATCOP_ERROR_LOG_0("enable_thin_ui_cfg EFS read failed");
      }
      else
      {
...
)
{
  ...
  DSATME_EX_ASSERT(slot_idx < DSAT_SLOT_IDX_MAX);
    ...
    case MMGSDI_REFRESH_EVT:
     if (IS_VALID_GW_APPS_ID(apps_id))
     {
      ...
      /* Invoke OK to refresh */
        ...
        {
          DS_ATCOP_ERROR_LOG_1("mmgsdi_ok to refresh failed with return status =d",
                    mmgsdi_status);
        }
      }
      ...
      {
        /* Clearing subscription status; it will be true only during subscription ready event */
        dsat_qcsimapp_info[apps_id].active = FALSE;
...
#define CPHS_VAS_OFFSET    0xC0 /* Value Added Service (VAS)group code*/
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Reading of CSP file FAILED");
  ...
  { 
   ...
   DS_ATCOP_ERROR_LOG_0("Value Added Service is NOT present");
...
#define OPERATOR_NAME_INDEX 1
  ...
  if( MMGSDI_SUCCESS != cmd_rd_ptr->cmd_status )
  {
   DS_ATCOP_ERROR_LOG_0("Reading of SPN file FAILED");
    SEND_FAIL_MMGSDI_REFRESH_COMPLETE(refresh_mask,apps_id)
    return DSAT_ASYNC_EVENT;
  }
  memset((void *)&me_ms_val->dsat_spn_info,0x00,sizeof(dsat_mmgsdi_spn_info_s_type));
  ...
  {
    DS_ATCOP_ERROR_LOG_1("SPN Display Info is invalid =d",
...
)
{
  ...
  if( MMGSDI_SUCCESS != cmd_rd_ptr->cmd_status )
  {
   DS_ATCOP_ERROR_LOG_0("Reading of IMSI file FAILED");
...
)
{
  ...
  DEBUG ( DSATME_EX_ASSERT(cb_expected == (cb_cmd_e_type)cnf_ptr->response_header.client_data) ;);
...
#endif /* defined(FEATURE_WCDMA) || defined(FEATURE_GSM) */
    default:
      DS_ATCOP_ERROR_LOG_1(" Invalid Client ref =d in read transparent cb clearing cmd buf",
                              temp_client_ref);
...
)
{
  ...
  switch ( client_ref )
  {
    ...
    case CUAD_MMGSDI_SIM_READ:
      ...
      break;
    default:
       DS_ATCOP_ERROR_LOG_1("Currently  client ref =d is not supported",client_ref);
       DSATME_EX_ASSERT(0); /* for testing purpose */
  }
...
)
{
  ...
  if ( MMGSDI_TRANSPARENT_FILE == file_type )
  {
    ...
    {
     DS_ATCOP_ERROR_LOG_0(" ATCoP MMGSDI Read Transparent failed");
      return FALSE;
    }
  }
  ...
  {  
    ...
    {
     DS_ATCOP_ERROR_LOG_0(" ATCoP MMGSDI Read Record failed");
      return FALSE;
    }
  }
  else
  {
   DS_ATCOP_ERROR_LOG_0(" Invalid file type for MMGSDI read");
    return FALSE;
  }
...
void dsatme_read_uim_files ( dsat_apps_id_e_type apps_id )
{
  ...
  for( i=0; (i < ARR_SIZE(cb_cmd_list)) && (cb_cmd_list[i] != CB_NONE) ; ++i )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Initial read failed for file =d",cb_cmd_list[i]);
    }
  }
...
)
{
  ...
  if ( mmgsdi_status != MMGSDI_SUCCESS )
  {
   DS_ATCOP_ERROR_LOG_0("ATCoP failed to post REFRESH COMPLETE to MMGSDI");
  }
...
)
{
  ...
  if ( mmgsdi_status != MMGSDI_SUCCESS)
  {
   DS_ATCOP_ERROR_LOG_0("Failed to Request Refresh FCN Registration");
  }
    ...
    while((index < me_ms_val->dsat_refresh_fcn_ind.num_files)&& (FALSE != pass_fail))
    {
      switch(me_ms_val->dsat_refresh_fcn_ind.file_list[index])
      {
        ...
        case MMGSDI_USIM_IMSI:
          ...
          break;
        default:
         DS_ATCOP_ERROR_LOG_0("Unknown file in file list ,pass_fail = FALSE");
          refresh_mask = DSAT_MMGSDI_REFRESH_MASK_CB;
      ...
      if ( CB_NONE != cb_cmd )
      {
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Initial read failed for file =d",cb_cmd);
