...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsime.c#2 $ $DateTime: 2019/11/20 10:24:50 $ $Author: kiras $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/20/19   ks      Added NR5G mode check as part of the bug fix arised in ATD.
07/05/18   skc     Fixed issue with AT+CPAS.
01/15/18   rk      Fixed issue related to AT+ICCID cmd.
12/22/17   skc     Added enhancement on supplementary service for IP Call sub-phase 2.
08/17/17   skc     Fixed issue related to AT+CLAC
08/03/17   skc     Replaced strcmp() by strncmp().
06/29/17   skc     Fixed issue with AT+CPUC to handle SIM Read failure.
02/07/17   slc     Removed limitation of slot id dependancy in Single SIM mode.
02/11/16   skc     Deprciating mmgsdi_option_type
03/02/15   sc      Enhanced +CLCK command for SIMLOCK unsupported case.
...
/* To convert HEX chars read from SIM card into its ASCII 
   equivalent as part of valid ICCID info */

#define DSAT_ICCID_VALUE_OFFSET 55
...
/* Macros to ease use of GSDI functions that change name and parms 
  ...
  #define UNBLOCK_PIN(pin_id, old_pin_data, new_pin_data, client_ref ) \
...
/*-------------------------------------------------------------------------
    Local definitions:
-------------------------------------------------------------------------*/
#define DSATETSIME_ASSERT(expression) \
         dsatetsime_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATETSIME_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATETSIME ASSERT
...
===========================================================================*/
static void dsatetsime_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATETSIME_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatetsime_assert_wrapper */
...
)
{
  ...
  if ( DSAT_OK != result )
  {
   DS_ATCOP_ERROR_LOG_0("Could not generate +CIND response");
...
)
{
  ...
  if ((NULL != call_event_ptr) && (0 != call_event_ptr->event_info.dsat_cm_call_info_list.number_of_active_calls))
  {
    ...
    /* Report voice call state over ps data call */
    uint8 i;
    boolean done = FALSE;
    ...
    for (i=0; (i < call_event_ptr->event_info.dsat_cm_call_info_list.number_of_active_calls) && !done; i++)
    {
      ...
      case CM_CALL_STATE_CONV:
        ...
        /* If there is a voice call in ORIG/CONV state, return early*/
        done = (call_event_ptr->event_info.dsat_cm_call_info_list.number_of_voice_calls >= 1)?TRUE:FALSE;
        break;    

      case CM_CALL_STATE_INCOM:
        call_state = CPAS_RINGING;
        /* Call with incoming state can only be voice call and if there is only one, return early */
        done = (call_event_ptr->event_info.dsat_cm_call_info_list.number_of_voice_calls == 1)?TRUE:FALSE;
        break;    
...
)
{
  ...
  if ( pin == NULL )
  {
   DS_ATCOP_ERROR_LOG_0("PIN value is NULL in dsatetsime_set_pin_status");
    return DSAT_ERROR;
  }
  ...
  {
    DS_ATCOP_ERROR_LOG_1("MMGSDI returned failure =d",(int)mmgsdi_result);
...
)
{
  ...
  if ( pin == NULL )
  {
   DS_ATCOP_ERROR_LOG_0("PIN value is NULL in dsatetsime_verify_pin");
    return DSAT_ERROR;
  }
...
)
{
  ...
  if ( pin == NULL )
  {
   DS_ATCOP_ERROR_LOG_0("PIN value is NULL in dsatetsime_change_pin");
    return DSAT_ERROR;
  }
  ...
  if ( new_pin == NULL )
  {
   DS_ATCOP_ERROR_LOG_0("New PIN value is NULL in dsatetsime_change_pin");
    return DSAT_ERROR;
  }
...
#define GENERATE_QCSIMSTAT_RESPONSE(slot_id) \
  ...
  switch ( mmgsdi_event )
  {
    case MMGSDI_CARD_INSERTED_EVT:
      ...
      if(CHECK_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_WRITE_CARD_PUP) )
      {
  ...
  case MMGSDI_CARD_ERROR_EVT:
    ...
    if(CHECK_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_WRITE_CARD_DOWN) )
    {
       SET_PENDING(DSAT_EXT_CFUND_IDX,0,DSAT_PENDING_CFUN_NONE)
      result = DSAT_OK;
    }
...
#define OPER_LEN_W_MNC2    5
  ...
  else if (tok_ptr->op == (NA|EQ|AR))
  {
      ...
      case DSAT_CPOL_SHORT_ALPHA_NUMERIC:
      {
        ...
        shortname.data_ptr = ( uint8 *)(char *)tok_ptr->arg[2];
                   ...
                   else
                   {
                    ...
                    DS_ATCOP_ERROR_LOG_0("CPOL network name lookup failed");
    ...
    else
    {
      ...
      default:
        DS_ATCOP_ERROR_LOG_1("Invalid +CPOL format: =d", cpol_format);
        return DSAT_ERROR;
      }
...
#define ACT_EUTRAN          0x40
  ...
  else
  {
    ...
    for (index = 0; index < pref_networks->length ; index++)
    {
      ...
      {
        /* Duplicate PLMN id */
       DS_ATCOP_ERROR_LOG_0("Attempt to write duplicate PLMN denied");
    ...
    if ( result == DSAT_OK )
    {
      ...
      {       
         ...
         else
        {
         ...
         DS_ATCOP_ERROR_LOG_0("Invalid PLMN Selector with access technology");
        }
      }
    }
    /* Execute Command */ 
...
#define MSISDN_REC_SIZE (PBM_MAX_NUM_LENGTH + PBM_MAX_TEXT_LENGTH + 24) 
  ...
  for(i=0; pbm_return == PBM_SUCCESS; i++)
  {
    ...
    if(pbm_return == PBM_SUCCESS)
    {
      DSATETSIME_ASSERT( res_buff_ptr != NULL );
...
{
#define PREP_ICCID(val)\
      if(val > 9)\
      {\
        tmp_iccid[idx1++] = DSAT_ICCID_VALUE_OFFSET + val;\
      }\
      else\
      {\
        tmp_iccid[idx1++] = '0' + val;\
      }
  ...
  if (MMGSDI_SUCCESS == mmgsdi_status)
  {
    ...
    for(idx = 0, idx1 = 0;idx < MMGSDI_ICCID_LEN && idx1 < (MMGSDI_ICCID_LEN * 2) ;idx++) 
    {
      ...
      PREP_ICCID(low_iccid);
      PREP_ICCID(high_iccid);
    }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Incorrect CNF type rcvd: =d", cnf);
  }
}
#endif /* FEATURE_DSAT_CFUN_CARD_POWER_CTL */
...
#else
         if (MMGSDI_SUCCESS != mmgsdi_send_apdu_ext( me_ss_val->dsat_mmgsdi_client_id,
                               dsat_get_current_slot_id(),
                               dsat_apdu_data,
...
)
{
  ...
  DSATETSIME_ASSERT(cnf_ptr != NULL);

  DS_AT_MSG4_HIGH("cnf = %d session_id = %d client data = %d status %d",
  ...
  if(MMGSDI_GET_FILE_ATTR_CNF == cnf)
  {
    if(cnf_ptr->get_file_attr_cnf.file_attrib.file_type == MMGSDI_TRANSPARENT_FILE)
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Mmgsdi get file attr failed");
        return;
      }
    }
    else
  ...
  else if((MMGSDI_READ_CNF == cnf)||(MMGSDI_WRITE_CNF == cnf))
  {
    ...
    switch(cnf_ptr->response_header.client_data)
    {
      ...
      case ICCID_MMGSDI_SIM_READ:
        ...
        {
          DS_ATCOP_ERROR_LOG_1("Received data NULL pointer ,Status =d",
...
boolean req_mmgsdi_sim_read(cb_cmd_e_type cb_cmd)
{
  ...
  case COPS_MMGSDI_SIM_READ:
    ...
    {
      DS_ATCOP_ERROR_LOG_1("NO APPS AVAILABLE =d", app_type);
  ...
  case CPUC_MMGSDI_SIM_READ:
    ...
    {
     DS_ATCOP_ERROR_LOG_0("NO APPS AVAILABLE");
      /* Default to USIM IMSI */
      file_name.file.file_enum = MMGSDI_GSM_PUCT;
    }
    break;
  ...
  default:
   DS_ATCOP_ERROR_LOG_0("UNKNOWN SIM FILE");
...
)
{
  ...
  DSATETSIME_ASSERT( NULL != imsi_ptr );
  DSATETSIME_ASSERT( NULL != imsi_buf );
  ...
  {
    DS_ATCOP_ERROR_LOG_1("No valid IMSI present to convert - length: =d",
              imsi_ptr->length);
    return DSAT_ERROR;
  }
...
)
{
  ...
  dsat_perso_user_type *user_data = NULL;
...
#define PIN_LEN_OK(pinstr)\
  ...
  if ( IS_PUK_STATE(DSAT_CPIN_SIM_PIN_REQUIRED) )
  {
    ...
    {
      result = me_cpin_unblock_pin ( tok_ptr, res_buff_ptr, MMGSDI_PIN1 );
    }
    else
    {
      result = DSAT_ERROR;
                                 ...
                                 (uint32)DSAT_CPIN_PH_FSIM_PIN_REQUIRED) )
  {
    ...
    user_data = (dsat_perso_user_type *)dsat_alloc_memory(sizeof(dsat_perso_user_type), FALSE);
    ...
    simlock_result = simlock_unlock(req_data,dsatme_sim_mmgsdi_perso_req_cb, user_data);

    if (simlock_result != SIMLOCK_SUCCESS)
    {
      dsatutil_free_memory(user_data);
    }
    result = dsat_map_simlock_result(simlock_result);
  }
  ...
  else if ( IS_PUK_STATE(DSAT_CPIN_SIM_PIN2_REQUIRED) )
  {
    ...
    if ( PIN_LEN_OK(temp0_mix_type) && PIN_LEN_OK(temp1_mix_type) ) 
    {
      result = me_cpin_unblock_pin ( tok_ptr, res_buff_ptr, MMGSDI_PIN2 );
    }
    else
    {
      result = DSAT_ERROR;
...
)
{
  ...
  if (2 == tok_ptr->args_found)
  {
    ...
    UNBLOCK_PIN(pin_id, old_pin_data, new_pin_data, req_submitter );
...
/*------------------------------------------------------------------------*/
  ...
  switch (cnf_hdr_ptr->response_type)
  {
   ...
   break;
   default:
      DS_ATCOP_ERROR_LOG_1("Not a valid mmgsdi CRSM cb type: =d", 
...
)
{
  ds_cmd_type *cmd_ptr = NULL;
  const mmgsdi_response_header_type *cnf_hdr_ptr  = NULL;
  const mmgsdi_send_apdu_ext_cnf_type   *cnf_apdu_ptr = NULL;
  ds_at_mmgsdi_apdu_resp_type       *cmd_apdu_ptr = NULL;

/*--------------------------------------------------------------*/
  
  DSATETSIME_ASSERT(cnf_ptr != NULL);
  cnf_hdr_ptr = &cnf_ptr->send_apdu_ext_cnf.response_header;
  cnf_apdu_ptr = &cnf_ptr->send_apdu_ext_cnf;  
  DSATETSIME_ASSERT(cb_csim_pending == cnf_hdr_ptr->client_data) ; 
  ...
  switch (cnf_hdr_ptr->response_type)
  {
   ...
   break;
   default:
      DS_ATCOP_ERROR_LOG_1("Not a valid mmgsdi sim cb rsp: =d", 
                           cnf_hdr_ptr->response_type);
      break;
  }
...
)
{
  ...
  DSATETSIME_ASSERT(NULL != resp_apdu);
...
)
{
  ...
  if ( num_bytes )
  {
    DSATETSIME_ASSERT(resp_apdu != NULL);
  }
  ...
  while ( num_bytes_rem )
  {
    ...
    if ( tmp_chr_ptr == NULL )
    {
     DS_ATCOP_ERROR_LOG_0("Error converting APDU to HEX chars");
      return DSAT_ERROR;
    }
...
FUNCTION  DSATETSIME_PARSE_TIME

DESCRIPTION
  This function extract time information from time string in the format HH:MM:SS.

DEPENDENCIES
  None

RETURN VALUE
  Return TRUE on successfull parse
  Otherwise FALSE
...
===========================================================================*/
/* ARGSUSED */
boolean dsatetsime_parse_time
(
  const byte *in_str,
  uint8 len,
  dsat_time_s_type *time_info
)
{
#define MAX_TIME_TOKEN_LENGTH 2
#define MAX_TIME_HOUR 23
#define MAX_TIME_MINUTE 59
#define MAX_TIME_SECOND 59

  const byte *str;
  enum { FINDING_HOUR = 0, FINDING_MINUTE, FOUND_TIME } time_state = FINDING_HOUR;
  byte delimiter = ':';  /* Initial delimiter for time string parsing */
  byte time_tok_str[MAX_TIME_TOKEN_LENGTH + 2];

  if(time_info == NULL || in_str == NULL)
  {
    return FALSE;
  }
  ...
  while(time_state != FOUND_TIME)
  {
    if(dsatutil_strtok(str, MAX_TIME_TOKEN_LENGTH, delimiter, time_tok_str) != DSAT_ERROR)
    {
      if(((time_tok_str[0] >= '0') && (time_tok_str[0] <= '9')) &&
         ((time_tok_str[1] >= '0') && (time_tok_str[1] <= '9')))
      {
        switch(time_state)
        {
          case FINDING_HOUR:
            time_info->hour = (time_tok_str[0] - '0')*10 +
                              (time_tok_str[1] - '0');
            time_state = FINDING_MINUTE;
            str += MAX_TIME_TOKEN_LENGTH + 1;
            break;

          case FINDING_MINUTE:
            time_info->minute = (time_tok_str[0] - '0')*10 +
                                (time_tok_str[1] - '0');
            ...
            {
              time_info->second = ((*(char *)str) - '0')*10 +
                                  ((*(char *)(str + 1)) - '0');
              time_state =  FOUND_TIME;
            }
            else
            {
              return FALSE;
  ...
  /* Check the range for hour, minute and second */
  if ((time_info->hour > MAX_TIME_HOUR) ||
      (time_info->minute > MAX_TIME_MINUTE)||
      (time_info->second > MAX_TIME_SECOND))
  {
    return FALSE;
  }
  return TRUE;
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    if ( tok_ptr->args_found != 1 )
    {
      DS_ATCOP_ERROR_LOG_1("+CSTF: Invalid Number of arguements  =d", tok_ptr->args_found);
      return(DSAT_ERROR);
    }
    if((VALID_TOKEN(0))&& 
    ...
    { 
      DS_ATCOP_ERROR_LOG_1("+CSTF: mode updation to =d failed", cstf_mode);
      result = DSAT_ERROR;
      return result;
    }
  }else if(tok_ptr->op == (NA|QU))
...
)
{
  ...
  if (tok_ptr->op == NA)
  {
    ...
    boolean found_at_fwd_reg = FALSE;

    #define CHECK_AT_FWD_CMD(table) if(table.attrib & AT_FWD_CMDS)\
    {\
      found_at_fwd_reg = FALSE; \
      ...
      {\
        ...
        { \
          if ( 0 ==  dsatutil_strcmp_ig_sp_case( (const byte *)fwd_at_cmd_p->cmd_name, (const byte *)table.name) ) \
          { \
            found_at_fwd_reg = TRUE;\
            break; \
          } \
      	} \
      	fwd_at_cmd_p = q_next(&qmi_at_fwd_list.at_fwd_cmd_q, &(fwd_at_cmd_p->link));\
      } \
      if(found_at_fwd_reg == TRUE) \
      {\
        continue;\
      }\
    }
    ...
    {
      CHECK_AT_FWD_CMD(dsat_basic_table[i])
    ...
    {
      CHECK_AT_FWD_CMD(dsat_basic_action_table[i])
      size = (word)snprintf(&(buffer[0]),
                            MAX_CMD_SIZE,
                            "%s\n",
    ...
    {
      CHECK_AT_FWD_CMD(dsat_sreg_table[i])
      size = (word)snprintf(&(buffer[0]),
                            MAX_CMD_SIZE,
                            "%s\n",
    ...
    {
      CHECK_AT_FWD_CMD(dsat_ext_table[i])
      size = (word)snprintf(&(buffer[0]),
                           MAX_CMD_SIZE,
                           "%s\n",
    ...
    {
      CHECK_AT_FWD_CMD(dsatetsi_ext_table[i])
      size = (word)snprintf(&(buffer[0]),
                            MAX_CMD_SIZE,
                            "%s\n",
         ...
         ( DSAT_MODE_NR5G == operating_mode ))
    {
      for (i=0;i<dsatetsi_ext_action_table_size;i++)
      {
        ...
        for (j=0;((j<6) &&(found_flag==0));j++)
        {
          ...
          (strncmp(name[j],(char *)dsatetsi_ext_action_table[i].name,
          MAX(strlen((char*)dsatetsi_ext_action_table[i].name), strlen(name[j])) )==0) )
          {
            found_flag = 1;
          }
        }
    ...
    */
    ...
    {
      CHECK_AT_FWD_CMD(dsat_vendor_table[i])
      size = (word)snprintf(&(buffer[0]),
                           MAX_CMD_SIZE,
                            "%s\n",
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid EAP request callback");
  }
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid EAP state");
    return DSAT_ERROR;
  }
...
)
{
  ...
  if(imsi_ptr == NULL)
  {
   DS_ATCOP_ERROR_LOG_0("Invalid ID");
    return DSAT_ERROR;
  }
...
/*-------------------------------------------------------------------------
  ...
  {
   DS_ATCOP_ERROR_LOG_0("EAP create instance failed ");
    dsatutil_free_memory(eap_req_params);
    return DSAT_ERROR;
  }
...
)
{
  ...
  if( tok_ptr->op == (NA))
  {
    ...
    if ( FALSE == dsatme_req_mmgsdi_get_file_attr( CUAD_MMGSDI_SIM_FILE_ATTR, apps_id ) )
    {
      ...
      DS_ATCOP_ERROR_LOG_1("Initial Get File attr failed for file =d",CUAD_MMGSDI_SIM_FILE_ATTR);
      return DSAT_ERROR;
    }
  }
  else
   ...
   /* Process EAP method */
    ...
    else
    {
     ...
     /* Process EAP method */
      ...
      if((len % 2) != 0)
      {
       DS_ATCOP_ERROR_LOG_0("Invalid packet data");
    ...
    if( me_ss_val->dsat_eap_info.eap_state == DSAT_EAP_STATE_AUTHENTICATING )
    {
      ...
      {
        dsm_free_buffer(eap_packet_data);
        DS_ATCOP_ERROR_LOG_1("EAP input =d specified while a different one in progress",curr_eap_method);
        dsatme_set_cme_error(DSAT_CME_OP_NOT_SUPPORTED, res_buff_ptr);
        return DSAT_CMD_ERR_RSP;
      }
      else
...
#define PSUEDONYM_PARAM 4
  ...
  if(tok_ptr->op == (NA|EQ|AR))
  {
    ...
    if(eap_session_id == me_ss_val->dsat_eap_info.eap_session_id)
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("Unsupported param");
        dsatme_set_cme_error(DSAT_CME_OP_NOT_SUPPORTED, res_buff_ptr);
        return DSAT_CMD_ERR_RSP;
      }
    ...
    {
     DS_ATCOP_ERROR_LOG_0("Invalid Session ID");
      dsatme_set_cme_error(DSAT_CME_EAP_INVALID_PARAM, res_buff_ptr);
      return DSAT_CMD_ERR_RSP;
