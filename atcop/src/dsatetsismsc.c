...
INITIALIZATION AND SEQUENCING REQUIREMENTS
   ...
   Copyright (c) 2002 - 2017 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsismsc.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/03/17   skc     Replaced strcmp() by strncmp().
09/21/15   skc     Fixed KW Issues
07/01/16   skc     Fixed CPMS display issue.
12/17/15   skc     Fixed KW Issues
10/12/15   skc     Fixed KW Issues
10/09/15   skc     Fixed CNMA issue seen due to MO SMS setting current command
                   to SMS_NONE before acking MT SMS
02/11/15   tk      Fixed issues in subscription specific WMS API calls.
01/08/15   sc      Fixed +CSCB AT command issue.
...
===========================================================================*/
#define DSATETSISMSC_ASSERT(expression) \
         dsatetsismsc_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATETSISMSC_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATETSISMSC ASSERT
...
===========================================================================*/
static void dsatetsismsc_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATETSISMSC_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatetsismsc_assert_wrapper */
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("dsatsmsi_pres_cmd not clear, equals =d",
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == (NA|QU)) )
  {
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      ...
      {
       DS_ATCOP_ERROR_LOG_0("CPMS: mem1 is needed");
        return DSAT_ERROR;
      }
    ...
    {
     DS_ATCOP_ERROR_LOG_0("+CPMS write command fails");
      result = DSAT_ERROR;
      DSAT_SMS_STATE_CLEAN();
    }
    return result;
  ...
  {
   DS_ATCOP_ERROR_LOG_0("+CPMS need at least one argument");
    result = DSAT_ERROR;
    DSAT_SMS_STATE_CLEAN();
    return result;
  }
...
#define INATL_IND 0x10
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    else
    {
      ...
      DSATETSISMSC_ASSERT(template_ptr == NULL); 
...
)
{
  ...
  unsigned int used_val;
  byte sr_storage[CPMS_MEM_STR_LEN + 1] = "SR";
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if( strncmp( (char *)sr_storage, (char *) mem_valstr, CPMS_MEM_STR_LEN + 1 ) == 0 )
    {
    ...
    if( tok_ptr->args_found == 1 )
    {
      ...
      sms_msg_ptr->msg_hdr.mem_store = 
        dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
                   DSATETSI_EXT_ACT_CPMS_ETSI_IDX,subs_id,1,NUM_TYPE), 
...
)
{
  ...
  {
    /* Length Validation */
    stat_val_str_len = (uint16)(strlen((char *)(stat_val)));
    if(stat_val_str_len > sizeof(s))
    {
      return WMS_TAG_NONE;	
    }
...
)
{
  ...
  *num_of_param = 0;
  /* Length Validation */
  param_str_len = (uint16)(strlen((char *)(param)));
  if(param_str_len > sizeof(stripped))
  {
    return FALSE;	
  }
