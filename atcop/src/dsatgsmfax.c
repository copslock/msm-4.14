...
Copyright (c) 2001 - 2012,2015 by Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatgsmfax.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
#define DSATGSMFAX_ASSERT(expression) \
         dsatgsmfax_assert_wrapper(__LINE__, expression)
...
#define CHECK_VALID_FAX_CMD() {\
  ...
  {\
   DS_ATCOP_ERROR_LOG_0("FAX call is not active");\
    return DSAT_ERROR;\
  }\
}
...
FUNCTION DSATGSMFAX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATGSMFAX ASSERT
...
===========================================================================*/
static void dsatgsmfax_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATGSMFAX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatgsmfax_assert_wrapper */
...
)
{
  ...
  DSATGSMFAX_ASSERT(mode != DSAT_ONLINE_DATA);
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Modem port is INVALID, set port= DATA_PORT");
