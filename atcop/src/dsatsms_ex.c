...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatsms_ex.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
8/23/19    sp      ATCOP changes to support new WMS callback
08/23/16   skc     Reverting back CR1020560.
06/21/16   skc     Updating CPMS mem3 value to default at bootup.
...
#define DSATSMS_EX_ASSERT(expression) \
         dsatsms_ex_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATSMS_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATSMS_EX ASSERT
...
===========================================================================*/
static void dsatsms_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATSMS_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatsms_ex_assert_wrapper */
...
)
{
          ...
          (sms_dsm_item_ptr = dsm_dequeue( &dsat_sms_wm )) != NULL )
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Invalid subs ID =d, continuing...", sms_info->subs_id);
      continue;
    }
...
(
  ...
  wms_cmd_err_info_s_type  err_info
)
{
  DEBUG(
...
void dsatsms_init_sms_ex ( void )
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Could not initialize with WMS");
  }
  ...
  if (result != WMS_CLIENT_ERR_NONE)
  {
   DS_ATCOP_ERROR_LOG_0("Could not register call back function with reg_cfg_cb");
  }
  ...
  if (result != WMS_CLIENT_ERR_NONE)
  {
   DS_ATCOP_ERROR_LOG_0("Could not register call back function with reg_msg_cb");
  }
...
#ifdef FEATURE_CDMA_SMS
  if ( subs_id == dsat_get_current_1x_subs_id() )
  {
...
)
{
  ...
  DSATSMS_EX_ASSERT (event_ptr != NULL);
  ...
  {
    DS_ATCOP_ERROR_LOG_1("unable to map, event:=d", event);
    return;
  }
       ...
       event == WMS_MSG_EVENT_WRITE_STS_REPORT   )
  {
    ...
    switch ( event_info.u.msg_status.msg_hdr.message_mode )
    {
      ...
      case WMS_MESSAGE_MODE_GW:
        ...
        break;
      default:
        DS_ATCOP_ERROR_LOG_1("Bad message mode: =d", 
...
)
{
  ...
  switch ( event_info.event )
  {
    ...
    case SMS_CFG_EVENT_MS_READY:
      ...
      {
        DS_ATCOP_ERROR_LOG_1("Invalid system type =d in ready event",
  ...
  LOG_MSG_QTRACE_HFT_HIGH(PS_TAG_ATCOP, ps_utils_convert_subs_sys_to_ps(event_info.subs_id),
                          "sms_cfg_cb_func, event:%d", event);
  /* Place the information in a DSM item */
  ds_cmd_buff = dsat_dsm_create_packet(&event_info, sizeof(sms_event_info_s_type), TRUE);
  if (NULL != ds_cmd_buff)
  {
...
)
{
  ...
  if ( wms_cfg_ms_get_gw_domain_pref( dsatsmsi_client_id,
                                      ...
                                      NULL) != WMS_OK_S )
  {
   DS_ATCOP_ERROR_LOG_0("fail to get domain pref info from WMS");
  }
#else
...
{
  DSATSMS_EX_ASSERT (event_info != NULL);
...
)
{
  ...
  if(dsatsmsi_pres_cmd.cmd_name != SMS_CGSMS)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("wrong domain pref =d",domain_pref);
    }
...
)
{
  ...
  if( (msg_ptr = q_get( msg_free_q_ptr )) == NULL )
  {
    ...
    DSATSMS_EX_ASSERT(msg_ptr != NULL);
    /* Add data to it */
...
{
  qsh_mdump_collect_high(&dsatsmsi_pres_cmd, sizeof(dsat_sms_cmd_s_type)); 
}
#endif /* FEATURE_QSH_MDUMP */
