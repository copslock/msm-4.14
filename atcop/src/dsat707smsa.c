...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsat707smsa.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
#define DSAT707SMSA_ASSERT(expression) \
         dsat707smsa_assert_wrapper(__LINE__, expression)
...
FUNCTION DSAT707SMSA_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSAT707SMSA ASSERT
...
===========================================================================*/
static void dsat707smsa_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSAT707SMSA_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsat707smsa_assert_wrapper */
  ...
  */
  ...
  if (NULL != res_buff)
  {
    ...
    *data_ptr++ = ',';
    ...
    data_ptr = psutil_itoa((uint32) temp707_ts_data->u.cdma.language, 
                              data_ptr, 10);
    *data_ptr++ = ',';
    
    /* Update the Result with <format> */
    data_ptr = psutil_itoa(
    ...
    *data_ptr++ = ',';
    ...
    data_ptr = psutil_itoa((uint32) cl_bd_data->user_data.data_len, 
                              data_ptr,
                             10);
    *data_ptr++ = ',';
    
    /* Update the Result with <priority> */
    data_ptr = psutil_itoa((uint32) temp707_ts_data->u.cdma.priority, 
                             data_ptr, 10);
    *data_ptr++ = ',';
    
    /* Update the Result with <privacy> */
    data_ptr = psutil_itoa((uint32) temp707_ts_data->u.cdma.privacy,
                              data_ptr, 10);
    *data_ptr++ = ',';
    ...
    if (temp707_ts_data->u.cdma.message_id.type == WMS_BD_TYPE_DELIVERY_ACK) 
    {
      ...
      data_ptr = psutil_itoa(3, data_ptr, 10);
    }
    else
    {
      data_ptr = psutil_itoa(
...
#else
    /* Update the Result with <length> */
    data_ptr = psutil_itoa((uint32) cl_bd_data->user_data.data_len, data_ptr, 10);
#endif /* FEATURE_DSAT_EXTENDED_SMS */
...
/*-------------------------------------------------------------------------*/
  ...
  DSAT707SMSA_ASSERT( ((event_info != NULL) == TRUE) );
...
/*-------------------------------------------------------------------------*/
     ...
     ( dsatsmsi_pres_cmd.cmd_name == SMS_CMGL) )
  {
    ...
    *data_ptr++ = ',';
    ...
    {
      /* Update the Result with <lang> */
      data_ptr = psutil_itoa((uint32) cl_ts_data->u.cdma.language, data_ptr, 10);
      *data_ptr++ = ',';
      ...
      *data_ptr++ = ',';
      ...
      /* Update the Result with <priority> */
      data_ptr = psutil_itoa((uint32) cl_ts_data->u.cdma.priority, 
                               data_ptr, 10);
      *data_ptr++ = ',';
    
      /* Update the Result with <privacy> */
      data_ptr = psutil_itoa((uint32) cl_ts_data->u.cdma.privacy, 
                               data_ptr, 10);
      *data_ptr++ = ',';
