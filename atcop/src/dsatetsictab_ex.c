...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatetsictab_ex.c#6 $ $DateTime: 2019/12/02 23:41:43 $ $Author: kiras $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/03/19   ks      Fixed COPS write command issue of unable to perform manual PLMN selection by updating the table to support NSA and NR5G SA.
...
04/12/16   skc     Added support for d_comp V.44.
07/09/15   skc     Setting default value of CTZU from 0 to 1 
...
/* StarTime#EndTime
   ...
   Delimiter '#': 1 length
   NULL Termination: 1
   Total Lenght: (8 * 2) + 2 + 1 
 */
word dsat_ccfc_time_start_end_string_len = (MAX_TIME_STRING_LENGTH * 2) + 2 + 1;

LOCAL const mixed_def_s_type dsat_ccfc_time_start_end_def_val =
{
  CONFIG | STRING | YES_QUOTE, (mixed_def_u_type *) &dsat_ccfc_time_start_end_string_len
};

LOCAL const mixed_def_s_type * dsat_ccfc_mixed_dfl[9] = 
{
  ...
  &dsat_ccfc_time_start_end_def_val
};

/* +CCFCU  */

#define CCFCU_NUMBERTYPE_STR_MAX_LEN 1
#define CCFCU_TON_STR_MAX_LEN 1
#define CCFCU_RULESET_STR_MAX_LEN 1

word dsat_ccfcu_numbertype_string_len = CCFCU_NUMBERTYPE_STR_MAX_LEN;

LOCAL const mixed_def_s_type dsat_ccfcu_numbertype_def_val =
{
  CONFIG | STRING | NO_QUOTE, (mixed_def_u_type *) &dsat_ccfcu_numbertype_string_len
};

word dsat_ccfcu_ruleset_string_len = CCFCU_RULESET_STR_MAX_LEN;

LOCAL const mixed_def_s_type dsat_ccfcu_ruleset_def_val =
{
  CONFIG | STRING | YES_QUOTE, (mixed_def_u_type *) &dsat_ccfcu_ruleset_string_len
};


LOCAL const mixed_def_s_type * dsat_ccfcu_mixed_dfl[10] = 
{
  &dsat_ccfc_reason_def_val,
  &dsat_ccfc_mode_def_val,
  &dsat_ccfcu_numbertype_def_val,
  &dsat_ccfc_type_def_val,
  &dsat_ccfc_number_def_val,
  &dsat_ccfc_class_def_val,
  &dsat_ccfcu_ruleset_def_val,
  &dsat_ccfc_subaddr_def_val,
  &dsat_ccfc_satype_def_val,
  &dsat_ccfc_time_def_val
};
...
{
  "0", "2", "7", "12", "13", ""  /* 0-GSM   2-UTRAN   7-E-UTRAN 12-NG-RAN 13-EUTRA-NR-DUAL*/
};

LOCAL const dsat_string_item_type dsat_cops_act_tststr [] =
{ "(0,2,7,12,13)" };
#elif defined (FEATURE_DSAT_LTE) 
LOCAL const dsat_string_item_type dsat_cops_act_valstr [][8] =
{
...
{
  {0, 0, 2}, /* defaulting timezone change reporting to "disable" */
};
...
{
  {1, 0, 1}, /* defaulting timezone update to "enable" */
};
...
/*--------------------------------------
  EMBMBS AT commands DFL 
 --------------------------------------*/
/* +CEMBMSCFG command */
LOCAL const dflm_type dsat_cembmscfg_dfl[] =
{ 
  { 0, 0, 1 }
  /* 0 - disable, 1 - enable*/
};
...
/* +CEMBMSSRV command */
LOCAL const dsat_string_item_type dsat_embms_n_valstr [][8] =
{
  "0", "1", NULL
};

LOCAL const dsat_string_item_type dsat_embms_n_tststr [] =
{ "(0,1)" };

LOCAL const dflm_type dsat_embms_n_list [] =
{
  { 0, 0 , 1 }
};

LOCAL const mixed_def_s_type dsat_embmssrv_n_def_val =
{
  CONFIG , (mixed_def_u_type *) &dsat_embms_n_list
};

LOCAL const dsat_string_item_type dsat_embms_service_valstr [][8] =
{
  "0", "1", "2", "3", "4", NULL
};

LOCAL const dsat_string_item_type dsat_embms_service_tststr [] =
{ "(0,1,2,3,4)" };

LOCAL const dflm_type dsat_embms_service_list[] =
{
  { 4 , 0 , 4 }
};

LOCAL const mixed_def_s_type dsat_embmssrv_service_def_val =
{
  CONFIG , (mixed_def_u_type *) &dsat_embms_service_list
};

LOCAL const word dsat_embmssrv_tmgi_string_len = TMGI_STRING_MAX_LENGTH;

LOCAL const mixed_def_s_type dsat_embmssrv_tmgi_dfl =
{
  CONFIG | STRING | YES_QUOTE,
  (mixed_def_u_type *) &dsat_embmssrv_tmgi_string_len
};

const mixed_def_s_type * dsat_cembmssrv_mixed_dfl[3] = 
{
  &dsat_embmssrv_n_def_val,
  &dsat_embmssrv_service_def_val,
  &dsat_embmssrv_tmgi_dfl
};

LOCAL const dflm_type dsat_cembmssai_dfl[2] =
{ 
  { 0, 0, 1 }, /* mode */
  { 0, 0, 1 }  /* embms prior - not supported */
};

/* +CMCCS Command */

LOCAL const dflm_type dsat_cmccs_dflm [] =
{
   {0, 0, 3}
};
...
const dsati_cmd_type dsatetsi_ext_action_table [] =
{
  ...
  { "+CBC",    READ_ONLY | COMMON_CMD | AT_FWD_CMDS,
  ...
  { "+CPIN",   EXTENDED | CONFIG | MIXED_PARAM | LIST | 
               LOCAL_TEST | RESTRICTED | UMTS_CMD | MULTI_SLOT | NOT_FORWARDABLE,
  ...
  { "+CIND",    EXTENDED | CONFIG | LIST | LOCAL_TEST | NO_RESET | RESTRICTED| MULTI_SUBS | COMMON_CMD| NOT_FORWARDABLE,
...
#ifdef FEATURE_DSAT_ETSI_DATA
  { "+CGATT",
    EXTENDED | CONFIG | LIST | LOCAL_TEST | NO_RESET| MULTI_SUBS | UMTS_CMD| NOT_FORWARDABLE,
  ...
  { "+CHUP",       ATTRIB_NONE | RESTRICTED | MULTI_SUBS | UMTS_CMD| NOT_FORWARDABLE,
  ...
  { "+COPS",    ABORT_CMD | EXTENDED | LOCAL_TEST | CONFIG | LIST | MIXED_PARAM | MULTI_SUBS | UMTS_CMD | NOT_FORWARDABLE,
  ...
  { "+CHLD",    ATTRIB_NONE | UMTS_CMD | NOT_FORWARDABLE,
...
#ifdef FEATURE_MMGSDI
  { "+CPOL",    ABORT_CMD | EXTENDED | LOCAL_TEST | CONFIG | LIST | MIXED_PARAM| MULTI_SUBS | UMTS_CMD| NOT_FORWARDABLE,
  ...
  { "+CLIP",     ABORT_CMD | EXTENDED | CONFIG | LOCAL_TEST | UMTS_CMD| NOT_FORWARDABLE,
    SPECIAL_NONE,    1, DSATETSI_EXT_ACT_CLIP_ETSI_IDX,      &dsat_clip_dflm[0]},

  { "+COLP",     ABORT_CMD | EXTENDED | CONFIG | LOCAL_TEST | UMTS_CMD| NOT_FORWARDABLE,
    SPECIAL_NONE,    1,DSATETSI_EXT_ACT_COLP_ETSI_IDX,      &dsat_colp_dflm[0]},

  { "+CDIP",     ABORT_CMD | EXTENDED | CONFIG | LOCAL_TEST | UMTS_CMD| NOT_FORWARDABLE,
    SPECIAL_NONE,    1, DSATETSI_EXT_ACT_CDIP_ETSI_IDX,    &dsat_cdip_dflm[0]},

  { "+CTFR",    EXTENDED | LOCAL_TEST | LIST | MIXED_PARAM | UMTS_CMD| NOT_FORWARDABLE,
    SPECIAL_NONE,    4,  DSATETSI_EXT_ACT_CTFR_ETSI_IDX,  &dsat_ctfr_mixed_dfl[0] },

  { "+CLIR",      ABORT_CMD | EXTENDED | CONFIG | LOCAL_TEST | UMTS_CMD| NOT_FORWARDABLE,
  ...
  { "+CMCCS",     EXTENDED | CONFIG | LOCAL_TEST | UMTS_CMD,
    SPECIAL_NONE,   1, DSATETSI_EXT_ACT_CMCCS_ETSI_IDX, dsat_cmccs_dflm },

  { "+CCMMD",     EXTENDED | UMTS_CMD,
    SPECIAL_NONE,   3, DSATETSI_EXT_ACT_CCMMD_ETSI_IDX, NULL },

  { "+CCFCU",	EXTENDED | LOCAL_TEST | LIST | MIXED_PARAM | UMTS_CMD,
    SPECIAL_NONE,    10,DSATETSI_EXT_ACT_CCFCU_ETSI_IDX,   &dsat_ccfcu_mixed_dfl[0] },
...
const dsati_cmd_type dsatetsi_ext_table [] =
{
  ...
  { "+CREG",    EXTENDED | CONFIG | LOCAL_TEST | MULTI_SUBS | UMTS_CMD| NOT_FORWARDABLE,
    SPECIAL_NONE,    1,DSATETSI_EXT_CREG_IDX,    dsat_creg_dfl },

  { "+CGREG",    EXTENDED | CONFIG | LOCAL_TEST| MULTI_SUBS | UMTS_CMD| NOT_FORWARDABLE,
  SPECIAL_NONE,    1, DSATETSI_EXT_CGREG_IDX,     dsat_cgreg_dfl },
#ifdef FEATURE_DSAT_LTE

  { "+CEREG",    EXTENDED | CONFIG | LOCAL_TEST| MULTI_SUBS | UMTS_CMD| NOT_FORWARDABLE,
  ...
  ,{ "+CEMBMSCFG",  EXTENDED | CONFIG | UMTS_CMD,
     SPECIAL_NONE, 1 ,DSATETSI_EXT_CEMBMSCFG_IDX, &dsat_cembmscfg_dfl }

  ,{ "+CEMBMSSRV",  EXTENDED | CONFIG | MIXED_PARAM | LOCAL_TEST | UMTS_CMD,
     SPECIAL_NONE, 3 ,DSATETSI_EXT_CEMBMSSRV_IDX, &dsat_cembmssrv_mixed_dfl[0] }

  ,{ "+CEMBMSDATA",  ATTRIB_NONE | UMTS_CMD ,
     SPECIAL_NONE, 0 ,DSATETSI_EXT_CEMBMSDATA_IDX, NULL }

  ,{ "+CEMBMSCNT",  ATTRIB_NONE | UMTS_CMD,
     SPECIAL_NONE, 0 ,DSATETSI_EXT_CEMBMSCNT_IDX, NULL }
  
  ,{ "+CEMBMSSAI",  EXTENDED | CONFIG | LOCAL_TEST | UMTS_CMD ,
     SPECIAL_NONE, 2 ,DSATETSI_EXT_CEMBMSSAI_IDX, &dsat_cembmssai_dfl[0] }
