...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatcmif_ex.c#3 $ $DateTime: 2019/11/22 03:36:18 $ $Author: kiras $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/22/19   ks      Added Rac info parameter to CGREG command.
...
08/18/17   rk      Fixed issue related to network registration status.
08/17/17   skc     Fixed issue with at+chld=2 during MPTY calls
08/01/17   skc     Made changes to handle CHLD and CLCC for SRVCCed MPTY Call.
05/18/17   rg      Added support for ATA command to answer voice 
                   call after LTE->G SRVCC
...
12/23/15   skc     Fixed issue where wrong call id is stored during one 
                   active call and one held call and at+chld=2 is issued.
10/08/15   skc     Adding condition for enabling of dsat_power_off only when
...
#define DSATCMIF_EX_ASSERT(expression) \
         dsatcmif_ex_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATCMIF_EX_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATCMIF_EX ASSERT
...
===========================================================================*/
static void dsatcmif_ex_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATCMIF_EX_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatcmif_ex_assert_wrapper */
...
)
{
  ...
  DSATCMIF_EX_ASSERT( ((call_id_ptr != NULL) == TRUE) );

  if ( num_call_ids > CM_CALL_ID_MAX || !IS_VALID_SUBS_ID(subs_id))
  {
    return FALSE;
  }
...
FUNCTION DSATCMIF_ANSWER_CALL

DESCRIPTION
  This command answer a voice or video call by calling cm_call_cmd_answer
...
if( call_mode == CM_CALL_MODE_INFO_IP )
  {
    ...
    /* Set IP answer parameters based on call attribute received in incoming call event
       and AT$QCCALLATTR setting */

    if(call_da_val->ip_call_info.call_attrib_info.video_attrib != CM_CALL_ATTRIB_NONE &&
       (DSAT_QCCALLATTR_CALL_TYPE_VT ==
        (dsat_qccallattr_call_type_e_type)dsatutil_get_val(DSAT_VENDOR_QCCALLATTR_IDX,0,0,NUM_TYPE)))
    {
      answer_params_ptr->ans_params.ip_ans.call_type = CM_CALL_TYPE_VT;
      answer_params_ptr->ans_params.ip_ans.call_attrib.audio_attrib = CM_CALL_ATTRIB_TX_RX;

      call_attr = (dsat_num_item_type)dsatutil_get_val(DSAT_VENDOR_QCCALLATTR_IDX,0,1,NUM_TYPE);
      /* Setting video attribute based on AT$QCCALLATTR */
      if(call_attr == DSAT_QCCALLATTR_CALL_ATTR_RX)
      {
        answer_params_ptr->ans_params.ip_ans.call_attrib.video_attrib = CM_CALL_ATTRIB_RX_ONLY;
      }
      else if(call_attr == DSAT_QCCALLATTR_CALL_ATTR_TX)
      {
        answer_params_ptr->ans_params.ip_ans.call_attrib.video_attrib = CM_CALL_ATTRIB_TX_ONLY;
      }
      else
      {
        answer_params_ptr->ans_params.ip_ans.call_attrib.video_attrib = CM_CALL_ATTRIB_TX_RX;
      }
    }
    else
    {
      /* Default values: Answer as voice call */
      answer_params_ptr->ans_params.ip_ans.call_type = CM_CALL_TYPE_VOICE;
      answer_params_ptr->ans_params.ip_ans.call_attrib.audio_attrib = CM_CALL_ATTRIB_TX_RX;
      answer_params_ptr->ans_params.ip_ans.call_attrib.video_attrib = CM_CALL_ATTRIB_NONE;
    }
  }
...
)
{
  ...
  DS_AT_MSG3_MED("subs_id = %d, etsicall_was_call_active = %d call_type %d",
  ...
  switch (call_event->event)
  {
    case CM_CALL_EVENT_INCOM:
      ...
      {
        /* Save Call attributes for Incoming call with call type voice or VT */
        if((call_event->event_info.call_type == CM_CALL_TYPE_VOICE ) ||
           (call_event->event_info.call_type == CM_CALL_TYPE_VT ))
        {
          call_da_val->ip_call_info.call_attrib_info.audio_attrib = 
            call_event->event_info.dsat_mode_info.info.ip_call.call_attrib_info.audio_attrib;
          call_da_val->ip_call_info.call_attrib_info.video_attrib = 
           call_event->event_info.dsat_mode_info.info.ip_call.call_attrib_info.video_attrib;
        }
    ...
    case CM_CALL_EVENT_SRVCC_COMPLETE_IND:
      {
        result = dsatvoice_call_event_ho_complete(subs_id, call_event);
      }
      break;
    default:
      break;
...
)
{
  ...
  DS_AT_MSG4_HIGH("Call cmd handler cmd_err %d [Applicable for only voice] cause %d alpha len %d cmd %d",
            call_cmd->cmd_err, call_cmd->err_cause, call_cmd->alpha.len, call_cmd->cmd);
...
===========================================================================*/
dsat_subs_mode_e_type cmif_determine_subs_mode
(
  const ds_at_cm_ss_event_type *event_ptr
)
{
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Either msg ptr is NULL or No of Stack is 0: Ignored SS event processing");
    return result;
  }
  ...
  if (!IS_VALID_SUBS_ID(subs_id))
  {
    DS_ATCOP_ERROR_LOG_1("Invalid subs ID =d", subs_id);
    return DSAT_ASYNC_EVENT;
  }
    ...
    case CM_SS_EVENT_SRV_CHANGED:
    {
      ...
      switch(event_ptr->event_info[DSAT_STACK_MAIN].sys_mode)
      {
        ...
        case SYS_SYS_MODE_HDR:
          ...
          {
            ph_ss_ms_val = dsat_get_base_addr_per_subs(DSAT_SS_PH_MS_VALS, dsat_get_current_1x_subs_id(), TRUE);
...
)
{
  ...
  /*Unified CM change fields into ATCoP change fields*/
      ...
      case CM_SS_EVENT_EMERG_NUM_LIST:
        if ( ( event_ptr->emerg_num_list->emerg_num_list.num_list_len > 0 ) &&
             ( event_ptr->emerg_num_list->emerg_num_list.num_list_len <= CM_MAX_EMERGENCY_NUM_COUNT ) )
        {
          ...
          if (NULL != info_ptr->emerg_num_list)
          {
            dsatutil_memscpy(info_ptr->emerg_num_list,
                             ...
                             &event_ptr->emerg_num_list->emerg_num_list,
                             sizeof(cm_emerg_num_list_s_type));
  ...
  if ( TRUE == post_ss_event )
  {
    ...
    if( FALSE == local_cm_msg_wm_full )
    {
      ...
      for(stack_id=DSAT_STACK_MAIN;(stack_id<event_ptr->number_of_stacks) && (stack_id < DSAT_STACK_MAX) ;stack_id++)
      {
        ...
        info_ptr->mm_info.daylight_saving_adj_avail =
          event_ptr->stack_info[stack_id].mode_info.gw_info.mm_information.daylight_saving_adj_avail;
        ...
        info_ptr->mm_info.daylight_saving_adj =
          event_ptr->stack_info[stack_id].mode_info.gw_info.mm_information.daylight_saving_adj;
        ...
        info_ptr->tac_5g = event_ptr->stack_info[stack_id].rat_mode_info.nr5g_mode_info.tac_5g;
        info_ptr->restrict_dcnr = event_ptr->stack_info[stack_id].rat_mode_info.lte_mode_info.restrict_dcnr;
        info_ptr->sib2_upr_layer_ind_value = event_ptr->stack_info[stack_id].rat_mode_info.lte_mode_info.sib2_upr_layer_ind_value;
#ifdef FEATURE_GSM_RR_CELL_CALLBACK
    ...
    else
    {
      ...
      DS_ATCOP_ERROR_LOG_1("CM message queue full - dropping event: =d", event);
      post_ss_event = FALSE;
    }
  }
...
)
{
  ...
  case CM_PH_CMD_SYS_SEL_PREF:
    ...
    {
      DS_ATCOP_ERROR_LOG_1("CM PH command error: =d",ph_cmd->cmd_err);
...
)
{
  ...
  if(event_info_ptr->pref_info.network_band_pref == SD_SS_BAND_PREF_NO_CHANGE )
  {
...
)
{
  ...
  LOG_MSG_QTRACE_HFT_HIGH(PS_TAG_ATCOP, ps_utils_convert_subs_sys_to_ps(subs_id),
                          "Processing CM PH event: %d, OPRT mode %d, device mode %d, eps mode %d subs_id:%d",
                          ph_event->event,
                          ph_event->event_info.oprt_mode,
                          ph_event->event_info.device_mode,
                          ph_event->event_info.eps_mode,
                          subs_id);
  ...
  switch(ph_event->event)
  {
    ...
    case CM_PH_EVENT_SYS_SEL_PREF:
    {
      ...
      /* Set current value */
      dsatact_set_ws46_current_value(pref_info_ptr->network_rat_mode_pref, subs_id);
          ...
          (TRUE == cmif_is_gw_subscription_available(subs_id, ph_event) ) )
      {
        ...
        switch (ph_ss_ms_val->dsat_net_reg_state.cmph_pref.network_sel_mode_pref)
        {
          ...
          case CM_NETWORK_SEL_MODE_PREF_LIMITED_SRV:
            ...
            break;
          default:
            DS_ATCOP_ERROR_LOG_1(" network_sel_mode_pref = %d not handled",
                                   ph_ss_ms_val->dsat_net_reg_state.cmph_pref.network_sel_mode_pref);
            /* No update required */
            break;
        }
...
)
{
  ...
  while ( (dsm_item_ptr = dsm_dequeue( &dsat_cm_msg_wm )) != NULL)
  {
    ...
    default:
      DS_ATCOP_ERROR_LOG_1("Command = =d", msg_ptr->cmd);
    }
...
void dsatcmif_init_cmif_ex ( void )
{
...
#else
  switch (nv_rtre_control_ext((uint16)dsat_get_current_1x_subs_id()))
#endif /* FEATURE_DATA_MPSS_AT20_DEV */
  {
    ...
    default:
     DS_ATCOP_ERROR_LOG_0("Invalid RTRE control setting from NV");
  ...
  if (TRUE != cm_ph_cmd_get_ph_info( dsatcmif_ph_cmd_cb_func,
                                     ...
                                     dsatcm_client_id ))
  {
   DS_ATCOP_ERROR_LOG_0("Problem requesting initial CM PH info ");
  }
...
if ( dsatcmif_is_voice_call_type(event_ptr->call_type)
  ...
  {
    DS_AT_MSG5_MED("ATCOP: cm event cb, event: %d, type: %d, state: %d, call_id %d mode_info_type %d",
                   event, event_ptr->call_type, event_ptr->call_state,
                   event_ptr->call_id, event_ptr->mode_info.info_type);
      ...
      if( NULL != event_ptr )  /*lint !e774 */
      {
        ...
        switch (event_ptr->mode_info.info_type)
        {
          ...
          case CM_CALL_MODE_INFO_IP:
            ...
            /* Store all IP call related information for voice or VT Call */
            if(CM_CALL_TYPE_VOICE == event_ptr->call_type ||
               CM_CALL_TYPE_VT == event_ptr->call_type)
            {
              call_event->event_info.dsat_mode_info.info.ip_call.call_attrib_info.audio_attrib =
                   event_ptr->mode_info.info.ip_call.call_attrib_info.audio_attrib;
              call_event->event_info.dsat_mode_info.info.ip_call.call_attrib_info.video_attrib =
                   event_ptr->mode_info.info.ip_call.call_attrib_info.video_attrib;

              call_event->event_info.dsat_mode_info.info.ip_call.call_modify_err_status =
                   event_ptr->mode_info.info.ip_call.call_modif_err_status;
              call_event->event_info.dsat_mode_info.info.ip_call.modify_error_code =
                   event_ptr->mode_info.info.ip_call.modify_error_code;
              call_event->event_info.dsat_mode_info.info.ip_call.modify_cause_code =
                   event_ptr->mode_info.info.ip_call.modify_cause_code;

              DS_AT_MSG5_HIGH("CM modify_error_code %d modify_cause_code %d modify error status %d audio %d video %d",
                   event_ptr->mode_info.info.ip_call.modify_error_code,
                   event_ptr->mode_info.info.ip_call.modify_cause_code,
                   event_ptr->mode_info.info.ip_call.call_modif_err_status,
                   event_ptr->mode_info.info.ip_call.call_attrib_info.audio_attrib,
...
)
{
  ...
  DSATCMIF_EX_ASSERT (((event_ptr != NULL) == TRUE));
  ...
  case CM_PH_EVENT_SIM_AVAILABLE:	  
      ...
      ph_event->event_info.pref_info.network_rat_mode_pref = event_ptr->ph_mode_pref;
      ...
      if( event_ptr->asubs_id == SYS_MODEM_AS_ID_2 )
      {
        ph_event->event_info.is_hybr_gw_subscription_available = 
      	                           event_ptr->is_gw_subscription_available;
      }
      if( event_ptr->asubs_id == SYS_MODEM_AS_ID_3 )
      {
  ...
  default:
    DS_ATCOP_ERROR_LOG_1("Unsupported CM PH event: =d", event);
  }
  return;
...
)
{
  ...
  if ( SYS_SRV_STATUS_SRV == event_ptr->event_info[stack_id].srv_status )
  {
...
#endif /* FEATURE_DSAT_LTE */
      if(event_ptr->event_info[stack_id].sys_mode == SYS_SYS_MODE_NR5G)
      {
        (void) dsatutil_memscpy((void*)&net_reg_state_ptr->net_id_info.plmn_lac.lac,
                              sizeof(sys_lac_type),
                              (void*)&event_ptr->event_info[stack_id].tac_5g,
                              sizeof(sys_5g_tac_s_type));
      }
    }
    else
    {
    ...
    switch ( event_ptr->event_info[stack_id].sys_mode )
    {
      ...
      case SYS_SYS_MODE_TDS:
#endif /*  FEATURE_TDSCDMA */
        if(SYS_SRV_DOMAIN_CS_PS   == event_ptr->event_info[stack_id].srv_domain ||
           SYS_SRV_DOMAIN_PS_ONLY == event_ptr->event_info[stack_id].srv_domain)
        {
          if(event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSDPA_SUPP_CELL ||
             event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSDPAPLUS_SUPP_CELL ||
             event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSDPAPLUS_64QAM_SUPP_CELL ||
             event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_DC_HSDPAPLUS_SUPP_CELL)
          {
            net_reg_state_ptr->act = DSAT_COPS_ACT_UTRAN_HSDPA;
          }
          else if(event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSUPA_SUPP_CELL)
          {
            net_reg_state_ptr->act = DSAT_COPS_ACT_UTRAN_HSUPA;
          }
          else if(event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSDPA_HSUPA_SUPP_CELL ||
                  event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSDPAPLUS_HSUPA_SUPP_CELL ||
                  event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_HSDPAPLUS_64QAM_HSUPA_SUPP_CELL ||
                  event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_DC_HSDPAPLUS_HSUPA_SUPP_CELL ||
                  event_ptr->event_info[stack_id].cell_srv_ind.hs_ind == SYS_HS_IND_DC_HSDPAPLUS_DC_HSUPA_SUPP_CELL)
          {
            net_reg_state_ptr->act = DSAT_COPS_ACT_UTRAN_HSDPA_HSUPA;
          }
          else
          {
        ...
        if(event_ptr->event_info[stack_id].sys_mode == SYS_SYS_MODE_WCDMA )
        {
          net_reg_state_ptr->csg_rat = DSAT_COPS_CSG_RAT_UMTS;
        }
#ifdef FEATURE_TDSCDMA
        if(event_ptr->event_info[stack_id].sys_mode == SYS_SYS_MODE_TDS )
        {
    ...
    case SYS_SYS_MODE_GSM:
      if((SYS_SRV_DOMAIN_CS_PS   == event_ptr->event_info[stack_id].srv_domain ||
          SYS_SRV_DOMAIN_PS_ONLY == event_ptr->event_info[stack_id].srv_domain) && 
          SYS_EGPRS_SUPPORT_AVAIL == event_ptr->event_info[stack_id].cell_srv_ind.egprs_supp)
      {
        net_reg_state_ptr->act = DSAT_COPS_ACT_GSM_EGPRS;
      }
      else
      {
...
#ifdef FEATURE_DSAT_LTE
    case SYS_SYS_MODE_LTE:
      if( FALSE == event_ptr->event_info[stack_id].restrict_dcnr &&
          TRUE == event_ptr->event_info[stack_id].sib2_upr_layer_ind_value)
      {
        net_reg_state_ptr->act = DSAT_COPS_ACT_EUTRA_NR;
      }
      else
      {
...
#endif /* FEATURE_DSAT_LTE */
    case SYS_SYS_MODE_NR5G:
      net_reg_state_ptr->act = DSAT_COPS_ACT_NGRAN;
      break;
    default:
...
)
{
  ...
  if (DSAT_COPS_ASTATE_NULL == ph_ss_ms_val->dsatetsicall_network_list.cmd_state || 
      SYS_SRV_STATUS_SRV == event_ptr->event_info[stack_id].srv_status )
  {
    dsatetsicall_creg_state_machine(event_ptr, stack_id, subs_id);
...
)
{
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Unsupported system mode");
    return;
  }
  ph_ss_val = dsat_get_base_addr(DSAT_SS_PH_SS_VALS, TRUE);
  if ((NULL == ss_info_ptr) || (NULL == ph_ss_val))
    ...
    cm_status = (cm_client_status_e_type)
                  ((int32)cm_status |
                   (int32)cm_mm_client_call_reg(dsatcm_client_id,
                                  ...
                                  CM_CALL_EVENT_ABRV_ALERT,
                                  NULL ));
   /* Registering to CM Events related to Call modification */
   cm_status = (cm_client_status_e_type)
                  ((int32)cm_status |
                   (int32)cm_mm_client_call_reg(dsatcm_client_id,
                                  cmif_call_event_cb_func,
...
)
{
        ...
        call_type == CM_CALL_TYPE_VT))
  {
    ...
    DSATCMIF_EX_ASSERT(t_ptr->user_data != NULL);
...
)
{
  ...
  if(( call_type == CM_CALL_TYPE_VOICE || call_type == CM_CALL_TYPE_VT ) &&
...
void dsatcmif_initialize_to_default( dsat_mem_type mem_type,void* ptr)
{
  ...
  if(NULL == ptr)
  {
   DS_ATCOP_ERROR_LOG_0("NULL pointer");
    return;
  }
  switch(mem_type)
  {
  ...
  /* HDR CSQ Initailzation */
       ...
       ms->last_reported_tz      = DSAT_TZ_INVALID;
       ms->last_reported_dst_adj = DSAT_DST_ADJ_DEFAULT;
       ms->dsat_mode_urc_info.mode_val    = AT_MODE_NONE;
       ms->dsat_mode_urc_info.report_flag = FALSE;
       ms->ims_service_status = FALSE;
     }
     break;
    ...
    case DSAT_CALL_DA_VALS:
     {
       ...
       da->call_info_before_connect.call_id = CM_CALL_ID_INVALID;
       da->call_info_before_connect.info_type = CM_CALL_MODE_INFO_NONE;
       da->etsicall_num_participant_in_conf = 0;
       da->conf_linked_call_id = CM_CALL_ID_INVALID;
       da->handover_complete = FALSE;
       da->last_seq_num =  CALL_SEQNUM_INVALID;
       /* Initializing call modification parameters */
       da->call_modify_param.req_call_type = CM_CALL_TYPE_NONE;
       da->call_modify_param.modify_response = FALSE;
       da->call_modify_param.last_ccmmd_neg_status = DSAT_NEG_STATUS_NONE;
       /* Initialize to no valid sequence numbers */
       /* Initialize conference participants info */
       for(i = 0; i< CM_CALL_ID_MAX ; i++)
       {
         ...
         da->dsat_seqnum_callid[i].is_conf_participant = FALSE;
         da->conf_participant_info[i].dsat_call_id = CM_CALL_ID_INVALID;
         da->conf_participant_info[i].seq_number = CALL_SEQNUM_INVALID;
         da->conf_participant_info[i].user_uri_len = 0;
         da->conf_participant_info[i].user_uri[0] = '\0';	
         da->conf_participant_info[i].status = MMODE_QMI_CALL_STATUS_DISCONNECTED;
       }
...
)
{
  ...
  switch (cmd_ptr->hdr.cmd_id )
  {
    ...
    default:
      DS_ATCOP_ERROR_LOG_1("Unknown event =d",cmd_ptr->hdr.cmd_id);
      break;
  }
}
...
{
  qsh_mdump_collect_high(&dsat_cm_msg_q, sizeof(q_type));
  qsh_mdump_collect_high(&dsat_cm_msg_wm, sizeof(dsm_watermark_type)); 
}
#endif /* FEATURE_QSH_MDUMP */
