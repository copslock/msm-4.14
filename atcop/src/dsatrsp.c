...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/src/dsatrsp.c#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/30/17   rk      Removed DS Flag FEATURE_DSAT_EXT_CLIENT_SUPPORT.
02/17/15   tk      Fixed CSD call port none issue.
...
#define DSATRSP_ASSERT(expression) \
         dsatrsp_assert_wrapper(__LINE__, expression)
...
FUNCTION DSATRSP_ASSERT_WRAPPER()

DESCRIPTION
  Wrapper function for DSATRSP ASSERT
...
===========================================================================*/
static void dsatrsp_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("DSATRSP_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* dsatrsp_assert_wrapper */
...
)
{
  ...
  *in_ptr_ptr = NULL;
  ...
  DSATRSP_ASSERT( ((out_ptr_ptr != NULL && *out_ptr_ptr != NULL) == TRUE) );  
  ...
  default:
   ...
   DS_ATCOP_ERROR_LOG_0("Invalid type of response.");
    break;
  }
  ...
  DSATRSP_ASSERT( dest_ptr < dest_end_ptr );
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Incorrect basic result code =d", result_code);
    return NULL;
  }
  ...
  if ( NULL != raw_rsp_ptr )
  {
    ...
    else
    {
      ...
      tmp_ptr = psutil_itoa( result_code_basic,
...
)
{
                           ...
                           DSAT_BASIC_Q_IDX,0,0,NUM_TYPE) == 0 )
  {
            ...
            {
              port= DS3G_SIOLIB_DATA_PORT;
      DS_ATCOP_ERROR_LOG_0("Port is None setting it to Data Port");
   }
    /* Result codes are not suppressed so send result code. */
...
)
{
  if( DS3G_SIOLIB_PORT_NONE == port )
  {
    ...
    if( DS3G_SIOLIB_PORT_NONE == port )
    {
      DS_ATCOP_ERROR_LOG_1("Invalid port =d",port);
      return;
    }
  }
  /* If we are processing ATH command we are supposed to continue
...
#define BYTES_TO_KB_FACTOR 1000
  ...
  */
                 ...
                 DSAT_BASIC_N_E_IDX,0,0,NUM_TYPE) == AMP_E_RADIO_RATE )
  {
    ...
    {
      (void)psutil_itoa( dsat_report_rate_val.rate.cs_call.waiur,
                     rate_str,
                     10 );
    }
    else if ( dsat_report_rate_val.call_type == DSAT_CALL_TYPE_PS )
    {
      ...
      (void)psutil_itoa( dsat_report_rate_val.rate.ps_call.data_rate,
                     rate_str,
                     10 );
    }
    else if ( dsat_report_rate_val.call_type == DSAT_CALL_TYPE_1X )
    {
      (void)psutil_itoa( dsat_report_rate_val.rate.cdma_call.data_rate,
                     rate_str,
                     10 );
    }
    else
    {
     DS_ATCOP_ERROR_LOG_0("Invalid call type for data rate ");
    }
  }
...
)
{
                         ...
                         DSAT_EXT_DR_IDX,0,0,NUM_TYPE) == 1)
  {
    ...
    {
      DS_ATCOP_ERROR_LOG_1("Incorrect return code type =d", compression);
      return;
    }
...
(void) dsatutil_memscpy((void*)dsat_report_rate_val.rate.ps_call.apn,
  ...
  {
   DS_ATCOP_ERROR_LOG_0("Invalid call type for connect rate updation");
  }
...
)
{
  ...
  {
    DS_ATCOP_ERROR_LOG_1("Free item timer handler incorrectly called: timer ID - =d", 
               *timer_id_ptr );
