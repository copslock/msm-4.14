...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/3gpp/dsmgr/inc/dstask_v.h#14 $ $DateTime: 2020/03/23 02:00:38 $ $Author: sdhoot $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/10/19   my      Added new s2b cmd for epdg_call_setup and throttle timer cb
04/03/19   my      Added mppm cmd DS_CMD_MPPM_XCAP_CONFIG_EVENT
01/15/19  scb      Added support in MPPM to handle RPLMN CHG event
11/21/18   js      Added s2b cmd for DPD check
10/10/18   ak      Added s2b cmd for resolver bring up cmd 
07/26/18   ak      Added mppm and s2b cmd for device mode 
06/08/18   js      Added new s2b cmd for sub available
...
04/30/15   cb      Added new cmd for WFC RAT preference
04/22/15   fn      Added new cmd for ANDSF event processing
04/16/15   cb      Added new s2b commands to handle wp flow enabled/disabled   
02/20/15   sd      FC Task removal changes.
02/19/15   ab      Added a new s2b command to handle wp addr change ev
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  IPA WAIT signals
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
#define DS_IPA_REG_WAIT_SIGNAL      0x02000000  
...
typedef enum
{
  ...
  DS_CMD_FLOW_MGR_TIMER_EXPIRED                = 5,
  /* Flow manager timer expired */

  DS_CMD_MCFG_REFRESH_IND                      = 6,
  /* command to process MCFG refresh ind */

  DS_CMD_CFCM_THERMAL_RUNAWAY_IND              = 7,
  /* command to process CFCM thermal runaway ind */

  DS_CMD_DS3GTPUTMGR_TIMER_EXPIRED             = 8,
  /* Notification of timer expiry */

  DS_CMD_NOTIFY_IMS_PDN_STATUS                 = 9,
  /* Cmd to notify IMS PDN status */

  DS_CMD_SIO_TIMER_EXPIRED                     = 20,
  /* Notification of SIO timer expiry */
  ...
  DS_CMD_SIO_CLOSE_COMPLETE                    = 23,
  /* Cmd from SIOLIB when sio_close is done */

  DS_CMD_SIO_DSM_MEM_CTRL                      = 24,
  /* Cmd from SIOLIB */

  DS_CMD_SIO_DTR_CHANGED                       = 25,
  /* DTR changed event notification */

  DS_CMD_SIO_CD_CHANGED                        = 26,
  /* CD changed event notification */

  DS_CMD_ENTER_ONLINE_CMD_TX_FLUSH             = 27,
  /* Goto online flush TX */

  DS_CMD_ENTER_ONLINE_DATA_TX_FLUSH            = 28,
  /* Goto online-data, flush TX */

  DS_CMD_ENTER_ONLINE_DATA_NO_TX_FLUSH         = 29,
  /* Goto online-data, don't flush TX */

  DS_CMD_COMPLETE_ONLINE_CMD_SWITCH            = 30,
  /* OK to finish online-cmd switch   */

  DS_CMD_COMPLETE_ONLINE_DATA_SWITCH           = 31,
  ...
  DS_CMD_CM_SS_EV_MAX_BEARER_SUPPORT           = 103,
  /* Cmd for  DS_CMD_CM_SS_EV_MAX_BEARER_SUPP */
  ...
  DS_CMD_CM_PH_SUB_NOT_AVAIL                   = 112,
  /* Cmd for CM SUB info - not avail */
  ...
  DS_CMD_CM_TAC_OR_LAC_CHG                     = 124,
  /*  Cmd for tac/lac change */
  ...
  DS_CMD_DS3GDSDIF_MT_CSFB_HYST_TIMER_EXP      = 165,
  /* Cmd for DS3GDSDIF MT CSFB estab hysteresis timer expired */

  DS_CMD_DSD_LTE_EMERG_CALL_EVENT              = 166,
  /*Cmd for LTE PS Emergency call is in progress*/

  DS_CMD_PROCESS_UL_TPUT_TIMER_CMD             = 167,
  ...
  DS_CMD_DSMGR_LOW_LATENCY_CMD                 = 174,
  /*Cmd for Diag request from QXDM*/
  DS_CMD_PROCESS_ULDL_TIMER_CMD                = 175,

  DS_CMD_DSD_APM_DEL_ATTACH_PROFILES           = 176,
 /* Cmd for deleting attach profiles */

  DS_CMD_DSD_APM_SET_ATTACH_PDN_LIST           = 177,
  /* Cmd for APM SET attach pdn list */

  DS_CMD_DSD_SUBS_CAPABILITY_CHG               = 178,
  /* Cmd for subs capability change */

  DS_CMD_PROCESS_DL_TPUT_TIMER_CMD             = 180,

  DS_CMD_DS3G_DDS_SWITCH_REQ                   = 182,
  /* Cmd to handle DDS switch request */

  DS_CMD_DS3G_DDS_SWITCH_ALLOWED_ERR           = 183,
  /* Cmd to handle DDS switch allowed request error  */

  DS_CMD_DS3G_DUAL_STANDBY_PREF_ERR            = 184,
  /* Cmd to handle DDS switch request error */
  DS_CMD_DS3G_PS_REG_REQ                       = 185,
  /* Cmd for PS registration trigger*/
#ifdef FEATURE_DS_QSH_INTERFACE
  DS_CMD_PROCESS_QSH_CMD                       = 186,
  /*Cmd to process QSH cmd*/
#endif /* FEATURE_DS_QSH_INTERFACE */
#ifdef FEATURE_LAPP
  DS_CMD_DSD_NR5G_ACQ_LOST_EVENT               = 187,
  /* Cmd to handle 5G state change */

  DS_CMD_DSD_NR5G_APN_INFO_EVENT               = 188,
  /* Cmd to handle APN info event for NSA */
#endif /* FEATURE_LAPP */

#ifdef FEATURE_DATA_NR5G_SA 
  DS_CMD_CM_PDU_SESSION_CB_ERR                 = 189,

  DS_CMD_CM_PDU_SESSION_EST_ACCEPT             = 190,

  DS_CMD_CM_PDU_SESSION_EST_REJECT             = 191,

  DS_CMD_CM_PDU_SESSION_RELEASE_CMD            = 192,
  
  DS_CMD_CM_PDU_SESSION_MODIFY_CMD             = 193,

  DS_CMD_CM_PDU_SESSION_RELEASE_REJ            = 194,

  DS_CMD_3GPP_NR5G_BEARER_UPDATE_IND           = 195,

  DS_CMD_CM_SS_EV_SESSION_REACTIVATION_IND     = 196,
  /* Cmd for CM_SS_EVENT_SESSION_REACTIVATION_IND */
 
  DS_CMD_CM_SS_EV_SESSION_REACTIVATION_REJ     = 197,
  /* Cmd for CM_SS_EVENT_PDU_SESSION_REACTIVATION_REJ */

  DS_CMD_CM_SS_EV_BEARER_UPDATE_IND_TIMER_EXP  = 198,

  DS_CMD_CM_PDU_AUTH_COMPLETE_CB_ERR  = 199,

  DS_CMD_CM_PDU_SESSION_AUTH_CMD = 200,

  DS_CMD_CM_PDU_SESSION_AUTH_COMPLETE_RSP = 201,
#endif /*FEATURE_DATA_NR5G_SA */

  /*-------------------------------------------------------------------------
    Logging
  -------------------------------------------------------------------------*/
  DS_LOGGING_CMD                               = 202,
  DS_CMD_DS3G_TIMER_EXPIRED                    = 203,
  /* Notification of timer expiry */
  /*-------------------------------------------------------------------------
    DSSNET6 commands
  -------------------------------------------------------------------------*/
  DS_CMD_DSSNET6_PHY_LINK_UP_EV                = 204,
  DS_CMD_DSSNET6_PHY_LINK_GONE_EV              = 205,
  DS_CMD_DSSNET6_PHY_LINK_DOWN_EV              = 206,
  DS_CMD_DSSNET6_LINK_UP_EV                    = 207,
  DS_CMD_DSSNET6_LINK_DOWN_EV                  = 208,
  DS_CMD_DSSNET6_LINK_RESYNC_EV                = 209,
  DS_CMD_DSSNET6_NET_UP_EV                     = 210,
  DS_CMD_DSSNET6_NET_DOWN_EV                   = 211,
  DS_CMD_DSSNET6_NET_ABORTED_EV                = 212,
  DS_CMD_DSSNET6_PPP_CLOSE_EV                  = 213,
  DS_CMD_DSSNET6_FLOW_DELETED_EV               = 214,
  DS_CMD_DSSNET6_RS_RA_START_EV                = 215,
  /* End of DSMGR Commands */

  DS_CMD_CM_HANDLE_MODIFY_PDU_SESSION_REQ_ERR  = 216,
  DS_CMD_CM_PDU_SESSION_MODIFY_REJ             = 217,

  DS_CMD_DS3G_UI_DISPLAY_HDLR_SYS_INFO         = 218,

  DS_CMD_DSD_NR5G_EMERG_CALL_EVENT              = 219,
  /*Cmd for NR5G PS Emergency call is in progress*/
  DS_CMD_DS3G_MO_DATA_PROTECT_TIMER_EXPIRED    = 220,
  /*Cmd for TRM flow activity timer*/
  DS_CMD_DS3G_TRM_FLOW_ACTIVITY_TIMER_EXPIRED  = 221,

#ifdef FEATURE_DATA_NR5G_SA 
  DS_CMD_3GPP_RESTRICTED_SERVICE_AREA_IND      = 222,
#endif /*FEATURE_DATA_NR5G_SA */

  DS_CMD_DS3G_FLUSH_DATA_CMD                   = 223,


#ifdef FEATURE_DATA_NR5G_SA 
  DS_CMD_3GPP_EPS_BEARER_CTXT_STATUS_IND       = 224,

  DS_CMD_NR5G_PDU_MODIFY_REJ_BACKOFF_TIMER_EXP = 225,
#endif /*FEATURE_DATA_NR5G_SA */
  /*-------------------------------------------------------------------------
  ...
  DS_CMD_3GPP_PERIODIC_TMR_EXP                          = 1022,
  /* Cmd for notifying hysterisis  timer expired */

  DS_CMD_3GPP_RRC_RESUME_FROM_INACTIVE            = 1023,
  /* Cmd for rrc_resume_req*/

  DS_CMD_3GPP_RRC_RESUME_RETRY_TMR_EXP            = 1024,
  /* Cmd for rrc_resume_exp*/
  ...
  DS_CMD_3GPP_GSNDCP_DEREG_WM_CNF                 = 1092,
  /* Cmd for confirmation of GSNDCP WM dereg */
  ...
  DS_CMD_3GPP_KAMGR_SYS_INFO                      = 1124,
  /* Cmd for KAM to process sys info */

  DS_CMD_3GPP_KAMGR_TIMER_EXPIRY_CB               = 1125,
  /* Cmd for KAM timer expiry */

  DS_CMD_3GPP_KAMGR_QMI_BRING_UP_IND              = 1126,
  /* Cmd for KAM QMI bring-up */

  DS_CMD_3GPP_KAMGR_QMI_TEAR_DOWN_IND             = 1127,
  /* Cmd for KAM QMI tear-down */

  DS_CMD_3GPP_KAMGR_DEFAULT_APN_CHG_IND           = 1128,
  /* Cmd to indicate KAMGR for change in default apn */
  ...
  DS_CMD_3GPP_UNUSED_PDN_TMR_EXP_CMD              = 1170,
  /* Cmd for unused PDN timer*/

  DS_CMD_3GPP_GENERIC_THROT_TIMER_EXP             = 1171,
  /* Cmd for Generic Throt Expiry*/
  ...
  DS_CMD_3GPP_PROCESS_ACTIVITY_REG                =1183,
  /* Cmd for handling data activity client reg */
  
  DS_CMD_DS3G_REPROCESS_UT_CONFIG              = 1184,
  ...
  DS_CMD_3GPP_PROFILE_PERSISTENT_OPTIMIZER        = 1194,
  /*cmd for persistent profile optimizer*/
  ...
  DS_CMD_3GPP_RMSM_STOP_PPP_DELAY_TIMER           = 1243,
  /* Cmd for RMSM stop ppp delay time expiry */

  DS_CMD_3GPP_RMSM_PPP_UP_DELAY_TIMER             = 1244,
  /* Cmd for RMSM ppp UP delay timer expiry */
  ...
  DS_CMD_3GPP_RPM_SAMPLING_TIMER_EXP              = 1264,
  /* Cmd for RPM sampling timer expiring */

  DS_CMD_3GPP_RRC_PCH_IND_PROC                    = 1280,
  /* Cmd for RRC PCH indication processing */

  DS_CMD_3GPP_RRC_STATE_IND_PROC                  = 1281,
  /* Cmd for RRC PCH indication processing */
#ifdef FEATURE_LAPP
  DS_CMD_3GPP_UI_DISPLAY_STATUS_CHG               = 1282,
  /* Cmd for UI display status change */
#endif

  DS_CMD_3GPP_AP_SLEEP_STATUS_IND                 = 1283,
  /* Cmd for AP sleep status indication */
  ...
  DS_CMD_3GPP_CM_PH_OPRT_MODE_CB_ERR              = 1294,

  DS_CMD_3GPP_CM_PH_OPRT_MODE_CB_NO_ERR           = 1295,

  DS_CMD_3GPP_CM_PH_INFO_CB_ERR                   = 1296,

  DS_CMD_3GPP_CM_PH_INFO_CB_NO_ERR                = 1297,

  DS_CMD_3GPP_TD_CONFIG_UL_DATA_RATE              = 1300,
  /* Cmd for configuring TD UL data rate */

  DS_CMD_3GPP_PROC_WRRC_DATA_RATE_IND             = 1301,
  /* Cmd for processing WRRC data rate ind */
  ...
  DS_CMD_3GPP_UMTS_LB_MODE4                       = 1321,
  /* Cmd for handling umts loop mode4  */

  DS_CMD_3GPP_UMTS_LB_CLOSE_READY                 = 1322,
  /* Cmd for posting close ready */
  ...
  DS_CMD_3GPP_LB_UL_RAB_REG_CNF                   = 1357,

  DS_CMD_3GPP_LB_DL_RAB_REG_CNF                   = 1358,

  DS_CMD_3GPP_PDN_APN_PARAM_CHG_TMR_EXP           = 1359,

  DS_CMD_3GPP_DO_SILENT_REDIAL                    = 1360,

  DS_CMD_3GPP_CONFIGURE_ULDL_DATA_RATE            = 1361,

  DS_CMD_3GPP_IFACE_TEARDOWN_INVALID_DNS_ADDR     = 1362,
  /* Cmd for PDN teardown in case of INVALID DNS */

  DS_CMD_3GPP_IFACE_TEARDOWN_INVALID_PCSCF_DNS_ADDR = 1363,

  DS_CMD_3GPP_UPLINK_DATA_RATE_IND                  = 1364,

  DS_CMD_3GPP_DOWNLINK_ESTM_DATA_RATE_IND           = 1365,
  /* Cmd for PDN teardown in case of INVALID DNS */

  DS_CMD_3GPP_APM_WRITE_ATTACH_PROFILE_INFO         = 1366,
  /*Cmd for writing attach profile info into EFS*/

  DS_CMD_3GPP_PLMN_LIST_CHG                         = 1367,
  /* Cmd for PLMN List change processing */

  DS_CMD_3GPP_PRE_ATTACH_TIMER_EXP                 = 1368,
  /*Cmd for pre attach timer callback*/

  DS_CMD_3GPP_PRE_ATTACH_SYS_INFO                  = 1369,
  /*Cmd for PS SYS EV received*/

  DS_CMD_3GPP_WRITE_HPLMN_LIST                     = 1371,
  /* Cmd for writing hplmn list into EFS */

 /* Cmd for 3GPP iface event callback handler */
  DS_CMD_3GPP_INVOKE_IFACE_EV_CB_HDLR               = 1372,

  /* Cmd for pref sys changed APN API Override List */
  DS_CMD_3GPP_APN_PREF_SYS_CHG                     = 1373,

  DS_CMD_3GPP_APN_SWITCH_SYS_INFO                  = 1374,
  /*Cmd for LTE PARA SYS EV received*/
  
  DS_CMD_3GPP_NON_EMPTY_CB_CMD                     = 1375,
  /*Cmd for non_empty cb processing*/
    DS_CMD_3GPP_FLOW_DISABLE_TMR_EXP               = 1376,
  /*Cmd for 3GPP Flow Disable Timer Expiry*/

  DS_CMD_3GPP_DATA_SAMPLING_TMR_EXP                = 1377,
  /* Cmd for notifying data sampling timer expired */
  DS_CMD_3GPP_UDT_BARRING_TIMER_EXPIRED            = 1378,
  /* Cmd for handling UDT barring timer expiry */
  DS_CMD_3GPP_SET_UL_TPUT_THRES_CRITERIA           = 1379,
  /* Cmd for setting UL throughput threshold criteria */ 
  DS_CMD_3GPP_SET_DL_TPUT_THRES_CRITERIA           = 1380,
  /* Cmd for setting DL throughput threshold criteria */

  DS_CMD_3GPP_GRANT_BYTES_RECEIVED                  = 1381,
  /* Cmd for grant bytes received */

  DS_CMD_3GPP_FLOW_CTL_CMD                          = 1382,
  /* Cmd for wm flow enable and disable */

  DS_CMD_3GPP_FLOW_ACK_TMR_EXP                      = 1383,
  /* Cmd for flow ack timer expiry */

  DS_CMD_3GPP_FLOW_QMAP_ACK_CNF                     = 1384,
   /* Cmd for flow ack qmap ack cnf */

  DS_CMD_3GPP_LB_UL_RAB_DEREG_CNF                   = 1385,

  DS_CMD_3GPP_DPM_INFO_CNF                          = 1386, 
  
  DS_CMD_3GPP_PDN_TCP_RETRANS_PROTECT_TMR_EXP       = 1387,

  DS_CMD_3GPP_IFACE_TCP_LEVEL_TRM_REQ_INFO          = 1388,

  DS_CMD_3GPP_OMADM_SET_PSDATAOFF_INFO              = 1389,

  DS_CMD_3GPP_OTASN_PDN_TMR_EXP                     = 1390,
  /* Cmd for notifying OTASN PDN timer expired */

  DS_CMD_3GPP_GONE_EMPTY_CB                         = 1391,
   /* Cmd for gone empty callback */

  /* 
   * Cmd used for logging flow control latency
   * information 
  */ 
  DS_CMD_3GPP_LOG_FLOW_CTRL_LATENCY                 = 1392,

  /*
   * Cmd used for processing the phys link event callback
   * called from the PS task
   */
  DS_CMD_3GPP_PHYS_LINK_EV_CB                       = 1393,

  /* Cmd for notifying Prohibit timer expired */
  DS_CMD_3GPP_PROH_TIMER_CMD                        = 1394,

  /*cmd to process back  pressure*/
  DS_CMD_3GPP_BEARER_CNTXT_TMR_DYN_BACK_PRESSURE    = 1395,

  DS_CMD_3GPP_LB_DL_RAB_DEREG_CNF                   = 1396,

  /*cmd to process nsa bearer config change */
  DS_CMD_3GPP_BEARER_CONFIG_CHG_IND                 = 1397,
  /* cmd used to perform NSA switch queue
     operations */
  DS_CMD_3GPP_BEARER_Q_OPS_IND                      = 1399,

  /* cmd used to perform NSA switch failure
     related operations */
  DS_CMD_3GPP_NSA_SWITCH_FAILURE_IND               = 1400,

  /* Cmd for notifying BURST INTERVAL calculation enabled */
  DS_CMD_3GPP_SET_DL_BURST_INTERVAL_CALC            = 1402,
  DS_CMD_3GPP_OMADM_SET_XCAP_CFG_INFO               = 1403,

  DS_CMD_3GPP_BEARER_TRANSCATION_COMPLETE_IND       = 1404,

  DS_CMD_3GPP_LB_SET_LB_REQ                         = 1405,

#ifdef FEATURE_DATA_NR5G_SA 
  DS_CMD_3GPP_UE_INIT_MODIFY_QOS                    = 1406,
#endif
  DS_CMD_3GPP_GET_L2NR_CRITERIA_CMD                 = 1407,

#ifdef FEATURE_DATA_NR5G_SA
  DS_CMD_NR5G_URSP_IWLAN_HO_WAIT_TMR_EXP            = 1408,
#endif /* FEATURE_DATA_NR5G_SA */

  /* Cmd used for unblocking slice throttling*/
  DS_CMD_3GPP_SLICE_THROT_UNBLOCK_SLICE_TIMER_EXP   = 1409,

  /* Cmd used for indicating any change in profile */
  DS_CMD_3GPP_PROFILE_DB_GENERIC_CHANGE_IND         = 1410,

  DS_CMD_3GPP_APN_SWITCH_DETACH_OR_DISCONNECT       = 1411,

  DS_CMD_3GPP_FLOW_CONTROL_TMR_FCF                  = 1412,
  /*-------------------------------------------------------------------------
  ...
  DS_CMD_707_DELAY_OTASN_PDN_TEARDOWN_TIMER_EXPIRED
                                                  = 2071,
...
/*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
  DS_CMD_RSVP_SOCKET_EVENT                        = 2125,

  /*-------------------------------------------------------------------------
    Cmd to process notification from CM 
  -------------------------------------------------------------------------*/
  DS_CMD_707_MCFG_NOTIFY                          = 2126,

  /*-------------------------------------------------------------------------
    Cmd to process notification from DS3G for Thermal mitigation 
  -------------------------------------------------------------------------*/
  DS_CMD_3GPP2_TM_NOTIFY                          = 2127,

  /*-------------------------------------------------------------------------
    SRLTE Enabled . 
  -------------------------------------------------------------------------*/
  DS_CMD_CAM_SRLTE_ENABLED                         = 2128,

  /*-------------------------------------------------------------------------
    Cmd to process notification from DS profile  for MIN APN updation 
  -------------------------------------------------------------------------*/
  DS_CMD_707_MANDATORY_APN_UPDATE                  = 2129,

  /*-------------------------------------------------------------------------
    Cmd to process notification from DS mmgsdi for subscription ready
  -------------------------------------------------------------------------*/
  DS_CMD_707_SUBSCRIPTION_READY                    = 2130,

  DS_CMD_707_PRI_FLOW_DISABLE_TMR_EXPIRY           = 2131,

#ifdef FEATURE_DATA_FLOW_MGMT
  /*-------------------------------------------------------------------------
    This command is used to disable flow on a particular physlink after
    receiving flow control IOCTL from PS
  -------------------------------------------------------------------------*/
  DS_CMD_707_PHYS_LINK_DISABLE_FLOW                = 2132,

  /*-------------------------------------------------------------------------
    This command is used to enable flow on a particular physlink after
    receiving flow control IOCTL from PS
  -------------------------------------------------------------------------*/
  DS_CMD_707_PHYS_LINK_ENABLE_FLOW                 = 2133,

  /*--------------------------------------------------------------------------
    This command is posted every 50 ms. It increments the unblock_timer_count
    upon each expiry and tries to enable flow on that physlink after the
    count reaches 10 (if current WM count < low WM)
  ---------------------------------------------------------------------------*/
  DS_CMD_707_BURST_PERIODIC_TMR_EXPIRY           = 2134,

  /*--------------------------------------------------------------------------
    This command is posted when WM goes empty. As part of this flow enable   
    indication will be sent to AP
  ---------------------------------------------------------------------------*/
  DS_CMD_707_BURST_WM_GONE_EMPTY                   = 2135,
  /*-------------------------------------------------------------------------
    This command is posted as part of handling the PS IOCTL to get the
    allowed grant bytes on a particular physlink
  -------------------------------------------------------------------------*/
  DS_CMD_707_BURST_RECVD_GRANT_BYTES               = 2136,

  /*--------------------------------------------------------------------------
    This command is posted every 10 ms. Prohibit timer expiry
  ---------------------------------------------------------------------------*/
  DS_CMD_707_BURST_PROHIBIT_TMR_EXPIRY           =   2137,
#endif /* FEATURE_DATA_FLOW_MGMT */

  DS_CMD_707_SYNC_ESPM_IND                         = 2138,
  ...
  DS_CMD_WLAN_PROXY_WLAN_UNAVAILABLE                 = 3008,
  ...
  DS_CMD_EPC_NV_REFRESH                        = 4014,
  DS_CMD_EPC_PENDING_TEARDOWN_TIMER_EXP_PROC   = 4015,
 ...
 /* DS_CMD_MPPM_NULL_APN_RESOLVED_INFO callback cmd */
  ...
  DS_CMD_MPPM_ROAMING_STATUS_EVENT              = 5014,
  /* DS3GEVENTMGR_ROAMING_STATUS_EV Event callback cmd*/

  DS_CMD_MPPM_ROAMING_PROFILE_UPDATE_EVENT_CB    = 5015,
  /* DS Profile update Event callback cmd*/

  DS_CMD_MPPM_PROFILE_UPDATE_EVENT_CB = 5016,
  /* DS Profile update Event callback cmd*/

  DS_CMD_MPPM_ANDSF_EVENT_CB                   = 5017,
  /* Andsf event cmd*/

  DS_CMD_MPPM_RAT_PREFERENCE                    = 5018,
 /* DS_CMD_MPPM_RAT_PREFERENCE callback cmd */

  DS_CMD_MPPM_DATA_ROAMING_UI_CHANGED_EVENT     = 5019,
  /* DS3GEVENTMGR_DATA_ROAMING_CHANGED_EV Event callback cmd*/

  DS_CMD_MPPM_WIFI_SETTINGS_CHANGE               = 5020,
 /* DS_CMD_MPPM_RAT_PREFERENCE callback cmd */

  DS_CMD_MPPM_WLAN_IDI_TYPE_EVENT                = 5021,
  /* DS3GEVENTMGR_WLAN_IDI_TYPE_EV Event callback cmd*/

  DS_CMD_MPPM_IMSI_INFO_EVENT                    = 5022,
  /* DS3GEVENTMGR_MMGSDI_IMSI_INFO_EV Event callback cmd*/

  DS_CMD_MPPM_IMPI_INFO_EVENT                    = 5023,
  /* DS3GEVENTMGR_MMGSDI_IMPI_INFO_EV Event callback cmd*/

  DS_CMD_MPPM_WLAN_CAP_SUBS_CHANGE               = 5024,
  /* DS_CMD_MPPM_WLAN_CAP_SUBS_CHANGE callback cmd */

  DS_MPPM_WWAN_THROTTLE_TIMER_CB_CMD             = 5025,
  /* DS_MPPM_WWAN_THROTTLE_TIMER_CB callback cmd */

  DS_CMD_MPPM_IWLAN_S2B_THROTTLE_INFO_CMD        = 5026,
  /*Handle IWLAN_S2B_THROTTLE info cmd*/

  DS_CMD_MPPM_EPDG_ADDR_USABILITY_EVENT          = 5027,
  /*Handle EPDG address usability cmd*/

  DS_CMD_MPPM_WLAN_PREF_IP_AVAILABLE_EVENT       = 5028,
  /*Handle DS3GEVENTMGR_WLAN_PREF_IP_AVAILABLE_EV info cmd*/

#ifdef FEATURE_LAPP
  DS_CMD_MPPM_5G_APN_INFO_EVENT                  = 5029,
  /*Handle DS3GEVENTMGR_5G_APN_INFO_EV Event callback cmd*/
#endif /*FEATURE_LAPP */

  DS_CMD_MPPM_IWLAN_UNAUTH_EMERGENCY_STATUS_EVENT       = 5030,
  /*Handle DS3GEVENTMGR_IWLAN_UNAUTH_EMERGENCY_STATUS info cmd*/

  DS_CMD_MPPM_AP_ASSIST_MODE_EVENT               = 5031,
  /*Handle PS_SYS_EVENT_AP_ASST_APN_PREF_SYS_MODE info cmd*/

  DS_CMD_MPPM_AP_ASSIST_APN_PREF_SYS_CHANGE      = 5032,
  /*Handle PS_SYS_EVENT_AP_ASST_APN_PREF_SYS_CHANGE info cmd*/

  DS_CMD_MPPM_T_WIFI_SWITCH_HYS_TIMER_CB			  = 5033,
  /* WIFI_SWITCH_HYS timer callback cmd*/

  DS_CMD_MPPM_DEVICE_MODE_EVENT_CB           = 5034,
  /*Handle DS3GSUBSMGR_DEVICE_MODE_EV Event callback cmd*/

  DS_CMD_MPPM_AP_ASSIST_IWLAN_S2B_CAPABILITY_EVENT      = 5035,
  /*Handle PS_SYS_EVENT_WLAN_SET_WLAN_CAPABILITY info cmd*/

  DS_CMD_MPPM_XCAP_CONFIG_EVENT           = 5036,
  /*Handle DS_CMD_MPPM_XCAP_CONFIG_EVENT info cmd*/

  DS_CMD_MPPM_AP_ASSIST_INTENT_RESP_TIMER_CB             = 5037,
  /* AP ASSIST INTENT Resp timer callback cmd*/

#ifdef FEATURE_DATA_NR5G_SA
  DS_CMD_MPPM_RPLMN_CHG_EVENT                            = 5038,
#endif /*FEATURE_DATA_NR5G_SA */

  DS_CMD_MPPM_PROFILE_READY_EVENT_CB                     = 5039,
  /* Profile ready event callback cmd */  
 ...
 /*-------------------------------------------------------------------------
   ...
   DS_CMD_ATCOP_VOICE_CONF_PARTICIPANTS_IND_CMD  = 6051,
   DS_CMD_ATCOP_EMBMS_AVAIL_TMGI_LIST            = 6052,
   DS_CMD_ATCOP_EMBMS_INTEREST_TMGI_LIST         = 6053,
   DS_CMD_ATCOP_EMBMS_SAI_LIST_UPDATE            = 6054,
   DS_CMD_ATCOP_EMBMS_ACTIVE_TMGI_LIST           = 6055,
   DS_CMD_ATCOP_SETEMP_REQ_CMD                   = 6056,
   DS_CMD_ATCOP_EMBMS_TMGI_DEACT_CNF             = 6066,
   DS_CMD_ATCOP_EMBMS_TMGI_ACT_CNF               = 6067,
   DS_CMD_ATCOP_SRVCC_HO_COMPLETE_IND            = 6068,
   DS_CMD_ATCOP_IMS_SRV_STATUS_IND               = 6069,
  ...
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_IFACE_ENABLED  = 7015,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_TIMER_CALLBACK = 7016,
  DS_CMD_IWLAN_S2B_IFACE_IPSEC_IPFLTR_CHANGED      = 7017,
  DS_CMD_IWLAN_S2B_WP_FLOW_CHANGED                 = 7018,
  DS_CMD_IWLAN_S2B_PLMN_LIST_CHANGED               = 7019,
  DS_CMD_IWLAN_S2B_WP_INFO_CHANGED                 = 7020,
  DS_CMD_IWLAN_S2B_PDN_STATUS_CHANGED              = 7021,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_TTL_TIMER_CALLBACK = 7022,
  DS_CMD_IWLAN_S2B_LPM_MODE                        = 7023,
  DS_CMD_IWLAN_S2B_WIFI_OFF                        = 7024,
  DS_CMD_IWLAN_S2B_PDN_KEEP_ALIVE_GUARD_TIMER_CB   = 7025,
  DS_CMD_IWLAN_S2B_IKEV2_PROCESS_NAT_KA_STATUS     = 7026,
  DS_CMD_IWLAN_S2B_NAT_KA_OFFLOAD_GUARD_TIMER_CB   = 7027,
  DS_CMD_IWLAN_S2B_PDN_HO_GUARD_TIMER_CB           = 7028,
  DS_CMD_IWLAN_S2B_SUBS_NA_PDN_KA_GUARD_TIMER_CB   = 7029,
  DS_CMD_IWLAN_S2B_IMSI_AVAIL_EV                   = 7030,
  DS_CMD_IWLAN_S2B_TRANSPORT_CONFIG                = 7031,
  DS_CMD_IWLAN_S2B_IMPI_INFO_CB                    = 7032,
  DS_CMD_IWLAN_S2B_CONFIG_VOWIFI_SRVC_EV           = 7034,
  DS_CMD_IWLAN_S2B_IWLAN_DISABLED                  = 7035,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_TTL_TIMER_v6_CALLBACK = 7036,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_TEARDOWN_SM    = 7037,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_RESOLVE_DNS_SM = 7038,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_PCO_ADDR              = 7039,
  DS_CMD_IWLAN_S2B_EPDG_PING_MGR_SESS_CALLBACK     = 7040,
  DS_CMD_IWLAN_S2B_EPDG_PING_MGR_WP_IFACE_UP       = 7041,
  DS_CMD_IWLAN_S2B_EPDG_PING_MGR_WP_IFACE_DOWN     = 7042,
  DS_CMD_IWLAN_S2B_EPDG_PING_MGR_FAILURE_RETRY_TIMER_CALLBACK = 7043,
  DS_CMD_IWLAN_S2B_RPLMN_CHANGED                   = 7044,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_RESOLVE_FQDN_SM = 7045,
  DS_CMD_IWLAN_S2B_SET_APN_INFO_EV                  = 7046,
  DS_CMD_IWLAN_S2B_IMSI_PUBLIC_KEY_UPDATE_EV        = 7047,
  DS_CMD_IWLAN_S2B_EM_CALL_DOWN                     = 7048,
  DS_CMD_IWLAN_S2B_EM_CALL_WAITING_GUARD_TIMER_CB   = 7049,
  DS_CMD_IWLAN_S2B_LEVEL_THROTTLE_PDN_TIMER_CB      = 7050,
  DS_CMD_IWLAN_S2B_LEVEL_THROTTLE_PLMN_TIMER_CB     = 7051,
  DS_CMD_IWLAN_S2B_LEVEL_THROTTLE_EPDG_TIMER_CB     = 7052,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_DNS_RESOLVED_SM = 7053,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_THROTTLE_EV     = 7054,
  DS_CMD_IWLAN_S2B_ALL_NON_EM_CALLS_DOWN            = 7055,
  DS_CMD_IWLAN_S2B_NON_EM_CALLS_TEARDOWN_WAITING_GUARD_TIMER_CB = 7056,
  DS_CMD_IWLAN_S2B_TAC_LAC_INFO_CB                  = 7057,
  DS_CMD_IWLAN_S2B_EPDG_SELECTION_MGR_DNS_CALLBACK  = 7058,
  DS_CMD_IWLAN_S2B_EPDG_SELECTION_MGR_NAPTR_TTL_TIMER_CALLBACK = 7059,
  DS_CMD_IWLAN_S2B_EPDG_SELECTION_MGR_NAPTR_RETRY_TIMER_CALLBACK = 7060,
  DS_CMD_IWLAN_S2B_ALL_NON_REDIRECT_PDNS_DOWN       = 7061,
  DS_CMD_IWLAN_S2B_REDIRECT_PDN_WAITING_GUARD_TIMER_CB   = 7062,
  DS_CMD_IWLAN_S2B_REDIRECT_HO_HYSTERESIS_TIMER_CB   = 7063,
  DS_CMD_IWLAN_S2B_EMERG_CALL_STATUS_CB              = 7064,
  DS_CMD_IWLAN_S2B_VOICE_CALL_STATUS_CB              = 7065,
  DS_CMD_IWLAN_S2B_LTE_HO_FAIL_54_EV_CB              = 7066,
  DS_CMD_IWLAN_S2B_SUB_AVAIL_EV                      = 7067,
  DS_CMD_IWLAN_S2B_DEVICE_MODE_EVENT_CB              = 7068,
  DS_CMD_IWLAN_S2B_EPDG_ADDR_RESLVR_V4_OR_V6_CALL_BRINGUP = 7069,
  DS_CMD_IWLAN_S2B_IKEV2_PROCESS_DPD_CHECK           = 7070,
  DS_CMD_IWLAN_S2B_IOCTL_IKEV2_PING                  = 7071,
  DS_CMD_IWLAN_S2B_IKEV2_PING_CBACK                  = 7072,
  DS_CMD_IWLAN_S2B_EPDG_CALL_SETUP_TIMER_CB          = 7073,
  DS_CMD_IWLAN_S2B_THROTTLE_TIMER_CB                 = 7074,
  ...
  /*-------------------------------------------------------------------------
    External commands for OEM (9000 - 9100)
  -------------------------------------------------------------------------*/
