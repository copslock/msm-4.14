...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/inc/dsatetsicall.h#1 $ $DateTime: 2019/10/12 17:41:50 $ $Author: mplcsds1 $
...
typedef enum ccfc_reason_e
{
  ...
  DSAT_CCFCU_REASON_COMM_DEF = 6,
  DSAT_CCFCU_REASON_COMM_NOT_LOGGED_IN = 7,
  DSAT_CCFCU_REASON_MAX
} ccfc_reason_e_type;

typedef enum ccfcu_numbertype_e
{
  DSAT_CCFCU_NUMBERTYPE_INVALID        = 0,
  DSAT_CCFCU_NUMBERTYPE_URI            = 1,
  DSAT_CCFCU_NUMBERTYPE_NUMBER         = 2,
  DSAT_CCFCU_NUMBERTYPE_MAX
}dsat_ccfcu_numbertype_e_type;
...
typedef struct ss_cf_user_s
{
  ...
  cm_call_forward_number_e_type fwd_type;         /* Forwarded to number or URI */
  uint8 type;                                     /* Address type     */
  uint8 satype;                                   /* Subaddress type  */
  uint8 time;                                     /* Time value       */
  cm_ip_sups_time_info_s_type  sups_time_info;    /* Time based parameter */
  ...
  cm_forward_to_addr_uri_s_type *fwd_to_uri;      /* Forwarded to URI */
} ss_cf_user_s_type;
...
typedef enum
{
  DSAT_NEG_STATUS_NONE = 0,
  DSAT_NEG_STATUS_DESIRED = 1,
  DSAT_NEG_STATUS_PROPOSED = 2,
  DSAT_NEG_STATUS_ACCEPT = 3,
  DSAT_NEG_STATUS_REJECT = 4
}dsat_neg_status_e_type;

/* +CMCCS Command */

typedef enum
{
  DSAT_CMCCS_VAL_DISABLED = 0,
  DSAT_CMCCS_VAL_LIST_SUPPORTED_SUPS = 1,
  DSAT_CMCCS_VAL_ENABLED_BASIC = 2,
  DSAT_CMCCS_VAL_ENABLED_BASIC_SUPS = 3
}dsat_cmccs_val_e_type;

typedef enum
{
  DSAT_CMCCSI_NUMBERTYPE_NOT_VALID = 0,
  DSAT_CMCCSI_NUMBERTYPE_URI_FORMAT = 1,
  DSAT_CMCCSI_NUMBERTYPE_GENERIC_FORMAT =2
}dsat_cmccsi_numbertype_e_type;

/* +QCCALLATTR Command */
/* Call type */
typedef enum
{
  DSAT_QCCALLATTR_CALL_TYPE_VOICE = 0,
  DSAT_QCCALLATTR_CALL_TYPE_VT = 1
}dsat_qccallattr_call_type_e_type;
/* Call Attribute */
typedef enum
{
  DSAT_QCCALLATTR_CALL_ATTR_TXRX = 0,
  DSAT_QCCALLATTR_CALL_ATTR_RX = 1,
  DSAT_QCCALLATTR_CALL_ATTR_TX = 2  
}dsat_qccallattr_call_attribute_e_type;
...
/* Stucture for call modification parameters */
typedef struct
{
  cm_call_type_e_type    req_call_type;
  boolean                modify_response;
  dsat_neg_status_e_type last_ccmmd_neg_status;  
}dsat_call_modify_param_s_type;
...
/* To track call before connect state after Call origination */
  ...
  /* To track VoLTE conference participants */
  uint8 etsicall_num_participant_in_conf;
  dsat_conference_participant_info conf_participant_info[CM_CALL_ID_MAX];
  cm_call_id_type conf_linked_call_id;
  /* The flag will be used if handover happen from LTE to GW i.e. SRVCC for MPTY Call */
  boolean handover_complete;
  /* IP Call parameters useful during answering the incoming IP call */
  dsat_cm_ip_call_info_s_type ip_call_info;
...
/* To keep track of the sequence of setting up or receiving the calls 
  ...
  /* Storing sequence number of last ended call */
  int last_seq_num;
  dsat_call_modify_param_s_type call_modify_param;
  /* Mode Info */
...
DESCRIPTION
  This function takes the result from the command line parser and executes 
  it. It executes +CCFC/$QCCCFC command, which controls the call forwarding
...
FUNCTION DSATETSICALL_EXEC_CCFCU_CMD

DESCRIPTION
  This function takes the result from the command line parser and executes 
  it. It executes +CCFCU command, which controls the call forwarding
  supplementary service acording to 3GPP TS 22.072, 22.082 and 24.604.
  It supports call forward to number in URI format.
...
===========================================================================*/
/* ARGSUSED */
dsat_result_enum_type dsatetsicall_exec_ccfcu_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
FUNCTION  DSATETSICALL_EXEC_CMCCS_CMD

DESCRIPTION
  This command activates or deactivates a call monitoring function in the ME. If call monitoring is activated,
  the ME informs about events for calls with URCs.
...
===========================================================================*/
/*ARGSUSED*/
dsat_result_enum_type dsatetsicall_exec_cmccs_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
...
FUNCTION  DSATETSICALL_EXEC_CCMMD_CMD

DESCRIPTION
  This command allows the control of media used in multimedia call.
...
===========================================================================*/
/*ARGSUSED*/
dsat_result_enum_type dsatetsicall_exec_ccmmd_cmd
(
  dsat_mode_enum_type mode,             /*  AT command mode:            */
  const dsati_cmd_type *parse_table,    /*  Ptr to cmd in parse table   */
  const tokens_struct_type *tok_ptr,    /*  Command tokens from parser  */
