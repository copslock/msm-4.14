...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.HI.1.0.c8/Main/modem_proc/datamodem_r/interface/atcop/inc/dsati.h#8 $ $DateTime: 2019/11/22 03:36:18 $ $Author: kiras $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/22/19   ks      Added Rac info parameter to CGREG command.
11/20/19   ks      Supported for NR5G as part of similar bug in ATD.
10/30/19   ks      Added 5G enums for +COPS.
07/16/19   ya      Remove IMS dependency on ATCOP
05/15/18   skc     Fixed issue with AT+CPAS.
03/27/18   skc     Fixed issue with CREG and CGREG URCs.
02/23/18   skc     Added support for +CCFCU.
01/23/18   skc     Fixed issue while flushing queued URCs.
...
08/03/17   skc     Fixed PLMN display issue.
08/03/17   skc     Featurized timer related to EAP.
...
08/10/15   skc     Removing TUPLE_TYPE_IPv4_254
...
02/17/15   tk      Fixed issue in subscription not available event handling.
...
#define dsat_alloc_memory(size, graceful) \
  (TRUE == graceful) ? \
  dsatutil_validate_memory_msg_error(modem_mem_alloc(size, MODEM_MEM_CLIENT_DATA), \
                                     size, __FILENAME__, __LINE__) \
  : \
  dsatutil_validate_memory_err_fatal(modem_mem_alloc(size, MODEM_MEM_CLIENT_DATA), \
                                     size, __FILENAME__, __LINE__)
...
#define IS_VALID_1X_APPS_ID(apps_id) \
  ((DSAT_APPS_ID_1X_PRIMARY <= apps_id) && (apps_id < DSAT_APPS_ID_1X_MAX))
...
#define MAX_TIME_STRING_LENGTH 8 /* HH:MM:SS format */
...
/* Brief structure of audio and video related attributes for multimedia call */
typedef struct dsat_cm_ip_call_attrib_info_s{
...
}dsat_cm_ip_call_attrib_info_s_type;

typedef struct dsat_cm_ip_call_info_s
{
  dsat_cm_ip_call_attrib_info_s_type call_attrib_info;
  
  boolean                       call_modify_err_status;
...
typedef union dsat_cm_call_mode_info_u
{
  ...
  dsat_cm_ip_call_info_s_type        ip_call;
      /* Information related to IP Call */
...
typedef struct dsat_cm_call_state_info_list_s
{
  ...
  byte                           number_of_voice_calls;
  /* Calls with call type VOICE or VT. At a time only one active call can be there.
     It is possible that there can be hold call in conversation state. */
...
typedef struct
{
  ...
  sd_ss_band_pref_e_type                network_band_pref;
...
#endif /* FEATURE_DSAT_LTE */
  boolean                              restrict_dcnr;
  uint8                                sib2_upr_layer_ind_value;
#ifdef FEATURE_LTE_TO_1X
  boolean                              is_csfb_call_active;
#endif /* FEATURE_LTE_TO_1X */
  sys_5g_tac_s_type                    tac_5g;
  uint16                               cm_req_id;
  boolean                              last_reported_tz_changed;
  boolean                              last_reported_dst_adj_changed;
...
typedef struct
{
  ...
  mmgsdi_len_type                      data_len;    /* Data length  */
...
typedef struct
{
  ...
  uint16                               lac;/*lac info*/
  mmgsdi_5g_tac_type                   tac_5g;/*tac info*/
...
/* Structures to track VoLTE Conference call participants */
typedef struct
{
  ...
  int seq_number;
  uint8 dsat_call_id;
}dsat_conference_participant_info;
...
typedef struct
{
  ...
  dsat_conference_participant_info  call_info[MMODE_QMI_NUM_CONF_PARTICIPANTS_MAX];
}dsat_voice_conf_participants_info;
...
} dsat_srvcc_ho_comp_list_s_type;
...
{
  DSAT_COPS_ACT_GSM               = 0,  /* GSM access technology */
  DSAT_COPS_ACT_GSM_COMP          = 1,  /* GSM Compact access technology */
  DSAT_COPS_ACT_UTRAN             = 2,  /* UMTS access technology */
  DSAT_COPS_ACT_GSM_EGPRS         = 3,  /* EGPRS access technology */
  DSAT_COPS_ACT_UTRAN_HSDPA       = 4,  /* HSDPA access technology */
  DSAT_COPS_ACT_UTRAN_HSUPA       = 5,  /* HSUPA access technology */
  DSAT_COPS_ACT_UTRAN_HSDPA_HSUPA = 6,
  DSAT_COPS_ACT_EUTRAN            = 7,  /* LTE access technology */
  DSAT_COPS_ACT_EC_GSM_IOT        = 8,  /* EC-GSM-IoT access technology */
  DSAT_COPS_ACT_EUTRAN_NB_S1      = 9,  /* NB-IoT access technology */
  DSAT_COPS_ACT_EUTRA_5GCN        = 10, /* LTE connected to 5G Core Network */
  DSAT_COPS_ACT_NR_5GCN           = 11, /* NR connected to 5G Core Network */
  DSAT_COPS_ACT_NGRAN             = 12, /* NG-RAN access technology */
  DSAT_COPS_ACT_EUTRA_NR          = 13, /* Dual connectivity of LTE with NR */
...
typedef enum
{
  ...
  DSAT_VENDOR_QCRSCP_IDX      = 13073,
  DSAT_VENDOR_QCECNO_IDX      = 13074,
  DSAT_VENDOR_QCPCOLIST_IDX   = 13075,
  DSAT_VENDOR_QCMSISDN_IDX    = 13076,
  DSAT_VENDOR_QCPCOMSISDN_IDX = 13077,
  DSAT_VENDOR_QCCALLATTR_IDX  = 13078,
  DSAT_VENDOR_QCIMSSRV_IDX    = 13079,
  DSAT_VENDOR_MAX_IDX         = 13080
...
typedef enum
{
  ...
  DSATETSI_EXT_ACT_CMCCS_ETSI_IDX  = 14083,
  DSATETSI_EXT_ACT_CCMMD_ETSI_IDX  = 14084,
  DSATETSI_EXT_ACT_CCFCU_ETSI_IDX  = 14085,
...
typedef enum
{
  ...
  DSATETSI_EXT_CEMBMSCFG_IDX   = 15051,
  DSATETSI_EXT_CEMBMSSRV_IDX   = 15052,
  DSATETSI_EXT_CEMBMSDATA_IDX  = 15053,
  DSATETSI_EXT_CEMBMSCNT_IDX   = 15054,
  DSATETSI_EXT_CEMBMSSAI_IDX   = 15055,
  DSATETSI_EXT_MAX_IDX         = 15056
...
typedef enum
{
  ...
  DSAT_ERR_PTR_VARIABLE_NULL         = 13,
  DSAT_ERR_NV_EFS_READ               = 14,
  DSAT_ERR_NV_EFS_WRITE              = 15,
  DSAT_ERR_NV_EFS_CREATE_DIR         = 16,
  DSAT_ERR_NV_EFS_CREATE_CONFIG_FILE = 17,
  DSAT_ERR_NV_EFS_CLOSE_CONFIG_FILE  = 18,
  DSAT_ERR_NV_EFS_OPEN_CONFIG_FILE   = 19,
  DSAT_ERR_NV_EFS_GET_CONF_FILE      = 20
} dsat_err_e_type;
...
/*dsat_cpol_res_type*/
  ...
  DSAT_PENDING_MRU_UPDATE_RESULT           = 114, /* MRU update -> MRU update result */
  DSAT_PENDING_MRU_RESULT                  = 115, /* MRU update result -> Online -> Result */
  ...
  DSAT_PENDING_ACT_DEACT_TMGI     = 165,   /* Activation and Deactivation of TMGI pending */
  DSAT_PENDING_ACT_TMGI           = 166,   /* Activation of TMGI Pending */
  DSAT_PENDING_DEACT_TMGI         = 167,   /* Deactivation of TMGI pending */
  DSAT_PENDING_TMGI_AVAIL_LIST    = 168,   /* Query for Available list of TMGI pending */
  DSAT_PENDING_ACT_AVAIL_LIST     = 169,   /* Query for Available list of TMGI and Activation of TMGI pending */

  DSAT_PENDING_QCIMSSRV_READ      = 170,
  DSAT_PENDING_QCIMSSRV_NONE      = 171,
  DSAT_PENDING_QCIMSSRV_WRITE     = 172,

  /* To differentiate between AT+CCFC and AT$QCCCFC command */
  DSAT_PENDING_CCFC_LEGACY        = 176,
  DSAT_PENDING_CCFC_EXTENDED      = 177,
  /* +CCFCU */
  DSAT_PENDING_CCFCU              = 178,
  
  DSAT_PENDING_CMD_NONE             = 180
...
typedef enum
{
  ...
  DSAT_TABLE_707_HDR                = 14,
  DSAT_TABLE_707_SMS                = 15,
  DSAT_TABLE_707_VENDOR_PARA        = 16,
  DSAT_TABLE_707_VENDOR_ACTION      = 17,
...
dsat_num_item_type dsat_qcgansm_val; /* Defaulting to NONE */
  ...
  dsat_num_item_type dsat_qccallattr_val[2];
...
typedef struct
{
  ...
  dsat_num_item_type dsat_cmccs_val;
...
typedef struct
{
  ...
  dsat_num_item_type dsat_embmscfg_val;
  dsat_mixed_param_val_type dsat_embmssrv_val[3];
  dsat_num_item_type dsat_embmssai_val[2];
...
#define DSAT_LEGACY_PROFILE_NUM_MAX 42
...
(
  cm_pref_chg_req_s_type *sys_sel_pref_params_ptr
);
#endif /* FEATURE_DSAT_LTE*/
/*===========================================================================
...
/*===========================================================================
 FUNCTION DSAT_MAP_SIMLOCK_RESULT
 
 DESCRIPTION
   This function maps SIMLOCK result code to internal result code.
 
 DEPENDENCIES
   None
 
 RETURN VALUE
   dsat_result_enum_type - Returns internal result code.
 
 SIDE EFFECTS
   None.
 ===========================================================================*/
dsat_result_enum_type dsat_map_simlock_result
(
  simlock_result_enum_type simlock_result
);
