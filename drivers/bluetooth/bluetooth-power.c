...
static const struct of_device_id bt_power_match_table[] = {
	...
	{	.compatible = "csr,csr8811" },  /* askey@bruno 20190523, add power control for csr8811. */
	{}
};
...
/* askey@bruno 20190523, add power control for csr8811. */
static int bt_configure_gpios_by_num(int bt_gpio_num, char *bt_gpio_name, int on)
{
	int rc = 0;
	int bt_gpio = bt_gpio_num;

	BT_PWR_DBG("bt_gpio= %d on: %d", bt_gpio, on);

	if (on) {
		rc = gpio_request(bt_gpio, bt_gpio_name); //bt_sys_rst_n, bt_sys_vreg_n, bt_sys_pwr_n
		if (rc) {
...
{
#if 1 /* askey@bruno 20190523, add power control for csr8811. */
  int rc = 0;
  if(on)
  {
    rc = bt_configure_gpios_by_num(bt_power_pdata->bt_gpio_sys_pwr,"bt_sys_pwr_n",on);
    if (rc) {
  			BT_PWR_ERR("Unable to set bt power\n");
  			return rc;  
    }
    rc = bt_configure_gpios_by_num(bt_power_pdata->bt_gpio_sys_vreg,"bt_sys_vreg_n",on);
    if (rc) {
  			BT_PWR_ERR("Unable to set bt vreg\n");
  			return rc;  
    }    
    rc = bt_configure_gpios_by_num(bt_power_pdata->bt_gpio_sys_rst,"bt_sys_rst_n",on);
    if (rc) {
  			BT_PWR_ERR("Unable to set bt reset\n");
  			return rc;  
    }    
  } 
  else
...
gpio_fail:
		if (bt_power_pdata->bt_gpio_sys_pwr > 0)
			gpio_free(bt_power_pdata->bt_gpio_sys_pwr);
		if (bt_power_pdata->bt_gpio_sys_vreg > 0)
			gpio_free(bt_power_pdata->bt_gpio_sys_vreg);
		if (bt_power_pdata->bt_gpio_sys_rst > 0)
...
static int bt_power_populate_dt_pinfo(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		/* askey@bruno 20190523, add power control for csr8811. qca,bt-vreg-gpio */
		bt_power_pdata->bt_gpio_sys_vreg =
			of_get_named_gpio(pdev->dev.of_node,
						"qca,bt-vreg-gpio", 0);
		if (bt_power_pdata->bt_gpio_sys_vreg < 0)
			BT_PWR_ERR("bt-vreg-gpio not provided in device tree");
      
		/* askey@bruno 20190523, add power control for csr8811. qca,bt-power-gpio */
		bt_power_pdata->bt_gpio_sys_pwr =
			of_get_named_gpio(pdev->dev.of_node,
						"qca,bt-power-gpio", 0);
		if (bt_power_pdata->bt_gpio_sys_pwr < 0)
			BT_PWR_ERR("bt-power-gpio not provided in device tree");
