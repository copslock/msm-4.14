...
#include "atl_ring.h"
#include "atl2_fw.h"
//mike@askey, 20191217, netlink event when link up/down
#include <linux/kobject.h>

//mike@askey, 20191217, netlink event when link up/down
static void atl_kobject_uevent_send(struct atl_hw *hw, char *pin_name, int value)
{
	char *envp[4];
	int env_offset = 0;
	char type_buf[64];
	...
	memset(name_buf,0,64);
	memset(value_buf,0,64);
	
	snprintf(type_buf, sizeof(type_buf),"TYPE=%s", "AQC");
	envp[env_offset++] = type_buf;    
	snprintf(name_buf, sizeof(name_buf),"NAME=%s", pin_name);
	envp[env_offset++] = name_buf;   
	snprintf(value_buf, sizeof(value_buf),"VALUE=%d", value);
	envp[env_offset++] = value_buf;     
	envp[env_offset] = NULL;    
	kobject_uevent_env(&hw->pdev->dev.kobj, KOBJ_CHANGE, envp); // customize uevent
	//kobject_uevent(&sdev->dev->kobj, KOBJ_CHANGE);  // standard uevent     
	dev_err(&hw->pdev->dev,"kobject_uevent_env, type = %s, name = %s, value=%d\n","AQC",pin_name, value);
}
...
//mike@askey, 20191217, netlink event when link up/down			
			atl_kobject_uevent_send(hw,"LAN",1);
...
//mike@askey, 20191217, netlink event when link up/down
			atl_kobject_uevent_send(hw,"LAN",0);
		}
	}
