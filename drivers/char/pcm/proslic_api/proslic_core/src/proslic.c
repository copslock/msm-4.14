...
//leaf add
#if 0
void ProSLIC_FSK8Write(proslicChanType_ptr pProslic, int fsk8)
{
  uInt8 data = ReadReg(pProslic,pProslic->channel,PROSLIC_REG_OMODE);
  if (fsk8)
  {
    data |= 0x80;
  }
...
void ProSLIC_FSKAskeySetup (int preset, int bits, int fifo_depth)
{
  uInt8 eb = bits ? 0x80 : 0;
#ifdef SI3217X
...
#ifdef TEST_USE_DTMF
    printf("current_data[%d]=%x intsActive=%x\n", i, *currentData, intsActive);
    if(digit[0] && i == PROSLIC_REG_IRQ1 + 1)
#else
    if(i == PROSLIC_REG_IRQ1 + 1)
#endif
      *currentData |= 0x10;
