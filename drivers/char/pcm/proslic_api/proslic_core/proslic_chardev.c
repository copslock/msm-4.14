...
//bruno@askey 20190807, added linux/uaccess.h, due to asm/uaccess.h does not have copy_to_user on kernel > 4.11
...
static struct class *proslic_class;
static struct device *pdevice[PROSLIC_NUM_PORTS];

//for cid
static struct _cid_msg cid_data;
/*****************************************************************************************************/

extern int proslic_get_irq(void);
void proslic_preamble_init(char *buf, int len);
static int proslic_open(struct inode *node, struct file *fp)
{
  ...
  for(i = 0; i < port->numberOfChan; i++)
  {
    ...
    ProSLIC_SetMuteStatus(chan, PROSLIC_MUTE_ALL);
  }
...
static int proslic_close(struct inode *node, struct file *fp)
{
  ...
  module_put(THIS_MODULE);
  disable_irq(proslic_get_irq());
  proslic_clear_int_data();
  return 0;
}
...
{
  int rc = 0;
  char *msg_start = cid_data.msg;
  char *msg_stop = cid_data.msg + cid_data.msg_len;
  char bufsz = cid_data.msg_len - (char)(cid_data.ptr - msg_start);
  unsigned char wsend;

  if(cid_data.msg_len <= 0)
  {
    mdelay(5 * (cid_data.fifo_depth + 1) * 8 / 6);//wait depth byte done
    ProSLIC_DisableCID(chanPtr);
    return 1;
  }

  wsend = min(bufsz, FSKBUF_AVAILIABLE);
  rc = ProSLIC_SendCID(chanPtr, cid_data.ptr, wsend);
  cid_data.ptr += wsend;
  if(cid_data.ptr >= msg_stop)
  {
    cid_data.ptr = 0;
    cid_data.msg_len = 0;
  }
...
#endif
{
  ...
  switch(cmd)
  {
   ...
   case PROSLIC_IOCTL_GET_RINGER_COUNT:
    ...
    case PROSLIC_IOCTL_SET_MUTE:
      rc = ProSLIC_SetMuteStatus(chanPtr, chan_if.byte_value);
      rc &= ~1;
      break;
...
#define C_VOLUME(x) ((x) * 36 / 100 - 30)
    ...
    case PROSLIC_IOCTL_SEND_CID:
      ...
      if(!rc)
        ProSLIC_EnableCID(chanPtr);
      break;

    case PROSLIC_IOCTL_DRZ_SET:
      ProSLIC_DCFeedSetup(chanPtr, chan_if.byte_values[0]);
      ProSLIC_RingSetup(chanPtr, chan_if.byte_values[1]);
      ProSLIC_ZsynthSetup(chanPtr, chan_if.byte_values[2]);
      break;
...
int proslic_api_char_dev_init()
{
  ...
  {
    proslic_class = class_create(THIS_MODULE, PROSLIC_CHARDEV_BASE_NAME);
    if (IS_ERR(proslic_class)) {
      pr_err("%s: class_create() failed ENOMEM\n", __func__);
      rc = -ENOMEM;
      unregister_chrdev_region(MAJOR(proslic_dev),
          PROSLIC_NUM_PORTS);
      return rc;
    }
  }
  for(i = 0; i < PROSLIC_NUM_PORTS; i++)
  {
    ...
    {
      char devn [16];
      sprintf(devn, PROSLIC_CHARDEV_BASE_NAME"%d", i);
      pdevice[i] = device_create(proslic_class, NULL, proslic_dev + i, NULL, devn);
      if(IS_ERR_OR_NULL(pdevice[i])){
	pr_err("%s: device_create() failed for proslic:%d\n",__func__, i);
        rc = -ENOMEM;
...
void proslic_api_char_dev_quit()
{
  ...
  if(proslic_file_info)
  {
    ...
    {
      cdev_del( &(proslic_file_info[i].cdev) );
      device_del(pdevice[i]);
    }

    class_destroy(proslic_class);
    kfree(proslic_file_info);
    proslic_file_info = NULL;
  }
